# coding: utf-8

"""
    YouTube Data API v3

    The YouTube Data API v3 is an API that provides access to YouTube data, such as videos, playlists, and channels.

    The version of the OpenAPI document: v3
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from abcdefgji_ti.api.playlists_api import PlaylistsApi


class TestPlaylistsApi(unittest.TestCase):
    """PlaylistsApi unit test stubs"""

    def setUp(self) -> None:
        self.api = PlaylistsApi()

    def tearDown(self) -> None:
        pass

    def test_youtube_playlists_delete(self) -> None:
        """Test case for youtube_playlists_delete

        """
        pass

    def test_youtube_playlists_insert(self) -> None:
        """Test case for youtube_playlists_insert

        """
        pass

    def test_youtube_playlists_list(self) -> None:
        """Test case for youtube_playlists_list

        """
        pass

    def test_youtube_playlists_update(self) -> None:
        """Test case for youtube_playlists_update

        """
        pass


if __name__ == '__main__':
    unittest.main()
