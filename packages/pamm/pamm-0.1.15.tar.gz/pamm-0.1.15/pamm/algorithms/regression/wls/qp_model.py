...
class QPRegressionAlgorithm(RegressionAlgorithm):
    ...
    def _add_bias(self, x: np.ndarray, min: np.ndarray, max: np.ndarray) -> tuple[np.ndarray]:
        if self._fit_bias:
            return (np.hstack([np.ones((x.shape[0], 1)), x]), np.vstack([-100000, min]), np.vstack([100000, max]))
        return x, min, max

    def fit(self, x: np.ndarray, y: np.ndarray, r: np.array, beta_min: np.ndarray, beta_max: np.ndarray) -> None:
