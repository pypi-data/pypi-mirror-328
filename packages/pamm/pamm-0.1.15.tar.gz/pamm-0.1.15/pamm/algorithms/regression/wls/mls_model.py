...
class ModifyRegressionAlgorithm(RegressionAlgorithm):
    def _add_bias(self, x: np.ndarray, q: np.ndarray, b_apr: np.ndarray) -> tuple[np.ndarray]:
        if self._fit_bias:
            return (np.hstack([np.ones((x.shape[0], 1)), x]), np.diag(np.hstack([0, np.diag(q)])), np.vstack([0, b_apr]))
        return x, q, b_apr

    def fit(self, x: np.ndarray, y: np.ndarray, w: np.ndarray, q: np.ndarray, b_apr: np.ndarray) -> None:
        """Fit model.
