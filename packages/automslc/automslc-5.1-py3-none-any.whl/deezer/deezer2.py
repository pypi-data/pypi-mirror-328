...
from gbackup import CloudFlareHelper
...
def init_deezer_session(proxy_server,token=""):
    global session
    header = {
        'Pragma': 'no-cache',
    ...
    session.cookies.update({'arl': token, 'comeback': '1'})
...
def get_jwt():
    ...
    jwt = session.post(url).json()['jwt']
    return jwt
def get_lyric_sync(trackid):
    try:
        jwt= get_jwt()
        url="https://pipe.deezer.com/api"
        headers={"user-agent" : "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/114.0",
                              "authorization": "Bearer " + jwt}
        jdata = {"operationName": "GetLyrics", "variables": {"trackId": trackid},
        ...
        return requests.post(url, json=jdata, headers=headers).json()
...
def download_by_id(track_id, info_download={}, ip= None, output="/tmp", quality_text='128'):
    ...
    init_deezer_session("http://jcqpvqte-1:tedzuonzv900@p.webshare.io:80", token_txt)
    song = get_song_infos_from_deezer_website('track', track_id)

    file_name = song['ISRC'] + "_" + track_id + "_" + quality + extension
    tmp_file = song['ISRC'] + "_tmp_" + track_id + "_" + quality + extension
    path_name = output + "/" + file_name
    path_tmp= output + "/" + tmp_file
    download_song(song, path_name)
    cf =CloudFlareHelper('cdn_cacherecords')
    url_128=cf.upload(path_name,"deez")
    # cmd = f"ffmpeg -i \"{path_tmp}\" -b:a 128000 -ar 44100 -c:a mp3 \"{path_name}\"";
    # os.system(cmd)
    os.remove(path_name)
    rs=get_lyric_sync(track_id)
    info_download['isrc'] = song['ISRC']
    info_download['url_128'] = url_128
    info_download['lyric'] = ''
