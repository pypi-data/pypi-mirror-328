...
def verifica_processos_emsys_abertos():
    try:
...
def kill_contabil_processes():
    try:
        ...
        verifica_processos_emsys_abertos()
    except Exception as error:
        console.print(f"Error: {error}")
...
async def integracao_contabil(task: RpaProcessoEntradaDTO) -> RpaRetornoProcessoDTO:
    try:
        ...
        for attempt in range(max_attempts):
            try:
                app = Application(backend="win32").connect(title="Selecione o Usuário para autenticação")
        ...
        for attempt in range(1,max_attempts+1):
            console.print(f"Verificacao {attempt} ( maximo {max_attempts} ) - aguardando 1 minuto...")
            field = get_text_from_window(main_window, (1033, 770, 1301, 785))
            if "finalizado" in field:
                found = True
                break
            # if "validando" in field:
            #     return RpaRetornoProcessoDTO(
            #         sucesso=False,
            #         retorno=f"Erro durante o processo integração contabil, nenhum lote para integrar...",
            #         status=RpaHistoricoStatusEnum.Falha, tags=[RpaTagDTO(descricao=RpaTagEnum.Tecnico)]
            #     )
            await worker_sleep(30)
        if not found:
        ...
        for attempt in range(1, max_attempts_dados_consistentes+1):
            ...
            try:
                ...
                if total_debito != total_credito:
                    if not dados_consistentes_is_checked:
                        ...
                        if total_debito == total_credito:
                            ...
                            return RpaRetornoProcessoDTO(
                                sucesso=False,
                                retorno="Os valores de crédito e débito divergem mesmo após clicar em lotes consistentes",
                ...
                console.print("Nao foi possivel encontrar os campos diretamente, tentando via ocr...")
                total_debito = get_text_from_window(main_window, (854, 722, 956, 747)).replace("\n","")
                await worker_sleep(1)
                total_credito = get_text_from_window(main_window, (1055, 722, 1162, 747)).replace("\n","")
                await worker_sleep(1)
                diferenca = get_text_from_window(main_window, (1233, 722, 1336, 747)).replace("\n", "")
                await worker_sleep(1)
