...
def compute_mosaicity(
    moments: dict[int, numpy.ndarray], dimension1: int, dimension2: int
):
    norm_dim1 = compute_normalized_component(moments[dimension1][0])
    norm_dim2 = compute_normalized_component(moments[dimension2][0])

    return hsv_to_rgb(compute_hsv(norm_dim1, norm_dim2))
...
def compute_orientation_dist_data(
    ...
    hsv_key = dataset.compute_orientation_colorkey(dimension1, dimension2)
