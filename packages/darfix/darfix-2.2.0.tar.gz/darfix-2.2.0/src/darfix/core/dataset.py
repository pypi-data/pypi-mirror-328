...
from darfix.core.imageRegistration import apply_shift
...
from darfix.core.mapping import fit_2d_data
from darfix.core.mapping import fit_data
...
class ImageDataset:
    ...
    def compute_orientation_colorkey(self, dimension1: int, dimension2: int):
        x_data = numpy.linspace(-1, 1, self.__dims[dimension1].size * SCALE_FACTOR)
        y_data = numpy.linspace(-1, 1, self.__dims[dimension2].size * SCALE_FACTOR)
    ...
    def compute_orientation_dist(
        ...
        dim1_size = self.__dims[dimension1].size
        dim2_size = self.__dims[dimension2].size

        ori_dist = numpy.zeros(dim1_size * dim2_size)
        ...
        return ori_dist.reshape((dim2_size, dim1_size)).T
