...
from .init_parameters import InitParameters, KeyParameters
...
class ConfigurationManager:
    ...
    def evaluate_running_experiment(
        ...
        if self.init_parameters.is_get_or_create() or self.init_parameters.is_get():
            ...
            result = key_params.match_with_experiment(experiment)
            if result.ok():
        ...
        LOGGER.debug(
            "Key parameters doesn't match, the current running experiment will be ended automatically."
        )
        return None
