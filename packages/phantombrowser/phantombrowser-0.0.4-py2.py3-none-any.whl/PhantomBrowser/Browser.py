...
class Browser:
    ...
    def download(
        ...
        **kwargs: Any,
        ...
        if not filename:
            ...
            if not filename:
                uri = urlparse(url)
                filename = os.path.basename(uri.path) or f"downloaded_{uri.netloc}.html"
                if "." not in filename:
    ...
    def _downloader(self, *args: Any, **kwargs: Any) -> None:
        try:
    ...
    def downloader(
        self, urls: List[str], folder: Optional[str] = None, **kwargs: Any
    ) -> None:
        if folder:
        ...
        for url in urls:
            ...
            p = multiprocessing.Process(target=self._downloader, kwargs=kwargs)
            process.append(p)
            p.start()
        for p in process:
            p.join()
