    ...
    def dump(self):
        ...
        with RedisFileLock(self.redis_client, os.path.join(self.folder, self.task_id)):
            with open(os.path.join(self.folder, self.task_id), "wb") as target_file:
                pickle.dump(
                    self.data, file=target_file, protocol=pickle.HIGHEST_PROTOCOL
                )
...
class CeleryExecutor(object):
    ...
    def cleanup_task_results(
        in_dir, out_dir, redis_host, redis_port, task_expires, result_expires
    ):
            ...
            if not redis_client.exists(redis_key):
                ...
                if stale:
                    try:
                        ...
                        with RedisFileLock(
                            redis_client, os.path.join(out_dir, file_name)
                        ):
                            if os.path.exists(os.path.join(out_dir, file_name)):
                                os.remove(os.path.join(out_dir, file_name))
                                logger.debug(
                                    f"Deleted stale file: {os.path.join(out_dir, file_name)}"
                                )
                    except OSError:
                        pass
                ...
                if stale:
                    try:
                        with RedisFileLock(redis_client, file_path):
                            os.remove(file_path)
                        with RedisFileLock(
                            redis_client, os.path.join(in_dir, file_name)
                        ):
        ...
        while True:
            cursor, keys = redis_client.scan(
                cursor=cursor, match=f"{CeleryExecutor.CELERY_BROKER_PREFIX}*"
            )
            if not keys:
                break
            for key in keys:
    ...
    def set_cleanup_task(self):
        ...
        self.app.task(
            CeleryExecutor.cleanup_task_results, name="celery.backend_cleanup"
        )
    ...
    def start(self):
        ...
        for queue, queue_config in self.queue_config.items():
            ...
            if queue_config.max_memory_per_child is not None:
                command.append(
                    f"--max-memory-per-child={queue_config.max_memory_per_child}"
                )
        ...
        normalized_ratios = {
            key: value / total_ratio
            for key, value in CeleryExecutor.BACKEND_COMM_THREADS_RATIOS.items()
        }
        allocated_workers = {
            key: round(self.n_workers * ratio)
            for key, ratio in normalized_ratios.items()
        }
        while sum(allocated_workers.values()) != self.n_workers:
            diff = self.n_workers - sum(allocated_workers.values())
            key = max(
                allocated_workers,
                key=lambda k: normalized_ratios[k]
                - (allocated_workers[k] / self.n_workers),
            )
    ...
    def close(self):
        ...
        celery_files = [
            ...
            if f.startswith("celerybeat-schedule")
        ]
    ...
    def serialized_io(self, task_fn):
        ...
        def wrapper(task):
            ...
            serialized_data = CelerySerialized(
                folder=self.in_folder, redis_client=self.redis_client
            )
            ...
            if os.path.isfile(os.path.join(self.in_folder, task_id)):
                with RedisFileLock(
                    self.redis_client, os.path.join(self.in_folder, task_id)
                ):
    ...
    async def terminate_task(self, task_id):
        ...
        def _remove_task_data(celery_app, in_folder, out_folder, task_id):
            ...
            if os.path.isfile(os.path.join(out_folder, task_id)):
                with RedisFileLock(
                    self.redis_client, os.path.join(self.out_folder, task_id)
                ):
                    os.remove(os.path.join(out_folder, task_id))

        await asyncio.get_running_loop().run_in_executor(
            self.set_thread_pool, _terminate_task, self.app, task_id
        )
        await asyncio.get_running_loop().run_in_executor(
            self.io_thread_pool,
            _remove_task_data,
    ...
    async def get_task_result(self, task_id, remove=False):
        def _try_get_task_data(celery_app, task_id):
            celery_app.AsyncResult(
                task_id
            ).get()  # will raise if the task raised an exception

        def _read_task_data(out_folder, task_id):
            serialized_data = CelerySerialized(
                folder=out_folder, redis_client=self.redis_client
            )
            serialized_data.set_task_id(task_id)
        ...
        await asyncio.get_running_loop().run_in_executor(
            self.get_thread_pool, _try_get_task_data, self.app, task_id
        )
    ...
    async def send_and_wait_task(
        self, task_fn, args=[], kwargs={}, timeout=60, **options
    ):
        ...
        try:
            while not ready:
                ...
                if time.time() - start_time > timeout:
                    await self.terminate_task(task_id)
                    raise TimeoutError(
                        "Task has not completed within the alloted time amount!"
                    )
...
class FastAPICeleryTaskRoute(Route):
    ...
    def define_routes(self):
        ...
        async def get_task(
            task_id: str, pop: bool = Query(False)
            ...
            if ready:
                ...
                try:
                    result = await self.celery_executor.get_task_result(
                        task_id, remove=pop
                    )
            ...
            if state in ["FAILURE", "RETRY", "REVOKED"]:
                error_response = DefaultErrorRoute.format_error(
                    exc, debug=True
                ).body.decode("utf-8")
                data = {
                    "task_state": state,
