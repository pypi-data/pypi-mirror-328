    ...
    def call_infer(self, message: dict, callback: Optional[Any] = None, **kwargs):
        ...
        try:
            ...
            for key, value in kwargs.items():
                param_type = "FP32" if isinstance(value, float) else "INT32" if isinstance(value, int) else "BYTES"
            ...
            response = requests.post(self.base_url, json=input_data, headers=self.headers, timeout=7200)
            if response.status_code != 200:
                raise Exception(
