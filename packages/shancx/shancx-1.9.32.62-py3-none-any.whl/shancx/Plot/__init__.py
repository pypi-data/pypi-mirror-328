...
def plotGrey(img,,name="plotGrey", cmap='gray', title='Image'):
    now_str = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
    img = img.squeeze()  # 去掉 batch 维度并转换为 numpy 数组
    plt.imshow(img, cmap='gray')
    plt.title(f"Image ")
    plt.axis('off')  # 不显示坐标轴
    plt.savefig(f'./plotGrey/{name}_{now_str}.png')
    plt.close()


import matplotlib.pyplot as plt
def plotMat(matrix,name='plotMat',title='Matrix Plot', xlabel='X-axis', ylabel='Y-axis', color_label='Value', cmap='viridis'):
    now_str = datetime.datetime.now().strftime("%Y%m%d%H%M%S")    
    ...
    plt.ylabel(ylabel)
    plt.tight_layout()
    plt.savefig(f'./plotMat/{name}_{now_str}.png')
    plt.close()
 
import datetime
import matplotlib.pyplot as plt
from mpl_toolkits.axes_grid1 import make_axes_locatable
def plotRadar(array_dt,name="plotRadar", ty="CR"):
    now_str = datetime.datetime.now().strftime("%Y%m%d%H%M%S")    
    if len(array_dt.shape) == 2 and ty == "pre":
        ...
        outpath = f"./plotRadar/{name}_pre_{now_str}.png"
        crDir(outpath)
        plt.savefig(outpath)
        plt.close()    
    else:
        ...
        cax = divider.append_axes("right", size="5%", pad=0.05)
        plt.colorbar(im, cax=cax)        
        outpath = f"./plotRadar/{name}_CR_{now_str}.png"
        crDir(outpath)
        plt.savefig(outpath)
        plt.close()
...
from mpl_toolkits.axes_grid1 import make_axes_locatable
import os
def plotVal( epoch=0,*datasets, title=["input","prediction","truth"], save_dir="plots", cmap='summer'):
...
def plotValplus(epoch=0, *datasets, title=["input", "prediction", "truth"], save_dir="plots", cmap='summer'):
    num_datasets = len(datasets)
    title = title or [f"data{i}" for i in range(num_datasets)]
    ncols = int(np.ceil(np.sqrt(num_datasets)))
    nrows = int(np.ceil(num_datasets / ncols))
...
def plot_grid_data(data, titles=None, save_dir="plots", name="temp",
    ...
    plt.close()
def plotTr(base_up, base_down, name="plotTr", save_dir="plotTr",  cmap="summer"): #viridis
    """
    组合数据并调用绘图函数    
    Args:
