...
def _get_remote_fs():
    ...
    获取远端 filesystem 信息
    """
    url = "http://127.0.0.1:30443/agent/rpc"
    headers = {
        "Content-Type": "application/json"
    }
    data = {
        "app": "nodes.on-invoke-rpc.windmill",
        "method": "GET",
        "params": "/sts"
    }
    ...
    if response.status_code == 200:
        ...
        raise Exception("Failed to get remote filesystem info.")
...
def import_models(args, windmill_client, job_client):
    """
    ...
    create_task_event_req = CreateEventRequest(
        ...
        task_name="task-import_model",
        kind=EventKind.Succeed,
        reason="模型导入成功",
        message="模型包成功创建并导入本地 filesystem")

    try:
        fs_remote_json = _get_remote_fs()
    except Exception as e:
        create_task_event_req.kind = EventKind.Failed
        create_task_event_req.reason = "无法获取远端 fs 信息"
        create_task_event_req.message = "模型导入失败，无法获取远端 filesystem 信息"
    ...
    # 构建远端 filesystem
    fs_remote = {
        "host": fs_remote_json.get("host", ""),
        "kind": fs_remote_json.get("kind", ""),
        "endpoint": fs_remote_json.get("endpoint", ""),
        "credential": {
            "accessKey": fs_remote_json.get("credential", {}).get("accessKey", ""),
            "secretKey": fs_remote_json.get("credential", {}).get("secretKey", "")
        },
        "config": {
            "disableSSL": fs_remote_json.get("config", {}).get("disableSSL", ""),
            "region": fs_remote_json.get("config", {}).get("region", ""),
        }
    }
    # 构建远端 filesystem
    # fs_remote = {
    ...
    # TODO: 这个 tigs 应该不需要了, 盒子上没有前端展示模型包的结构
    model_list = []
    ...
    try:
        for model in model_list:
            ...
            # 这里需要 windmill 中的 fs 能够访问宿主机上的文件
...
def run():
    ...
    model_artifact_list = import_models(args, windmill_client, job_client)
