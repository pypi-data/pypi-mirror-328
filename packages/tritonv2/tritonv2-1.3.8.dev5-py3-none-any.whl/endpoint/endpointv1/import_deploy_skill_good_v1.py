...
def _delete_model_file(base_path, model_name, version):
    """
    删除模型文件

    Args:
        base_path (str): 模型文件基本路径, e.g. "/data/model/workspaces/xxx/modelstores/bbb/model"
        model_name (str): 模型名称(模型路径), e.g. "fire_ensemble"
        version (str): 实际的模型版本号, e.g. "1"
    """

    delete_file_path = os.path.join(base_path, model_name, version)
    try:
...
def import_models(args, windmill_client):
    ...
    model_version = {}  # key = 原子模型 localName, value = version

    local_model_base_path = args.model_base_path
    model_store = parse_modelstore_name(args.model_store_name)
    job_name = parse_job_name(args.job_name)
    tags = {}

    # 盒子模型包存放路径, e.g. /data/model/workspaces/xxx/modelstores/bbb/model
    local_model_path = str(os.path.join(local_model_base_path, args.model_store_name, "model"))
    ...
    try:
        for model in model_list:
            ...
            resp = windmill_client.create_model(
                ...
                # "file" 开头, 并且盒子上 filesystem 类型为 "file", 不会执行模型文件上传操作
                artifact_uri=("file://" + model["metadata"]["artifact"]["uri"]))

            # 将模型文件从云端 copy 到宿主机
            dest_path = os.path.join(local_model_path, model["metadata"]["localName"], resp.artifact.version)
            try:
                download_by_filesystem(fs_remote, model["metadata"]["artifact"]["uri"], dest_path)
            except Exception as e:
                tags = {
                    "errorCode": "400",
                    "errorMessage": "无法下载模型文件！"
                }
                bcelogger.error(f"ImportDeploySkillJob {args.job_name} Failed: could not download model artifact")
                windmill_client.update_job(workspace_id=job_name.workspace_id,
                                           project_name=job_name.project_name,
                                           local_name=job_name.local_name,
                                           tags=tags)
    ...
    for artifact in ret_artifact_list:
        ...
        model_version[model_name.local_name] = artifact["version"]
    for name, version in model_version.items():
        sub_model_path = os.path.join(local_model_path, name, version)
        ...
        for sub_model_name, sub_model_info in step.items():
            if sub_model_name not in model_version:
                tags = {
                    "errorCode": "400",
...
def create_deploy_job(args, model_artifact, windmill_client):
    ...
    try:
        ...
        for compute in compute_list:
            if ("accelerator" in compute.tags) and (compute.tags["accelerator"] == accelerator):
                endpoint_compute_name = compute.name
                break
        if endpoint_compute_name == "":
            if len(compute_list) > 0:
                endpoint_compute_name = compute_list[0].name
            else:
                tags = {
                    "errorCode": "400",
                    "errorMessage": "部署服务失败，无法获取计算资源！"
    ...
    try:
        for model_artifact in model_artifact_list:
            ...
            for i in range(2, len(artifacts.result)):
                if artifacts.result[i].version != model_artifact.version:
                    windmill_client.delete_artifact(object_name=artifact_name.object_name,
                                                    version=artifacts.reault[i].version)
                    if not _delete_model_file(local_model_path, model_name.local_name, artifacts.reault[i].version):
            ...
            while processed_count < total_count:
                ...
                processed_count += len(artifacts.result)
                for artifact in artifacts.result:
                    if artifact.version != model_artifact.version:
                        windmill_client.delete_artifact(object_name=artifact_name.object_name, version=artifact.version)
                        if not _delete_model_file(local_model_path, model_name.local_name, artifact.version):
                            bcelogger.error(f"DeleteModels Failed: {traceback.format_exc()}")
    except Exception as e:
        bcelogger.error(f"DeleteModels Failed: {traceback.format_exc()}")
