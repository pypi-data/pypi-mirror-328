...
BioSAK Usearch16S -i CleanData -x fna -o op_dir -t 12 -f -ref SILVA_138.2.fa
...
# This is a wrapper for the following Usearch modules, refer to the output cmds.txt for details.
...
def Usearch16S(args):
    ...
    if run_blca is True:
        cmd_5  = 'usearch -uchime2_ref %s/s05_AllSamples_denoised.fasta -db %s -strand plus -mode high_confidence -notmatched %s/s06_AllSamples_unoise_nc.fasta' % (op_dir, blca_ref_seq, op_dir)
    with open(cmd_txt, 'a') as cmd_txt_handle:
    ...
    if run_blca is False:
        ...
        # run blca
        blca(unoise_nc_fa_split_dir, 'fa', s10_blca_op_dir, blca_ref_seq, blca_ref_tax, num_threads)
...
if __name__ == '__main__':
    ...
    Usearch16S_parser.add_argument('-r',       required=False,                       help='SILVA reference sequences')
    Usearch16S_parser.add_argument('-ref',     required=False,                       help='reference sequences')
    Usearch16S_parser.add_argument('-blca',    required=False, action="store_true",  help='perform classification with BLCA')
    Usearch16S_parser.add_argument('-tax',     required=False,                       help='reference sequence taxonomy')
