...
class H2OGPTEAsync:
    ...
    async def create_collection(
        ...
        collection = _rest_to_client_exceptions(
            lambda: rest_client.create_collection(request, _headers=headers)
        )
    ...
    async def get_all_directory_stats(self, chat_session_id: str) -> dict:
        res = await self._lang(
            "get_all_directory_stats", chat_session_id=chat_session_id
        )
        return res
