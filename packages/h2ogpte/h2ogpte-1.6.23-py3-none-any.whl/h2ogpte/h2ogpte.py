...
class H2OGPTE:
    ...
    def create_collection(
        ...
        collection = _rest_to_client_exceptions(
            lambda: rest_client.create_collection(
    ...
    def set_collection_size_limit(
        ...
        if not (
            H2OGPTE.MIN_COLLECTION_SIZE_BYTES
            <= byte_limit
            <= H2OGPTE.MAX_COLLECTION_SIZE_BYTES
        ):
            raise ValueError(
                "Size limit must be a positive integer and within the range 1MiB - 10GiB."
            )

        res = self._db("set_collection_size_limit", collection_id, byte_limit)
        return _to_id(res)
    ...
    def remove_collection_size_limit(self, collection_id: str) -> str:
        ...
        res = self._db("remove_collection_size_limit", collection_id)
        return _to_id(res)
    ...
    def get_all_directory_stats(self, chat_session_id: str) -> dict:
        res = self._lang("get_all_directory_stats", chat_session_id=chat_session_id)
        return res

    def get_h2ogpt_system_stats(self) -> dict:
        res = self._lang("get_h2ogpt_system_stats")
        return res
