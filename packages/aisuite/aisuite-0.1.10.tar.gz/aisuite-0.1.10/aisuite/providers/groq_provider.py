...
class GroqMessageConverter(OpenAICompliantMessageConverter):
    ...
    def convert_request(self, messages):
        """
        Convert messages to Groq format, ensuring no reasoning_content is present
        """
        converted_messages = super().convert_request(messages)
        
        # Clean up any reasoning_content fields
        for message in converted_messages:
            if 'reasoning_content' in message:
                message['content'] = message.pop('reasoning_content')
