...
class _opt_agent_model:
    ...
    def predict(self,X_predict:np.ndarray,return_std:bool = False,return_cov:bool = False) -> np.ndarray:
        ...
        X_predict : np.ndarray
            predict points, must be (n,m) like, where n is the number of predict points
...
class _opt_plot:
    def __init__(self,flist,x_vec,method,visual = "all",_show_opc:bool = True):
        ...
        if "advanced" in visual:
            ...
            with ProcessPoolExecutor() as executor:
                future_pc = executor.submit(_opt_plot._pc, flist, x_vec)
                future_scatter = executor.submit(_opt_plot._scatter_matrix, df, x_vec.shape[1])
                future_pca = executor.submit(_opt_plot._PCA, df.copy(), x_vec)
                future_tsne = executor.submit(_opt_plot._t_SNE, df.copy(), x_vec)
                
                fig_1 = future_pca.result()
                fig_2 = future_tsne.result()
                df_scalar = future_pc.result()
                sc_figure = future_scatter.result()
            
            ## show()
            fig_1.show()
            fig_2.show()
            sc_figure.canvas.draw()
    ...
    @staticmethod
    def _scatter_matrix(df, x_vec_shape_1):
        print("scatter matrix")
        from seaborn import pairplot
        grid = pairplot(df, vars=[f"x{i}" for i in range(x_vec_shape_1)],
                        hue="cost", palette='viridis', diag_kind='hist',
                        plot_kws={'alpha': 0.8}, height=8.1 / (x_vec_shape_1 + 0.5))
...
class base_optimizer:
    ...
    def optimization(self): assert 0 # method has not beend defined for this optimizer
    ...
    def error(self,err:str) -> Exception:
        ...
        """
        err_msg = _optimize_Exception.err_dict(err)
        try:
            self._logging(err_msg)
        except:
            pass
        raise _optimize_Exception(err_msg)

    def __exit__(self):
        self.__del__()
        
    def __del__(self):
        print("optimizer stop working")
...
if __name__ == "__main__":
    ...
    path = "labopt_logs/lab_opt_2025_02_19/optimization__2025-02-19-10-42__ISMA__.txt"
