...
from khmer_text_transliteration.config import PARENT_DIR


all_pronunciations = load_data(
    os.path.join(PARENT_DIR, "data", "raw", "eng_khm_data.txt")
)
...
correct_khmer_normalized = flat_khmer_word(
    os.path.join(PARENT_DIR, "data", "raw", "khmer_words.txt")
)
