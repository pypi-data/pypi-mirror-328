...
NUMBERTYPES = (int, int, float, complex, np.float16, np.float32,
               np.float64, np.int16, np.int32, np.int64, np.int8)
STRINGTYPES = (str, str)
    ...
    def draw_new(self, i=None, ax=None, cb=False, rescale=True):
        ...
        if kind == 'scatter':
            if tmp_values is None:
                ...
                if 'c' not in dargs.keys() and 'color' not in dargs.keys():
                    dargs['c'] = tmp_values
                plot = ax.scatter(tmp_x, tmp_y, **dargs)
        elif kind == 'plot':
