...
def extrapolate_until_wall(field, direction='x', position=0.,
    ...
    if direction == 'x':
        if position < axe_x[0]:
            ext_field.extend(nmb_left=int((axe_x[0] - position)/dx),
                             inplace=True)
        else:
            ext_field.extend(nmb_right=int((position - axe_x[1])/dx),
                             inplace=True)
    else:
        if position < axe_y[0]:
            ext_field.extend(nmb_down=int((axe_y[0] - position)/dy),
                             inplace=True)
        else:
            ext_field.extend(nmb_up=int((position - axe_y[1])/dy),
                             inplace=True)
    # ScalarField
    if isinstance(field, ScalarField):
        # adding wall
