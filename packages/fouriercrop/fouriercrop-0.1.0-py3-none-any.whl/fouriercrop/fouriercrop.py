...
class FourierCrop:
    ...
    This class crops the central region of a tensor after a Fourier transform,
    with options for normalization and padding.
    ...
    Methods:
        ...
        norm(x: torch.Tensor, dim: Tuple = (-3, -2, -1), epsilon: float = 1e-6) -> torch.Tensor:
            Normalizes a tensor by its mean and standard deviation.

        fft(x: torch.Tensor, dim: Tuple = (-3, -2, -1), norm: str = "ortho") -> torch.Tensor:
            Applies 3D Fast Fourier Transform (FFT) to input data.

        ifft(x: torch.Tensor, dim: Tuple = (-3, -2, -1), norm: str = "ortho") -> torch.Tensor:
    ...
    def __init__(
        ...
        """
        Initializes the FourierCrop module with specified padding mode, dimensions, and epsilon value.

        Args:
            pad_mode (int, optional): Determines the cropping function to use.
            dim (Tuple, optional): Dimensions over which to perform operations like FFT and normalization (default=(-3, -2, -1)).
            epsilon (float, optional): Small value to avoid division by zero during normalization (default=1e-6).
        """
        super().__init__()
        self.crop_func = {0: self.crop_center, 1: self.pad_center, 2: self.crop_center_pad}[pad_mode]
    ...
    def crop_center(x: torch.Tensor, bin_factor: int = 2) -> torch.Tensor:
        ...
        Args:
            ...
            bin_factor (int, optional): Factor determining the size of the cropped region. Defaults to 2.
        ...
        crop_slice = tuple(slice(input - target, input + target) for input, target in zip(input_center, target_center))
