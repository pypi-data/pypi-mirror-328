...
class BaseDownloader(object):
    ...
    def download_tasks(self, tasks, executor):
        ...
        for task_info in tasks:
            ...
            for file_info in task_info["files"]:
                ...
                file_info["output_dir"] = ensure_writable_output_path(file_info, task_info, self.output_path)
    ...
    def attempt_download(self, file_info):
        ...
        if file_info["output_dir"] is None:
            identifier = f"{file_info['job_id']}::{file_info['task_id']}::{file_info['relative_path']}"
            logger.debug("Download destination is None for file %s.", identifier)
            return self.generate_file_done_event(
                file_info, 
                error=f"Download destination is None for file {identifier}."
            )
...
def ensure_writable_output_path(file_info, task_info, user_specified_path=None):
    """
    ...
    """
    fallback_path = get_fallback_path(task_info["job_id"])
    identifier = f"{file_info['job_id']}::{file_info['task_id']}::{file_info['relative_path']}"
    if user_specified_path:
        logger.debug("Using user-specified output directory: %s", user_specified_path)
        full_user_specified_path = os.path.join(user_specified_path, task_info["job_id"])
        logger.debug("Using user-specified output directory: %s", full_user_specified_path)
        if ensure_path_valid(full_user_specified_path):
            return full_user_specified_path
        if ensure_path_valid(fallback_path):
            logger.info("Using fallback output directory: %s for file %s", fallback_path, identifier)
            return fallback_path
        logger.warning("No valid output directory found for file %s.", identifier)
        return None
    else:
        logger.debug("No user-specified output directory. Using job's output directory.")
        output_path = file_info["output_dir"]
        if ensure_path_valid(output_path):
            logger.debug("Using job's output directory: %s", output_path)
            return output_path
        if ensure_path_valid(fallback_path):
            logger.info("%s is not valid. Using fallback output directory: %s for file %s", output_path, fallback_path, identifier)
            return fallback_path
        logger.warning("No valid output directory found for file %s.", identifier)
        return None

def get_fallback_path(job_id):
    return os.path.expanduser(os.path.join("~", FALLBACK_DOWNLOADS_FOLDER, job_id))

def ensure_path_valid(path):
    """
    Ensure that a path is valid and writable.

    This function ensures:
    1. The path is a posix path, or a Windows path on a Windows system
    2. The directory can be created and is writable
    ...
    """
    os_name = os.name
    is_drive_letter_path = re.match(r"^[a-zA-Z]:", path)
    
    if is_drive_letter_path and (os_name != "nt"):
        return False
    try:
