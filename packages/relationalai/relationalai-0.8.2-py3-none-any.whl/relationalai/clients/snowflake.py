...
from typing import Any, Dict, Iterable, Optional, Tuple, List, Literal, cast
...
class Resources(ResourcesBase):
    ...
    def poll_use_index(
        ...
        source_info = self._get_source_info(sources)
    ...
    def get_export_code(self, params: ExportParams):
        ...
        source_references = self._get_source_references()
        source_relations = [
            "'" + table_name.replace(".", "_").lower() + "'"
            for (_, _, table_name) in self.source_elements
        ]
        python_code = process_jinja_template(
            ...
            py_inputs=py_inputs,
            py_outs=py_outs,
            source_references=", ".join(source_references),
            source_relations=", ".join(source_relations),
    ...
    def create_export(self, params: ExportParams):
        source_info = self._get_source_info(self.sources)
        params.sources = self._get_source_elements(source_info)
        ...
        if not self.config.get("use_graph_index", USE_GRAPH_INDEX):
            ...
            model = params.proc_database
            engine_name = params.engine
            program_span_id = debugging.get_program_span_id()
            self.poll_use_index(app_name, self.sources, model, engine_name, program_span_id)
    ...
    def _get_source_elements(self, source_info: dict[str, SourceInfo]):
        app_name = self.get_app_name()
        missing_sources = []
        invalid_sources = {}
        source_elements = []
        for source, info in source_info.items():
            if info['type'] is None:
                ...
                source_elements.append((app_name, info['type'], source))
    ...
    def _get_source_references(self, source_info: Optional[dict[str, SourceInfo]] = None):
        if source_info is not None:
            source_elements = self._get_source_elements(source_info)
        else:
            source_elements = self.source_elements
        return [f"{app_name}.api.object_reference('{object_type}', '{table_name}')" for (app_name, object_type, table_name) in source_elements]
