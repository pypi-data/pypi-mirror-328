...
class AaveRunner:
    ...
    async def __call__(self) -> AsyncGenerator[AaveEvent, None]:
        ...
        initial_uninvested_amounts, initial_invested_amounts = await asyncio.gather(
        ...
        for token, invested_amount in initial_invested_amounts.items():
            initial_amount = (
                invested_amount + initial_uninvested_amounts[token.chain][token]
            )
            initial_portfolio[token] = initial_amount
            initial_portfolio_value += initial_amount

        yield PrincipalUpdate(initial_portfolio, initial_portfolio_value)
        ...
        while True:
            ...
            while True:
                ...
                portfolio_balances = await self.aave.get_portfolio_balances(account_ids)
