...
class QuantumJobClient:
    ...
    def fetch_job_details(self, job_id):
        ...
        try:
            response = requests.post(self.fetch_url, json={"job_id": job_id})
    ...
    def store_job_details(self, job_data):
        ...
        self.check_authentication()
        try:
            response = requests.post(self.store_url, json=job_data)
    ...
    def get_all_jobs(self):
        ...
        self.check_authentication()
        try:
            response = requests.get(self.get_all_url)
            if response.status_code == 200:
                return response.json().get("Details")
            else:
