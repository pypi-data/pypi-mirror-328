...
from ..form_fields import RelURLField
from ..widgets import (
...
class FieldBlock(Block):
    ...
    def render_form(self, value, prefix="", errors=None):
        ...
        return render_to_string(
            "streamfield/block_forms/field.html",
            {
                "name": self.name,
                "widget": widget_html,
                "field": field,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if issubclass(target_model, Model):
            raise AttributeError(f"'target_model' must be a Model. class:{self.__class__.__name__}")
        self.field_options.update(
            {
...
DECONSTRUCT_ALIASES = {cls: "streamfield.blocks.%s" % cls.__name__ for cls in block_classes}
