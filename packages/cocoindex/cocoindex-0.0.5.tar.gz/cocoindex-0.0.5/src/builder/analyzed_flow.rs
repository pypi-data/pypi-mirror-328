...
use crate::{
    ...
    service::error::{shared_ok, ApiError, SharedError, SharedResultExt},
    setup,
};
use anyhow::Result;
...
impl AnalyzedFlow {
    pub async fn from_flow_instance(
        ...
        let execution_plan_fut = async move {
            shared_ok(Arc::new(
        ...
        let result = Self {
            ...
            data_schema,
            desired_state,
            execution_plan: Some(execution_plan_fut),
            // TODO: Check the setup status after we can apply setup changes.
            //   execution_plan: if setup_status_check.is_up_to_date() {
            //       Some(execution_plan_fut.await?)
            //   } else {
            //       None
            //   },
        };
    ...
    pub async fn get_execution_plan(&self) -> Result<Arc<plan::ExecutionPlan>> {
        let execution_plan = self
            .execution_plan
            .as_ref()
            .ok_or_else(|| api_error!("Flow setup is not up to date"))?
            .clone()
            .await
            .std_result()?;
        Ok(execution_plan)
