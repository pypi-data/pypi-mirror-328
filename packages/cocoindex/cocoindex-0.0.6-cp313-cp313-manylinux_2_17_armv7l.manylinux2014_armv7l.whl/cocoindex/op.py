...
from typing import get_type_hints, Protocol, Any, Callable
...
class OpSpec(Protocol):
    """Spec for any operation (source, transform, etc.)."""

    _op_category: OpCategory

    def kind(self) -> str:
        """The kind of the operation."""

    def spec(self) -> dict[str, Any]:
        """The spec of the operation."""


class _SimpleOpSpec(OpSpec):
    """Base class for ops supported by the native engine (Rust)."""
...
def spec_class(category: OpCategory) -> Callable[[type], type]:
    """Decorate a class to provide an op spec."""

    def _decorator(cls: type) -> type:
        data_cls: type = dataclass(cls)
        class _inner(data_cls, _SimpleOpSpec):
            _op_category = category

        _inner.__name__ = cls.__name__
        return _inner
