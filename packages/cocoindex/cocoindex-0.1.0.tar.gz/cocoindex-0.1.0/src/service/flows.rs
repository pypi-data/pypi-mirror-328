...
pub async fn evaluate_data(
    ...
    let execution_plan = fl.get_execution_plan().await?;
    let data_builder = if let Some(key) = query.key {
        let key_field = collection_schema
            .key_field()
            .ok_or_else(|| api_error!("field {} does not have a key", query.field))?;
        let key = value::KeyValue::from_strs(key, &key_field.value_type.typ)?;
        evaluator::evaluate_source_entry(
            &execution_plan,
            field_idx as u32,
            &schema,
            std::iter::once(key),
...
pub async fn build_index(
    ...
    let fl = &lib_context.with_flow_context(&flow_name, |ctx| ctx.flow.clone())?;
    let execution_plan = fl.get_execution_plan().await?;
    indexer::build_index(&execution_plan, &fl.data_schema, &lib_context.pool).await?;
    Ok(Json(()))
}
