...
pub async fn index_source_entry<'a>(
    plan: &ExecutionPlan,
    source_op_idx: u32,
...
async fn index_source(
    ...
    for existing_key_json in existing_keys_json.into_iter() {
        ...
        all_keys_set.insert(existing_key.as_key()?);
    }

    join_all(all_keys_set.into_iter().map(|key| async move {
        let result = index_source_entry(plan, source_op_idx, schema, &key, pool).await;
        if let Err(e) = result {
...
pub async fn build_index(
    ...
    join_all(
        (0..plan.source_ops.len())
            .into_iter()
            .map(|source_op_idx| async move {
                let result = index_source(plan, source_op_idx as u32, schema, pool).await;
                if let Err(e) = result {
                    error!("Error indexing source {}: {}", source_op_idx, e);
                }
            }),
    )
    .await;
