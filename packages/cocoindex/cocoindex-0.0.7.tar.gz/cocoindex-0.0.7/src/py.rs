...
use crate::ops::{interface::ExecutorFactory, py_factory::PyTransformFactory, register_factory};
use crate::server;
...
#[pyfunction]
fn start_server(py: Python<'_>) -> PyResult<()> {
    py.allow_threads(|| -> anyhow::Result<()> {
        let lib_context =
            get_lib_context().ok_or_else(|| api_error!("Cocoindex is not initialized"))?;
        let server = lib_context
            .runtime
            .block_on(server::init_server(lib_context.clone()))?;
...
#[pyfunction]
fn register_transform_factory(name: String, py_transform_factory: Py<PyAny>) -> PyResult<()> {
    let factory = PyTransformFactory {
        py_transform_factory,
    };
    register_factory(name, ExecutorFactory::SimpleOp(Arc::new(factory))).into_py_result()
}
...
impl SimpleSemanticsQueryHandler {
    ...
    pub fn new(
        ...
        let handler = lib_context
            .runtime
            .block_on(py.allow_threads(|| {
                query::SimpleSemanticsQueryHandler::new(
                    flow.0.clone(),
                    target_name,
...
fn cocoindex_engine(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(register_transform_factory, m)?)?;
