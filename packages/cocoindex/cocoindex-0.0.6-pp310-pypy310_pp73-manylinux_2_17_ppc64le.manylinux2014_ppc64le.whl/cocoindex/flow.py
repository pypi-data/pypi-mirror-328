...
class DataSlice:
    ...
    def transform(self, spec: op.OpSpec, /, name: str | None = None) -> DataSlice:
        """
        ...
        return _create_data_slice(
            ...
            lambda target_scope, name: self._engine_flow_builder.transform(
                ...
                args,
                target_scope,
                self._field_name_builder.build_name(name, prefix=_to_snake_case(spec.kind())+'_'),
            ),
            name)
...
class DataCollector:
    ...
    def export(self, name: str, target_spec: op.OpSpec, /, *,
        ...
        self._flow_builder.internal_flow_builder.export(
            name, target_spec.kind(), json.dumps(target_spec.spec()), json.dumps(index_options), self._engine_data_collector)
...
class FlowBuilder:
    ...
    def add_source(self, spec: op.OpSpec, /, name: str | None = None) -> DataSlice:
        """
...
def _create_lazy_flow(name: str | None, fl_def: Callable[[FlowBuilder], None]) -> Flow:
    """
...
def add_flow_def(name: str, fl_def: Callable[[FlowBuilder], None]) -> Flow:
