...
from .types import McpServer, McpCategory, Category
...
def save_mcp_sever(server_data: McpServer) -> Optional[McpServer]:
    ...
    Args:
        server_data (McpServer): A dictionary containing server information with the following fields:
            ...
            - author (str): Server author username
            - package_name (str): Python package name (must be a valid Python package name)
            - slug (str): URL-friendly identifier (lowercase, hyphens only)
            - description (str): Server description
            - category (str): Server category name (must match existing category)
            - features (List[str]): List of server feature strings
            - github_url (str): GitHub repository URL (must be a valid GitHub URL)
