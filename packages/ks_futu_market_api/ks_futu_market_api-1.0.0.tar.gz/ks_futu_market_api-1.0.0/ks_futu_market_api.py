...
from ks_trade_api.utility import extract_vt_symbol, generate_vt_symbol, is_stock_like
...
SUBTYPE_KS2MY = {
    ...
    KsSubscribeType.K_MINUTE_5: SubType.K_5M,
    KsSubscribeType.K_MINUTE_15: SubType.K_15M,
    KsSubscribeType.K_MINUTE_30: SubType.K_30M
}
...
INTERVAL_KS2MY = {
    ...
    KsInterval.MINUTE_5: KLType.K_5M,
    KsInterval.MINUTE_15: KLType.K_15M,
    KsInterval.MINUTE_30: KLType.K_30M
}
...
class KsFutuMarketApi(BaseMarketApi):
        ...
        if ret == RET_OK:
            if df:
                ...
                for index, quote in data.iterrows():
                    ...
                    tz = CHINA_TZ if exchange == KsExchange.SEHK else US_EASTERN_TZ
                    dt = tz.localize(parse(f'{quote.data_date} {quote.data_time}')).astimezone(CHINA_TZ)
    ...
    def query_history_n(
        ...
        data_k['volume'] = data_k.volume.astype(str).apply(Decimal)
        data_k['turnover'] = data_k.turnover.astype(str).apply(Decimal)
        data_k['open'] = data_k.open.astype(str).apply(Decimal)
        data_k['high'] = data_k.high.astype(str).apply(Decimal)
        data_k['low'] = data_k.low.astype(str).apply(Decimal)
        data_k['close'] = data_k.close.astype(str).apply(Decimal)
        ret_df = data_k[['vt_symbol', 'datetime', 'interval', 'volume', 'turnover', 'open', 'high', 'low', 'close']]
        return KS_RET_OK, data_k[['vt_symbol', 'datetime', 'interval', 'volume', 'turnover', 'open', 'high', 'low', 'close']]
