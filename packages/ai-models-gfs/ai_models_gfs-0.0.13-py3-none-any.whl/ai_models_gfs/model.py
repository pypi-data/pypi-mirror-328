        ...
        for file in self.download_files:
            ...
            if not os.path.exists(asset):
                ...
                try:
                    ...
                    LOG.error("multiurl TypeError: %s", str(te))
                    LOG.warning("Trying urllib instead...")
                    try:
                        urllib.request.urlretrieve(download_url, temp_download)
                    except Exception as e2:
                        LOG.error("Both multiurl and urllib failed: %s", str(e2))
                        continue  # Skip this file if both fail
