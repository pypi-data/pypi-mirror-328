...
def arr2im(arr, vmin=False, vmax=False, pwr=1.0, cmap=None, gamma=1.0):
    ...
    mmax = np.percentile(arr2, 99) if vmax is False else vmax
...
class MyWindow(QMainWindow):
    ...
    def load_shm(self):
        ...
        self.myslice = 0
        if self.naxis == 3:
            self.myDRK = np.zeros_like(self.mySHM.get_data()[0])
        else:
            ...
            if self.live_counter < mycntr:
                if self.naxis == 2:
                    ...
                    if self.required_slice != -1:
                        ...
                        self.data_img = self.mySHM.get_data(False, True)[
                            mycntr % self.mySHM.mtdata["size"][0]]
