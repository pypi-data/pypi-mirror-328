...
def main():
    ...
    if os.path.isdir(pkg_path) and any('isaac' in d for d in os.listdir(pkg_path)):
        ...
        isaac_sim_path = input("If you want to use a different one, please type in the path containing isaac-sim.sh here (press enter to skip) >>> ")
        ...
        isaac_sim_path = input("If you have already installed it in a custom location, please type in the path containing isaac-sim.sh here >>> ")

    # Check if isaac-sim.sh exists
    while not os.path.isfile(os.path.join(isaac_sim_path, 'isaac-sim.sh')):
        isaac_sim_path = input(f"isaac-sim.sh not found in \033[4m{isaac_sim_path}\033[0m! Make sure you have entered the correct path >>> ")
    ...
    print(f"\nGRUtopia successfully installed! Please run \033[4mconda deactivate && conda activate {os.path.basename(conda_prefix)}\033[0m to activate the environment.\n")
