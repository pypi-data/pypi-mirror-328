...
import os
import argparse
from GFHunter import preprocessing
from GFHunter import transcriptome_based_detection
from GFHunter import crossing_reference_based_refinement
...
def sub_cmd_detect(args):
    ...
    cnums = (args.min_read_length, args.max_exon_boundary, args.overlap_precent, args.min_clustering_length, args.trans_based_align_type)
    result_file, POA_file, supporting_reads = transcriptome_based_detection.main(vars(args)['<readfile>'], vars(args)['<indexdir>'], args.middlefile, args.threads, args.print_middle_output, cnums)
    #step2
    print('-' * 50)
    print('CROSS-REFERENCING-BASED FUSION REFINEMENT')
    print('-' * 50)
    POA_file = args.middlefile + 'step1.POA_result.fasta'
    crossing_reference_based_refinement.main(args.output + '.csv', POA_file, result_file, vars(args)['<indexdir>'], args.middlefile, args.threads, args.print_middle_output, args.least_support_reads)
