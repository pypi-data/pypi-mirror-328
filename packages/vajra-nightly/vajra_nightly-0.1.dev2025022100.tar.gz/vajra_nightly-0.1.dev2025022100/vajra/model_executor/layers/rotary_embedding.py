...
from vajra._kernels_C import rotary_embedding
from vajra._model_executor_C import RotaryEmbedding as RotaryEmbeddingC
...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if self.use_native_execution_backend:
            self.native_handle = RotaryEmbeddingC(
                head_size,
                rotary_dim,
    ...
    def forward(
        ...
        if self.use_native_execution_backend:
            self.native_handle.forward(positions, query, key)
            return query, key
...
def _yarn_linear_ramp_mask(
    low: float, high: float, dim: int, dtype: torch.dtype, device: str
) -> torch.Tensor:
    if low == high:
...
class YaRNScalingRotaryEmbedding(RotaryEmbedding):
    ...
    def _compute_inv_freq(self, base: Union[int, float]) -> torch.Tensor:
        ...
        inv_freq_interpolation = 1.0 / (base * pos_freqs)
