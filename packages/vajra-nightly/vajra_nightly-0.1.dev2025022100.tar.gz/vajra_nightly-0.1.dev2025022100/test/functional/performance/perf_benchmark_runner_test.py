...
def performance_comparison_session() -> Generator[Any, None, None]:
    """
    ...
    try:
            ...
            if not most_recent_run_id:
                ...
                if found_artifact:
                    ...
                    if not prev_json_files:
                        log_and_print_warning(
                            "No JSON file found in baseline artifact."
                        )
                    else:
                        prev_json_path = prev_json_files[0]
                        baseline_metrics = load_json_file(prev_json_path)
                        log_and_print_info(f"Baseline metrics loaded.")
                else:
        ...
        if config.overwrite_baseline:
            log_and_print_warning(
                f"Error retrieving baseline metrics, but overwriting baseline for branch {config.baseline_branch}."
            )
        else:
            log_and_print_error(f"Error during baseline artifact retrieval: {e}")

    yield baseline_metrics  # test suite execution saves new performance results
    ...
    if baseline_metrics:
        try:
            ...
            log_and_print_info(f"Loaded new metrics for comparison.")
            ...
            if perf_regression:
                if not config.overwrite_baseline:
                    log_and_print_error("Performance regression detected!")
                    pytest.fail(
                        "Performance regression detected in one or more metrics. Check output for details."
                    )
                else:
                    log_and_print_warning(
                        f"Performance regression detected, but overwriting baseline for branch {config.baseline_branch}."
                    )

        except Exception as e:
            console.print_exception(show_locals=True)
            log_and_print_error(
                f"Error during performance comparison and result saving: {e}"
            )
