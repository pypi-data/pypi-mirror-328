...
def pytest_runtest_logreport(report: TestReport) -> None:
    ...
    if report.when == "call":
        if report.outcome == "passed":
            if _pytest_config.option.verbose > 0:
...
def pytest_terminal_summary(terminalreporter) -> None:
    ...
    if markexpr == "performance":
        console.print(
            "\nNote: For the purposes of this report, a passing performance test "
            "only means that it run correctly, not that there wasn't a performance regression."
        )
