...
class DataSeries:
    ...
    def __init__(
        ...
        self.data_series: deque[tuple[Any, float]] = deque()
    ...
    def consolidate(
        ...
        for x, y in self.data_series:
            res[x].append(y)
        self.data_series = deque((x, sum(y) / len(y)) for x, y in res.items())

        # sort by x
        self.data_series = deque(sorted(self.data_series, key=lambda x: x[0]))
    ...
    def print_series_stats(
        self, df: pd.DataFrame, plot_name: str, y_name: Optional[str] = None
    ) -> None:
    ...
    def plot_step(
        ...
        if x_cumsum:
            df[self.x_name] = df[self.x_name].cumsum()
        ...
        fig = px.line(
            ...
            y=self.y_name,
            markers=True,
            labels={"x": y_axis_label, "y": x_axis_label},
        )
        ...
        if wandb.run:
            ...
            wandb_df = wandb_df.rename(
                columns={self.y_name: y_axis_label, self.x_name: x_axis_label}
            )
    ...
    def plot_cdf(
        self, path: str, plot_name: str, y_axis_label: Optional[str] = None
    ) -> None:
