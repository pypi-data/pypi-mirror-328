...
class Base(Device):
    ...
    _param_map = {
        ...
        "channel_1": {
            ...
            "input_range_dBV": ("I1RG{Q}", lambda x: int(x.split(",")[0]), int),
            "auto_range": ("A1RG{Q}", ParamProxy.decode_bool, int),
            "auto_range_up_only": ("I1AR{Q}", lambda x: x == "0", lambda x: int(not x)),
        },
        "channel_2": {
            ...
            "input_range_dBV": ("I2RG{Q}", lambda x: int(x.split(",")[0]), int),
            "auto_range": ("A2RG{Q}", ParamProxy.decode_bool, int),
            "auto_range_up_only": ("I2AR{Q}", lambda x: x == "0", lambda x: int(not x)),
        },
        "input_auto_offset": ("IAOM{Q}", ParamProxy.decode_bool, int),
        "display_A": {
...
class FFT(Base):
    ...
    _average_mode_map = ParamProxy.CodeMap({"none": "0", "vector": "1", "rms": "2", "peak": "3"})
    ...
    _param_map = Base._param_map | {
        ...
        "start_Hz": (["FSTR{Q} 0,", "FSTR{Q} 2,"], float, ParamProxy.ValSet({0, 0.0})),  # TODO verification
        "stop_Hz": (["FSPN{Q} 0,", "FSPN{Q} 2,"], float, _verify_fft_span),  # TODO verification
        "average": (["FAVG{Q} 0,", "FAVG{Q} 2,"], ParamProxy.decode_bool, int),  # TODO must always be true for FFT
        "average_mode": (["FAVM{Q} 0,", "FAVM{Q} 2,"], *_average_mode_map.de_en_code),
        ...
        **kwargs,
        ...
        Args:
            ...
            channel_1 (dict, optional): Channel 1 settings. Passed in **kwargs.
                ...
                - `input_range_dBV` _float, optional_ - input range in dBV.
                - `auto_range` _bool, optional_ - auto range enabled.
                - `auto_range_up_only` _bool, optional_ - only increase range.
            channel_2 (dict, optional): Channel 2 settings. Passed in **kwargs.
            input_auto_offset (bool, optional): Input auto offset. Passed in **kwargs.
            ...
            average_mode (Literal["none", "vector", "rms", "peak"], optional): Averaging mode. Passed in **kwargs.
    ...
    def measure_stitched(
        ...
        **kwargs,
        ...
        """
        stop_Hz_parsed = FFT._verify_fft_span(stop_Hz)
