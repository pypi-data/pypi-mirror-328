    ...
    def save_zip(
        ...
        *,
        ...
        with ZipFile(filename, "w") as archive:
            with TextIOWrapper(archive.open(self._make_filename("tsv"), "w"), encoding="utf-8") as f:
                self._serialize_to(f, delim)
            for name, attachment in self.attachments.items():
                with archive.open(name, "w", encoding="utf-8") as f:
                    f.write(attachment)
        return filename
    ...
    def load(filename: Path, delim: str = None) -> "Measurement":
        ...
        if filename.suffix == ".zip":
            with ZipFile(filename, "r") as archive:
                with TextIOWrapper(archive.open(archive.namelist()[0], "r"), encoding="utf-8") as f:
