...
class ClusterStatus(BaseModel):
    """
    Per-block temperature readings for a single cluster.
    """  # noqa: E501

    id: Optional[StrictInt] = Field(
        default=None, description="The cluster's ID relative to a carrier board."
    )
    m0: float = Field(alias="/M0")
    m1: float = Field(alias="/M1")
    u: float = Field(alias="/U")
    c: float = Field(alias="/C")
    i: float = Field(alias="/I")
    sh: float = Field(alias="/SH")
    __properties: ClassVar[List[str]] = ["id", "/M0", "/M1", "/U", "/C", "/I", "/SH"]
