...
class ModuleDependenciesRewriteUtil:
    ...
    def rewrite(self, reWriteOnlyChange=False):
        ...
        packageConfig = self._recursionDeps(depsJson["packages"])  #  直接。间接依赖的依赖树维护
        packageConfigJson = self._recursionDirectDeps(depsJson["packages"])  # 直接依赖库的依赖树维护
        ...
        for index, moduleName in enumerate(depsSortedList):
            ...
            for node in packageConfig[moduleName]:
                if self.moduleNameList.__contains__(node.name) and node.type.indirect and node.name != moduleName:
