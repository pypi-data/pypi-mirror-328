...
from .sinks import MT5UserSink, MT5DealSink, ServerInfo, MT5PositionSink, MT5OrderSink, MT5SummarySink, MT5PriceSink, MT5BookSink
...
class MT5ConnectionPools:
    ...
    def __init__(self, servers=None):
        ...
        self._price_sinks: Dict[int, MT5PriceSink] = {}
    ...
    def _get_or_create_sinks(self, server_config: MT5ServerConfig) -> tuple[MT5UserSink, MT5DealSink, MT5PositionSink, MT5OrderSink, MT5SummarySink, MT5PriceSink, MT5BookSink]:
        ...
        if server_config.id not in self._user_sinks:
            ...
            self._price_sinks[server_config.id] = MT5PriceSink(server_info)
            self._book_sinks[server_config.id] = MT5BookSink(server_info)
            
        return self._user_sinks[server_config.id], self._deal_sinks[server_config.id], self._position_sinks[server_config.id], self._order_sinks[server_config.id], self._summary_sinks[server_config.id], self._price_sinks[server_config.id], self._book_sinks[server_config.id]
        ...
        try:
            ...
            for server_id, manager in self._demo_pools.items():
                if manager._connected:
                    ...
                    user_sink, deal_sink, position_sink, order_sink, summary_sink, price_sink, book_sink = self._get_or_create_sinks(config)
                    ...
                    manager.setup_price_sink(price_sink)
    ...
    def add_price_callback(self, event: str, callback: Callable):
        """
        Add callback for price events
        
        The callback should accept two parameters:
        - data: The event data from MT5
        - server_info: ServerInfo object containing server id, name, and type
        """
        print(f"Adding price callback for {event}")
        # Add callback to all price sinks
        for sink in self._price_sinks.values():
            sink.add_callback(event, callback)
    ...
    def connect_server(self, server_id: int) -> bool:
        ...
        try:
            ...
            user_sink, deal_sink, position_sink, order_sink, summary_sink, price_sink, book_sink = self._get_or_create_sinks(server)
            ...
            connection.setup_price_sink(price_sink)
