...
from ..sinks import MT5UserSink, MT5DealSink, MT5PositionSink, MT5OrderSink, MT5SummarySink, MT5PriceSink, MT5BookSink
...
class MT5ConnectionManager:
  def __init__(self, server_config: MT5ServerConfig, data_folder: Optional[str] = None):
    ...
    self._price_sink: Optional[MT5PriceSink] = None
  ...
  def setup_price_sink(self, sink: MT5PriceSink) -> bool:
    """Setup price event sink"""
    if not self._connected:
      raise MT5ConnectionError("Must be connected to setup sinks")
    
    if self._price_sink:
      self._manager.PriceUnsubscribe(self._price_sink)

    self._price_sink = sink
    return self._manager.PriceSubscribe(sink)
