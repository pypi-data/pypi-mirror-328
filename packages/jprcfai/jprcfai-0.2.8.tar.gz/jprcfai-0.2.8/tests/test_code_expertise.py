from jprcfai.complex_coding_tasks import (
...
def test_modify_single_file(capsys):
    ...
    requested_changes = (
        "Modify the arguments used in the main function from (5, 6) to (10, 21)"
    )
    new_file_code = apply_changes_to_code(file_code, requested_changes, "low", False)
...
def test_big_task(capsys):
    with capsys.disabled():
        temp_project_dir = create_temporary_directory_from(
            "tests/files/initial_project"
        )
        ...
        execute_major_coding_task(
            ...
            "The team calculated the expect output for the test command and after the changes it should be (60, 37)",
            temp_project_dir,
