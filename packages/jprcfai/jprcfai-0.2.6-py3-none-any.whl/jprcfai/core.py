...
def load_prompt_cache() -> Dict[str, CacheEntry]:
    ...
    and purges expired entries (older than 24 hours).
    """
    ...
    for key in list(cache.keys()):
        ...
        if not isinstance(entry, dict) or "timestamp" not in entry:
            del cache[key]
        elif now - entry["timestamp"] > 86400:  # 24 hours = 86400 seconds
            del cache[key]
    return cache
...
def set_cached_value(key_str: str, value: str) -> None:
    """
    Sets the value in the cache with the current timestamp.
    """
    global PROMPT_CACHE
    PROMPT_CACHE[key_str] = {"value": value, "timestamp": time.time()}
    save_prompt_cache(PROMPT_CACHE)
...
def unroll_prompt(prompt: str, visited: Optional[Set[Any]] = None) -> str:
    ...
    3. [#PLACEHOLDER_LOAD_CONTENT_FROM_SCRIPT_EXECUTION ([<full_command>], <is_custom_dir>)]
       -> Executes the provided script command, captures its output, and recursively
          ...
          (jprcfai) and its path is resolved relative to this file.

    Caching Enhancement:
    This function now uses a persistent cache stored in the user cache directory for 'jprcfai'.
    Each cached item is stored with a timestamp and is considered expired 24 hours after
    the moment of caching. Expired items are not used and are removed from the cache.
    """
    global PROMPT_CACHE
