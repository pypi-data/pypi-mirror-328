...
class StepRunner:
    ...
    def _do_execute_action_install(self, step: RunbotStepConfig) -> int:
        ...
        return rc + self._execute_to_filters(step, log_filters, warnings_list)
    ...
    def _do_execute_action_tests(self, step: RunbotStepConfig) -> int:
        ...
        output_dir = self.env.abs_curr_dir.joinpath(step.unittest_output)
        ...
        if step.coverage:
            _logger.info("Save Coverage recording...")
            coverage_watcher.html_report()
            coverage_watcher.xml_report()
            coverage_watcher.save()
        return abs(rc) + self._execute_to_filters(step, log_filters, warnings_list)
    ...
    def _execute_to_filters(
        ...
        if log_filters or warnings_list:
            ...
            res = runbot_run_test.execute_test_after_odoo(
                ...
                test_runner=runbot_run_test.get_test_runner(self.env.abs_curr_dir.joinpath(step.unittest_output)),
            )
    ...
    def setup_warning_filter(self, warning_filters: RunbotPyWarningsFilter) -> None:
