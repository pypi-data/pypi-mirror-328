...
class TestStep2(common.TransactionCase):
    ...
    def test_logger1(self):
        ...
        """
        self.env["runbot.logger"].log_msg("My info message")
        self.env["runbot.logger"].log_msg("My warning message", level=logging.WARNING)
        self.env["runbot.logger"].log_msg("My critical message", level=logging.CRITICAL)
        self.env["runbot.logger"].log_msg("My error message", level=logging.ERROR)
    ...
    def test_logger2(self):
        ...
        with self.assertLogs() as logs:
            self.env["runbot.logger"].log_msg("My message1", level=logging.WARNING, logger_name=None)
        ...
        with self.assertLogs("odoo.addons.runbot_tester.models.runbot_tester") as l2:
            self.env["runbot.logger"].log_msg("My message2", level=logging.ERROR)
        ...
        with self.assertLogs("custom.logger") as l3:
            self.env["runbot.logger"].log_msg("My message3", level=logging.CRITICAL, logger_name="custom.logger")
    ...
    def test_logger3(self):
        ...
        """
        self.env["runbot.logger"].log_msg("message INFO", level=logging.INFO)
        self.env["runbot.logger"].log_msg("message DEBUG", level=logging.DEBUG)
        if odoo.release.version >= "14.0":
            # Logger level RUNBOT activated in odoo 14 and more
            self.env["runbot.logger"].log_msg("message RUNBOT", level=logging.RUNBOT)
