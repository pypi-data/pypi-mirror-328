...
from odoo_runbot.runbot_env import RunbotExcludeWarning, RunbotStepConfig
...
def test_log_setup_1_step():
    ...
    with log_handler.start_warning_log_watcher(step) as log_filters:
        ...
        assert len(global_handler.filters) == 1, "One warnings rule in step eq 1 filter in log handler"
        assert isinstance(global_handler.filters[0], ExcludeWarningFilter), "contains only ExcludeWarningFilter"
        assert global_handler.filters[0].exclude == warn_rule
        assert not global_handler.filters[0].name, "No pre-filter on the logger name"

    log_filters()
    assert not log_handler.get_handler().filters, "After stop no more warnings rule registered"
...
def test_log_setup_2_step():
    ...
    with log_handler.start_warning_log_watcher(step2) as extractor2:
        ...
        global_handler = log_handler.get_handler()
        assert len(global_handler.filters) == 2, "2 filter in step 2"
        with log_handler.start_warning_log_watcher(step1) as extractor1:
            assert len(global_handler.filters) == 3, "1 filter in step 1 + 2 filters in step2"
            assert all(isinstance(_filter, ExcludeWarningFilter) for _filter in global_handler.filters), (
        ...
        assert len(log_handler.get_handler().filters) == 2, "Still have the 2 filter from step2"
    ...
    assert len(filter_warnings2) == 3, "2 Filter in step2 + 1 for the default"
...
def test_log_warnings_as_test():
    ...
    filters = extractor()
    test_result = execute_test_after_odoo(filters)
    assert test_result.wasSuccessful(), "Should catch the 2 log line above"
    assert test_result.testsRun == 2, "My Step contains 1 warnings, and the default catch all"
...
def test_start_and_stop_watching():
    ...
    with log_handler.start_warning_log_watcher(step) as extractor:
        ...
        assert len(handler_post_start.filters) == 1
    extractor()
...
def test_no_rule_default_catcher():
    ...
    result = execute_test_after_odoo(log_filter)
...
def test_catch_critical():
    ...
    assert log_handler.get_handler() not in logging.root.handlers, "The handler is not activated"
    with log_handler.start_warning_log_watcher(step) as extractor:
        logging.getLogger("my_logger").critical("critical msg")
        logging.getLogger("my_logger").error("error msg")
        logging.getLogger("my_logger").exception("exception msg")
        logging.getLogger("my_logger").warning("warning msg")
        logging.getLogger("my_logger").debug("debug msg")
        logging.getLogger("my_logger").info("info msg")

    log_filter = extractor()
    result = execute_test_after_odoo(log_filter)
    assert not result.wasSuccessful(), "Should catch the 2 log line above"
    assert result.testsRun == 2, "Default catcher + rule"
