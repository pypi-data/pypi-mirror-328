...
class RunbotStepConfig:
    ...
    unittest_output: str = dataclasses.field(default="test_results")
...
class RunbotToolConfig:
    ...
    def load_from_toml_data(cls, runbot_data: dict) -> RunbotToolConfig:
        ...
        """
        global_warnings = log_filter_by_odoo_version() + [
            RunbotExcludeWarning(
                name=data.get("name", f"Global Logger Filter {_idx}"),
                regex=data["regex"],
                logger=data.get("logger"),
                min_match=data.get("min-match"),
                max_match=data.get("max-match"),
            )
            for _idx, data in enumerate(runbot_data.get("log-filters", []))
        ]
        ...
        for _idx_step, step_data in enumerate(runbot_data.get("step")):
            ...
            log_filter = []
            for _idx_log_filter, data in enumerate(step_data.get("log-filters", []), start=len(global_warnings) + 1):
                _data = data
            ...
            step_obj = RunbotStepConfig(
                ...
                unittest_output=step_data.get("unittest-output", runbot_data.get("unittest-output")),
                ...
                log_filters=global_warnings + log_filter,
            )
