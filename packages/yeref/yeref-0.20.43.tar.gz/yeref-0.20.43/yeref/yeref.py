...
async def return_file_id(BOT_TID, FILE_NAME, MSG_TYPE, IS_LINK, BASE_D, EXTRA_D, MEDIA_D):
    ...
    try:
        ...
        while cnt > 0:
            try:
                sql = "SELECT OWNER_TID, BOT_STATUS, BOT_TOKEN, BOT_VARS FROM \"BOT\" WHERE BOT_TID=$1"
                data = await db_select_pg(sql, (int(BOT_TID),), BASE_D)
                if not len(data): return
                OWNER_TID, BOT_STATUS, BOT_TOKEN, BOT_VARS = data[0]
                ...
                if 'restricted' in str(e).lower():
                    ...
                    text = l_tools_has_restricted[BOT_LZ]
...
async def facade_get_fid(bot, chat_id, KEYS_JSON, BOT_TID, dst, msg_type, IS_LINK, BASE_D, EXTRA_D, MEDIA_D):
    ...
    FILE_NAME = None
    try:
        file_id, file_id_note, file_type, FILE_NAME, IS_LINK, BOT_TOKEN = await return_file_id(BOT_TID, dst, msg_type, IS_LINK,
