...
async def train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, ENT_TYPE, EXTRA_D, BASE_D, lz, prompt='',
                            BOT_TOKEN_=None):
...
async def pst_gen_ent2(bot, chat_id, lc, lz, POST_TYPE, POST_MEDIA, ENT_TID, PROJECT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D, EXTRA_D, BASE_D, prompt='', ENT_TOKEN_=None):
    result = dst = None
    try:
                            ...
                            for api_key in provider_keys:
                                try:
                                    ...
                                    if provider_name == 'prodia':
                                        ...
                                        attempts = 8
                                        status_url = url + f'text2image/status/{job_id}'
                                        await asyncio.sleep(2)
        ...
        for item in data:
            try:
                ...
                if USER_ISPAID == 1 and USER_DTPAID and (
                    ...
                    tzinfo=timezone.utc)).days > 31:
                    result.append(
                        item)  # else:  #     sql = "UPDATE USER SET USER_USERNAME=$1, USER_FULLNAME=$2 WHERE USER_TID=$3"  #     await db_change(sql, (get_.username, get_.full_name, USER_TID,), BASE_D)
            except TelegramRetryAfter as e:
                logger.info(log_ % f"TelegramRetryAfter {e.retry_after}")
                await asyncio.sleep(e.retry_after + 1)
            except Exception as e:
