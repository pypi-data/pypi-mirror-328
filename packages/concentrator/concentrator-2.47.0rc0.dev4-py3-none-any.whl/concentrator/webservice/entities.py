...
class AdaptationProgramDocInfo(NotBlankUnicode):
    class Annotations(NotBlankUnicode.Annotations):
        doc = u'Реквизиты документа, подтверждающего ограничение в здоровье'
...
class ScheduleData(ComplexModelWithNamespace):
    ...
    _type_info = [
        ('ScheduleType', Integer.customize(
            min_occurs=0, max_occurs="unbounded"))
    ]

    class Annotations(ComplexModelWithNamespace.Annotations):
        doc = (u'Контейнер, содержит информацию '
               u'о режимах пребывания')
