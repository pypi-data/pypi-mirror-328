...
class DiagnosticContext:
    ...
    def __exit__(
        ...
        self.collector.logger.debug("Leaving diagnostic context %r", self.__class__._diagnostic_context.get(), stacklevel=3)
...
class DiagnosticCollector(DiagnosticContext):
    ...
    def raise_if_errors(self, stacklevel: int = 1) -> None:
        """
        ...
        if bool(self):
            ...
            self.logger.debug("Raising exception from %r: %s", self, self.errors, stacklevel=stacklevel + 1)
    ...
    def __exit__(
        ...
        if exc_val is None or isinstance(exc_val, DiagnosticException):
            self.raise_if_errors(stacklevel=2)
