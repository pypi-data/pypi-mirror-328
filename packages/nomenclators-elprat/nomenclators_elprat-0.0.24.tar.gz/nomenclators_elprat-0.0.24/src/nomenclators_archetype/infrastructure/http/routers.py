...
class BaseNomenclatorRouter(APIRouter, Generic[M, R]):
    ...
    def _include_default_routes(self):
            ...
            try:
                ...
                return [
                    ...
                    for domain in self.service.list_items(
                        ...
                        filters=param.filter.mapped() if param and param.filter else None,
                    )
                ]
        ...
        async def update_item_by_id(item: SchemaSimpleNomenclatorUpdater = Body(...)) -> R:
            try:
        ...
        async def create_item(item: SchemaSimpleNomenclatorCreator = Body(...)) -> R:
            try:
                return self.mapper.map_from_domain_to_schema(
