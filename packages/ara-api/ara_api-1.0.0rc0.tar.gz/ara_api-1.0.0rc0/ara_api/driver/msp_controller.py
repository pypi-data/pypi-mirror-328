...
class MultirotorControl:
    ...
    def __init__(self, transmitter: Transmitter, logger):
        ...
        self.SENSOR_DATA = {
            ...
            "optical_flow": [
                ...
                0,
                0,
            ],  # quality, flowRate[X], flowRate[Y], bodyRate[X], bodyRate[Y]
        }
            ...
            while True:
                ...
                if dataHandler["state"] == 0:  # sync char 1
                    ...
                    if (
                        ...
                        == MultirotorControl.JUMBO_FRAME_SIZE_LIMIT
                    ):
    ...
    def save2eprom(self):
        ...
        return self.send_RAW_msg(
            MultirotorControl.MSPCodes["MSP_EEPROM_WRITE"], data=[]
        )
    ...
    def set_RX_MAP(self, new_rc_map):
        ...
        return self.send_RAW_msg(
            MultirotorControl.MSPCodes["MSP_SET_RX_MAP"], new_rc_map
        )
    ...
    def set_FEATURE_CONFIG(self, mask):
        ...
        return self.send_RAW_msg(
            MultirotorControl.MSPCodes["MSP_SET_FEATURE_CONFIG"], data
        )
    ...
    def process_recv_data(self, dataHandler):
        ...
        if code == 0:  # code==0 means nothing was received...
            ...
            if not dataHandler["unsupported"]:
                processor = MultirotorControl.__dict__.get(
                    "process_" + MultirotorControl.MSPCodes2Str[code]
                )
    ...
    def process_MSPV2_INAV_OPTICAL_FLOW(self, data):
        self.SENSOR_DATA["optical_flow"][0] = self.readbytes(
            data, size=8, unsigned=True
        )
        self.SENSOR_DATA["optical_flow"][1] = radians(
            self.readbytes(data, size=16, unsigned=False)
        )
        self.SENSOR_DATA["optical_flow"][2] = radians(
            self.readbytes(data, size=16, unsigned=False)
        )
        self.SENSOR_DATA["optical_flow"][3] = radians(
            self.readbytes(data, size=16, unsigned=False)
        )
        self.SENSOR_DATA["optical_flow"][4] = radians(
            self.readbytes(data, size=16, unsigned=False)
        )

    def process_MSP_ODOM(self, data):
        self.SENSOR_DATA["odom"]["position"][0] = (
            self.readbytes(data, size=32, unsigned=False) / 100_000
        )
        self.SENSOR_DATA["odom"]["position"][1] = (
            self.readbytes(data, size=32, unsigned=False) / 100_000
        )
        self.SENSOR_DATA["odom"]["position"][2] = (
            self.readbytes(data, size=32, unsigned=False) / 100_000
        )
