...
from driver import serialize, UDPTransmitter, TCPTransmitter, SerialTransmitter
from protos import api_pb2 as api_pb2
from protos import api_pb2_grpc as api_pb2_grpc
...
class MSPDriverManagerGRPC(api_pb2_grpc.DriverManagerServicer):
    ...
    def __init_logging__(self, log_directory="log"):
        ...
        self.driver_formater = logging.Formatter(
            "%(asctime)s - %(levelname)s - %(message)s"
        )
        self.driver_handler = logging.FileHandler(
            os.path.join(log_directory, "msp_driver.log")
        )
    ...
    def update_data(self):
        ...
        while True:
            ...
            logging.info("[SENSOR_DARA]:" + str(self.msp_controller.SENSOR_DATA))
            time.sleep(1 / 50)
    ...
    async def GetMotorDataRPC(self, request, context):
        ...
        if self.analyzer_state:
            ...
            print("[MOTOR]: " + "Get error, only available with analyzer")
    ...
    async def GetAnalogDataRPC(self, request, context):
        ...
        if self.analyzer_state:
            ...
            print("[ANALOG]: " + "Get error, only available with analyzer")
            self.state_logging.error(
    ...
    async def SendRcDataRPC(self, request, context):
        ...
        if not self.analyzer_state:
            ...
            print("[RCIN]: " + "Send error, not available when the analyzer is on")
