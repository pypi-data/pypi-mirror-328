...
from navigation.nav_fetcher import DataFetcher

from . import AccelPage, GyroPage, MotorPage, AnalogPage
...
class AnalyzerServiceAPP(customtkinter.CTk):
    def __init__(self, csv_dir):
        ...
        self.page_frame.pack(side="left", fill="both", expand=True, padx=20, pady=20)
    ...
    def create_buttons(self):
        motor_button = customtkinter.CTkButton(
            self.button_frame, text="Motor", command=lambda: self.show_page("Motor")
        )
        motor_button.pack(pady=10, padx=10)

        analog_button = customtkinter.CTkButton(
            self.button_frame, text="Analog", command=lambda: self.show_page("Analog")
        )
        analog_button.pack(pady=10, padx=10)

        accel_button = customtkinter.CTkButton(
            self.button_frame, text="Accel", command=lambda: self.show_page("Accel")
        )
        accel_button.pack(pady=10, padx=10)

        gyro_button = customtkinter.CTkButton(
            self.button_frame, text="Gyro", command=lambda: self.show_page("Gyro")
        )
