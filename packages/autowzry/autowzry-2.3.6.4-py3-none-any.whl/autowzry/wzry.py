...
class wzry_figure:
    ...
    def __init__(self, Tool=None):
        ...
        self.对战火球按钮 = Template(r"tpl1740116233645.png", record_pos=(0.032, 0.104), resolution=(960, 540))
...
# 状态判断
    ...
    def 判断对战中(self, 处理=False, acce=False):
        ...
        while initloop or self.判断对战中(处理=False):
            ...
            # 3技能英雄点击1技能
            if max(self.移动端.resolution) != 960:
                self.Tool.existsTHENtouch(self.图片.对战31技能,  keystr="对战31技能")
            else:
                self.Tool.touch_record_pos(record_pos=self.图片.对战31技能.record_pos, resolution=self.移动端.resolution, keystr="对战31技能")
            #
            # 火焰山火球
            if self.对战模式 in ["火焰山"]:
                self.Tool.touch_record_pos(record_pos=self.图片.对战火球按钮.record_pos, resolution=self.移动端.resolution, keystr="火焰山火球")
