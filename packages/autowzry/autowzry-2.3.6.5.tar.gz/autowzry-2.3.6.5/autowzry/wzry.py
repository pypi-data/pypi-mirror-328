...
class wzry_figure:
    ...
    def __init__(self, Tool=None):
        ...
        self.对战主动技能 = Template(r"tpl1740124404016.png", record_pos=(0.116, 0.224), resolution=(960, 540))
        #
        self.友方血条 = []
        self.敌方血条 = []
    ...
    def 开始1v1对局(self, times=0):
        ...
        while True:
            ...
            if 自己曾经确定过匹配:
                ...
                if not 队友确认匹配:
                    # 强制点击,[避免:] 1v1有1/10的概率没有点击确定匹配扣1分
                    self.Tool.touch_record_pos(self.图片.确定匹配按钮.record_pos, self.移动端.resolution, "确定匹配按钮")
...
# 状态判断
    ...
    def 判断对战中(self, 处理=False, acce=False):
        ...
        self.Tool.timelimit(timekey="释放技能", limit=60, init=True)
        # 识别到的位置
        装备pos = False
        移动pos = False
        普攻pos = False
        ...
        while initloop or self.判断对战中(处理=False):
            ...
            #
            # 恢复按钮
            # 这种不重要的功能, 不进行识别, 肯定适配960x540, 其他分辨率无法保证
            if max(self.移动端.resolution) != 960:
                self.Tool.existsTHENtouch(self.图片.对战恢复按钮,  keystr="对战恢复按钮")
            else:
            ...
            # 非960x540的设备, 不开启这些功能, 免得出现问题
            if max(self.移动端.resolution) != 960:
                self.Tool.existsTHENtouch(技能图标,  keystr="对战技能图标")
            else:
                self.Tool.touch_record_pos(record_pos=技能图标.record_pos, resolution=self.移动端.resolution, keystr="对战技能图标")
                # 随机释放技能
                if self.Tool.timelimit(timekey="释放技能", limit=30+random.randint(0, 30), init=False):
