...
class DeprecatedTaxID(BaseModel):
    """Table DeprecatedTaxID.
    
    This table stores mappings of deprecated (old) taxIDs to their corresponding new taxIDs.
    """

    old_taxid = pw.IntegerField(null=False, unique=True, index=True)  # The deprecated taxID (must be unique)
    new_taxid = pw.IntegerField(null=False, index=True)  # The new taxID it maps to

    class Meta:
        indexes = (
            (('old_taxid', 'new_taxid'), True),
        )
