...
class TaxID(TaxaDB, TaxaDumpParser):
    ...
    def sci_name(self, taxid):
        ...
        """
        resolved_taxid = self.resolve_taxid(taxid)
        try:
            name = Taxa.get(Taxa.ncbi_taxid == resolved_taxid).tax_name
            return name
    ...
    def lineage_id(self, taxid, ranks=False, reverse=False):
        ...
        try:
            lineages = []
            # lineage_list = []
            current_lineage = Taxa.get(Taxa.ncbi_taxid == resolved_taxid).tax_name
            current_lineage_id = Taxa.get(Taxa.ncbi_taxid == resolved_taxid).ncbi_taxid
            parent = Taxa.get(Taxa.ncbi_taxid == resolved_taxid).parent_taxid
            rank = Taxa.get(Taxa.ncbi_taxid == resolved_taxid).lineage_level
    ...
    def has_parent(self, taxid, parent):
        ...
        """
        resolved_taxid = self.resolve_taxid(taxid)
        lineage_id = self.lineage_id(resolved_taxid)
        lineage_name = self.lineage_name(resolved_taxid)
