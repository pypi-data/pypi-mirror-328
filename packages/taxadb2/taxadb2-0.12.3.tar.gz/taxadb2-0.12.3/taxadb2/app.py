...
from taxadb2 import util
from taxadb2 import download
from taxadb2.version import __version__
from taxadb2.schema import DatabaseFactory, db, Taxa, Accession, DeprecatedTaxID
from taxadb2.parser import TaxaDumpParser, Accession2TaxidParser
...
def create_db(args):
    ...
    parser = TaxaDumpParser(nodes_file=os.path.join(args.input, 'nodes.dmp'),
                            ...
                            merged_file=os.path.join(args.input, 'merged.dmp'),
    ...
    if not DeprecatedTaxID.table_exists():
        logger.info('Creating table %s' % str(DeprecatedTaxID.get_table_name()))
        db.create_tables([DeprecatedTaxID])

    logger.info("Parsing files")
    taxa_info_list, deprecated_taxid_data = parser.taxdump()
    ...
    try:
        with db.atomic():
            for i in tqdm(range(0, total_size, args.chunk),
                          unit=' chunks', desc='INFO:taxadb2.app',
                          total=''):
    ...
    logger.info("Inserting Deprecated TaxID data")
    total_size = len(deprecated_taxid_data)
    try:
        with db.atomic():
            for i in tqdm(range(0, total_size, args.chunk),
                          unit=' chunks', desc='INFO:taxadb2.app (DeprecatedTaxID)',
                          total=''):
                DeprecatedTaxID.insert_many(deprecated_taxid_data[i:i+args.chunk]).on_conflict(
                    conflict_target=[DeprecatedTaxID.old_taxid],  # Target column(s) for conflict (unique)
                    preserve=[DeprecatedTaxID.old_taxid],  # Preserve the 'old_taxid' value
                    update={DeprecatedTaxID.new_taxid: DeprecatedTaxID.new_taxid}  # Optionally, update new_taxid
                ).execute()
    except OperationalError as e:
        print("\n")  # needed because the above counter has none
        logger.error("sqlite3 error: %s" % e)
        logger.error("Maybe retry with a lower chunk size.")
        sys.exit(1)
    logger.info('Table DeprecatedTaxID completed')
