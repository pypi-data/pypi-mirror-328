...
@contextmanager
def collect_send_update() -> Iterator[list[UpdateToUser[Any, Any]]]:
    data = []
    try:

        async def _send_update(m: UpdateToUser[Any, Any]) -> None:
            data.append(m)

        _set_send_update_var(_send_update)
        yield data
    finally:
        _send_update_var.set(None)
