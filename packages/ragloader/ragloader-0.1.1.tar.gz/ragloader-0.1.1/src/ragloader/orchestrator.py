...
from ragloader.db.qdrant_connector import QdrantConnector
...
class UploadOrchestrator:
    ...
    def extract_content(self, classified_document: ClassifiedDocument) -> ExtractedDocument:
        ...
        extracted_document:
            - parsed_content
            - category
            - (optional) extracted_content (JSON)
        """
    ...
    def split_document(self, extracted_document: ExtractedDocument) -> ChunkedDocument:
        ...
        chunks_data: list = [
            {"chunk_index": chunk.chunk_index, "content": chunk.content, "metadata": chunk.document_metadata}
            for chunk in chunked_document.chunks
        ]
        payload: dict = {
            "document_name": chunked_document.document_name,
            "document_content": chunked_document.document_content,
            "document_class": chunked_document.document_class,
            "document_chunks": chunks_data,
        }
        point: PointStruct = PointStruct(id=str(chunked_document.document_uuid), vector={}, payload=payload)
        with self.lock:
    ...
    def embed_document(self, chunked_document: ChunkedDocument):
        ...
        # document_chunks = chunked_document.chunks
        for embedded_chunk in embedded_document.embedded_chunks:
            point_id = random.randint(1, 1000000)
            point: PointStruct = PointStruct(
                id=point_id, vector=embedded_chunk.embedding, payload=embedded_chunk.payload
            )
            with self.lock:
