...
void RxEventCallback(UART_HandleTypeDef *huart, uint16_t Pos) {
  if(huart->Instance == USART2) { //Determine whether it is serial port 2
    ...
    if (receive_buff[receive_buff_cnt - 1] != '\n') {
      ...
      HAL_UARTEx_ReceiveToIdle_DMA(&huart2, (uint8_t*)(receive_buff + receive_buff_cnt), max_read);
    }
  }
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  HAL_UART_Transmit(&huart2, (unsigned char *)send_msg, 7, 0x200); // Send READY
  /* USER CODE END 2 */
  ...
  while (1)
  {
    if (receive_buff_cnt > 0 && receive_buff[receive_buff_cnt - 1] == '\n') {
      ...
      struct NNResult res = spikingNeuralNetworkInfer(inputs);
