/**
  ...
  * @brief   Template generating plain C code for the implementation of Integrate and Fire neuron
  */
  ...
  for (h = 0; h < INPUT_SAMPLESH; h++) {
    for (w = 0; w < INPUT_SAMPLESW; w++) {
      for (j = 0; j < INPUT_CHANNELS; j++) {
        ...
        tmp = scale(NUMBER_T, (LONG_NUMBER_T){{ node.layer.name }}_potent[h][w][j], WEIGHTS_SCALE_FACTOR - TMP_SCALE_FACTOR, OUTPUT_ROUND_MODE);
        tmp = tmp + scale(NUMBER_T, (LONG_NUMBER_T)input[h][w][j], INPUT_SCALE_FACTOR - TMP_SCALE_FACTOR, OUTPUT_ROUND_MODE);
