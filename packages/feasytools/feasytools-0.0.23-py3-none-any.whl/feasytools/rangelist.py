...
class RangeList:
    ...
    @overload
    def __init__(self, data: 'list[tuple[int,int]]', loop_times:int, loop_period:int): ...

    def __init__(self, data: 'Union[list[tuple[int,int]], ET.Element]',
                 loop_times:int = 1, loop_period:Optional[int] = None):
        ...
        if loop_times > 1 and len(data) >= 1:
            assert loop_period > data[-1][1], f"Loop period {loop_period} must be greater than end time {data[i][1]}."
            n = len(data)
    ...
    def toXMLNode(self, tag:str = "range") -> ET.Element:
        attr:dict[str, str] = {}
        if self.__lp: attr["loop_period"] = str(self.__lp)
        if self.__lt: attr["loop_times"] = str(self.__lt)
        elem = ET.Element(tag, attr)
        for d in self._d:
            if self.__lp and d[1] > self.__lp: break
