...
class SeriesInfoHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if not series_model:
            return self.reponse_json_error("获取系列信息失败")
        ...
        self.reponse_json_success(series_model)
    ...
    def post_async(self, *args, **kwargs):
        ...
        if not category_list:
            return self.reponse_json_error("请选择分类信息")
        ...
        if series_id == 0:
            ...
            if not series_model:
                return self.reponse_json_error("无法获取系列信息")
            
            if series_model.status == 1:
                return self.reponse_json_error("系列已上架，无法修改")
    ...
    def delete_async(self, *args, **kwargs):
        ...
        if series_model["status"] == 1:
            return self.reponse_json_error("系列已上架，无法删除")
