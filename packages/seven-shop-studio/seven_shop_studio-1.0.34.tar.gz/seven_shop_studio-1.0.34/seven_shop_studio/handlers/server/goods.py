...
class GoodsInfoHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        if not goods_model or goods_model["status"] == -1:
            return self.reponse_json_error("获取商品信息失败")
    ...
    def post_async(self):
        ...
        if not self.param_check(goods_info, goods_check_list):
            return self.reponse_json_error("商品表单未填写完整")
        ...
        if sku_list:
            ...
            for item in sku_list:
                if not self.param_check(item, sku_check_list):
                    return self.reponse_json_error("sku表单未填写完整")
        ...
        if goods_entity.id > 0:
            ...
            if old_goods_model.status == 1:
                return self.reponse_json_error("商品已上架，无法修改")
            ...
            if Decimal(goods_entity.price) <= 0:
                return self.reponse_json_error("价格错误")
    ...
    def delete_async(self, *args, **kwargs):
        ...
        if good_model_list:
            ...
            if status > 0:
                return self.reponse_json_error("请确认商品已下架后再删除")
    ...
    def post_async(self):
        ...
        if goods_list:
            ...
            if len(success_list) == 0 and len(fail_list) > 0:
                return self.reponse_json_error("商品已在推荐列表中")
            else:
                return self.reponse_json_success(desc="提交成功")
            
        else:
            return self.reponse_json_error("无法获取商品列表")
