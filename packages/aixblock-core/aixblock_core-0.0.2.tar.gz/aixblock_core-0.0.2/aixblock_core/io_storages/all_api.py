...
class AllExportStorageTypesAPI(APIView):
    ...
    def get(self, request, **kwargs):
        if request.query_params.get('is_global') and request.query_params.get('is_global') == 'true':
            return Response([{'name': s['name'], 'title': s['title']} for s in _common_storage_list if s['name'] != "s3-server"])
        else:
            return Response([{'name': s['name'], 'title': s['title']} for s in _common_storage_list])
...
class AllImportStorageListAPI(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        list_responses = sum([
            ...
            for s in _common_storage_list if s['name'] != "s3-server"
        ], [])
