...
class ComputeMarketplaceRentV2API(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        access_token = get_env("MASTER_TOKEN", "")
        if access_token != "":
            endpoint = get_env("MASTER_NODE", "https://test.aixblock.org")
            headers = {f"Authorization": "Token {access_token}"}
            response = requests.post(
                f"{endpoint}/api/compute_marketplace/rent/", headers=headers
            )
        ...
        for compute_vast in compute_rent_vast:
            ...
            def install_and_buy_vast(compute_vast_id, compute_vast_hours, type, price, disk_size):
                ...
                if not status_install:
                    ...
                    send_mail_refund(request.user, compute)
                ...
                if type == "storage":
                    ...
                    send_mail_s3(request.user, compute, email_user, pass_user)
                    ...
                    send_mail_rent_compute(request.user, compute, model_endpoint, tensorboard_endpoint, ddp_endpoint)

                else:
                    email_user = response["instances"]["extra_env"][4][1]
                    pass_user = response["instances"]["extra_env"][6][1]
                    send_mail_rent_compute_platform(request.user, compute, email_user, pass_user)
        ...
        for compute_exabit in compute_rent_exabit:
            ...
            def install_and_buy_exabit(compute_exabit_id, compute_exabit_hours, type, price):
                ...
                if type == "storage":
                    ...
                    send_mail_s3(request.user, compute, email_user, pass_user, response["data"]["ports"]["9001"])
                    send_mail_rent_compute_platform(request.user, compute, email_user, pass_user)
                    # update_minio_info(compute, email_user, pass_user)
        ...
        def install_and_buy_gpu(gpu_id, hours, type):
            ...
            try:
                ...
                if type == "storage":
                    ...
                    send_mail_s3(request.user, compute, minio_user_email, minio_password)
                elif type == 'model-training':
                    send_mail_rent_compute(request.user, compute, minio_port, user_email)
                else:
                    ...
                    send_mail_rent_compute_platform(request.user, compute, user_email, password)
                    if minio_user_email:
                        send_mail_s3(request.user, compute, minio_user_email, minio_password)
            except Exception as e:
                pass
        ...
        def thread_auto_provision(computes_preferences, email=None, type_remove=None):
            while computes_preferences.deleted_at is None:
                ...
                if settings.DJANGO_DB == "sqlite":
                    queryset = MLBackend.objects.filter(
                        id__in=all_pj_status_is_training, deleted_at__isnull=True
                    )
                    all_pj_is_training = list(
                        {obj["project_id"] for obj in queryset.values("project_id")}
                    )
                else:
                    all_pj_is_training = (
