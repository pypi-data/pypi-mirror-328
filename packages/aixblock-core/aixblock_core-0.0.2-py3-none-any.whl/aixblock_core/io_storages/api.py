...
class ImportStorageListAPI(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "s3-server" in request.path:
            regex_filter = request.data.get("regex_filter")
            project_id = request.data.get('project')

            user_email = request.user.email
            s3_server = S3Server(user_email)
            result = s3_server.create_service()

            storage = S3ImportStorage(title=result["bucketname"], 
                                    #   region_name="kwargs.get('region_name')",
                                      bucket=result["bucketname"], 
                                      aws_access_key_id = result["accessKey"],
                                      aws_secret_access_key = result["secretKey"], 
            ...
            def send_mail_s3():
                ...
                html_file_path =  './templates/mail/s3_server.html'
                ...
                # try:
                #     ssl_link = "https://" + convert_url(result["host_address"])
                # except Exception as e:
                #     ssl_link = result["host_address"]
                #     print(e)

                html_content = html_content.replace('[endpoint_minio]', result["host_address"])
                # html_content = html_content.replace('[endpoint_minio_api]', result["endpoint"])

                html_content = html_content.replace('[user_minio]', result["username"])
                html_content = html_content.replace('[pass_minio]', result["password"])

                data = {
                    "subject": f"AIxBlock | Confirmation of Your Storage Access",
...
class StorageFormLayoutAPI(generics.RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.path == "/api/storages/s3-server/form":
            form_layout_file = os.path.join(os.path.dirname(inspect.getfile(self.__class__)), 'form_layout_server.yml')
        else:
