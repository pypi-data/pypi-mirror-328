...
class Windows(CMake):
    ...
    def __init__(self,
        ...
        super().__init__(projectdir=projectdir if projectdir is not None else
                         ...
                         outdir=outdir if outdir is not None else Path('out')/'deploy'/'Windows')
    ...
    def deploy(self, tag: str) -> Deploy | None:
        ...
        return Deploy(rom_size=self._rom_size(self._outdir/tag/'Linux.exe', str(self.__size_bin)),
                      ram_size=self._ram_size(self._outdir/tag/'Linux.exe', str(self.__size_bin)),
