...
class TestModelsQuantizedResnet1D(TestCase):
    def setUp(self):
        ...
        self.__data = RawDataModel(sets=RawDataSets(train=train, test=test), name='test_quantizedresnet1d')
        self.__model_params = {
            'filters': (4, 6),
            'kernel_sizes': (5, 5),
    ...
    def test_quantized_resnet_1d_pytorch(self):
        ...
        from qualia_core.learningmodel.pytorch.quantized_layers1d import QuantizedConv1d, QuantizedMaxPool1d
        from qualia_core.learningmodel.pytorch.layers.quantized_layers import QuantizedIdentity, QuantizedLinear
        from torch.nn import Flatten, Linear, ReLU, AvgPool1d
        ...
        trainresult = qualia.train(self.__data,
                        train_epochs=1,
                        iteration=1,
                        model_name='test_quantized_resnet_1d_pytorch',
                        model=model,
                        model_params=self.__model_params,
                        optimizer={'kind': 'Adam'},
                        framework=framework,
                        )
        self.assertEqual(trainresult.name, 'test_quantized_resnet_1d_pytorch')
        ...
        self.assertEqual(type(trainresult.model.layers[0][0].pool1), QuantizedMaxPool1d)
        ...
        self.assertEqual(type(trainresult.model.layers[0][0].spool), QuantizedMaxPool1d)
        ...
        self.assertEqual(type(trainresult.model.postpool), QuantizedMaxPool1d)
        self.assertEqual(tuple(trainresult.model.postpool.kernel_size), (32,))
        self.assertEqual(tuple(trainresult.model.postpool.stride), (32,))
