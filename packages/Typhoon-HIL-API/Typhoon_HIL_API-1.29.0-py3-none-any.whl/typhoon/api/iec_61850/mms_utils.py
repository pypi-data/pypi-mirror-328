...
def export_config(config, file_name=None, file_path=None):
    ...
    if isinstance(config, dict):
        ...
        raise Exception("config is not a dictionary!")
...
def write_leaf_props_to_mms(config, sch_model, item_handle):
    ...
    #   Marko Boberic, December 2024
    # --------------------------------------------------------------------------------
    from typhoon.import_path.handlers.mms_server import (
        get_casted_type,
        get_casted_value,
        get_domain_name,
        get_path_with_fc,
    ...
    for node_path in config["node_trees"]:
        ...
        for leaf_path in leaf_path_subset:
            ...
            if value:
                leaf_values.append(
                    {
                        "domain": get_domain_name(leaf_path),
                        "name": get_path_with_fc(
                            leaf_path,
                            _get_object_fc(
                                config["scl"], config["ied_data"], leaf_path
                            ),
                        ),
                        "casted_type": get_casted_type(type_val),
                        "type": type_val,
                        "value": get_casted_value(value, type_val),
                        "default_value": get_casted_value(default_value, type_val),
                    }
                )
