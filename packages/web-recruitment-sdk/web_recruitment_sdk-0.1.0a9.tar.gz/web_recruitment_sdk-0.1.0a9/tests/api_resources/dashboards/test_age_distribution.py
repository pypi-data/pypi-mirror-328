...
class TestAgeDistribution:
    ...
    def test_method_list(self, client: WebRecruitmentSDK) -> None:
        age_distribution = client.dashboards.age_distribution.list()
        assert_matches_type(ChartResponse, age_distribution, path=["response"])
    ...
    def test_raw_response_list(self, client: WebRecruitmentSDK) -> None:
        response = client.dashboards.age_distribution.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        age_distribution = response.parse()
        assert_matches_type(ChartResponse, age_distribution, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: WebRecruitmentSDK) -> None:
        with client.dashboards.age_distribution.with_streaming_response.list() as response:
...
class TestAsyncAgeDistribution:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_list(self, async_client: AsyncWebRecruitmentSDK) -> None:
        age_distribution = await async_client.dashboards.age_distribution.list()
        assert_matches_type(ChartResponse, age_distribution, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncWebRecruitmentSDK) -> None:
        response = await async_client.dashboards.age_distribution.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        age_distribution = await response.parse()
        assert_matches_type(ChartResponse, age_distribution, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncWebRecruitmentSDK) -> None:
        async with async_client.dashboards.age_distribution.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
