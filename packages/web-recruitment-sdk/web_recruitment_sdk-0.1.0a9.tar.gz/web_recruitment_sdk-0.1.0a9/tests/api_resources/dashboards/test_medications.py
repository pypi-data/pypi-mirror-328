...
class TestMedications:
    ...
    def test_method_list(self, client: WebRecruitmentSDK) -> None:
        medication = client.dashboards.medications.list()
        assert_matches_type(ChartResponse, medication, path=["response"])
    ...
    def test_raw_response_list(self, client: WebRecruitmentSDK) -> None:
        response = client.dashboards.medications.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        medication = response.parse()
        assert_matches_type(ChartResponse, medication, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: WebRecruitmentSDK) -> None:
        with client.dashboards.medications.with_streaming_response.list() as response:
...
class TestAsyncMedications:
    ...
    async def test_raw_response_list(self, async_client: AsyncWebRecruitmentSDK) -> None:
        response = await async_client.dashboards.medications.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        medication = await response.parse()
        assert_matches_type(ChartResponse, medication, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncWebRecruitmentSDK) -> None:
        async with async_client.dashboards.medications.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
