...
class TestConditions:
    ...
    def test_method_list(self, client: WebRecruitmentSDK) -> None:
        condition = client.dashboards.conditions.list()
        assert_matches_type(ChartResponse, condition, path=["response"])
    ...
    def test_raw_response_list(self, client: WebRecruitmentSDK) -> None:
        response = client.dashboards.conditions.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        condition = response.parse()
        assert_matches_type(ChartResponse, condition, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: WebRecruitmentSDK) -> None:
        with client.dashboards.conditions.with_streaming_response.list() as response:
...
class TestAsyncConditions:
    ...
    async def test_method_list(self, async_client: AsyncWebRecruitmentSDK) -> None:
        condition = await async_client.dashboards.conditions.list()
        assert_matches_type(ChartResponse, condition, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncWebRecruitmentSDK) -> None:
        response = await async_client.dashboards.conditions.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        condition = await response.parse()
        assert_matches_type(ChartResponse, condition, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncWebRecruitmentSDK) -> None:
        async with async_client.dashboards.conditions.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
