...
from web_recruitment_sdk._utils import maybe_transform
...
from web_recruitment_sdk._exceptions import APIStatusError, APITimeoutError, APIResponseValidationError
...
from web_recruitment_sdk.types.admin.user_create_params import UserCreateParams
...
class TestWebRecruitmentSDK:
    ...
    def test_retrying_timeout_errors_doesnt_leak(self, respx_mock: MockRouter) -> None:
        respx_mock.post("/admin/users").mock(side_effect=httpx.TimeoutException("Test timeout error"))
        ...
        with pytest.raises(APITimeoutError):
            self.client.post(
                ...
                body=cast(
                    object,
                    maybe_transform(
                        dict(email="dev@stainlessapi.com", role_ids=["trially-system-admin"]), UserCreateParams
                    ),
                ),
    ...
    def test_retrying_status_errors_doesnt_leak(self, respx_mock: MockRouter) -> None:
        respx_mock.post("/admin/users").mock(return_value=httpx.Response(500))
    ...
    def test_retries_taken(
        ...
        respx_mock.post("/admin/users").mock(side_effect=retry_handler)

        response = client.admin.users.with_raw_response.create(
            email="dev@stainlessapi.com", role_ids=["trially-system-admin"]
        )
...
class TestAsyncWebRecruitmentSDK:
    ...
    async def test_retries_taken(
        ...
        response = await client.admin.users.with_raw_response.create(
            email="dev@stainlessapi.com", role_ids=["trially-system-admin"]
        )
