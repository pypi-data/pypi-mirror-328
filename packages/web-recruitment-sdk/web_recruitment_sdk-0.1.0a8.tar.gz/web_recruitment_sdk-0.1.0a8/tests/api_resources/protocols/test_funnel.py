...
class TestFunnel:
    ...
    def test_method_retrieve(self, client: WebRecruitmentSDK) -> None:
        funnel = client.protocols.funnel.retrieve(
            protocol_id=0,
        )
        assert_matches_type(ProtocolFunnelStats, funnel, path=["response"])
    ...
    def test_raw_response_retrieve(self, client: WebRecruitmentSDK) -> None:
        response = client.protocols.funnel.with_raw_response.retrieve(
            protocol_id=0,
        )
    ...
    def test_streaming_response_retrieve(self, client: WebRecruitmentSDK) -> None:
        with client.protocols.funnel.with_streaming_response.retrieve(
            protocol_id=0,
        ) as response:
...
class TestAsyncFunnel:
    ...
    async def test_raw_response_retrieve(self, async_client: AsyncWebRecruitmentSDK) -> None:
        response = await async_client.protocols.funnel.with_raw_response.retrieve(
            protocol_id=0,
        )
    ...
    async def test_streaming_response_retrieve(self, async_client: AsyncWebRecruitmentSDK) -> None:
        async with async_client.protocols.funnel.with_streaming_response.retrieve(
            protocol_id=0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
