...
def read_footprint_table(f, long=False):
    ...
    for col in wide_cols:
        ...
        if col != "fiber_names" and col != "haplotype":
            df[col] = df[col].apply(lambda x: [int(i) for i in x])
            
    if "haplotype" in df.columns:
        hap_types = df.haplotype.str.split(",").unique().to_list()
        for hap_type in hap_types:
            df[f"haplotype_{hap_type}_count"] = df.haplotype.apply(
                lambda x: x.count(hap_type)
            )
