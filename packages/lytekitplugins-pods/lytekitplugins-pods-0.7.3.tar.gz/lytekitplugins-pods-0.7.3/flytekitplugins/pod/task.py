...
def _sanitize_resource_name(resource: _task_models.Resources.ResourceEntry) -> str:
    return (
        _core_task.Resources.ResourceName.Name(resource.name).lower().replace("_", "-")
    )
    ...
    def __init__(
        ...
        if not pod_spec:
            raise _user_exceptions.FlyteValidationException(
                "A pod spec cannot be undefined"
            )
        if not primary_container_name:
            raise _user_exceptions.FlyteValidationException(
                "A primary container name cannot be undefined"
            )
    ...
    def _serialize_pod_spec(self, settings: SerializationSettings) -> Dict[str, Any]:
        ...
        for container in containers:
            ...
            if container.name == self.task_config.primary_container_name:
                ...
                container.command = sdk_default_container.args
                container.args = []
                ...
                container.env.append(
                    V1EnvVar("latch_runtime_info_dir", f"/opt/latch/{hash}"),
                )
