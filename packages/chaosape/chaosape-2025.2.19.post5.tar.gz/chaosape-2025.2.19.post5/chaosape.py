...
class ChaosApe:
    ...
    def get_function_categories(self):
        ...
        return list({category: list(functions) for category, functions in ChaosApe.FUNCTION_CATEGORIES.items()}.keys())

    def get_function_categories_as_list(self):
        """Returns a list of tuples, where each tuple contains (category, [functions])."""
        return [(category, list(functions)) for category, functions in ChaosApe.FUNCTION_CATEGORIES.items()]
