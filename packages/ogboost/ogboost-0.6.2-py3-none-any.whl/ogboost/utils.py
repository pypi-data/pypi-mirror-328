...
def generate_heterogeneous_learners(
    ...
    ----------
    templates : list
        A list of pre-initialized base learner instances that serve as templates.
    overrides : list of dict
        A list of dictionaries (one per template). Each dictionary maps hyperparameter
        names to callables that accept a random state (rng) and return a new sampled value.
    ...
    template_probs : array-like or str or None, optional
        ...
        length as `templates`. If the string "round_robin" is provided, then round-robin selection is used.
        If None, a uniform random distribution is used.
    ...
    -------
    list
        A flat list of newly generated base learner instances with hyperparameters updated
