...
def test_render_template(tmp_path):
    ...
    from pattern_templates_to_docs import render_template
...
def test_process_all_templates(tmp_path, monkeypatch):
    ...
    from pattern_templates_to_docs import process_all_templates

    # Mock output directories
    mock_relative = tmp_path / "relative"
    mock_local = tmp_path / "local"
    monkeypatch.setattr(
        "pattern_templates_to_docs.RELATIVE_OUTPUT_DIR",
        str(mock_relative)
    )
    monkeypatch.setattr(
        "pattern_templates_to_docs.LOCAL_OUTPUT_DIR",
        mock_local
    )
