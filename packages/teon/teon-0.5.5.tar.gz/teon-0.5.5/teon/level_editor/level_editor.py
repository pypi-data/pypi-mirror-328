...
class SMP(Widget):
    def __init__(self,console):
        super().__init__(position = (0.694,0.019),image = load_image("images/s0.png"),anchor = "topleft",scale = (0.6,0.2))
...
class SnapToGrid(Widget):
    def __init__(self,console):
        super().__init__(position = (0.768,0.019),anchor = "topleft",scale = 0.2,z = 100,image = load_image("images/stg0.png"))
        self.is_ui = True
        self.console = console
...
class Board(Widget):
    ...
    def input(self,key):
        ...
        if key == "f7":
            if self.index != None:
                if self.console.level.last_entity:
                    current_default = [self.console.level.last_entity.scale,self.console.level.last_entity.actual_visible,self.console.level.last_entity.actual_collidable,self.console.level.last_entity.actual_class,self.console.level.last_entity.z]
...
class Level:
    ...
    def input(self,key):
        if key == "left mouse" and not self.panel.hovered and not self.console.smp.hovered and not self.console.stg.hovered and self.console.smp.mode == 2:
            if not self.board.index == None and self.board.image_files[self.board.index]:
                ent = REntity(position = (int(self.game.mouse.world_position.x),int(self.game.mouse.world_position.y)),image = load_image(self.board.image_files[self.board.index]))
    ...
    def update(self):
        ...
        if self.console.stg.on and self.last_entity and not self.latch:
            ...
            self.last_entity.position.y = round(self.last_entity.position.y / (self.last_entity.scale.y * 100)) * (self.last_entity.scale.y * 100)
        if not self.last_entity == None:
            pygame.draw.rect(self.game.window.display,(255,255,255),self.last_entity.collider,2,2)
        self.caps_lock = pygame.key.get_mods() == pygame.KMOD_CAPS
