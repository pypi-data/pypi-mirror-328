...
skill_version_name_regex = re.compile(
...
class SkillVersionName(SkillName):
    """
    Skill version name, e.g. workspaces/:ws/skills/:localName/versions/:version
    """

    version: str


def new_skill_version_name(name: str) -> Optional[SkillVersionName]:
    """
    Create a new skill version name.

    Args:
        name: str, skill version name, e.g. workspaces/:ws/skills/:localName/versions/:version
    Returns:
        SkillVersionName, 解析成功返回SkillVersionName对象，否则返回None
    """

    match = skill_version_name_regex.match(name)
    if match:
        return SkillVersionName(**match.groupdict())
    return None
