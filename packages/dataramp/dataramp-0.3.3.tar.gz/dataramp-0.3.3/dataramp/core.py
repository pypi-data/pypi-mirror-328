...
class DataVersioner:
    ...
    def _calculate_hash(self, data: Union[pd.DataFrame, pd.Series]) -> str:
        ...
        return hashlib.sha256(pd.util.hash_pandas_object(data).values.tobytes()).hexdigest()

    def _generate_version_id(self, data_hash: str, version_format: str, name: str) -> str:
...
def data_save(
    ...
    **version_kwargs,
    ...
    data_path = Path(get_path("processed_data_path")) / f"{name}.{SUPPORTED_DATA_METHODS[method][1]}"
