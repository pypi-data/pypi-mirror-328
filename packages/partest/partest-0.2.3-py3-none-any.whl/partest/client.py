...
class ApiClient:
    ...
    async def _perform_request(self, client, method: str, url: str, params: Optional[Dict[str, Any]],
        ...
        if method == "GET":
            ...
            return await client.post(url, json=data, content=data_type, params=params, headers=headers, files=files)
        elif method == "PUT":
            return await client.put(url, json=data, content=data_type, params=params, headers=headers)
        elif method == "PATCH":
            return await client.patch(url, json=data, content=data_type, params=params, headers=headers)
        elif method == "DELETE":
