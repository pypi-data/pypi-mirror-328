...
class BackgroundSeriesImage(Series):
...
def image_to_base64(image_source: str) -> str:
    """
    Convert an image (local file or URL) to Base64 format.

    Args:
        image_source (str): The source of the image (local file path or URL).

    Returns:
        str: Base64 encoded image string with proper MIME type.

    Raises:
        ValueError: If the image file is not found or cannot be processed.
    """
    try:
        ...
        file_ext = image_source.split('.')[-1].lower()
...
class PointImageStyle(Series):
    def set_point_image_style(self, source: str):
        ...
        """
        base64_image = image_to_base64(source)
...
class PolarPointImageStyle(Series):
    def set_point_image_style(self, source: str):
        """
...
class VideoPointStyle(Series):
    def set_point_video_style(
        ...
        Args:
            ...
            fit_mode: How the video fits the point ('fit', 'stretch', etc.).
            position: Position of the video element on the chart (e.g., {"x": 0, "y": 0}).
            size: Size of the video element, e.g., {"width": 100, "height": 100}.
        """

        def convert_video_to_base64(file_path: str) -> str:
            if not os.path.exists(file_path):
                raise FileNotFoundError(f'Video file not found: {file_path}')
            with open(file_path, 'rb') as video_file:
                base64_data = base64.b64encode(video_file.read()).decode('utf-8')
            mime_type = 'video/mp4' if file_path.endswith('.mp4') else 'video/webm'
            return f'data:{mime_type};base64,{base64_data}'

        video_data_uri = convert_video_to_base64(video_source)
