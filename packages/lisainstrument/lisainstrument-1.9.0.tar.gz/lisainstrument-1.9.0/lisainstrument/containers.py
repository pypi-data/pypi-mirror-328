    ...
    def write(self, hdf5, dataset):
        ...
        for value in self.values():
            ...
            if size != 1 and len(value) != size:
                raise ValueError(
                    f"incompatible sizes in dictionary '{size}' and '{len(size)}'"
                )
            size = max(size, len(value))
        logger.debug(
            "Writing dataset of size '%s' with '%s' columns", size, len(self.indices())
        )
        # Write dataset
        dtype = np.dtype(
            {"names": self.indices(), "formats": len(self.indices()) * [np.float64]}
        )
