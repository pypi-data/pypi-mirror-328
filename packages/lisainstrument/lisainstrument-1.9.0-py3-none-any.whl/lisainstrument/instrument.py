...
class Instrument:
    ...
    def __init__(
        ...
        *,
        ...
        mosa_longitudinal_jitter_asds=2e-9,
        # Angular jitters
        sc_angular_jitter_asds=(5e-9, 5e-9, 5e-9),
        sc_angular_jitter_fknees=(8e-4, 8e-4, 8e-4),
        mosa_angular_jitter_asds=(2e-9, 2e-9),
        mosa_angular_jitter_fknees=(8e-4, 8e-4),
        mosa_angles="default",
            ...
            if isinstance(orbits, str) and orbits != "static":
                ...
                with File(orbits, "r") as orbitf:
                    ...
                    if version in SpecifierSet("== 1.*", True):
                        self.t0 = float(
                            orbitf.attrs["t0" if orbit_dataset == "tcb/ltt" else "tau0"]
                        )
        ...
        self.physics_et = (
            np.arange(self.physics_size, dtype=np.float64) * self.physics_dt
        )  # elapsed time
        ...
        logger.info(
            "Computing telemetry time vector (size=%s, dt=%s)",
            self.telemetry_size,
            self.telemetry_dt,
        )
        self.telemetry_t = (
        ...
        self.physics_et_covering_telemetry = (
            self.telemetry_t0
            - self.t0
            + np.arange(self.physics_size_covering_telemetry, dtype=np.float64)
            * self.physics_dt
        )
        ...
        self.telemetry_physics_slice = slice(
            first_sample, first_sample + self.physics_size
        )
        ...
        if tdir_modulations is None:
            self.tdir_modulations = {
                mosa: lambda times: 0 for mosa in ForEachMOSA.indices()
            }
        else:
        ...
        if clock_freqlindrifts == "default":
            # Default based on LISANode
            self.clock_freqlindrifts = ForEachSC(
        ...
        if clock_freqquaddrifts == "default":
            # Default based on LISANode
            self.clock_freqquaddrifts = ForEachSC(
        ...
        # Longitudinal jitters
        self.mosa_jitter_x_asds = ForEachMOSA(mosa_longitudinal_jitter_asds)
        ...
        if ttl_coeffs == "default":
            ...
            self.ttl_coeffs_local_phis = ForEachMOSA(
                lambda _: np.random.uniform(-2.2e-3, 2.2e-3)
            )
            self.ttl_coeffs_distant_phis = ForEachMOSA(
                lambda _: np.random.uniform(-2.4e-3, 2.4e-3)
            )
            self.ttl_coeffs_local_etas = ForEachMOSA(
                lambda _: np.random.uniform(-2.2e-3, 2.2e-3)
            )
        ...
        # Angular jitters
        self.sc_jitter_phi_asds = ForEachSC(sc_angular_jitter_asds[0])
        self.sc_jitter_eta_asds = ForEachSC(sc_angular_jitter_asds[1])
        self.sc_jitter_theta_asds = ForEachSC(sc_angular_jitter_asds[2])
        self.sc_jitter_phi_fknees = ForEachSC(sc_angular_jitter_fknees[0])
        self.sc_jitter_eta_fknees = ForEachSC(sc_angular_jitter_fknees[1])
        self.sc_jitter_theta_fknees = ForEachSC(sc_angular_jitter_fknees[2])
        self.mosa_jitter_phi_asds = ForEachMOSA(mosa_angular_jitter_asds[0])
        self.mosa_jitter_eta_asds = ForEachMOSA(mosa_angular_jitter_asds[1])
        self.mosa_jitter_phi_fknees = ForEachMOSA(mosa_angular_jitter_fknees[0])
        self.mosa_jitter_eta_fknees = ForEachMOSA(mosa_angular_jitter_fknees[1])
    ...
    def init_aafilter(self, aafilter):
        ...
        self.downsampled = lambda _, x: (
            x if np.isscalar(x) else x[:: self.physics_upsampling]
        )
        ...
        if aafilter is None:
            ...
            self.aafilter = lambda _, x: (
                x if np.isscalar(x) else lfilter(self.aafilter_coeffs, 1, x)
            )
    ...
    def design_aafilter(self, parameters):
        ...
        if method == "kaiser":
            ...
            taps = firwin(
                numtaps, (freq1 + freq2) / (2 * nyquist), window=("kaiser", beta)
            )
    ...
    def init_lock(self, lock):
        ...
        if lock == "six":
            ...
            if match:
                ...
                raise ValueError(
                    f"unsupported pre-defined locking configuration '{lock}'"
                )
        elif isinstance(lock, dict):
            logger.info("Using explicit locking configuration '%s'", lock)
            if set(lock.keys()) != set(self.MOSAS) or set(lock.values()) > set(
                ["cavity", "distant", "adjacent"]
            ):
    ...
    def init_fplan(self, fplan):
        ...
        if isinstance(fplan, str) and fplan == "static":
            logger.warning(
            ...
            with File(self.fplan_file, "r") as fplanf:
                ...
                if version in SpecifierSet("== 1.1.*", True):
                    ...
                    times = (
                        self.orbit_t0
                        + np.arange(fplanf.attrs["size"]) * fplanf.attrs["dt"]
                    )
                    interpolate = lambda x: InterpolatedUnivariateSpline(
                        times, x, k=1, ext="raise"
                    )(self.physics_t)
                    lock_beatnotes = {}
                    ...
                    try:
                        for mosa in self.MOSAS:
                            if self.lock[mosa] == "cavity":
                                ...
                                lock_beatnotes[mosa] = 1e6 * interpolate(
                                    fplanf[self.lock_config][f"isi_{mosa}"]
                                )
                                ...
                                lock_beatnotes[mosa] = (
                                    ...
                                    * sign
                                    * interpolate(
                                        fplanf[self.lock_config][f"rfi_{left_mosa}"]
                                    )
                                )
                    except ValueError as error:
                        logger.error("Missing frequency-plan information at \n%s")
                        raise ValueError(
                            "missing frequency-plan information, use longer file or adjust sampling"
                        ) from error
                    self.fplan = ForEachMOSA(lock_beatnotes)
                else:
                    raise ValueError(
                        f"unsupported frequency-plan file version '{version}'"
                    )
        else:
    ...
    def init_orbits(self, orbits, orbit_dataset):
        ...
        if orbits == "static":
            ...
            self.d_pprs = self.pprs.transformed(
                lambda _, x: 0 if np.isscalar(x) else np.gradient(x, self.physics_dt)
            )
            self.tps_wrt_tcb = ForEachSC(0)
            self.orbit_dataset = None
    ...
    def init_orbits_file_1_0(self, orbitf):
        ...
        try:
            ...
            self.pprs = ForEachMOSA(
                lambda mosa: pprs_const(mosa)
                + d_pprs(mosa).antiderivative()(self.physics_t)
            ...
            self.tps_wrt_tcb = ForEachSC(
                lambda sc: tps_wrt_tcb(sc)(self.physics_t_covering_telemetry)
            )
        except ValueError as error:
    ...
    def init_orbits_file_2_0(self, orbitf):
        ...
        times = (
            orbitf.attrs["t0"] + np.arange(orbitf.attrs["size"]) * orbitf.attrs["dt"]
        )
        interpolate = lambda data, t: InterpolatedUnivariateSpline(
            times, data, k=5, ext="raise"
        )(t)
        int_interpolate = lambda data, t: InterpolatedUnivariateSpline(
            times, data, k=5, ext="raise"
        ).antiderivative()(t)
        ...
        try:
            ...
            d_ppr_dataset = (
                orbitf["tcb/d_ltt"]
                if self.orbit_dataset == "tcb/ltt"
                else orbitf["tps/d_ppr"]
            )
            self.pprs = ForEachMOSA(
                lambda mosa: ppr_dataset[0, link_index[mosa]]
                + int_interpolate(d_ppr_dataset[:, link_index[mosa]], self.physics_t)
                - int_interpolate(d_ppr_dataset[:, link_index[mosa]], self.orbit_t0)
            )
            logger.debug("Interpolating proper pseudo-range derivatives")
            self.d_pprs = ForEachMOSA(
                lambda mosa: interpolate(
                    d_ppr_dataset[:, link_index[mosa]], self.physics_t
                )
            )
        except ValueError as error:
            logger.error("Missing orbit information at \n%s", self.physics_t)
        ...
        try:
            ...
            if self.orbit_dataset == "tcb/ltt":
                ...
                self.tps_wrt_tcb = ForEachSC(
                    lambda sc: interpolate(
                        dataset[:, sc_index[sc]], self.physics_t_covering_telemetry
                    )
                )
        except ValueError as error:
            logger.error(
    ...
    def init_gws(self, gws):
        ...
        if isinstance(gws, str):
            logger.info(
                "Interpolating gravitational-wave responses from GW file '%s'", gws
            )
            self.gw_file = gws
    ...
    def _init_gw_file_before_1_1(self, gwf):
        ...
        self.gw_group = None
        interpolate = lambda data, t: InterpolatedUnivariateSpline(
            gwf["t"][:], data, k=5, ext="zeros"
        )(t)
        self.gws = ForEachMOSA(
            lambda mosa: interpolate(gwf[f"l_{mosa}"][:], self.physics_t)
        )
    ...
    def _init_gw_file_1_1(self, gwf):
        ...
        self.gws = ForEachMOSA(
            lambda mosa: interpolate(
                gwf[f"{self.gw_group}/l_{mosa}"][:], self.physics_t
            )
        )
    ...
    def _init_gw_file_2(self, gwf):
        ...
        interpolate = lambda data, t: InterpolatedUnivariateSpline(
            times, data, k=5, ext="zeros"
        )(t)
        self.gws = ForEachMOSA(
            lambda mosa: interpolate(
                gwf[f"{self.gw_group}/y"][:, link_index[mosa]], self.physics_t
            )
        )
    ...
    def init_glitches(self, glitches):
        ...
        if isinstance(glitches, str):
            ...
            logger.info(
                "Interpolating glitch signals from glitch file '%s'", self.glitch_file
            )
            with File(self.glitch_file, "r") as glitchf:
                ...
                if version.is_devrelease or version.local is not None:
                    logger.warning(
                        "You are using a glitch file in a development version"
                    )
                if version >= Version("1.5.dev"):
                    logger.warning(
                        "You are using a glitch file in a version that might not be fully supported"
                    )
                if version >= Version("1.4.dev"):
                    self.init_glitch_file_1_4(glitchf)
                elif version >= Version("1.3.dev"):
                    self.init_glitch_file_1_3(glitchf)
                elif version >= Version("1.0"):
                    self.init_glitch_file_1_0(glitchf)
    ...
    def init_glitch_file_1_0(self, glitchf):
        """Init glitches from a glitch file version >= 1.0."""
        interpolate = lambda x, y, newx: InterpolatedUnivariateSpline(
            x, y, k=5, ext="const"
        )(newx)
        ...
        # Test-mass glitches
        self.glitch_tms = ForEachMOSA(
            lambda mosa: (
                0
                if f"tm_{mosa}" not in glitchf
                else interpolate(
                    glitchf["t"][:], glitchf[f"tm_{mosa}"][:], self.physics_t
                )
            )
        )

        # Laser glitches
        self.glitch_lasers = ForEachMOSA(
            lambda mosa: (
                0
                if f"laser_{mosa}" not in glitchf
                else interpolate(
                    glitchf["t"][:], glitchf[f"laser_{mosa}"][:], self.physics_t
                )
            )
        )

    def init_glitch_file_1_3(self, glitchf):
        """Init glitches from a glitch file version >= 1.3."""
        interpolate = lambda x, y, newx: InterpolatedUnivariateSpline(
            x, y, k=5, ext="const"
        )(newx)

        # Readout glitches
        self.glitch_readout_isi_carriers = ForEachMOSA(
            lambda mosa: (
                0
                if f"readout_isi_carrier_{mosa}" not in glitchf
                else interpolate(
                    glitchf["t"][:],
                    glitchf[f"readout_isi_carrier_{mosa}"][:],
                    self.physics_t,
                )
            )
        )
        self.glitch_readout_isi_usbs = ForEachMOSA(
            lambda mosa: (
                0
                if f"readout_isi_usb_{mosa}" not in glitchf
                else interpolate(
                    glitchf["t"][:],
                    glitchf[f"readout_isi_usb_{mosa}"][:],
                    self.physics_t,
                )
            )
        )
        self.glitch_readout_tmi_carriers = ForEachMOSA(
            lambda mosa: (
                0
                if f"readout_tmi_carrier_{mosa}" not in glitchf
                else interpolate(
                    glitchf["t"][:],
                    glitchf[f"readout_tmi_carrier_{mosa}"][:],
                    self.physics_t,
                )
            )
        )
        self.glitch_readout_tmi_usbs = ForEachMOSA(
            lambda mosa: (
                0
                if f"readout_tmi_usb_{mosa}" not in glitchf
                else interpolate(
                    glitchf["t"][:],
                    glitchf[f"readout_tmi_usb_{mosa}"][:],
                    self.physics_t,
                )
            )
        )
        self.glitch_readout_rfi_carriers = ForEachMOSA(
            lambda mosa: (
                0
                if f"readout_rfi_carrier_{mosa}" not in glitchf
                else interpolate(
                    glitchf["t"][:],
                    glitchf[f"readout_rfi_carrier_{mosa}"][:],
                    self.physics_t,
                )
            )
        )
        self.glitch_readout_rfi_usbs = ForEachMOSA(
            lambda mosa: (
                0
                if f"readout_rfi_usb_{mosa}" not in glitchf
                else interpolate(
                    glitchf["t"][:],
                    glitchf[f"readout_rfi_usb_{mosa}"][:],
                    self.physics_t,
                )
            )
        )
    ...
    def init_glitch_file_1_4(self, glitchf):
        """Init glitches from a glitch file version >= 1.4."""
        interpolate = lambda x, y, newx: InterpolatedUnivariateSpline(
            x, y, k=5, ext="const"
        )(newx)

        # Create glitch time array
        t = glitchf.attrs["t0"] + np.arange(glitchf.attrs["size"]) * glitchf.attrs["dt"]
        ...
        self.glitch_readout_isi_carriers = ForEachMOSA(
            lambda mosa: (
                ...
                if f"readout_isi_carrier_{mosa}" not in glitchf
                    ...
                    t, glitchf[f"readout_isi_carrier_{mosa}"][:], self.physics_t
                )
            )
        )
        self.glitch_readout_isi_usbs = ForEachMOSA(
            lambda mosa: (
                ...
                if f"readout_isi_usb_{mosa}" not in glitchf
                    ...
                    t, glitchf[f"readout_isi_usb_{mosa}"][:], self.physics_t
                )
            )
        )
        self.glitch_readout_tmi_carriers = ForEachMOSA(
            lambda mosa: (
                ...
                if f"readout_tmi_carrier_{mosa}" not in glitchf
                    ...
                    t, glitchf[f"readout_tmi_carrier_{mosa}"][:], self.physics_t
                )
            )
        )
        self.glitch_readout_tmi_usbs = ForEachMOSA(
            lambda mosa: (
                ...
                if f"readout_tmi_usb_{mosa}" not in glitchf
                    ...
                    t, glitchf[f"readout_tmi_usb_{mosa}"][:], self.physics_t
                )
            )
        )
        self.glitch_readout_rfi_carriers = ForEachMOSA(
            lambda mosa: (
                ...
                if f"readout_rfi_carrier_{mosa}" not in glitchf
                    ...
                    t, glitchf[f"readout_rfi_carrier_{mosa}"][:], self.physics_t
                )
            )
        )
        self.glitch_readout_rfi_usbs = ForEachMOSA(
            lambda mosa: (
                ...
                if f"readout_rfi_usb_{mosa}" not in glitchf
                    ...
                    t, glitchf[f"readout_rfi_usb_{mosa}"][:], self.physics_t
                )
            )
        )
        ...
        self.glitch_tms = ForEachMOSA(
            lambda mosa: (
                0
                if f"tm_{mosa}" not in glitchf
                else interpolate(t, glitchf[f"tm_{mosa}"][:], self.physics_t)
            )
        )
        ...
        self.glitch_lasers = ForEachMOSA(
            lambda mosa: (
                0
                if f"laser_{mosa}" not in glitchf
                else interpolate(t, glitchf[f"laser_{mosa}"][:], self.physics_t)
            )
        )
    ...
    def disable_all_noises(self, excluding=None):
        ...
        if "laser" not in excluding:
            self.disable_laser_noise()
        if "modulation" not in excluding:
            self.disable_modulation_noise()
        if "clock" not in excluding:
            self.disable_clock_noise()
        if "test-mass" not in excluding:
            self.disable_testmass_noise()
        if "backlink" not in excluding:
            self.disable_backlink_noise()
        if "oms" not in excluding:
            self.disable_oms_noise()
        if "ranging" not in excluding:
            self.disable_ranging_noises()
        if "angular-jitters" not in excluding:
            self.disable_angular_jitters()
        if "dws" not in excluding:
            self.disable_dws_noise()
        if "moc-time-correlation" not in excluding:
            self.disable_moc_time_correlation_noise()
        if "longitudinal-jitters" not in excluding:
            self.disable_longitudinal_jitters()
    ...
    def disable_longitudinal_jitters(self):
        ...
        self.mosa_jitter_x_asds = ForEachMOSA(0)
    ...
    def simulate(self, keep_all=False):
        ...
        .. warning::
            ...
            In the current implementation, signals that remain constant in time
            are represented as scalar values for optimization purposes. We
            invite users to manually broadcast these scalar values to numpy
            arrays if necessary, using :func:`numpy.broadcast_to`.

            For example, a simulation with all noises but test-mass noise turned
            off will return scalar zero values for the ISI and RFI measurements,
            and numpy arrays for the TMI measurements.
        ...
        self.moc_time_correlations = (
            self.moc_time_correlation_noises
        ...
        self.tdir_modulations_tseries = ForEachMOSA(
            lambda mosa: self.tdir_modulations[mosa](
                self.physics_et + self.scet_wrt_tps_local[mosa[0]]
            )
        )
        ...
        self.local_usb_offsets = self.local_carrier_offsets + self.modulation_freqs * (
            1 + self.clock_noise_offsets
        )
        ...
        self.local_usb_fluctuations = (
            ...
            * (self.clock_noise_fluctuations + self.modulation_noises)
        )
        ...
        carrier_fluctuations = (
            self.local_carrier_fluctuations
            - (self.central_freq + self.local_carrier_offsets)
            * (self.distant_ttls - self.mosa_jitter_xs)
            / c
        )
        ...
        self.distant_carrier_fluctuations = (
            ...
            * (self.local_ttls - self.mosa_jitter_xs)
            / c
        )
        ...
        usb_fluctuations = (
            self.local_usb_fluctuations
            - (self.central_freq + self.local_usb_offsets)
            * (self.distant_ttls - self.mosa_jitter_xs)
            / c
        )
        ...
        self.distant_usb_fluctuations = (
            ...
            - (self.central_freq + delayed_distant_usb_offsets)
            * (self.local_ttls - self.mosa_jitter_xs)
            / c
        )
        ...
        self.tps_isi_carrier_fluctuations = (
            self.distant_isi_carrier_fluctuations
            - self.local_isi_carrier_fluctuations
        ...
        self.tps_isi_usb_fluctuations = (
            self.distant_isi_usb_fluctuations
            - self.local_isi_usb_fluctuations
        ...
        self.tps_mprs = (
            self.scet_wrt_tps_local - self.scet_wrt_tps_distant + self.ranging_noises
        )
        ...
        logger.debug(
            "Propagating local carrier fluctuations to test-mass interferometer"
        )
        self.local_tmi_carrier_fluctuations = (
            self.local_carrier_fluctuations
            - (self.central_freq + self.local_tmi_carrier_offsets)
            * (self.mosa_jitter_xs - self.testmass_noises - self.glitch_tms)
            * 2
            / c
        )
        ...
        logger.debug(
            "Propagating local upper sideband fluctuations to test-mass interferometer"
        )
        ...
        self.tps_rfi_carrier_fluctuations = (
            self.adjacent_rfi_carrier_fluctuations
            - self.local_rfi_carrier_fluctuations
        ...
        self.tps_rfi_usb_fluctuations = (
            self.adjacent_rfi_usb_fluctuations
            - self.local_rfi_usb_fluctuations
        ...
        self.tps_wrt_scet = self.scet_wrt_tps_local.transformed(
        ...
        logger.debug(
            "Sampling inter-spacecraft carrier beatnote fluctuations to SCET grid"
        )
        ...
        logger.debug(
            "Sampling inter-spacecraft upper sideband beatnote offsets to SCET grid"
        )
        ...
        logger.debug(
            "Sampling inter-spacecraft upper sideband beatnote fluctuations to SCET grid"
        )
        ...
        self.scet_isi_dws_phis = self.tps_isi_dws_phis.transformed(
            self.timestamped, concurrent=self.concurrent
        )
        self.scet_isi_dws_etas = self.tps_isi_dws_etas.transformed(
            self.timestamped, concurrent=self.concurrent
        )

        logger.info("Sampling measured pseudo-ranges to SCET grid")
        self.scet_mprs = self.tps_mprs.transformed(
            self.timestamped, concurrent=self.concurrent
        )
        ...
        logger.debug(
            "Sampling test-mass upper sideband beatnote fluctuations to SCET grid"
        )
        ...
        logger.debug(
            "Sampling reference upper sideband beatnote fluctuations to SCET grid"
        )
        ...
        self.electro_isi = lambda mosa, x: self.interpolate(
            x, -self.electro_delays_isis[mosa]
        )
        self.electro_tmi = lambda mosa, x: self.interpolate(
            x, -self.electro_delays_tmis[mosa]
        )
        self.electro_rfi = lambda mosa, x: self.interpolate(
            x, -self.electro_delays_rfis[mosa]
        )

        logger.debug("Applying electronic delays to inter-spacecraft beatnotes")
        self.electro_isi_carrier_offsets = self.scet_isi_carrier_offsets.transformed(
            self.electro_isi, concurrent=self.concurrent
        )
        self.electro_isi_carrier_fluctuations = (
            self.scet_isi_carrier_fluctuations.transformed(
                self.electro_isi, concurrent=self.concurrent
            )
        )
        self.electro_isi_usb_offsets = self.scet_isi_usb_offsets.transformed(
            self.electro_isi, concurrent=self.concurrent
        )
        self.electro_isi_usb_fluctuations = self.scet_isi_usb_fluctuations.transformed(
            self.electro_isi, concurrent=self.concurrent
        )

        logger.debug("Applying electronic delays to test-mass beatnotes")
        self.electro_tmi_carrier_offsets = self.scet_tmi_carrier_offsets.transformed(
            self.electro_tmi, concurrent=self.concurrent
        )
        self.electro_tmi_carrier_fluctuations = (
            self.scet_tmi_carrier_fluctuations.transformed(
                self.electro_tmi, concurrent=self.concurrent
            )
        )
        self.electro_tmi_usb_offsets = self.scet_tmi_usb_offsets.transformed(
            self.electro_tmi, concurrent=self.concurrent
        )
        self.electro_tmi_usb_fluctuations = self.scet_tmi_usb_fluctuations.transformed(
            self.electro_tmi, concurrent=self.concurrent
        )

        logger.debug("Applying electronic delays to reference beatnotes")
        self.electro_rfi_carrier_offsets = self.scet_rfi_carrier_offsets.transformed(
            self.electro_rfi, concurrent=self.concurrent
        )
        self.electro_rfi_carrier_fluctuations = (
            self.scet_rfi_carrier_fluctuations.transformed(
                self.electro_rfi, concurrent=self.concurrent
            )
        )
        self.electro_rfi_usb_offsets = self.scet_rfi_usb_offsets.transformed(
            self.electro_rfi, concurrent=self.concurrent
        )
        self.electro_rfi_usb_fluctuations = self.scet_rfi_usb_fluctuations.transformed(
            self.electro_rfi, concurrent=self.concurrent
        )
        ...
        self.filtered_isi_carrier_offsets = (
            self.electro_isi_carrier_offsets.transformed(
                self.aafilter, concurrent=self.concurrent
            )
        )
        self.filtered_isi_carrier_fluctuations = (
            self.electro_isi_carrier_fluctuations.transformed(
                self.aafilter, concurrent=self.concurrent
            )
        )
        self.filtered_isi_usb_offsets = self.electro_isi_usb_offsets.transformed(
            self.aafilter, concurrent=self.concurrent
        )
        self.filtered_isi_usb_fluctuations = (
            self.electro_isi_usb_fluctuations.transformed(
                self.aafilter, concurrent=self.concurrent
            )
        )

        logger.debug("Filtering inter-spacecraft DWS measurements")
        self.filtered_isi_dws_phis = self.scet_isi_dws_phis.transformed(
            self.aafilter, concurrent=self.concurrent
        )
        self.filtered_isi_dws_etas = self.scet_isi_dws_etas.transformed(
            self.aafilter, concurrent=self.concurrent
        )

        logger.debug("Filtering measured pseudo-ranges")
        self.filtered_mprs = self.scet_mprs.transformed(
            self.aafilter, concurrent=self.concurrent
        )

        logger.debug("Filtering test-mass beatnotes")
        self.filtered_tmi_carrier_offsets = (
            self.electro_tmi_carrier_offsets.transformed(
                self.aafilter, concurrent=self.concurrent
            )
        )
        self.filtered_tmi_carrier_fluctuations = (
            self.electro_tmi_carrier_fluctuations.transformed(
                self.aafilter, concurrent=self.concurrent
            )
        )
        self.filtered_tmi_usb_offsets = self.electro_tmi_usb_offsets.transformed(
            self.aafilter, concurrent=self.concurrent
        )
        self.filtered_tmi_usb_fluctuations = (
            self.electro_tmi_usb_fluctuations.transformed(
                self.aafilter, concurrent=self.concurrent
            )
        )

        logger.debug("Filtering reference beatnotes")
        self.filtered_rfi_carrier_offsets = (
            self.electro_rfi_carrier_offsets.transformed(
                self.aafilter, concurrent=self.concurrent
            )
        )
        self.filtered_rfi_carrier_fluctuations = (
            self.electro_rfi_carrier_fluctuations.transformed(
                self.aafilter, concurrent=self.concurrent
            )
        )
        self.filtered_rfi_usb_offsets = self.electro_rfi_usb_offsets.transformed(
            self.aafilter, concurrent=self.concurrent
        )
        self.filtered_rfi_usb_fluctuations = (
            self.electro_rfi_usb_fluctuations.transformed(
                self.aafilter, concurrent=self.concurrent
            )
        )
        ...
        self.isi_carrier_offsets = self.filtered_isi_carrier_offsets.transformed(
            self.downsampled
        )
        self.isi_carrier_fluctuations = (
            self.filtered_isi_carrier_fluctuations.transformed(self.downsampled)
        )
        self.isi_usb_offsets = self.filtered_isi_usb_offsets.transformed(
            self.downsampled
        )
        self.isi_usb_fluctuations = self.filtered_isi_usb_fluctuations.transformed(
            self.downsampled
        )
        ...
        if self.prn_ambiguity is None:
            ...
            self.mprs = self.mprs_unambiguous.transformed(
                lambda _, x: np.mod(x, self.prn_ambiguity / c)
            )

        logger.debug("Downsampling test-mass beatnotes")
        self.tmi_carrier_offsets = self.filtered_tmi_carrier_offsets.transformed(
            self.downsampled
        )
        self.tmi_carrier_fluctuations = (
            self.filtered_tmi_carrier_fluctuations.transformed(self.downsampled)
        )
        self.tmi_usb_offsets = self.filtered_tmi_usb_offsets.transformed(
            self.downsampled
        )
        self.tmi_usb_fluctuations = self.filtered_tmi_usb_fluctuations.transformed(
            self.downsampled
        )

        logger.debug("Downsampling reference beatnotes")
        self.rfi_carrier_offsets = self.filtered_rfi_carrier_offsets.transformed(
            self.downsampled
        )
        self.rfi_carrier_fluctuations = (
            self.filtered_rfi_carrier_fluctuations.transformed(self.downsampled)
        )
        self.rfi_usb_offsets = self.filtered_rfi_usb_offsets.transformed(
            self.downsampled
        )
        self.rfi_usb_fluctuations = self.filtered_rfi_usb_fluctuations.transformed(
            self.downsampled
        )
        ...
        self.isi_carriers = self.isi_carrier_offsets + self.isi_carrier_fluctuations

        logger.debug("Computing total inter-spacecraft upper sideband beatnotes")
        self.isi_usbs = self.isi_usb_offsets + self.isi_usb_fluctuations

        logger.debug("Computing total test-mass carrier beatnotes")
        self.tmi_carriers = self.tmi_carrier_offsets + self.tmi_carrier_fluctuations

        logger.debug("Computing total test-mass upper sideband beatnotes")
        self.tmi_usbs = self.tmi_usb_offsets + self.tmi_usb_fluctuations

        logger.debug("Computing total reference carrier beatnotes")
        self.rfi_carriers = self.rfi_carrier_offsets + self.rfi_carrier_fluctuations

        logger.debug("Computing total reference upper sideband beatnotes")
        self.rfi_usbs = self.rfi_usb_offsets + self.rfi_usb_fluctuations
    ...
    def simulate_noises(self):  # pylint: disable=too-many-statements
        ...
        self.clock_noise_fluctuations_covering_telemetry = ForEachSC(
            lambda sc: noises.clock(
                fs=self.physics_fs,
                size=self.physics_size_covering_telemetry,
                asd=self.clock_asds[sc],
                seed=generate_subseed(self.seed, f"clock_{sc}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.integrated_clock_noise_fluctuations_covering_telemetry = ForEachSC(
            lambda sc: cumulative_trapezoid(
                np.broadcast_to(
        ...
        self.modulation_noises = ForEachMOSA(
            lambda mosa: noises.modulation(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.modulation_asds[mosa],
                seed=generate_subseed(self.seed, f"modulation_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.backlink_noises = ForEachMOSA(
            lambda mosa: noises.backlink(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.backlink_asds[mosa],
                fknee=self.backlink_fknees[mosa],
                seed=generate_subseed(self.seed, f"backlink_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.testmass_noises = ForEachMOSA(
            lambda mosa: noises.testmass(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.testmass_asds[mosa],
                fknee=self.testmass_fknees[mosa],
                fbreak=self.testmass_fbreak[mosa],
                frelax=self.testmass_frelax[mosa],
                shape=self.testmass_shape,
                seed=generate_subseed(self.seed, f"testmass_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.ranging_noises = ForEachMOSA(
            lambda mosa: self.ranging_biases[mosa]
            + noises.ranging(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.ranging_asds[mosa],
                seed=generate_subseed(self.seed, f"ranging_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.oms_isi_carrier_noises = ForEachMOSA(
            lambda mosa: noises.oms(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.oms_isi_carrier_asds[mosa],
                fknee=self.oms_fknees[mosa],
                seed=generate_subseed(self.seed, f"oms_isi_carrier_{mosa}"),
            ),
            concurrent=self.concurrent,
        )

        self.oms_isi_usb_noises = ForEachMOSA(
            lambda mosa: noises.oms(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.oms_isi_usb_asds[mosa],
                fknee=self.oms_fknees[mosa],
                seed=generate_subseed(self.seed, f"oms_isi_usb_{mosa}"),
            ),
            concurrent=self.concurrent,
        )

        self.oms_tmi_carrier_noises = ForEachMOSA(
            lambda mosa: noises.oms(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.oms_tmi_carrier_asds[mosa],
                fknee=self.oms_fknees[mosa],
                seed=generate_subseed(self.seed, f"oms_tmi_carrier_{mosa}"),
            ),
            concurrent=self.concurrent,
        )

        self.oms_tmi_usb_noises = ForEachMOSA(
            lambda mosa: noises.oms(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.oms_tmi_usb_asds[mosa],
                fknee=self.oms_fknees[mosa],
                seed=generate_subseed(self.seed, f"oms_tmi_usb_{mosa}"),
            ),
            concurrent=self.concurrent,
        )

        self.oms_rfi_carrier_noises = ForEachMOSA(
            lambda mosa: noises.oms(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.oms_rfi_carrier_asds[mosa],
                fknee=self.oms_fknees[mosa],
                seed=generate_subseed(self.seed, f"oms_rfi_carrier_{mosa}"),
            ),
            concurrent=self.concurrent,
        )

        self.oms_rfi_usb_noises = ForEachMOSA(
            lambda mosa: noises.oms(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.oms_rfi_usb_asds[mosa],
                fknee=self.oms_fknees[mosa],
                seed=generate_subseed(self.seed, f"oms_rfi_usb_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.dws_phi_noises = ForEachMOSA(
            lambda mosa: noises.dws(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.dws_asds[mosa],
                seed=generate_subseed(self.seed, f"dws_phi_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        self.dws_eta_noises = ForEachMOSA(
            lambda mosa: noises.dws(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.dws_asds[mosa],
                seed=generate_subseed(self.seed, f"dws_eta_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.moc_time_correlation_noises = ForEachSC(
            lambda sc: noises.moc_time_correlation(
                fs=self.telemetry_fs,
                size=self.telemetry_size,
                asd=self.moc_time_correlation_asds[sc],
                seed=generate_subseed(self.seed, f"moc_time_correlation_{sc}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        logger.info("Generating MOSA longitudinal jitters")
        self.mosa_jitter_xs = ForEachMOSA(
            lambda mosa: noises.longitudinal_jitter(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.mosa_jitter_x_asds[mosa],
                seed=generate_subseed(self.seed, f"mosa_jitter_x_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.sc_jitter_phis = ForEachSC(
            lambda sc: noises.angular_jitter(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.sc_jitter_phi_asds[sc],
                fknee=self.sc_jitter_phi_fknees[sc],
                seed=generate_subseed(self.seed, f"sc_jitter_phi_{sc}"),
            ),
            concurrent=self.concurrent,
        )
        self.sc_jitter_etas = ForEachSC(
            lambda sc: noises.angular_jitter(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.sc_jitter_eta_asds[sc],
                fknee=self.sc_jitter_eta_fknees[sc],
                seed=generate_subseed(self.seed, f"sc_jitter_eta_{sc}"),
            ),
            concurrent=self.concurrent,
        )
        self.sc_jitter_thetas = ForEachSC(
            lambda sc: noises.angular_jitter(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.sc_jitter_theta_asds[sc],
                fknee=self.sc_jitter_theta_fknees[sc],
                seed=generate_subseed(self.seed, f"sc_jitter_theta_{sc}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.mosa_jitter_phis = ForEachMOSA(
            lambda mosa: noises.angular_jitter(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.mosa_jitter_phi_asds[mosa],
                fknee=self.mosa_jitter_phi_fknees[mosa],
                seed=generate_subseed(self.seed, f"mosa_jitter_phi_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        self.mosa_jitter_etas = ForEachMOSA(
            lambda mosa: noises.angular_jitter(
                fs=self.physics_fs,
                size=self.physics_size,
                asd=self.mosa_jitter_eta_asds[mosa],
                fknee=self.mosa_jitter_eta_fknees[mosa],
                seed=generate_subseed(self.seed, f"mosa_jitter_eta_{mosa}"),
            ),
            concurrent=self.concurrent,
        )
        ...
        self.mosa_total_jitter_phis = (
            self.sc_jitter_phis.for_each_mosa() + self.mosa_jitter_phis
        )
        cos_mosa_angles = (self.mosa_angles * np.pi / 180).transformed(
            lambda _, x: np.cos(x)
        )
        sin_mosa_angles = (self.mosa_angles * np.pi / 180).transformed(
            lambda _, x: np.sin(x)
        )
    ...
    def lock_on_cavity(self, mosa):
        ...
        self.laser_noises[mosa] = noises.laser(
            ...
            seed=generate_subseed(self.seed, f"laser_{mosa}"),
        )
        ...
        self.local_carrier_fluctuations[mosa] = (
            self.laser_noises[mosa]
            + self.glitch_lasers[mosa]
    ...
    def lock_on_adjacent(self, mosa):
        ...
        logger.debug(
            "Computing carrier offsets for local beam %s locked on adjacent beam %s",
            mosa,
            adjacent(mosa),
        )
        self.local_carrier_offsets[mosa] = self.local_carrier_offsets[
            adjacent(mosa)
        ] - self.fplan[mosa] * (1 + self.clock_noise_offsets[sc(mosa)])

        logger.debug(
            "Computing carrier fluctuations for local beam %s "
            "locked on adjacent beam %s",
            mosa,
            adjacent(mosa),
        )
        adjacent_carrier_fluctuations = (
            self.local_carrier_fluctuations[adjacent(mosa)]
    ...
    def lock_on_distant(self, mosa):
        ...
        logger.debug(
            "Computing carrier offsets for local beam %s locked on distant beam %s",
            mosa,
            distant(mosa),
        )
        ...
        distant_carrier_offsets = (
            ...
            + (1 - self.d_pprs[mosa]) * delayed_distant_carrier_offsets
        )
        self.local_carrier_offsets[mosa] = distant_carrier_offsets - self.fplan[
            mosa
        ] * (1 + self.clock_noise_offsets[sc(mosa)])

        logger.debug(
            "Computing carrier fluctuations for local beam %s "
            "locked on distant beam %s",
            mosa,
            distant(mosa),
        )
        carrier_fluctuations = (
            ...
            * (self.distant_ttls[distant(mosa)] - self.mosa_jitter_xs[distant(mosa)])
            / c
        )
    ...
    def simulate_locking(self):
        ...
        for mosa, lock_type in self.lock.items():
            if lock_type == "cavity":
                ...
                raise ValueError(
                    f"invalid locking type '{self.lock[mosa]}' for laser '{mosa}'"
                )
        ...
        while dependencies:
            ...
            for mosa in available_mosas:
                if self.lock[mosa] == "cavity":
                    ...
                    logger.debug(
                        "Locking laser %s on adjacent laser %s", mosa, adjacent(mosa)
                    )
                    self.lock_on_adjacent(mosa)
                elif self.lock[mosa] == "distant":
                    logger.debug(
                        "Locking laser %s on distant laser %s", mosa, distant(mosa)
                    )
    ...
    def invert_scet_wrt_tps(self, scet_wrt_tps, sc):
        ...
        logger.debug(
            "Solving iteratively (tolerance=%s s, maxiter=%s)",
    ...
    def _write_metadata(self, hdf5):
        ...
        self._write_attr(
            ...
            "testmass_frelax",
            "testmass_shape",
            "oms_isi_carrier_asds",
            "oms_isi_usb_asds",
            "oms_tmi_carrier_asds",
            "oms_tmi_usb_asds",
    ...
    def write(self, output="measurements.h5", mode="w-", keep_all=False):
        """Run a simulation and write results to an HDF5 file.
        ...
        with File(output, mode) as hdf5:
            ...
            if keep_all:
                ...
                self.moc_time_correlation_noises.write(
                    hdf5, "moc_time_correlation_noises"
                )
                ...
                self.local_carrier_fluctuations.write(
                    hdf5, "local_carrier_fluctuations"
                )
                ...
                self.distant_carrier_fluctuations.write(
                    hdf5, "distant_carrier_fluctuations"
                )
                self.distant_usb_offsets.write(hdf5, "distant_usb_offsets")
                self.distant_usb_fluctuations.write(hdf5, "distant_usb_fluctuations")

                logger.debug(
                    "Writing propagated SCETs with respect to TCB to '%s'", output
                )
                ...
                self.adjacent_carrier_fluctuations.write(
                ...
                logger.debug(
                    "Writing local beams at inter-spacecraft interferometer to '%s'",
                    output,
                )
                self.local_isi_carrier_offsets.write(hdf5, "local_isi_carrier_offsets")
                self.local_isi_carrier_fluctuations.write(
                    hdf5, "local_isi_carrier_fluctuations"
                )
                self.local_isi_usb_offsets.write(hdf5, "local_isi_usb_offsets")
                self.local_isi_usb_fluctuations.write(
                    hdf5, "local_isi_usb_fluctuations"
                )

                logger.debug(
                    "Writing distant beams at inter-spacecraft interferometer to '%s'",
                    output,
                )
                self.distant_isi_carrier_offsets.write(
                    hdf5, "distant_isi_carrier_offsets"
                )
                self.distant_isi_carrier_fluctuations.write(
                    hdf5, "distant_isi_carrier_fluctuations"
                )
                self.distant_isi_usb_offsets.write(hdf5, "distant_isi_usb_offsets")
                self.distant_isi_usb_fluctuations.write(
                    hdf5, "distant_isi_usb_fluctuations"
                )
                ...
                self.tps_isi_carrier_fluctuations.write(
                    hdf5, "tps_isi_carrier_fluctuations"
                )
                self.tps_isi_usb_offsets.write(hdf5, "tps_isi_usb_offsets")
                self.tps_isi_usb_fluctuations.write(hdf5, "tps_isi_usb_fluctuations")

                logger.debug(
                    "Writing inter-spacecraft DWS measurements on TPS to '%s'", output
                )
                ...
                self.local_tmi_carrier_fluctuations.write(
                    hdf5, "local_tmi_carrier_fluctuations"
                )
                self.local_tmi_usb_offsets.write(hdf5, "local_tmi_usb_offsets")
                self.local_tmi_usb_fluctuations.write(
                    hdf5, "local_tmi_usb_fluctuations"
                )

                logger.debug(
                    "Writing adjacent beams at test-mass interferometer to '%s'", output
                )
                self.adjacent_tmi_carrier_offsets.write(
                    hdf5, "adjacent_tmi_carrier_offsets"
                )
                self.adjacent_tmi_carrier_fluctuations.write(
                    hdf5, "adjacent_tmi_carrier_fluctuations"
                )
                self.adjacent_tmi_usb_offsets.write(hdf5, "adjacent_tmi_usb_offsets")
                self.adjacent_tmi_usb_fluctuations.write(
                    hdf5, "adjacent_tmi_usb_fluctuations"
                )
                ...
                self.tps_tmi_carrier_fluctuations.write(
                    hdf5, "tps_tmi_carrier_fluctuations"
                )
                ...
                self.local_rfi_carrier_fluctuations.write(
                    hdf5, "local_rfi_carrier_fluctuations"
                )
                self.local_rfi_usb_offsets.write(hdf5, "local_rfi_usb_offsets")
                self.local_rfi_usb_fluctuations.write(
                    hdf5, "local_rfi_usb_fluctuations"
                )

                logger.debug(
                    "Writing adjacent beams at reference interferometer to '%s'", output
                )
                self.adjacent_rfi_carrier_offsets.write(
                    hdf5, "adjacent_rfi_carrier_offsets"
                )
                self.adjacent_rfi_carrier_fluctuations.write(
                    hdf5, "adjacent_rfi_carrier_fluctuations"
                )
                self.adjacent_rfi_usb_offsets.write(hdf5, "adjacent_rfi_usb_offsets")
                self.adjacent_rfi_usb_fluctuations.write(
                    hdf5, "adjacent_rfi_usb_fluctuations"
                )
                ...
                self.tps_rfi_carrier_fluctuations.write(
                    hdf5, "tps_rfi_carrier_fluctuations"
                )
                ...
                logger.debug(
                    "Writing inter-spacecraft beatnotes sampled to SCET grid to '%s'",
                    output,
                )
                self.scet_isi_carrier_offsets.write(hdf5, "scet_isi_carrier_offsets")
                self.scet_isi_carrier_fluctuations.write(
                    hdf5, "scet_isi_carrier_fluctuations"
                )
                ...
                logger.debug(
                    "Writing inter-spacecraft DWS measurements sampled to SCET grid to '%s'",
                    output,
                )
                ...
                logger.debug(
                    "Writing measured pseudo-ranges sampled to SCET grid to '%s'",
                    output,
                )
                self.scet_mprs.write(hdf5, "scet_mprs")

                logger.debug(
                    "Writing test-mass beatnotes sampled to SCET grid to '%s'", output
                )
                self.scet_tmi_carrier_offsets.write(hdf5, "scet_tmi_carrier_offsets")
                self.scet_tmi_carrier_fluctuations.write(
                    hdf5, "scet_tmi_carrier_fluctuations"
                )
                ...
                logger.debug(
                    "Writing reference beatnotes sampled to SCET grid to '%s'", output
                )
                self.scet_rfi_carrier_offsets.write(hdf5, "scet_rfi_carrier_offsets")
                self.scet_rfi_carrier_fluctuations.write(
                    hdf5, "scet_rfi_carrier_fluctuations"
                )
                ...
                logger.debug(
                    "Writing inter-spacecraft beatnotes with electronic delay to '%s'",
                    output,
                )
                self.electro_isi_carrier_offsets.write(
                    hdf5, "electro_isi_carrier_offsets"
                )
                self.electro_isi_carrier_fluctuations.write(
                    hdf5, "electro_isi_carrier_fluctuations"
                )
                self.electro_isi_usb_offsets.write(hdf5, "electro_isi_usb_offsets")
                self.electro_isi_usb_fluctuations.write(
                    hdf5, "electro_isi_usb_fluctuations"
                )

                logger.debug(
                    "Writing test-mass beatnotes with electronic delays to '%s'", output
                )
                self.electro_tmi_carrier_offsets.write(
                    hdf5, "electro_tmi_carrier_offsets"
                )
                self.electro_tmi_carrier_fluctuations.write(
                    hdf5, "electro_tmi_carrier_fluctuations"
                )
                self.electro_tmi_usb_offsets.write(hdf5, "electro_tmi_usb_offsets")
                self.electro_tmi_usb_fluctuations.write(
                    hdf5, "electro_tmi_usb_fluctuations"
                )

                logger.debug(
                    "Writing reference beatnotes with electronic delays to '%s'", output
                )
                self.electro_rfi_carrier_offsets.write(
                    hdf5, "electro_rfi_carrier_offsets"
                )
                self.electro_rfi_carrier_fluctuations.write(
                    hdf5, "electro_rfi_carrier_fluctuations"
                )
                self.electro_rfi_usb_offsets.write(hdf5, "electro_rfi_usb_offsets")
                self.electro_rfi_usb_fluctuations.write(
                    hdf5, "electro_rfi_usb_fluctuations"
                )

                logger.debug(
                    "Writing filtered inter-spacecraft beatnotes to '%s'", output
                )
                self.filtered_isi_carrier_offsets.write(
                    hdf5, "filtered_isi_carrier_offsets"
                )
                self.filtered_isi_carrier_fluctuations.write(
                    hdf5, "filtered_isi_carrier_fluctuations"
                )
                self.filtered_isi_usb_offsets.write(hdf5, "filtered_isi_usb_offsets")
                self.filtered_isi_usb_fluctuations.write(
                    hdf5, "filtered_isi_usb_fluctuations"
                )

                logger.debug(
                    "Writing filtered inter-spacecraft DWS measurements to '%s'", output
                )
                ...
                self.filtered_tmi_carrier_offsets.write(
                    hdf5, "filtered_tmi_carrier_offsets"
                )
                self.filtered_tmi_carrier_fluctuations.write(
                    hdf5, "filtered_tmi_carrier_fluctuations"
                )
                self.filtered_tmi_usb_offsets.write(hdf5, "filtered_tmi_usb_offsets")
                self.filtered_tmi_usb_fluctuations.write(
                    hdf5, "filtered_tmi_usb_fluctuations"
                )

                logger.debug("Writing filtered reference beatnotes to '%s'", output)
                self.filtered_rfi_carrier_offsets.write(
                    hdf5, "filtered_rfi_carrier_offsets"
                )
                self.filtered_rfi_carrier_fluctuations.write(
                    hdf5, "filtered_rfi_carrier_fluctuations"
                )
                self.filtered_rfi_usb_offsets.write(hdf5, "filtered_rfi_usb_offsets")
                self.filtered_rfi_usb_fluctuations.write(
                    hdf5, "filtered_rfi_usb_fluctuations"
                )
            ...
            logger.debug(
                "Writing downsampled inter-spacecraft DWS measurements to '%s'", output
            )
    ...
    def plot_fluctuations(self, output=None, skip=0):
        ...
        plot = lambda axis, x, label: axis.plot(
            self.t[skip:], np.broadcast_to(x, self.size)[skip:], label=label
        )
        for mosa in self.MOSAS:
