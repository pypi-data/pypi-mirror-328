...
def fileparts(the_file, relative=False):
    ...
    if (os.path.isdir(full_name)):
        ...
        tmp = filename.split('.')[:]    
        if (len(tmp) == 1):
            fname = tmp # Note: 'the_file' was a mere string!
        else: # ...or ensure any '.' are kept & only last one is seen as extension
            fname = '.'.join(tmp[:-1])
            fext = tmp[-1]
            # Note: This ensures any '.' in filename are kept & only last is seen as extension!  
