...
def gen_v1_cid(
    ...
    DEFAULT_URL = "https://conlab.visitkorea.or.kr/api/utils/flow/utils-flow/sequence"
    DEFAULT_PARAMS = {
        "sequenceName": "origin_content_id"
    }

    request_url = url or DEFAULT_URL
    request_params = params or DEFAULT_PARAMS
    
    try:
        response = requests.get(request_url, params=request_params, timeout=timeout)
        ...
        return {
            "success": False,
            "message": f"서버({request_url})에 연결할 수 없습니다. 네트워크 연결을 확인해주세요."
        }
