...
def gui_tkinter():
    ...
    dict
        ...
        - "code" (str): The tool ID. Your user ID will be prepended to the code to prevent overwriting existing workflows from other users.
        - "version" (str): The tool version.
        - "folder" (str): The directory path where the tool is stored.
        - "name" (str): The tool name or title. It will be the displayed name in the QMENTA Platform when you want to run it.
    ...
    def perform_selection_check(values):
        ...
        try:
            ...
            values["code"] = os.path.basename(values["folder"])  # Update tool ID to folder name
            ...
            raise_if_false(re.search(r"^(\d+\.)?(\d+\.)?(\*|\d+)$", values["version"]),
    ...
    button = ttk.Button(master=action_frame, text="Push to Platform", command=submit_form)
    ...
    ttk.Button(master=action_frame,
               text="Quit", command=quit_tool_creator, bootstyle="danger").pack(side="left", padx=10)
...
def main():
    ...
    # Update code to prevent workflow overwrite
    # if user.endswith("@qmenta.com"):
    #     content_build["code"] = "qmenta_" + content_build["code"]
    # else:
    #     content_build["code"] = user.split("@")[0] + "_" + content_build["code"]
    ...
    content_build.update({
        ...
        "deprecated": "0",
        "average_time": "1",
        "results_configuration": '{"tools":[],"screen":""}',
        "tags": "dev"
    })
