...
def main_fdr(search, score_cutoff=0.8, min_matched_peak=4):
    df = pd.read_csv(f'{search}_results_mces.tsv', sep='\t')
...
def scale_fdr(score_cutoff=0.8, min_matched_peak=4):
    mode = 'k10'
...
if __name__ == '__main__':
    ...
    min_score = 0.7
    min_peaks = 3
    main_fdr('k0', min_score, min_peaks)

    main_fdr('k10', min_score, min_peaks)

    scale_fdr(min_score, min_peaks)
