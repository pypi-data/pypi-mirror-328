...
class EvalScriptError(evrmore.core.ValidationError):
...
class MaxOpCountError(EvalScriptError):
    def __init__(self, **kwargs):
        super(MaxOpCountError, self).__init__(
            'max opcode count exceeded', **kwargs)
...
def _CastToBigNum(s, err_raiser):
    v = evrmore.core._bignum.vch2bn(s)
...
def _CheckMultiSig(opcode, script, stack, txTo, inIdx, flags, err_raiser, nOpCount):
    ...
    i += sigs_count
    if len(stack) < i-1:
        raise err_raiser(ArgumentsInvalidError, opcode,
                         "not enough sigs on stack")
    elif len(stack) < i:
...
def _UnaryOp(opcode, stack, err_raiser):
    ...
    stack.append(evrmore.core._bignum.bn2vch(bn))
    ...
    for (sop, sop_data, sop_pc) in scriptIn.raw_iter():
        ...
        if sop <= OP_PUSHDATA4:
            ...
            if sop == OP_1NEGATE or ((sop >= OP_1) and (sop <= OP_16)):
                v = sop - (OP_1 - 1)
                stack.append(evrmore.core._bignum.bn2vch(v))
                ...
                _CheckMultiSig(sop, tmpScript, stack, txTo,
                               inIdx, flags, err_raiser, nOpCount)
                ...
                check_args(1)
                stack.append(evrmore.core.serialize.Hash160(stack.pop()))

            elif sop == OP_HASH256:
                check_args(1)
                stack.append(evrmore.core.serialize.Hash(stack.pop()))
                ...
                if n < 0 or n >= len(stack):
                    err_raiser(
                        EvalScriptError, "Argument for %s out of bounds" % OPCODE_NAMES[sop])
                vch = stack[-n-1]
                if sop == OP_ROLL:
                    del stack[-n-1]
                stack.append(vch)
...
class VerifyScriptError(evrmore.core.ValidationError):
    pass
...
class VerifySignatureError(evrmore.core.ValidationError):
    pass
