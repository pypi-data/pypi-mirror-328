...
class BaostockDataCollector(AbstractDataCollector):
    ...
    def get_stock_basic_data(self, stock_code_list = [], filter = FilterType.NoFilter):
        ...
        Raises:
          DataCollectorError
        """
        if self.__has_stock_basic_cache(stock_code_list):
            self.__logger.info("get_stock_basic_data: 从缓存读取数据")
            return self.__get_stock_basic_data_from_local_cache(stock_code_list)
        else:
            self.__logger.info("get_stock_basic_data: 从线上读取数据")
            return self.__get_stock_basic_data_from_online(stock_code_list, filter)

    def __has_stock_basic_cache(self, stock_code_list):
        res = True
        if len(stock_code_list) == 0:
            return False

        for code in stock_code_list:
            res = res and self.__cache_util.has_stock_basic_cache(code)
        return res

    def __get_stock_basic_data_from_local_cache(self, stock_code_list):
        res_list = []
        for code in stock_code_list:
            res_list.append(self.__cache_util.get_stock_basic_data(code))

        return 0, res_list

    def __get_stock_basic_data_from_online(self, stock_code_list, filter):
        if not self.__login:
        ...
        if len(stock_code_list) == 0:
            ...
            while (rs_b.error_code == '0') & rs_b.next():
                ...
                if len(stock_basic_data) < 5 or stock_basic_data[4] not in required_typs:
                    self.__logger.error('__get_stock_basic_data_from_online: not required type. ', stock_basic_data)
                    continue
            ...
            for stock_code in stock_code_list:
                ...
                if rs_b.error_code != '0':
                    ...
                    return int(rs_b.error_code), res_list
        
                stock_basic_data = rs_b.get_row_data()
                if stock_basic_data[4] not in required_typs:
                    continue
                keys = ["code", "code_name", "ipoDate", "outDate", "type", "status"]
                dicts = {}
                for i in range(len(stock_basic_data)):
        ...
        if self.__cache_util:
            try:
                for basic_data in res_list:
                    db_res = self.__cache_util.save_stock_basic_data("", basic_data)
            except Exception as e:
