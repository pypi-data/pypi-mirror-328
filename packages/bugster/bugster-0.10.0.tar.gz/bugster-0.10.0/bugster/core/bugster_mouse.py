class BugsterMouse:
    ...
    def _create_highlight_element(self, x: float, y: float, size: int = 20):
    ...
    def click(self, x: float, y: float, **kwargs):
        ...
        if self._page.screenshot:
            if has_element:
                locator = self._page._page.locator("*:focus")
                self._page.screenshot_step(name="after_mouse_click", locator=locator)
            else:
                self._create_highlight_element(x, y)
