...
class ThemeLibrary:
    ...
    def generate_terminal_js(self, theme: ThemeColors) -> str:
        ...
        js = f"""
        console.log('Applying terminal theme...', {json.dumps(term_colors)});
...
def generate_terminal_themes(theme_library):
    """Convert JSON theme files into terminal_themes format."""
    terminal_themes = {}
    ...
    for theme_name in theme_library.get_theme_names():
        ...
        js_code = f"""
        console.log('Applying terminal theme for {theme_name}...');

        try {{
            // Set all terminal options
            term.setOption('theme', {json.dumps(term_colors)});
            term.setOption('fontFamily', 'Courier New');
            term.setOption('fontSize', 14);
            term.setOption('cursorBlink', true);
