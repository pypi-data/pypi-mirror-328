...
class Agentuity(object):
    ...
    def init(self, lib_path: str = None):
        ...
        if "AGENTUITY_SDK_SESSION_ID" in os.environ:
            self.session_id = os.environ["AGENTUITY_SDK_SESSION_ID"]
        else:
    ...
    def __execute(self, command: str, data: Dict[str, Any] = {}) -> Dict[str, Any]:
        ...
        logger.debug(
            f"Executing command: {command}, sessionid: {self.session_id} with data: {json_data}"
        )
        result = self.lib.Execute(session_id_bytes, command_bytes, json_bytes)
