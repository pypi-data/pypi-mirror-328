...
class Utils:
    ...
    @staticmethod
    def date_calculate(days: int, date: str = None, pattern='%Y-%m-%d'):
        """
        日期计算
        ...
        base_dt = datetime.strptime(date, pattern) if date else datetime.now()
        return (base_dt - timedelta(days=days)).strftime(pattern)
    ...
    @staticmethod
    def url__append_params(url: str, params: dict):
        """
        给 url 添加参数
