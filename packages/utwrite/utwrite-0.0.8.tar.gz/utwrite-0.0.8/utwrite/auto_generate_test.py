...
def make_test_res_data(module_path):
...
# [end] make_test_res_data

def _check_in_res_keys(st):
    ...
    for s in st:
        ...
        if g:
            ...
            if  k.lower().replace(' ','') in RES_KEYS:
                return True
    return False
# [end] _check_in_res_keys

def _break_docstrings_in_tests_and_results(func_doc_data):
    ...
    for func,doc in func_doc_data.items():
        ...
        for s in doc.split('\n'):
            ...
            if _check_in_res_keys(s):
                if not regex.search(s).groups()[0].startswith('#'):
                    result_found=True
        ...
        for tst,res in test_result_tuples:
            ...
            # - Get result section
            # capture result key
            res_key  = [k for k in RES_KEYS if k in ''.join(res).lower().replace(' ','')]
...
# [end] _break_docstrings_in_tests_and_results

def make_test_body_from_test_result_data(mod_test_data):
    ...
    for key, test_tuples in mod_test_data.items():
        ...
        if not test_tuples:
            ...
            tst_str += '\n%s@MISSINGTEST'%(spc*tab_count) # from unittest_cases imported as * from header
            tst_str += func
            tst_str += '\n%spass\n'%(spc*(tab_count+1))
            continue
        ...
        for tst,res in test_tuples:
            ...
            l = res_part+'self.assertEqual(%s,%s)'%(last_tst, res_val)
            if not res_type:
                l=parts[0] + last_tst
            ...
            if res_assert:
                if '(' in res_assert: # assert given as a full function call
                    try:
                        l = res_part + res_assert %(last_tst, res_val)
                    except TypeError:
                        l = res_part + res_assert.format(last_tst, res_val)
                else:
                    # result assert just as a method name
                    l = res_part+'%s(%s,%s)'%(res_assert, last_tst, res_val)
            else: #use default asserts
                try:
                    if isinstance(ast.literal_eval(res[1]), float):
                        l = res_part+'self.assertAlmostEqual(%s,%s)'%(last_tst, res[1])
                except:
                    pass
                ...
                if res_type in RES_KEYS:
                    ...
                    if raise_key:
                        l = res_part + 'with self.assertRaises(%s): %s'%(raise_key, last_tst)
    ...
    return r'%s'%tst_str
# [end] make_test_body_from_test_result_data

def build_test_file(
        module_path, write=True, output='', header=headers.HEADER['default'],
        test_case='BaseTestCase', verbose=True):
    ...
    Args:
        ...
        - output `str`: Default ''. Finds the `tests` directory based on the
          `module_path`
        - header `str`: Default headers.HEADER['default']. The default header
          for the test file. Expects to have everything required imported.
...
def get_module_unittest_deco(module_path, token=':Unittest decorator:'):
