...
class Proto():
	...
	checks = [
		(
			...
			lambda codes, codes_file : self.raise_exception(Exception(f"Code list is empty {codes_file}")) #correts code, or logs, or throws error
		)
	]
	...
	def __init__(self, name, trud_codes_path=None):
		if trud_codes_path is not None:
			if trud_codes_path.is_file():  
				...
				raise FileNotFoundError(f"Error: Read2 code file '{trud_codes_path}' does not exist. Please ensure you have installed TRUD correctly")
	...
	def process(self, codes, codes_file):	
		...
		for msg, cond, fix in self.checks:
			...
			if not cond(codes).all(): 
				...
				logger.debug(f"Check: {msg} {(~cond(codes)).sum()} failed, trying to fix")			
...
class Read2(Proto):
	...
	def __init__(self):
		super().__init__('read2', trud.TRUD_PROCESSED_DIR / 'read2.parquet')
		...
		self.checks = [
			(
				...
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"Code list is empty",
										  codes=codes,
			...
			(
				...
				lambda codes, codes_file : codes.str.pad(width=5, side='right', fillchar='.')
			),
			(
			...
			(
				...
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"Illegal code format, not alphanumeric dot",
										  codes=codes,
										  codes_file=codes_file,
			...
			(
				...
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"Codes do not exist in database",
										  codes=codes,
										  codes_file=codes_file,
...
class Read3(Proto):
	def __init__(self):
		super().__init__('Read3', trud.TRUD_PROCESSED_DIR / 'read3.parquet')
		...
		self.checks = [
			...
			(
				...
				lambda codes : codes.str.match(r"^[a-zA-Z0-9.]+$"),
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"QA Alphanumeric Dot",
										  codes=codes,
										  codes_file=codes_file,
			...
			(
				...
				lambda codes : self.in_database(codes, self.db, self.name),
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"QA In Database",
										  codes=codes,
										  codes_file=codes_file,
...
class Icd10(Proto):
	def __init__(self):
		super().__init__('icd10', trud.TRUD_PROCESSED_DIR / 'icd10.parquet')
		...
		self.checks = [
			(
				...
				lambda codes : pd.Series([len(codes) > 0]),
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"Code list is empty {codes_file}",
										  codes=codes,
										  codes_file=codes_file,
										  mask=None,
										  code_type=self.name)))		
			...
			(
				...
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"QA Too Short",
										  codes=codes,
										  codes_file=codes_file,
			...
			(
				...
				lambda codes, codes_file : codes.str.replace(".", "") #delete any dots in string
			...
			(
				...
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"QA Alphanumeric Capital",
										  codes=codes,
										  codes_file=codes_file,
			...
			(
				"In Database",
				lambda codes : ~(~self.in_database(codes, self.db, self.name) & ~self.in_database(codes, self.db, self.name + "_alt")),
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"QA In Database",
										  codes=codes,
										  codes_file=codes_file,
										  mask=~(~self.in_database(codes, self.db, self.name) & ~self.in_database(codes, self.db, self.name+"_alt")), 
										  code_type=self.name)))
			)
# 			(
# 				"ICD10 Regex",
...
class Snomed(Proto):
	def __init__(self):
		super().__init__('snomed', trud.TRUD_PROCESSED_DIR / 'snomed.parquet')
		...
		self.checks = [
			...
			(
				...
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"QA Too Long",
										  codes=codes,
										  codes_file=codes_file,
			...
			(
				...
				lambda codes, codes_file : self.raise_exception(			
					InvalidCodesException(CodesError(f"QA Numeric",
										  codes=codes,
										  codes_file=codes_file,
...
class Opcs4(Proto):
	def __init__(self):
		super().__init__('opcs4', trud.TRUD_PROCESSED_DIR / 'opcs4.parquet')
