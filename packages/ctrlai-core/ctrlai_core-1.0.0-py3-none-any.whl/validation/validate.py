...
def validate_ctrlai_json(data: Dict[str, Any]) -> None:
    ...
    Raises:
        jsonschema.ValidationError: If the data is invalid according to the schema.
    """
...
def validate_ctrlai_pydantic(data: Dict[str, Any]) -> CtrlAI:
    ...
    Raises:
        pydantic.ValidationError: If the data is invalid according to the model.
    """
    return CtrlAI(**data)

def validate_ctrlai_embedding(value: Union[str, Dict[str, Any]]) -> List[float]:
    """
    Generates an embedding for a Ctrl AI value.

    Args:
        value: The value to generate an embedding for. Can be either a string
...
if __name__ == '__main__':
    ...
    valid_data = {
        ...
        "relatedEntities": [],
        "embedding": [0.1, 0.2, 0.3] # Add mock embedding
    }

    # Example invalid Ctrl AI data (missing required field)
    invalid_data = {
        "@context": [
            "https://schema.org/",
            "https://ctrlai.com/schema/"
    ...
    try:
        ...
        print(f"✗ Invalid data (jsonschema): {e}")
