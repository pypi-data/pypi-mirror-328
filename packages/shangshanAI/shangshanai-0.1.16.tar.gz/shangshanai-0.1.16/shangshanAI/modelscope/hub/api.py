...
from shangshanAI.modelscope.hub.constants import (API_HTTP_CLIENT_MAX_RETRIES,
...
from shangshanAI.modelscope.hub.errors import (InvalidParameter, NotExistError,
...
from shangshanAI.modelscope.hub.git import GitCommandWrapper
from shangshanAI.modelscope.hub.repository import Repository
from shangshanAI.modelscope.hub.utils.utils import (get_endpoint, get_readable_folder_size,
...
from shangshanAI.modelscope.utils.file_utils import get_file_hash, get_file_size
from shangshanAI.modelscope.utils.logger import get_logger
from shangshanAI.modelscope.utils.repo_utils import (DATASET_LFS_SUFFIX,
...
class HubApi:
    ...
    def __init__(self,
        ...
        self.revision = {"ref": revision} if revision else {}
        ...
        ssl_context.options |= getattr(ssl, 'OP_LEGACY_SERVER_CONNECT', 0x00000004)
        # adapter = HTTPAdapter(max_retries=retry, ssl_context=)
    ...
    def get_model_files(self,
        ...
        project_id = self.get_project_id(model_id) if self.project_id is None else self.project_id
        headers = self.headers if headers is None else headers
        query_url_template = '{endpoint}/hub/api/v4/projects/{project_id}/repository/tree?recursive={recursive}'
        if revision:
            query_url_template += '&ref={revision}'
    ...
    def dump_datatype_file(dataset_type: int, meta_cache_dir: str):
        ...
        `shangshanAI.modelscope.utils.constant.DatasetFormations`.
        """
    ...
    def get_project_id(self, model_id: str):
        """根据用户名和项目名获取项目ID"""
        user_name, project_name = model_id.split('/')
        download_url_template = '{endpoint}/hub/api/v4/users/{user_name}/projects?search={project_name}'
        url = download_url_template.format(endpoint=get_endpoint(), user_name=user_name, project_name=project_name)
    ...
    def get_model_file_url(self,
                          model_id: str,
                          file_path: str,
                          revision: str = None) -> str:
        file_path = urllib.parse.quote_plus(file_path)
        project_id = self.get_project_id(model_id) if self.project_id is None else self.project_id
        download_url_template = '{endpoint}/hub/api/v4/projects/{project_id}/repository/files/{file_path}/raw'
        if revision:
            download_url_template += '?ref={revision}'
    ...
    def get_file_metadata(self,
        ...
        headers = self.headers if headers is None else headers
        url = self.get_model_file_url(model_id, file_path, revision=revision)
        metadata_response = self.session.head(
            url,
            headers=headers
        )
        metadata_response.raise_for_status()
        return dict(metadata_response.headers)
