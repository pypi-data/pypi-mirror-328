...
def modify_model(windmill_client, artifact_name, model_path):
    """
    ...
    for model in manifest_list.subModels:
        ...
        if model["localName"] != model_name.local_name:
            modify_model(windmill_client, model["artifact"]["name"], model_path)
        ...
        config_path = os.path.join(sub_model_version_path, "config.pbtxt")
