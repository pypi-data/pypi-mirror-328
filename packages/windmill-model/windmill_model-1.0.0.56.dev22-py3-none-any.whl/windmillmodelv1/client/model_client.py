...
class ModelClient(BceInternalClient):
    ...
    def create_model(
        ...
        if artifact_uri != "":
            artifact_uri = ArtifactClient(self.config, context=self.context).create_location_with_uri(
                uri=artifact_uri, object_name=object_name, style=style, source_filesystem=source_filesystem
            )
...
def update_model_metadata(model_metadata):
    ...
    """
    # 1. 调整 extraModels 为空
    if model_metadata.get("extraModels") is not None and len(model_metadata.get("extraModels")) != 0:
