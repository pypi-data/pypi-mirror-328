...
#            print " INIT =",self._y[:,-1]
    ...
    def _rhointpe(self, phi):
        ...
        if phi > 0:
            ...
            kmax = phi - (self.B-1)/(1-self.C)

            P1 = ((1-self.B) + (1-self.C)*phi)/gamma(2.5) * (kmax**1.5 - phi**1.5)
            P2 = (1-self.C)/gamma(3.5)* (kmax**2.5 - phi**2.5)
            ...
            if (kmax > 0):
                print phi, kmax, " emin = ",(self.B-1)/(1-self.C)
                P1 = ((1-self.B) + (1-self.C)*phi)/gamma(2.5) * kmax**1.5 
                P2 = (1-self.C)/gamma(3.5)* kmax**2.5 
