...
class KFoldCrossValidator(CrossValidator):
    ...
    def _concatenate_predictions(self,
        ...
        if DatasetKeys.PROBABILITIES.value in datasets[0]:
            ...
            probabilities_df = pd.concat(dsets, axis=0)
            retval.probabilities = TableFactory(probabilities_df)  # type: ignore[abstract]  # pylint: disable=abstract-class-instantiated
