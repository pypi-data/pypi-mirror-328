...
def classification_results() -> Tuple[Dataset, Dataset]:
    ...
    pred.probabilities = TableFactory({'target': [.3, .4, .5, .5]})  # type: ignore[abstract]  # pylint: disable=abstract-class-instantiated
...
def test_roc_curve(classification_results: Tuple[Dataset, Dataset]) -> None:
    ...
    pd.testing.assert_frame_equal(got_designator1.dataframe, want)
