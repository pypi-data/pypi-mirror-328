...
log = Logger(__file__, to_stdout=True, level=Level.INFO).logger()
...
class NeighborState(
    ...
    def __init__(self, columns: Optional[List[str]] = None) -> None:
        if columns is not None:
            # This is here because mypy is missing typing issues on the columns.
            columns = self._cast_columns(columns=columns)
    ...
    def columns(self, value: Optional[List[str]]) -> None:
        ...
        if value is not None:
            # This is here because mypy is missing typing issues on the columns.
            value = self._cast_columns(columns=value)
        self._columns = value
    ...
    def state_differs(self,
        ...
        changed = bool(state_distance > distributed.fit_eps)
...
class DistributedAlgorithmInstance(FittableAlgorithmInstance):  # pylint: disable=too-many-public-methods,line-too-long
    ...
    def __init__(self,
        ...
        if kwargs:
            raise HyperparamError(
                f'Unexpected hyperparams to DistributedAlgorithmInstance: {kwargs}')
    ...
    def _resolve_neighbor_column_mismatch(self,
                                          dataset: Dataset,
                                          mismatched_neighbors: Dict[NodeID, Set[str]]) -> Dataset:
        ...
        message = (
            ...
            + '\n'.join(f'Neighbor node {node_id} has columns {cols}.'
                        for node_id, cols in mismatched_neighbors.items())
            + f'\n columns we see in the data: {self._columns}. \n'
        )
        log.error(message)
        raise NotImplementedError(message)

    def _get_columns(self, dataset: Dataset) -> List[str]:
        '''Get the columns of the dataset.'''
        if DatasetKeys.DATAFRAME.value in dataset:
            # TODO(piggy): Decide if we need to remove the target column.
            retval = dataset.dataframe.columns
        elif DatasetKeys.COVARIATES.value in dataset:
            retval = dataset[DatasetKeys.COVARIATES.value].columns
        else:
            raise NoDataError('No data found in dataset.')
    ...
    def _is_empty(self, dataset: Dataset) -> bool:
        ...
        keys_to_check = [DatasetKeys.DATAFRAME.value, DatasetKeys.COVARIATES.value]
        for k in keys_to_check:
            if k in dataset:
                df_k = pd.DataFrame()
                try:
                    df_k = pd.DataFrame(dataset[k])
                    if df_k.shape[1] > 0:  # at least one column
                        return False  # Found non-empty dataframe.
                except Exception as err:  # pylint: disable=broad-exception-caught
                    log.error('Could not transform item at key %s to dataframe: %s\n',
                              k, err)
        return True  # Found no non-empty dataframes.
    ...
    def _detect_convergence(self, state_changed: bool) -> None:
        ...
        neighbors_differ = [d > self._distributed.fit_eps for d in neighbor_distances]
    ...
    def _update_predict_state(self):
        ...
        if self._my_state is not None:
            with self._lock_predict_state:
                self._predict_state = self.my_state_copy
