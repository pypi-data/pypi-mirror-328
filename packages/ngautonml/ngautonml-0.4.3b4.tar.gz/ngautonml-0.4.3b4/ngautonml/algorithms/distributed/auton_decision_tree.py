...
class AutonDecisionTreeInstance(DistributedAlgorithmInstance):
    ...
    def _prepare_data(self,
                      dataset: Optional[Dataset]) -> Tuple[
                          np.float64, np.ndarray, np.ndarray, List[str]]:
        if dataset is None:
            ...
            if DatasetKeys.DATAFRAME.value in dataset:
                orig_cov = dataset.dataframe.copy()
                cols = list(orig_cov.columns)
                orig_cov.drop(columns=[target.name], inplace=True)
                orig_tar = dataset.dataframe.loc[:, target.name]
                x = orig_cov.to_numpy()
                y = orig_tar.to_numpy().flatten()
            else:
                cov_df = dataset[DatasetKeys.COVARIATES.value]
                assert isinstance(cov_df, pd.DataFrame), (
                    f'BUG: expected dataframe, instead found {cov_df} of type {type(cov_df)}'
                )
                x = cov_df.to_numpy()
                cols = list(cov_df.columns)
                y = dataset[DatasetKeys.TARGET.value][target.name].to_numpy().flatten()
        ...
        def recursive_fit(d: int,  # pylint: disable=too-many-statements,too-many-locals,too-many-branches
            ...
            if (self._max_depth is None or d < self._max_depth) \
                ...
                for f in self._random_state.permutation(self._my_state.n_in):
                    ...
                    loss[w_l < self._min_sample_weight] = np.infty
                    loss[w_r < self._min_sample_weight] = np.infty
                    loss[vol_l < self._min_vol] = np.infty
                    loss[vol_r < self._min_vol] = np.infty
                    ...
                    eps = 1e-5  # a tolerance to accommodate numerical imprecision
    ...
    def _predict(self, dataset: Optional[Dataset], **kwargs) -> Optional[Dataset]:
        ...
        if DatasetKeys.DATAFRAME.value in dataset:
            x = dataset.dataframe.to_numpy()
        elif DatasetKeys.COVARIATES.value in dataset:
            x = dataset[DatasetKeys.COVARIATES.value].to_numpy()
        else:
            log.error('Predict called on empty or malformed dataset: %s',
                      dataset)
            return None
