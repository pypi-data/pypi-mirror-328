...
from ..wrangler.dataset import Dataset, DatasetKeyError, DatasetValueError, RoleName
...
class SklearnMetric(SupervisedMetric):
    ...
    def _calculate(self,
        ...
        if self.needs_proba:
            try:
                proba_df = pred.probabilities.as_(pd.DataFrame)  # type: ignore[attr-defined]
                if proba_df.shape[1] <= 2:
                    # Assume binary classification
                    predictions_for_sklearn = proba_df.iloc[:, 0].to_numpy()
                else:
                    predictions_for_sklearn = proba_df.to_numpy()
    ...
    def calculate_roc_curve(self, pred: Dataset, ground_truth: Optional[Dataset] = None) -> Dataset:
        ...
        retval = pred.output()
        retval.dataframe = pd.DataFrame({'fpr': fpr, 'tpr': tpr, 'thresholds': thresholds})
        return retval
