...
class SklearnAlgorithmInstance(SklearnLikeAlgorithmInstance):
    ...
    def fit(self, dataset: Optional[Dataset]):
        ...
        # TODO(piggy): This logic belongs in Dataset.target and Dataset.covariates, not here.
        # This code allows a single 'dataframe' table instead of separate 'target' and 'covariates'.
        if DatasetKeys.TARGET.value not in dataset:
            if dataset.metadata.target is not None:
                target_for_sklearn = dataset.dataframe[dataset.metadata.target.name]
            else:
                # This is for fitting algorithms that don't need a target.
                target_for_sklearn = None
        else:
            target_data = dataset[DatasetKeys.TARGET.value]
            if isinstance(target_data, pd.DataFrame):
                assert dataset.metadata.target is not None
                target_for_sklearn = target_data[dataset.metadata.target.name]
            else:
                target_for_sklearn = pd.Series(list(target_data))

        if DatasetKeys.COVARIATES.value not in dataset:
            covariate_data = dataset.dataframe
            if isinstance(covariate_data, pd.DataFrame):
                assert dataset.metadata.target is not None
                covariates = dataset.dataframe.drop(
                    dataset.metadata.target.name, axis=1)
            else:
                covariates = list(covariate_data)
        else:
    ...
    def predict(self, dataset: Optional[Dataset]) -> Optional[Dataset]:
        ...
        try:
            ...
            elif DatasetKeys.DATAFRAME.value in dataset:
                df = dataset[DatasetKeys.DATAFRAME.value]
                if (dataset.metadata.target is not None
                        and dataset.metadata.target.name in df.columns):
                    df.drop(dataset.metadata.target.name, axis=1, inplace=True)
        ...
        if not is_empty(probabilities_result):
            ...
            retval.probabilities = TableFactory({  # type: ignore[abstract]  # pylint: disable=abstract-class-instantiated
