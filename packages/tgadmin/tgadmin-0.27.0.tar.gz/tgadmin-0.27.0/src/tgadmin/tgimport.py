...
def upload_modified(
    ...
    data_str = ET.tostring(
        etree_data, encoding='utf8', method='xml', default_namespace=default_namespace
    )
...
def check_path_and_rewrite(imex_map, data_path, element, ignore_warnings):
    ...
    if path in imex_map:
        ...
        message = f"'{element.text}' is referenced in portalconfig, but not found in import. Did you forget to add an collection with additional assets?"
        if not ignore_warnings:
            raise TextgridImportException(message)
        else:
            log.warning(f'{message} - but ignore warnings is enabled')
