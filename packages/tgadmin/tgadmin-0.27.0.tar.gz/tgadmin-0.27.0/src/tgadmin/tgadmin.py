...
from tgclients.auth import (
    ...
    TG_STANDARD_ROLE_MANAGER,
    TG_STANDARD_ROLE_OBSERVER,
)
...
def find_user(client, name, mail, org):
    ...
    tgauth: TextgridAuth = client.tgauth
    res = tgauth._client.service.getIDs(
        auth=client.sid, name=name, mail=mail, organisation=org
    )  # TODO: add to tgclients
    tabulate_tgauth_response(res, ['ePPN', 'name', 'mail', 'organisation'])
...
def add_role(
    ...
    if not (admin or manager or editor or observer):
        click.echo(f'Please specify at least one role you want {eppn} to have in {project_id}')
        sys.exit(1)
...
def remove_role(
    ...
    if not (admin or manager or editor or observer):
        click.echo(f'Please specify at least one role you want {eppn} to lose in {project_id}')
        sys.exit(1)

    if admin:
        _remove_role_from_project(client, project_id, eppn, TG_STANDARD_ROLE_ADMINISTRATOR)
    if manager:
        _remove_role_from_project(client, project_id, eppn, TG_STANDARD_ROLE_MANAGER)
    if editor:
        _remove_role_from_project(client, project_id, eppn, TG_STANDARD_ROLE_EDITOR)
    if observer:
        _remove_role_from_project(client, project_id, eppn, TG_STANDARD_ROLE_OBSERVER)

    print_project_members(client, project_id)


def _remove_role_from_project(client, project_id: str, eppn: str, role: str) -> bool:
    """TODO: add to tgclients"""
    rolename = role + ',' + project_id + ',Projekt-Teilnehmer'
    return client.tgauth._client.service.deleteMember(auth=client.sid, username=eppn, role=rolename)
...
def print_project_members(client, project_id):
    ...
    tabulate_tgauth_response(res, list(res[0]))


def tabulate_tgauth_response(data, keys):
    """A tgauth response consists of a list of objects, specify which keys to show."""
    tab = []
    for entry in data:
        tab.append([entry[key] for key in keys])
    click.echo(tabulate(tab, headers=keys, tablefmt='github'))
...
def contents(client, project_id):
    ...
    click.echo(f'project {project_id} contains {project_contents.hits} files:')
...
def clean_op(
    ...
    if not force:
        ...
        if int(project_contents.hits) > limit:
            click.echo(f' ...and ({int(project_contents.hits) - limit}) more objects')

    if not force and not click.confirm('Do you want to delete all these objects'):
        sys.exit(0)
    else:
        with click.progressbar(
            length=int(project_contents.hits),
...
def update_imex(
    ...
    Argument 1 is the IMEX file, argument 2 the path where the data
        ...
        for path, textgrid_uri in bar:
            ...
            with open(filename, 'rb') as the_data:
                ...
                if is_aggregation(metadata.object_value):
                    ...
                    for ore_aggregates in the_dataXML_root.findall('.//ore:aggregates', NAMESPACES):
                    ...
                    the_data = ET.tostring(the_dataXML_root, encoding='utf8', method='xml')
                    ...
                    if is_edition(metadata.object_value):
                        ...
                        click.echo(
                            f'changing isEdition of for {filename} from {work_path} to {work_uri}'
                        )
...
def put_aggregation(client, project_id, aggregation_files, threaded, ignore_warnings):
    ...
    try:
        res = aggregation_import(
            client.crud, client.sid, project_id, files, threaded, ignore_warnings
        )
...
def portalconfig(xslt):
    ...
    click.secho(f"""Config file templates and avatar example image downloaded.
...
def publish(client, textgrid_uri, really_publish):
    ...
    if is_collection(metadata) or is_edition(metadata):
        jobid = client.publish.publish(client.sid, textgrid_uri, dry_run=dryrun)
    elif is_portalconfig(metadata) or is_readme(metadata):
        publish_technical_object(client.publish._url, client.sid, textgrid_uri, dry_run=dryrun)
        jobid = textgrid_uri
    else:
        sys.exit(1)
...
def publish_technical_object(
    url, sid: str, textgrid_uri: str, ignore_warnings: bool = False, dry_run: bool = True
) -> str:
    """TODO: move to tgclients"""
    params = {
        'sid': sid,
    ...
    response = requests.get(
        url + '/' + textgrid_uri + '/publishWorldReadable',
        params=params,
...
def export_batch(client, textgrid_uris, output_path: str = './tgexport'):
    ...
    click.secho(
        f"""Finished. You may change your data in dir '{output_path}' and re-import with 'tgadmin update-imex {output_path}/.INDEX.imex'.
