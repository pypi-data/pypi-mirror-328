...
def get_data(api_key: str):
    ...
    """
    with urlopen(f"http://206.12.95.102/transfer_capacities_copper?key={api_key}") as response:
        response_content = response.read()
        json_response = json.loads(response_content)
        modeled_attributes = pd.json_normalize(json_response)

    modeled_attributes = modeled_attributes[
        ['balancing_area_from', 'balancing_area_to', 'ttc_winter']
    ].rename(columns={'balancing_area_from': 'region', 'balancing_area_to': 'variable', 'ttc_winter': 'value'})
