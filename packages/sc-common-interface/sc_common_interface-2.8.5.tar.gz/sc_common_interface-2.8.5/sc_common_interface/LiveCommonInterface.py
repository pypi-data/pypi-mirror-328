...
def get_receipt_sales_id(data):
    ...
    if flag:
        #先从列表查出一个最新结束的直播间，还是不返回要求再创建直播和链接直播
