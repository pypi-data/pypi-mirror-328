...
class InputData:
    ...
    def add_data(self, label, content):
        ...
        if label == "pubs":
            content = self.format_pubs(content)
    ...
    def check_label(self, label, content):
        ...
        if not label in ["pubs", "molecule-info"]:
            raise Exception("Only 'pubs' or 'molecule-info' can be specified as a input data labels.")
        if label == "pubs" and type(content) != list:
            raise Exception("For a 'pubs' data label the content of data must be a list of PUBs. Each PUB must be a tuple containing a quantum circuit, optionally second a list of circuit parameters and optionally third a number of shots.")

    def format_pubs(self, content):
        pubs = []
        for pub in content:
            pubs.append(self.validate_and_serialize_pub(pub))
        return pubs
    ...
    def validate_and_serialize_pub(self, pub):
        ...
        elif quantum_circuit.num_parameters != len(paramaters):
        ...
        if paramaters is not None and not all(isinstance(item, (int, float)) for item in paramaters):
            raise Exception("The 'paramaters' setting in a PUB must be a list of floating point numbers.")
