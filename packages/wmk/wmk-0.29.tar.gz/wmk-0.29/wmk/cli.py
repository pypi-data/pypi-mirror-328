...
@click.option(
    ...
    required=False,
    multiple=True,
    help='Dependencies to skip during download'
)
@click.option(
    '--manual',
    is_flag=True,
    default=False,
    help='Pause for confirmation before creating archive'
)
def package(target, name, platform, only_tracked, additional_files, build_version, python_version, skip, manual):
    """Download Python packages and create archive"""
    packager = Packager(target, platform, only_tracked, additional_files, build_version, python_version, skip)
