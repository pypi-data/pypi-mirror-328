...
class TableStep(InferenceStep):
    ...
    """
    step_type: ClassVar[StepTypes] = field(default=StepTypes.TABLE)
    helper_prompt: str = ""

@dataclass
class RowStep(InferenceStep):
    """
    Use this step to extract data that is already in tabular format (eg.: CSV).
    """
    step_type: ClassVar[StepTypes] = field(default=StepTypes.ROWS)

@dataclass
class ReverseGeoStep(InferenceStep):
    """
    Use this step to perform reverse geolocation based on the source field.
    """
    step_type: ClassVar[StepTypes] = field(default=StepTypes.REVERSE_GEO)

@dataclass
class NormaliseStep(BaseStep):
    """
    ...
    for the same information, hence the need to normalise the column names.

    Encapsulates the configuration necessary to perform the 'normalise' step.
    ...
    :type mode: NormaliseModes
    :ivar helper_prompt: Optional helper text or prompt information.
    :type helper_prompt: str
    """
    step_type: ClassVar[StepTypes] = field(default=StepTypes.NORMALISE)
...
class ValidateStep(BaseStep):
    ...
    """
    step_type: ClassVar[StepTypes] = field(default=StepTypes.VALIDATE)
    mode: ValidateModes
    columns: List[str]
