...
from eigenmorphic.eigenvalues import dimension_eigenvalues
...
def balanced_pair_algorithm(s, w=None, pv=None, getgraph=0, getbp=0, verb=0):
    """
...
def has_pure_discrete_spectrum(s, verb=0):
    ...
    if verb > 1:
        print("incidence matrix:")
        print(m)
    b, pv, _ = max(m.eigenvectors_left())
    pv = pv[0]
    if verb > 1:
        print("pv = %s" % pv)
    # compute the dimension of the Q-vector space generated by eigenvalues
    d = dimension_eigenvalues(s)
    if verb > 0:
        print("dimension eigenvalues :", d)
    db = b.minpoly().degree()
    if d < db:
        if verb > 0:
            print("not enough eigenvalues to have pure discrete spectrum : %s < %s" % (d, db))
        return False
    # find a power of s that have fixed point
    p = 1
    while (len((s**p).fixed_points()) == 0):
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        for n in range(1,1000): # browse possible lengths
            ...
            for w in sw:
                ...
                futures.append((w,executor.submit(balanced_pair_algorithm, s, w, pv)))
            for _ in range(40*n):
                time.sleep(.1)
                # test if one task has finished
                futures2 = []
                for w,task in futures:
                    if task.done():
                        res = task.result()
                        if res or is_prefix(w+w[:1], s):
                            if verb > 0:
