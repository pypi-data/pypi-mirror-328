...
class EventType:
    ...
    def read(self, read: PbdfReader) -> None:
        ...
        self.params = [read.bytes(self.param_size) for _ in range(read.i32())]

    def write(self, write: PbdfWriter) -> None:
        write.str(self.name)
        write.i32(self.param_size)
        write.i32(len(self.params))
        [write.bytes(x) for x in self.params]
...
class EventTypeFile:
    ...
    def calc_param_size_total(self) -> int:
        return sum([t.param_size * len(t.params) for t in self.types])
...
class AnimFile:
    ...
    def read(self, read: PbdfReader, route_sector_count: int) -> None:
        self.event_list = read.any(EventList)
        ...
        self.sector_instances = [read.any(AnimInstanceList) for _ in range(route_sector_count)]

    def write(self, write: PbdfWriter, route_sector_count: int) -> None:
        write.any(self.event_list)
...
class SoundEmitter:
    exist: bool = False
    position: vec3 = field(default_factory=vec3_new)
    unknown: bytes = field(default_factory=lambda: bytes(36))
    sound_id: int = 0

    def read(self, read: PbdfReader) -> None:
        self.exist = read.b32()
        self.position = read.vec3()
        self.unknown = read.bytes(36)
        self.sound_id = read.i32()

    def write(self, write: PbdfWriter) -> None:
        write.b32(self.exist)
        write.vec3(self.position)
        write.bytes(self.unknown)
        write.i32(self.sound_id)


@dataclass
class SoundFile:
    emitters: list[SoundEmitter] = field(default_factory=list)

    def read(self, read: PbdfReader) -> None:
        self.emitters = [read.any(SoundEmitter) for _ in range(read.i32())]

    def write(self, write: PbdfWriter) -> None:
        write.i32(len(self.emitters))
        [write.any(x) for x in self.emitters]
...
class Circuit:
    ...
    sound_filename: str = "NEANT"
    sound_file: SoundFile = field(default_factory=SoundFile)
    ...
    def read(self, read: PbdfReader) -> None:
        ...
        if read.format.version < 6:
            self.sound_filename = read.str()
            if self.sound_filename.upper() != "NEANT":
                self.sound_file = read.any(SoundFile)
    ...
    def write(self, write: PbdfWriter) -> None:
        ...
        if write.format.version < 6:
            write.str(self.sound_filename)
            if self.sound_filename.upper() != "NEANT":
                write.any(self.sound_file)
