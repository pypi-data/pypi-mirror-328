...
def graph(function):
    ...
    # Compute critical points and filter for finite real values
    ex = critical_points(function)
    ex = [float(val) for val in ex if is_finite(val)]

    # Compute x-limits
    x_min = min(ex) - 1 if ex else -10
    x_max = max(ex) + 1 if ex else 10

    # Handle empty or invalid x-limits gracefully
    if x_min >= x_max or not ex:
        x_min, x_max = -10, 10
