...
def get_task_template(node, **kwargs):
    ...
    use_job_override = bool(node.parm("job_override_checkbox").eval())
    job_override_value = node.parm("job").eval()
    ...
    if use_job_override:
        data['job_override'] = "--job {}".format(job_override_value)

    print("Setting job override {}, {}".format(use_job_override, job_override_value))
    ...
    return "{hserver}hython \"{script}\" -f {first} {last} {step} -d {driver} {job_override} \"{hipfile}\"".format(**data)
