...
""",
    ...
    def create(
        ...
        **kwargs,
        ...
        response = Util().request_apiv2(
            "POST", ENGAGEMENTS_URL, api_key, data=request_json
        )
        return response
    ...
    def _create(self):
        ...
        parser = argparse.ArgumentParser(
            description="Create an engagement on DefectDojo",
        ...
        optional.add_argument("--repo_url", help="Link to source code management")
        optional.add_argument(
            "--branch_tag",
        ...
        optional.add_argument(
            "--product_version", help="Version of the product the engagement tested"
        )
        optional.add_argument(
            "--tracker", help="Link to epic or ticket system with changes to version."
        )
        optional.add_argument(
            "--tag", help="Engagement tag (can be used multiple times)", action="append"
        )
        optional.add_argument(
            "--local_dedup",
    ...
    def _close(self):
        # Read user-supplied arguments
        parser = argparse.ArgumentParser(
            description="Close an engagement on DefectDojo",
    ...
    def update(
        ...
        **kwargs,
        ...
        # Make the request
        response = Util().request_apiv2(
            "PATCH", ENGAGEMENTS_ID_URL, api_key, data=request_json
        )
        return response
    ...
    def _update(self):
        # Read user-supplied arguments
        parser = argparse.ArgumentParser(
            description="Update a engagement on DefectDojo",
        ...
        optional.add_argument(
            "--desc", help="Engagement description", metavar="DESCRIPTION"
        )
        optional.add_argument(
            "--product_id", help="ID of the product the engagement belongs to"
        )
        optional.add_argument(
            "--lead_id", help="ID of the user responsible for this engagement"
        )
        optional.add_argument(
            "--start_date", help="Engagement starting date", metavar="YYYY-MM-DD"
        )
        optional.add_argument(
            "--end_date", help="Engagement ending date", metavar="YYYY-MM-DD"
        )
        optional.add_argument(
            "--type", help="Engagement type", choices=["Interactive", "CI/CD"]
        )
    ...
    def list(self, url, api_key, name=None, product_id=None, **kwargs):
        ...
        # Make the request
        response = Util().request_apiv2(
            "GET", ENGAGEMENTS_URL, api_key, params=request_params
        )
        return response
    ...
    def _list(self):
        # Read user-supplied arguments
        parser = argparse.ArgumentParser(
            description="List an engagement on DefectDojo",
        ...
        optional.add_argument("--name", help="Engagement name")
        optional.add_argument("--product_id", help="Product ID")
        parser._action_groups.append(optional)
        # Parse out arguments ignoring the first three (because we're inside a sub_command)
        args = vars(parser.parse_args(sys.argv[3:]))
    ...
    def _reopen(self):
        # Read user-supplied arguments
        parser = argparse.ArgumentParser(
            description="Reopen an engagement on DefectDojo",
