...
""",
    ...
    def import_(
        ...
        **kwargs,
        ...
        response = Util().request_apiv2(
            "POST", IMPORT_SCAN_URL, api_key, files=files, data=request_json
        )
        return response
    ...
    def _import(self):
        ...
        parser = argparse.ArgumentParser(
            description="Import findings (scan results) to DefectDojo",
            usage="defectdojo findings import RESULT_FILE [<args>]",
            formatter_class=RichHelpFormatter,
        )
        optional = parser._action_groups.pop()
        required = parser.add_argument_group("required arguments")
        parser.add_argument("result_file", help="File with the results to be imported")
        required.add_argument("--scanner", help="Type of scanner", required=True)
        ...
        optional.add_argument(
            "--tag_test", help="Test tag (can be used multiple times)", action="append"
        )
        optional.add_argument(
            "--note",
        ...
        optional.add_argument(
            "--skip_duplicates",
            help="Dont import duplicates " "(requires deduplication) (default = False)",
            action="store_true",
        )

        optional.add_argument("--version", help="Current version of the project")
        ...
        if args["note"] is not None:
            ...
            tmp_args["test_id"] = import_out[
                "test"
            ]  # Get the test ID from the import output
    ...
    def _reimport(self):
        ...
        required.add_argument("--test_id", help="Test to reimport", required=True)
        ...
        **kwargs,
        ...
        if test_type is not None:
            ...
            for tt in test_type:
                if type(tt) is str:
                    ...
                    temp_response = Util().request_apiv2(
                        "GET", API_URL + "/test_types/", api_key, params=temp_params
                    )
            ...
            if len(test_type_ids) == 1:
                ...
                return self.list_multiple_test_types(
                    url, api_key, test_type_ids, **request_params
                )
    ...
    def _list(self):
        # Read user-supplied arguments
        parser = argparse.ArgumentParser(
            description="List findings stored on DefectDojo",
        ...
        optional.add_argument(
            "--active",
            help="List only actives findings",
            action="store_true",
            dest="active",
        )
        optional.add_argument(
            "--inactive",
            help="List only inactives findings",
            action="store_false",
            dest="active",
        )
        optional.add_argument(
            "--closed", help="List only closed/mitigated fidings", action="store_true"
        )
        optional.add_argument(
            "--valid",
            ...
            if args["json"] is True:  # If --json flag was passed
                ...
                if findings_amount > 0:
                    ...
                    if args["product_id"] is not None:  # If a product id was passed
                        ...
                        findings_list_url = response.request.url.replace(
                            "api/v2/findings/",
                    ...
                    for finding in json_out["results"]:
                        ...
                        if (
                            len(finding["title"]) <= 70
                        ):  # Truncate title bigger then 70 chars
                            table["Title"].append(finding["title"])
                        else:
                            table["Title"].append(finding["title"][:70] + "...")
                        table["URL"].append(
                            args["url"] + "/finding/" + str(finding["id"])
                        )
                    print(tabulate(table, headers="keys", tablefmt="fancy_grid"))

                    # Exit
                    if (
                        args["fail_if_found"] != "NULL"
                    ):  # If --fail_if_found flag was passed
    ...
    def update(self, url, api_key, finding_id, active=None, mitigated=None, **kwargs):
        ...
        # Make the request
        response = Util().request_apiv2(
            "PATCH", FINDINGS_ID_URL, api_key, data=request_json
        )
        return response
    ...
    def _update(self):
        # Read user-supplied arguments
        parser = argparse.ArgumentParser(
            description="Update a finding on DefectDojo",
        ...
        optional.add_argument(
            "--active",
            help="Set finding as active (true) or inactive (false)",
            choices=["true", "false"],
        )
        optional.add_argument(
            "--mitigated",
            help="Indicates if the finding is mitigated (true) or not (false)",
            choices=["true", "false"],
        )
        parser._action_groups.append(optional)
    ...
    def _close(self):
        # Read user-supplied arguments
        parser = argparse.ArgumentParser(
            description="Close a finding on DefectDojo",
    ...
    def add_note(
        ...
        # Make the request
        response = Util().request_apiv2(
            "POST", FINDINGS_ID_NOTES_URL, api_key, data=request_json
        )
        return response
