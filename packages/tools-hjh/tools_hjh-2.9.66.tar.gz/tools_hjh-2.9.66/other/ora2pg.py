...
copy_data 现在有限支持PG到Oracle和PG到PG的数据复制，P2O或P2P不支持分页，不支持检查点
copy_index 复制索引
copy_pk 复制主键
copy_fk 复制外键
copy_uk 复制唯一键
...
copy_table：RAW(16)类型，同时设置默认值为sys_guid()，那么会转为PG的UUID类型。其他情况转为bytea，想转为VARCHAR(32)需要手动调整
copy_data，O2P时，is_clear=true会提前清空表
copy_data，O2P时，oracle中的chr(0)会被强行替换为chr(32)
copy_data，O2P时，oracle中date和timestamp类型中的0000年会被强行替换为0001年，00月替换为01月，00日替换为01日
copy_data，O2P时，收集元数据时，会count表，is_count_full=true则强制走全表，避免索引与表不同步的情况，因此大表可能比较慢
copy_data，O2P时，RAW(16)类型，且配置文件中设置is_use_rawtohex=true，会调用rawtohex()函数处理数据后存入PG
...
is_use_rawtohex = conf_map['is_use_rawtohex']
...
def get_table_map_list(src_db):
    ...
    for table_mess in tables.split('[--split--]'):
        ...
        if src_table != '*':
            ...
            for table_from_sql in tables_from_sql:
                if '-->' in table_mess and dst_db_type == 'oracle':
                    table_map = (src_schema, table_from_sql[0], dst_schema, table_from_sql[0].upper(), src_where)
                elif '-->' in table_mess and dst_db_type == 'pgsql':
                    table_map = (src_schema, table_from_sql[0], dst_schema, table_from_sql[0].lower(), src_where)
                elif '-->' not in table_mess and dst_db_type == 'oracle':
                    table_map = (src_schema, table_from_sql[0], src_schema.upper(), table_from_sql[0].upper(), src_where)
                elif '-->' not in table_mess and dst_db_type == 'pgsql':
                    table_map = (src_schema, table_from_sql[0], src_schema.lower(), table_from_sql[0].lower(), src_where)
                if table_map not in table_map_list and table_map[0] + '.' + table_map[1] not in exclude_tables:
                    table_map_list.append(table_map)
...
def truncate_table(dst_db, table_map):
    try:
        ...
        log.warning('PG执行SQL失败', truncate_table_sql, str_exception(_))
    finally:
        dst_cur.close()
        dst_conn.close()
...
def get_data_from_oracle(table_map, mess, table_scn, col_desc, table_metadata=None, page=None, page_rn=None):
    ...
    def ready():
        try:
            ...
            for col in cols_for_copy:
                ...
                elif 'RAW' in str(col[1]) and col[2] == 16 and is_use_rawtohex == 'true':
                    once_num = once_num_lob
                    merge_col[idx] = [idx2]
                    idx2 = idx2 + 1
            ...
            log.error('到Oracle获取数据失败，准备阶段出错', my_mess, str_exception(_))
    ...
    try:
        ...
        # log.error('到Oracle获取数据失败', mess, '获取Oracle连接超时60s，请使用后面SQL，使用copy_from_sql补数', select_sql, str_exception(_))
    ...
    try: 
        ...
        log.error('到Oracle获取数据失败，抽数阶段出错', mess, str_exception(_))
        return None
    finally:
        src_db.close()
        dst_db.close()
...
def save_to_pg(dst_db, table_map, rows, rows_size, cols, mess, select_time, table_scn, is_exists_blob):
    ...
    if is_only_insert == 'true' or is_exists_blob:
        try:
            ...
            log.error('写入PG失败', mess, str_exception(_))
            return
    else:
        try:
            ...
            log.warning('写入PG失败，改为insert重试', mess, str_exception(_))
            try:
                num = dst_db.run(insert_sql, rows)
                if num == -1:
                    raise Exception('unknown error, num = -1')
        ...
        if is_clear == 'true':
            ...
            for rs in rss:
                ...
                try:
                    ...
                    log.error('PG执行SQL失败', drop_index_sql, str_exception(_))
            ...
            if sql.startswith('create table '):
                ...
                try:
                    ...
                    if 'already exists' in str(_) and is_clear == 'true':
                        log.warning('PG存在重名对象，自动改名重试', sql, str_exception(_))
                        ...
                        try:
                            ...
                            log.error('PG执行SQL失败', sql, str_exception(_))
                    else:
                        log.error('PG执行SQL失败', sql, str_exception(_))
    ...
    def copy_one_table(src_db, dst_db, table_map, run_mode):
        ...
        if run_mode == 'copy_table':
            try:
                ...
                log.error('PG执行SQL失败', drop_sql, str_exception(_))
                ...
                if run_mode == 'copy_table':
                    try:
                        ...
                        if sql.startswith('create table ') and 'partition of' not in sql:
                            log.error('PG创建表失败', sql, str_exception(_))
                            break
                        log.error('PG执行SQL失败', sql, str_exception(_))
                elif run_mode == 'get_table_ddl':
                    log.out(sql + ' ;')
    ...
    for schema in schemas:
        ...
        for row in rows:
            ...
            try:
                ...
                log.error('PG执行SQL失败', create_sql, str_exception(_))
...
def compare_index():
    ...
    try:
        ...
        log.error('比较索引数量时，Oracle连接失败', str_exception(_))
...
def compare_constraint():
    ...
    try:
        ...
        log.error('比较键数量时，Oracle连接失败', str_exception(_))
...
def pg2xxx_copy_data():
    ...
    pp = ProcessPool(parallel_num)
    for table_map in table_map_list:
        pp.run(pg2xxx_copy_data_one, (table_map,), name=table_map[0] + '.' + table_map[1], error_callback=print_error)
...
def pg2xxx_copy_data_one(table_map):
    src_db = DBConn(src_db_type, src_ip, src_port, src_database, src_read_username, src_read_password)
    dst_db = DBConn(dst_db_type, dst_ip, dst_port, dst_database, dst_username, dst_password)
    src_schema = table_map[0]
    src_table = table_map[1]
    ...
    try:
        ...
        log.error('PG到' + dst_db_type + '的导数失败', src_schema + '.' + src_table + '-->' + dst_schema + '.' + dst_table, str_exception(_))
    finally:
        src_db.close()
        dst_db.close()
