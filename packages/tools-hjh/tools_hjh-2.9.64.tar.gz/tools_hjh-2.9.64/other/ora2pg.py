...
drop_fk 删除PG端已存在的外键
pg2ora_copy_data 用于从PG同步数据到Oracle，需要表提前建好，is_clear参数会生效，支持并行，但不支持分页，tables中不支持写*，但可以写多个表和where条件，没有检查点，不会对数
...
copy_data，RAW(16)类型，且配置文件中设置is_use_rawtohex=true，会调用rawtohex()函数处理数据后存入PG
...
def pg2ora_copy_data():
    ...
    pp = ProcessPool(parallel_num)
    for table_map in table_map_list:
        pp.run(pg2ora_copy_data_one, (table_map,), error_callback=print_error)
    pp.wait()
...
def pg2ora_copy_data_one(table_map):
    src_db = DBConn(src_db_type, src_ip, src_port, src_database, src_read_username, src_read_password)
    dst_db = DBConn(dst_db_type, dst_ip, dst_port, dst_database, dst_username, dst_password)
    ...
    try:
        ...
        log.error('pg2ora_copy_data失败', src_schema + '.' + src_table + '-->' + dst_schema + '.' + dst_table, str(_))
    finally:
        src_db.close()
        dst_db.close()
