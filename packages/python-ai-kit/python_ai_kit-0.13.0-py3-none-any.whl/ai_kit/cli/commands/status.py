...
def status_command() -> None:
    ...
    table.add_row(
        Text("🔑 Required API Keys", style="bold yellow"),
        "",
        ""
    )
    ...
    # Add API keys section
    table.add_row(
        Text("🔑 Optional API Keys", style="bold yellow"),
        "",
        ""
    )

    for key in LiteLLMConfig.optional_api_keys():
        value = os.getenv(key)
        has_key = value is not None
        status_style = "green" if has_key else "blue"
        status_icon = "✓" if has_key else "✗"
        url = LiteLLMConfig.API_KEY_URLS.get(key, "#")
        link_text = f"[white link={url}]{truncate_url(url)}[/]" if url != "#" else ""
        table.add_row(
