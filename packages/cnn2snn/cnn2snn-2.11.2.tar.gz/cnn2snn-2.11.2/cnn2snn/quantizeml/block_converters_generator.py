...
def generate_block_converters(model):
    ...
    for id, block in enumerate(blocks):
        ...
        if not block_converter:
            layers = [type(layer).__name__ for layer in non_skippable]
            supported_patterns = _display_supported_patterns(layers)
            raise RuntimeError(f"Invalid block pattern to conversion. Receives {layers}. "
