...
class Matrix3x3(MatrixBase):
    ...
    def diagonal(cls, value: Union[NUM, Iterable[NUM]]) -> "Matrix3x3":
        ...
        if hasattr(value, "__iter__"):
            ...
            return cls(value[0], 0.0, 0.0, 0.0, value[1], 0.0, 0.0, 0.0, value[2])
        return cls(value, 0.0, 0.0, 0.0, value, 0.0, 0.0, 0.0, value)
    ...
    def _force_dim(
        ...
        if b:
            le = len(v)
            if le == 3:
                v = (v[0], fill, fill, fill, v[1], fill, fill, fill, v[2])
            elif le != 9:
                raise MatrixException(ERROR_3D_MESSAGE) from None
        elif not b:
            with_fill = (v, fill, fill, fill, v, fill, fill, fill, v)
            with_scalar = (v, v, v, v, v, v, v, v, v)
    ...
    @classmethod
    def _force_dim_vec(cls, v: MATRIX_TYPE, sizes: Tuple[int] = (3, 9)) -> Tuple[NUM]:
        try:
            v = flatten(v)
    ...
    def __matmul__(self, other: MATRIX_TYPE) -> Union["Matrix3x3", Vector3D]:
        ...
        if len(other) == 9:
            m11 = self.m11 * other[0] + self.m21 * other[1] + self.m31 * other[2]
            m12 = self.m12 * other[0] + self.m22 * other[1] + self.m32 * other[2]
            m13 = self.m13 * other[0] + self.m23 * other[1] + self.m33 * other[2]
            m21 = self.m11 * other[3] + self.m21 * other[4] + self.m31 * other[5]
            m22 = self.m12 * other[3] + self.m22 * other[4] + self.m32 * other[5]
            m23 = self.m13 * other[3] + self.m23 * other[4] + self.m33 * other[5]
            m31 = self.m11 * other[6] + self.m21 * other[7] + self.m31 * other[8]
            m32 = self.m12 * other[6] + self.m22 * other[7] + self.m32 * other[8]
            m33 = self.m13 * other[6] + self.m23 * other[7] + self.m33 * other[8]
    ...
    def __rmatmul__(self, other: MATRIX_TYPE) -> Union["Matrix3x3", Vector3D]:
        ...
        if len(other) == 9:
            m11 = other[0] * self.m11 + other[3] * self.m12 + other[6] * self.m13
            m12 = other[1] * self.m11 + other[4] * self.m12 + other[7] * self.m13
            m13 = other[2] * self.m11 + other[5] * self.m12 + other[8] * self.m13
            m21 = other[0] * self.m21 + other[3] * self.m22 + other[6] * self.m23
            m22 = other[1] * self.m21 + other[4] * self.m22 + other[7] * self.m23
            m23 = other[2] * self.m21 + other[5] * self.m22 + other[8] * self.m23
            m31 = other[0] * self.m31 + other[3] * self.m32 + other[6] * self.m33
            m32 = other[1] * self.m31 + other[4] * self.m32 + other[7] * self.m33
            m33 = other[2] * self.m31 + other[5] * self.m32 + other[8] * self.m33
    ...
    def __getitem__(self, index: Union[int, slice]) -> Union[List[Vector3D], Vector3D]:
    ...
    def __eq__(self, value: MATRIX_TYPE) -> Tuple[bool]:
        try:
            ...
            return tuple(map(lambda t: t[0] == t[1], zip(self._values, values)))
        except ValueError:
            raise MatrixException(ERROR_3D_MESSAGE) from None
    ...
    @classmethod
    def rotation(cls, axis: Iterable[NUM], angle: NUM) -> "Matrix3x3":
    ...
    def inverse(cls, value: MATRIX_TYPE) -> "Matrix3x3":
        ...
        adjugate = Matrix3x3(d11, -d21, d31, -d12, d22, -d32, d13, -d23, d33)
