...
ERROR_COMP_MESSAGE = "Vectors must have equal number of " "dimensions for comparison."
ERROR_SET_MESSAGE = "Can only set a vector to a " "number or an iterable of numbers."
ERROR_COMP_ELEM_MESSAGE = (
    "Can only compare a vector to a " "number or an iterable of numbers."
)
ERROR_SET_ELEM_MESSAGE = (
    "Can only set elements of a vector to a " "number or an iterable of numbers."
)
ERROR_SET_PROPERTY = (
    "Can not set individual components. Try setting the property " "directly instead."
)
...
def _safe_vector_op(message: str):
    def _safe_vector_op_wrapped(func: Callable[[], T], *args, **kwargs) -> T:
        try:
...
class VectorBase(ABC):
    ...
    def _iter_conv_set(self, iterable: Iterable[NUM], length: int) -> Tuple[NUM]:
        values = tuple(float(v) for v in iterable)
    ...
    def __setitem__(self, index: Union[int, slice], value: Union[NUM, Iterable[NUM]]):
        try:
    ...
    def __format__(self, format_spec: str) -> str:
        return "({})".format(
            ", ".join(map(lambda x: format(x, format_spec), self._values))
        )
    ...
    def __repr__(self) -> str:
        return "{}({})".format(
            self.__class__.__qualname__, ", ".join((str(x) for x in self._values))
        )
