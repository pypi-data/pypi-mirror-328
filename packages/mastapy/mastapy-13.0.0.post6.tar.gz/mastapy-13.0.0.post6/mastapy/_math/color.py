...
class Color(Vector4D):
    ...
    def wrap(cls, value: Any) -> "Color":
        try:
            ...
            raise VectorException("Value to wrap has no R, G, B or A component.")
    ...
    def __format__(self, format_spec: str) -> str:
        return "({})".format(
            ", ".join(map(lambda x: format(x, format_spec), map(int, self._values)))
        )

    def __repr__(self) -> str:
        return "{}({})".format(
            self.__class__.__qualname__, ", ".join((str(int(x)) for x in self._values))
        )
