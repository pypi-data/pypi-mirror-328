...
from hestia_earth.aggregation.cycle.utils import (
    AGGREGATION_KEYS, format_for_grouping, _update_cycle, format_country_results
)
...
fixtures_folder = os.path.join(fixtures_path, 'cycle', 'countries')
...
def test_aggregate(
    ...
    with open(os.path.join(fixtures_path, 'cycle', 'terms', filename), encoding='utf-8') as f:
        cycles = json.load(f)
    ...
    results = list(map(format_country_results, results))
    results = list(map(_update_cycle(None, start_year, end_year, SOURCE, functional_unit, False), results))
