...
class OpenAILanguageModel(LanguageModel):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        """
        response: ChatCompletion = self._client.chat.completions.create(  # type: ignore[call-overload]
            messages=messages,  # type: ignore[arg-type]
            model=model,
            tools=[tool] if tool else NOT_GIVEN,  # type: ignore[list-item]
            max_tokens=max_tokens if max_tokens else NOT_GIVEN,
            frequency_penalty=frequency_penalty if frequency_penalty else NOT_GIVEN,
    ...
    async def agenerate(
        ...
        **kwargs: Any,
        ...
        Returns:
            Generation: Generated response based on the input messages and model parameters.
        """
        response: ChatCompletion = await self._aclient.chat.completions.create(  # type: ignore[call-overload]
            messages=messages,  # type: ignore[arg-type]
            model=model,
            tools=[tool] if tool else NOT_GIVEN,  # type: ignore[list-item]
            max_tokens=max_tokens if max_tokens else NOT_GIVEN,
