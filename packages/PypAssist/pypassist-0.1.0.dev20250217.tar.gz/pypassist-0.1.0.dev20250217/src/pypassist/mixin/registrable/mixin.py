...
class Registrable:
    ...
    def __init_subclass__(cls, *, register_name=None):
        ...
        Raises:
            ...
            DuplicateRegistrationError: If the name is already registered
        """
        if Registrable in cls.__bases__:
            if not hasattr(cls, cls._REGISTER_NAME):
                raise AttributeError(
                    f"Registrable classes must have a {cls._REGISTER_NAME!r} attribute"
                )
            setattr(cls, getattr(cls, "_REGISTER_NAME"), {})
        else:
            register_name = register_name or cls.get_registration_name()
            ...
            try:
                old_cls = register[register_name]
                LOGGER.warning(
                ...
                LOGGER.debug(
                    "Registering %s with name %s", cls.__qualname__, register_name
                )
    ...
    def get_base_registry(cls):
        ...
        Traverses the MRO to find the first non-Registrable class
        that defines the appropriate register attribute.
    ...
    @classmethod
    def get_registration_name(cls):
        """
        Get the registration name for this class.

        If the class name ends with its base registry name,
        removes that suffix. For example, CustomMetric with
        base Metric becomes "custom".

        Returns:
            str: The inferred registration name
        """
        base = cls.get_base_registry()
        if cls.__name__.endswith(base.__name__):
            return cls.__name__[: -len(base.__name__)]
        return cls.__name__
    ...
    @classmethod
    def get_registered(cls, name, retry_with_reload=True, submod="type"):
        """
        ...
        Args:
            ...
            submod: Subdirectory name for reload if needed (default: "type")
    ...
    @classmethod
    def register_subtypes(cls, submod="type"):
        """
        ...
        Args:
            submod: Subdirectory name where subtypes are located (default: "type")
                    The directory should be at the same level as the base class module
    ...
    @classmethod
    def collect_settings_types(cls, submod="type", settings_attr="SETTINGS_DATACLASS"):
        """
    ...
    @classmethod
    def reload(cls, submod="type", clear=True):
        """
    ...
    @classmethod
    def _import_subtypes(cls, submod="type"):
        """
    ...
    @classmethod
    def _get_subtypes_settings(cls, submod="type", settings_attr="SETTINGS_DATACLASS"):
        """
