...
class GitlabAdapter(GitAdapter):
    ...
    def _standardize_gitlab_project(
        ...
        repo_name = (
            gitlab_repo.name
            if not redact_names_and_urls
            else GitAdapter.repo_redactor.redact_name(gitlab_repo.name)
        )
        url = gitlab_repo.web_url if not redact_names_and_urls else None
        default_branch_name: str | None = get_attribute(gitlab_repo, 'default_branch')
    ...
    def _standardize_gitlab_branch(
        ...
        if not gitlab_branch:
            return None
        branch_name = str(get_attribute(gitlab_branch, 'name', default=''))
        if not branch_name:
            return None
    ...
    def _standardize_gitlab_pull_request_gql(
        ...
        head_repo_name = gitlab_pr['sourceProject']['name']
        head_repo_url = gitlab_pr['sourceProject']['webUrl']
        standardized_head_repository = StandardizedShortRepository(
            ...
            name=(
                head_repo_name
                if not redact_names_and_urls
                else GitAdapter.repo_redactor.redact_name(head_repo_name)
            ),
            url=head_repo_url if not redact_names_and_urls else None,
        )
        base_repo_name = gitlab_pr['targetProject']['name']
        base_repo_url = gitlab_pr['targetProject']['webUrl']
        standardized_base_repository = StandardizedShortRepository(
            ...
            name=(
                base_repo_name
                if not redact_names_and_urls
                else GitAdapter.repo_redactor.redact_name(base_repo_name)
            ),
            url=base_repo_url if not redact_names_and_urls else None,
        )
        ...
        base_branch = (
            gitlab_pr['targetBranch']
            if not redact_names_and_urls
            else GitAdapter.branch_redactor.redact_name(gitlab_pr['targetBranch'])
        )
        head_branch = (
            gitlab_pr['sourceBranch']
            if not redact_names_and_urls
            else GitAdapter.branch_redactor.redact_name(gitlab_pr['sourceBranch'])
        )
        ...
        return StandardizedPullRequest(
            ...
            url=gitlab_pr['webUrl'] if not redact_names_and_urls else None,
    ...
    def _standardize_gitlab_commit_gql(
        ...
        return StandardizedCommit(
            ...
            url=gitlab_commit['webUrl'] if not redact_names_and_urls else None,
    ...
    def get_organizations(self) -> List[StandardizedOrganization]:
        ...
        # Scrub sensitive info from gitlab orgs
        for org in orgs:
            if self.config.git_redact_names_and_urls:
                org.name = GitAdapter.organization_redactor.redact_name(org.name)
                org.url = None
    ...
    def _backfill_pr_closed_at(
        ...
        # Fetch 100 pr iids at a time. This should prevent sending too many ids at a time via queryparams
        for pr_key_batch in batch_iterable(pr_keys, batch_size=100):
    ...
    def get_prs(
        ...
        # If we need to backfill closed_at due to compat reasons, fill in generator values via the api in batches.
