...
class ApiRoutes:
    ...
    DOWNLOAD_PRESIGNED_FILE = "files/presigned"
...
class ApiClient:
    ...
    def stream_download_file_request(
        ...
        response = self.client.get(url=url, params={"path": read_path.as_posix()})

        if response.status_code != 200:
            detail = response.json().get("detail")
            raise ValueError(f"""Failed to make server call for  request Url: {route}, {detail}""")

        presigned_url: str = response.json()["url"]

        with self.client.stream("GET", presigned_url) as response:
