...
class DataRouteHelper(RouteHelper):
    ...
    def _load_profile(
        ...
        storage_client = client.get_storage_client(request.app.state.config)
    ...
    def get_versions_page(
        ...
        registry: CardRegistry = request.app.state.registries.data
        versions = registry.list_cards(name=name, limit=500, sort_by_timestamp=True)
...
class ProjectRouteHelper(RouteHelper):
    ...
    def get_unique_projects(self, project_registry: CardRegistry) -> List[str]:
        ...
        projects = project_registry.list_cards(limit=1000, sort_by_timestamp=True)
