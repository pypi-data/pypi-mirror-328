...
def factor_addindumv(factor = None,  dbo = None , dts = '20100101', dte = '20241101' , freq = 'm' , offset='first' ):
    ...
    factor = factor.merge( indu[['sid','INDEX_INFO_WINDCODE','S_CON_INDATE', 'S_CON_OUTDATE']].fillna('20990101') , how ='left', on ='sid' )
    ...
    eodprice['updownlimit'] = (eodprice['S_DQ_OPEN']>eodprice['S_DQ_STOPPING'])&(eodprice['S_DQ_OPEN']<eodprice['S_DQ_LIMIT'])
    # 第二日开盘价
    ...
    # 合并交易日收盘价
    nexttrade = nextopen.join(nextlowuplimit).join(eodprice['S_DQ_ADJCLOSE'].to_frame('adjclose'))
    nexttrade.index = nexttrade.index.set_levels([pd.to_datetime(nexttrade.index.levels[0].astype(str)), nexttrade.index.levels[1]])

    # step7 合并价格
    factor = factor.join(nexttrade)
    factor =  factor.join ( factor.next_open.unstack().pct_change(fill_method=None).shift(-1).stack().to_frame('nextadjopenpct') )
    factor =  factor.join( factor.adjclose.unstack().pct_change(fill_method=None).shift(-1).stack().to_frame('adjclosepct'))
...
def factor_neutralization(df = None, col = ['val_mv'], factorname = 'factor_value'):
    ...
    def neutralization(df_,col):
        ...
        #行业 + 按照指定的因子中性化
            ...
            dfr = pd.get_dummies(df_, columns=['INDEX_INFO_WINDCODE'], dtype=int)
            X = dfr [col + [col0 for col0 in dfr.columns if col.startswith('INDEX_INFO_WINDCODE_')]]
            y = dfr['factor_value']
            X = sm.add_constant(X)
            model = sm.OLS(y, X).fit()
            # 提取残差
...
def calcugroupinfo(group_resdict, freq = 'm'):
    ...
    for gp in list(key_value_pairs):
        ...
        exr.append([gp, year_r , exret, yearstd,yearsharpe ,wr,wr2 ])

    exr = pd.DataFrame(exr, columns=['分组', '年化收益', '超额收益', '年化波动', '夏普比例', '胜率', '超额胜率'])
    ...
    longshort = pd.Series([year_r,yearstd, smbsharpe,smbwr, 100 * group_resdict[max_i]['turnoverratio'].mean() , 100 * group_resdict[min_i]['turnoverratio'].mean()]  ,index=  ['多空年化收益',  '多空年化波动', '多空夏普比例', '多空胜率', '多头平均换手率','空头平均换手率'] )
    cumprodr = pd.concat(dfnormalreturn,axis=1)
    dfcumprodr = pd.concat(dfreturn, axis=1)
    smb =100* smb
    ymr = longshortyearmonthret(smb = smb )

    smb = pd.concat([group_resdict[max_i].exfeereturn - df_.benchmark , group_resdict[min_i].exfeereturn- df_.benchmark] , axis=1)
    ...
    smb['多空超额最大回撤（%）'] = (100*(smb['多空超额累计净值'].cummax() - smb['多空超额累计净值'])/smb['多空超额累计净值'].cummax()).replace(np.inf,np.nan).fillna(0)

    return longshort,cumprodr,exr,dfcumprodr,smb,ymr
...
def plotresults(regression_anares,ic_annres,longshort, IC_series,cumprodr,exr,dfreturn,smb,ymr,name  ):
    ...
    # 添加右侧 y 轴
    line4.extend_axis(
        yaxis=opts.AxisOpts(name="多空超额最大回撤（%）", position="right",is_inverse = True,name_location ='center',name_gap = 25)
    )

    line4.add_yaxis("多空超额最大回撤（%）", smb["多空超额最大回撤（%）"].tolist(), areastyle_opts = opts.AreaStyleOpts(opacity=0.5), yaxis_index=1 )
