...
from heaobject.root import Permission, DesktopObjectDict, ViewerPermissionContext, ViewerPermissionContext, Share, DesktopObject, json_loads
...
async def _insert_s3_object_from_sqs(request: web.Request, messages: list[dict]) -> Response:
    ...
    async def message_to_item_gen(messages: list[dict]):
        try:
            for msg in messages:
                body = json_loads(msg['Body']) if msg.get('Body') else None
                records = body['Records'] if body and body.get('Records') else []
                for r in records:
                    if r.get('s3'):
                        s3 = r['s3']
    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        try:
            ...
            if not root_account_prop or not sqs_url_prop:
                logger.debug("Missing account and SQS properties that are required.")
            ...
            async with SQSClientContext(request=request,
                ...
                while True:
                    try:
                        resp = await loop.run_in_executor(None,  partial( sqs_client.receive_message,QueueUrl=sqs_url_prop,
                            MaxNumberOfMessages=10, WaitTimeSeconds=int(pull_rate_prop) if pull_rate_prop else 20 ))

                        messages_raw = resp.get('Messages', [])  # This is list[MessageTypeDef]
                        if not messages_raw:
                            logger.debug("No messages received. Waiting...")
                            continue
                        #mypy fix
                        messages: list[dict] = [dict(msg) for msg in messages_raw]
                        # Process messages in bulk
                        inserted = await _insert_s3_object_from_sqs(request,  messages)

                        if inserted.status == 404 :
                            logger.debug("failed to insert into index")
                            return inserted

                        # Collect successfully processed messages for batch deletion
                        delete_entries: list[DeleteMessageBatchRequestEntryTypeDef] = [
                        ...
                        if delete_entries:
                            await loop.run_in_executor(None,  partial(sqs_client.delete_message_batch,
                                QueueUrl=sqs_url_prop,
                                Entries=delete_entries))
                            logger.debug(f"Deleted {len(delete_entries)} messages in batch.")
