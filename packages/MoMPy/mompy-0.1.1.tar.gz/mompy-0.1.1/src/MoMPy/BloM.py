...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct  4 11:51:19 2024
...
def check_if_id_BloM(element,map_table,rank_1_projectors,commuting_elements,orthogonal_projectors):
    ...
    """ 
        ...
        This is adapted to Block Matrices of operators. That is, the cyliicity
        of the trace is removed.
...
def block_normalisation_contraints(element,list_identities_in):
    ...
    for term in list_identities:
        for dd in range(len(term)):
            for pp in range(len(term[dd])):
                for yy in range(len(element)):
                    string = []
...
def BlockMatrix(S_1,S_2,higher_order_elements,rank_1_projectors,orthogonal_projectors,commuting_elements,states):
            ...
            for term in new_lists:
                if term not in already_counted:
                    ...
                    for hh in range(len(term)):
                        if term[hh] in commuting_elements: #if the element is in the list of commuting elements
                            for ii in range(len(new_vec)+1):
                                if term[np.mod(hh+ii+1,len(new_vec))] not in states: # If the following element is not a state that does not commute with anything
                                    new_vec = Commute(new_vec,np.mod(hh+ii,len(new_vec))) # permute members (monomials) of the element
                                    if new_vec not in new_lists:
                                        new_lists += [new_vec]
                                    if new_vec not in equivalences_vec:
    ...
    Block_Matrix = np.zeros((len(Mexp),len(Mexp)),dtype=int) # This is the output Gama matrix with numbers in each element
    ...
    for j in range(len(Mexp)):
        for i in range(len(Mexp)):
            #print(Mexp[i][j])
            Block_Matrix[i][j] = fmap(map_table,Mexp[i][j])
            #Block_Matrix[j][i] = Block_Matrix[i][j]
    
    list_of_eq_indices = np.unique(Block_Matrix) # Unique elements in M
    ...
    return Block_Matrix,map_table,S,list_of_eq_indices,Mexp
