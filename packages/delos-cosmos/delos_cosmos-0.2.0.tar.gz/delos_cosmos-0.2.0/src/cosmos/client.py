...
class APIClient(BaseModel):
    ...
    def _make_streaming_request(
        ...
        **kwargs: Any,
        ...
        try:
            with request_func(url, **request_args) as response:
                response.raise_for_status()
                yield from process_streaming_response(response)
...
class CosmosClient(APIClient):
    ...
    def llm_chat_stream(
        ...
        **kwargs: Any,
        ...
        data.update(kwargs)
        yield from self._make_streaming_request(endpoint=Endpoints.LLM.CHAT_STREAM.value, data=data)
