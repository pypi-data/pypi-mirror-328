...
class Logger:
    def __init__(self, prefix: str = None, space: bool = False):
        self.WHITE: str = "\u001b[37m"
    ...
    def message(self, level: str, message: str, start: int = None, end: int = None) -> str:
        ...
        return f"{self.space}{self.prefix}{time_now} {self.GRAY}[{level}{self.GRAY}] {self.WHITE}-> {self.GRAY}[{message}{self.GRAY}]{timer}"
    ...
    def PETC(self):
        input(f"{self.space}{self.GRAY}[{self.MAGENTA}Press Enter To Continue{self.GRAY}]")
