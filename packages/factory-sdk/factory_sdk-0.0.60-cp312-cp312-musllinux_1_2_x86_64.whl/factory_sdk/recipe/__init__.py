...
class RecipeBuilder:
    ...
    def _validate_preprocessor(self, directory: str):
        ...
        text_train_embeddings, image_train_embeddings = compute_embeddings(
            dataset["train"], reloaded_fn, **embed_config
        )

        text_test_embeddings, image_test_embeddings = compute_embeddings(
        ...
        for split in ["train", "test"]:
            ...
            embeddings = {
                "text": (
                    text_train_embeddings if split == "train" else text_test_embeddings
                ),
                "image": (
                    image_train_embeddings
                    if split == "train"
                    else image_test_embeddings
                ),
            }

            embeddings_path = os.path.join(split_dir, "embeddings.npz")
            embeddings = clean_dict(embeddings)
            np.savez_compressed(embeddings_path, **embeddings, allow_pickle=False)
    ...
    def _gen_preprocessed_samples(
        ...
        for split in dataset:
            ...
            for i in tqdm(
                ...
                desc=f"Testing preprocessor against {split} split",
            ):
                try:
        ...
        try:
            ...
            if recipe.last_revision is not None:
                ...
                if (
                    ...
                    response = input()
                    if response.lower() != "y":
                        print("[yellow]🤖 Recipe upload cancelled.[/yellow]")
                        exit()
