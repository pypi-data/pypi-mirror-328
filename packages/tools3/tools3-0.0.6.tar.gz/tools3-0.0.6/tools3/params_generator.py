...
class ParamsGenerator:
    def __init__(self, params, value_formatters, expected, include_field_names=False, separator="_"):
        """
        初始化 ParamsGenerator
        :param params: 参数字典，定义参数及其取值范围
        :param value_formatters: 参数值格式化规则
    ...
    def generate_case_name(self, case_data, include_field_names=True, case_number=""):
        ...
        for param, value in case_data.items():
            # 获取当前参数的格式化规则
            formatter_config = self.value_formatters.get(param, {})
            ...
            if include_field_names:
                case_name_parts.append(f"{param}_{value_str}")
            else:
    ...
    def generate_cases(self):
        ...
        for pairs in product(*self.params.values()):
            ...
            # 将生成的测试用例添加到列表中
            self.test_cases.append(case_with_fields)
