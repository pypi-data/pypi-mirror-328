...
class Disk:
    ...
    @property
    def stable_path(self) -> t.Optional[str]:
    ...
    def __udev_property(self, name: str) -> t.Optional[str]:
...
class DiskSetup:
    ...
    def __init__(self, *partitions: "PartitionSetup", identify_by: t.Iterable[str], do_repartition: bool = True,
                 partition_table_type: str = "gpt", name: t.Optional[str] = None):
        """
...
class _PartitionSetup:
    ...
    def __init__(self, *, fs_type: t.Optional["_PartitionType"] = None, mountpoint: t.Optional[str] = None,
                 label: t.Optional[str] = None, do_format: bool = True):
        """
    ...
    def mountpoint_spec(self, partition: "Partition") -> t.Optional["Mountpoint"]:
        if self.mountpoint:
...
TPartitionSetupSize = t.Union[int, t.Callable[[PartitionSizingEvent], int], None]


class PartitionSetup(_PartitionSetup):

    def __init__(self, *, index: t.Optional[int] = None, fs_type: t.Optional["_PartitionType"] = None,
                 mountpoint: t.Optional[str] = None, label: t.Optional[str] = None, do_format: bool = True,
                 use_in_raid: t.Optional[str] = None, size: TPartitionSetupSize = None,
                 start_at_mb: t.Optional[float] = None, flag_bootable: bool = False):
        """
...
class RaidPartitionSetup(_PartitionSetup):
    ...
    def __init__(self, raid_name: str, *, fs_type: t.Optional["_PartitionType"] = None,
                 mountpoint: t.Optional[str] = None, label: t.Optional[str] = None, do_format: bool = True,
...
class Partition:
    ...
    def uuid(self) -> t.Optional[str]:
        try:
            return subprocess.check_output(["blkid", "-p", "-s", "UUID", "-o", "value",
                                            self.path]).decode().strip()
        except subprocess.CalledProcessError:
            return None

    @property
    def partuuid(self) -> t.Optional[str]:
        try:
            return subprocess.check_output(["blkid", "-p", "-s", "PART_ENTRY_UUID", "-o", "value",
                                            self.path]).decode().strip()
        except subprocess.CalledProcessError:
            return None
...
class DiskPartition(Partition):
    ...
    def __init__(self, disk: Disk, part_no: int):
        super().__init__(disk.partition_path(part_no))
        self.__disk = disk
        self.__part_no = part_no

    @property
    def fstype(self) -> t.Optional["_PartitionType"]:
        parttype_str = _lsblk([self.path])["blockdevices"][0].get("parttype")
        if parttype_str:
...
class RaidPartition(Partition):
    ...
    def storage_devices(self) -> t.Iterable[Partition]:
        ...
        for line in subprocess.check_output(["mdadm",
                                             "--detail", "--scan", "--verbose", self.path]).decode().split("\n"):
            line = line.strip()
...
def size(*, b: float = 0, kb: float = 0, kib: float = 0, mb: float = 0, mib: float = 0, gb: float = 0, gib: float = 0,
         tb: float = 0, tib: float = 0, pb: float = 0, pib: float = 0) -> int:
    """
...
class _PartitionType:
    ...
    def __init__(self, gpt_uuid: str, mbr_id: t.Optional[str] = None, mkfs_command: t.Optional[list[str]] = None,
                 fstab_type_name: t.Optional[str] = None):
    ...
    def __eq__(self, other):
        return isinstance(other, _PartitionType) and other.gpt_uuid == self.gpt_uuid
    ...
    @staticmethod
    def by_gpt_uuid(parttype_uuid: str) -> t.Optional["_PartitionType"]:
...
class PartitionTypes:
    ...
    UNUSED = _PartitionType(
        gpt_uuid="00000000-0000-0000-0000-000000000000",
        mbr_id="0"
    )
    ...
    RAID = _PartitionType(
        gpt_uuid="A19D880F-05FC-4D3B-A006-743F0F84911E",
        mbr_id="fd"
    )
...
class RaidSetup:
    ...
    def create(self, *, do_create: bool = True) -> RaidPartition:
        ...
        if do_create:
            subprocess.check_call(["mdadm", "--create", dev_path, "--level=1", "--run",
                                   f"--raid-devices={len(self.partitions)}", *self.partitions])
...
class OrderedPartitionSetupsEntry:
    ...
    @property
    def partition_setup(self) -> t.Optional[PartitionSetup]:
...
def effective_partition_setup_order(partition_setups: list[PartitionSetup]) -> list[OrderedPartitionSetupsEntry]:
    ...
    return [OrderedPartitionSetupsEntry(i_partition_setup + 1, partition_setup)
...
class Mountpoint:
    ...
    def mount(self, *, prefix: t.Optional[str] = None, create_before: bool = True):
...
class DiskIntent:
    ...
    def udev_rule_for_alias(self) -> str:
        ...
        return (f'{disk_filter}, ENV{{DEVTYPE}}=="disk", SYMLINK+="{self.setup.name}"\n'
    ...
    def repartition(self) -> None:
        ...
        asterisk_parts = 0
        partition_sizes = {partition: (partition.size(PartitionSizingEvent(self.disk.size_bytes))
                                       if callable(partition.size) else partition.size)
...
def find_partition_for_setup(disk: Disk, disk_setup: DiskSetup, partition: PartitionSetup) -> DiskPartition:
