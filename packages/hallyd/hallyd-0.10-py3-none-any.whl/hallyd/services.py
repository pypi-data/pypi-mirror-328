...
import hallyd.io as _io
import hallyd.lang as _lang
import hallyd.bindle as _bindle


def create_service(name: t.Optional[str], runnable: "TRunnable") -> t.ContextManager["ServiceSetup"]:
...
def create_interval_task(name: t.Optional[str], runnable: "TRunnable") -> t.ContextManager["IntervalTaskSetup"]:
...
def create_calendar_task(name: t.Optional[str], runnable: "TRunnable") -> t.ContextManager["CalendarTaskSetup"]:
...
def create_next_boot_task(name: t.Optional[str], runnable: "TRunnable") -> t.ContextManager["NextBootTaskSetup"]:
...
class _TaskSetup:
    ...
    _runnable: t.Optional["TRunnable"] = None
    _description: t.Optional[str] = None
    _user: t.Optional[t.Union[str, int]] = None
    _group: t.Optional[t.Union[str, int]] = None
    ...
    def run_as_user(self, user: t.Union[str, int], group: t.Optional[t.Union[str, int]] = None) -> None:
        self._user = user
        self._group = group
    ...
    class _WithDependencies:
        ...
        def add_dependency(self, unit_name: str, *,
                           afterwards: bool = False, success_required: bool = False, optional: bool = False):
            self._dependencies = tuple([*self._dependencies,
                                        self.Dependency(unit_name, afterwards, success_required, optional)])
...
class ServiceSetup(_TaskSetup._WithDependencies, _TaskSetup):
    ...
    _enabled: bool = True
    _restart_delay: t.Optional[int] = __DEFAULT_RESTART_DELAY
    _as_oneshot: bool = False
    _startup_context: t.Optional[t.Iterable[str]] = None
    _post_stop: t.Optional["TCmdLineRunnable"] = None
    _options: t.Iterable[t.Callable[[str], None]] = ()

    def do_not_start_instantly(self, do_not_start_instantly: bool = True, *,
                               not_even_enable: t.Optional[bool] = None) -> None:
    ...
    def with_restart_delay(self, restart_delay: t.Optional[int]) -> None:
...
class CalendarTaskSetup(_TimedTaskSetup):
    ...
    def schedule_daily(self, *, time: t.Optional[str] = None) -> None:
        self._calendars = tuple([*self._calendars, self._DailyCalendar(time)])

    def schedule_weekly(self, *, time: t.Optional[str] = None, day_of_week: int = 7) -> None:
        self._calendars = tuple([*self._calendars, self._WeeklyCalendar(time, min(max(1, day_of_week), 7))])

    def schedule_monthly(self, *, time: t.Optional[str] = None, day: int = 1) -> None:
        self._calendars = tuple([*self._calendars, self._MonthlyCalendar(time, day)])

    def schedule_yearly(self, *, time: t.Optional[str] = None, day: int = 1, month: int = 1) -> None:
...
def _create_setup_context(name: t.Optional[str], runnable: "TRunnable", setup_type: type["_TTaskSetup"],
                          create_func) -> t.ContextManager["_TTaskSetup"]:
...
class _SystemdBackend(_ServiceBackend, _CalendarTaskBackend, _IntervalTaskBackend, _NextBootTaskBackend):
    ...
    @staticmethod
    def _short_name(name: str) -> tuple[str, t.Optional[str]]:
    ...
    def __create(setup) -> None:
        ...
        if isinstance(setup, CalendarTaskSetup):
            ...
            for calendar in setup._calendars:
                ...
                if isinstance(calendar, CalendarTaskSetup._DailyCalendar):
                    ...
                    timer_opts.append(("OnCalendar",
                                       f"*-{calendar.month}-{calendar.day} {calendar.time or default_time}"))
                else:
        ...
        with _SystemdBackend.__lock:
            ...
            service_spec_path.set_data(str(_SystemdBackend.__setup_to_object_spec(setup, service_spec_path)),
                                       readable_by_all=True)

            if timer_opts:
                timer_opts_str = "\n".join([f"{k}={v}" for k, v in timer_opts])
                _fs.Path(f"/etc/systemd/system/{name}.timer").set_data(f"[Timer]\n"
                                                                       f"{timer_opts_str}\n"
                                                                       f"[Install]\n"
    ...
    @staticmethod
    def __setup_to_object_spec(setup: _TaskSetup,
                               service_spec_path: t.Optional[_fs.Path] = None) -> "_UnitSpec":
        def full_name(s):
        ...
        if isinstance(setup, ServiceSetup):
            ...
            if setup._post_stop:
                unit_spec.service.add_value(
                    "ExecStopPost", _SystemdBackend.__runnable_to_spec_info(setup._post_stop, unit_spec,
                                                                            service_spec_path,
        ...
        unit_spec.service.set_value(
            "ExecStart", _SystemdBackend.__runnable_to_spec_info(setup._runnable, unit_spec, service_spec_path,
                                                                 discard_output=setup._discard_output,
                                                                 interactive=interactive, once=once))

        return unit_spec

    @staticmethod
    def __runnable_to_spec_info(runnable: "TRunnable", unit_spec: "_UnitSpec", service_spec_path: t.Optional[_fs.Path],
                                *, interactive: bool = False, once: bool = False, discard_output: bool = False) -> str:
        ...
        if isinstance(runnable, Runnable):
            ...
            base64_data = base64.b64encode(_bindle.dumps(dict(
                ACTION=runnable, SVC_PATH=service_spec_path, INTERACTIVE=interactive, ONCE=once)).encode()).decode()
            ...
            aux_bin = _fs.Path(__file__).parent("_aux/services_helper__call_action.py")
    ...
    class _UnitSpec:
        ...
        def __str__(self):
            ...
            for section_name, section in [("Unit", self.__unit),
    ...
    class _Unit(Service, IntervalTask, CalendarTask):
        ...
        def override(self, wants: list[str] = (), requires: list[str] = (),
                     ...
                     reset_wanted_by: bool = False, reset_required_by: bool = False,
                     ) -> None:
            def config_string(new_list, do_reset, config_key):
                return (f"{config_key}=\n" if do_reset else "") + f"{config_key}=" + " ".join(new_list) + "\n"

            install_section = "\n".join([config_string(*args) for args in ((wanted_by, reset_wanted_by, "WantedBy"),
                                                                           (required_by, reset_required_by,
                                                                            "RequiredBy"))])
            unit_section = "\n".join([config_string(*args) for args in ((wants, reset_wants, "Wants"),
...
_TCmdLineRunnablePart = t.Union[str, _fs.TInputPath]
TCmdLineRunnable = t.Union[t.Iterable[_TCmdLineRunnablePart], _TCmdLineRunnablePart]
TCallableRunnable = t.Union[t.Callable[[], None], t.Callable[[Runnable], None]]
TRunnable = t.Union[TCmdLineRunnable, Runnable, TCallableRunnable, None]

TServiceKey = t.Union[Service, str]
TIntervalTaskKey = t.Union[IntervalTask, str]
TCalendarTaskKey = t.Union[CalendarTask, str]
TNextBootTaskKey = t.Union[NextBootTask, str]
