...
async def test_inbox_unauthorized(
    uri_func, data, headers, test_client, my_actor  # noqa
):
...
@pytest.mark.parametrize("uri_func", [lambda x: x.inbox_uri, lambda x: "/shared_inbox"])
async def test_inbox(test_client, my_actor, mock_broker, uri_func):  # noqa
...
@pytest.mark.parametrize("uri_func", [lambda x: x.inbox_uri, lambda x: "/shared_inbox"])
async def test_inbox_no_digest(test_client, my_actor, mock_broker, uri_func):  # noqa
    response = test_client.post(
        uri_func(my_actor),
        json={"actor": "owner"},
        headers={
...
@pytest.mark.parametrize("uri_func", [lambda x: x.inbox_uri, lambda x: "/shared_inbox"])
async def test_inbox_unprocessable(test_client, my_actor, uri_func):  # noqa
    body = b'{"xxxx"}'
    key, val = content_digest_sha256_rfc_9530(body)
