...
from cattle_grid.model.exchange import UpdateIdentifierAction
...
async def base_urls_for_actor(actor: Actor) -> list[str]:
    account = await account_for_actor(actor.actor_id)
    return await allowed_base_urls(account)
