...
def job_init(
    ...
    Initialize a job
    Returns:
        Tuple of an init success, optional message, job id, and upload map
    """
    ...
    if status != "success":
        return False, message, id, upload_map
    ...
    return True, message, id, upload_map
...
def _normalize_download_map(download_map: Dict[str, str]) -> Dict[str, str]:
    """
    Given a download map with file paths as posix paths, normalize them to whatever the local OS uses.
    """
    return {os.path.normpath(k): v for k, v in download_map.items()}
...
def job_pull(
    ...
    download_map = _normalize_download_map(res.get("download_map"))
