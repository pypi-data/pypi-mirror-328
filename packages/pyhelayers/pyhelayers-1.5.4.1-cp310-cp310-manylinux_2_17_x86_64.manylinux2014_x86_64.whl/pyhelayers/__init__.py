...
from ._pyhelayers_cppwrappers import *
from . import _pyhelayers_cppwrappers
...
# Rename public objects' module name from 'pyhelayers._pyhelayers_cppwrappers'
# to 'pyhelayers', and expose them in __all__
for name in dir(_pyhelayers_cppwrappers):
    if not name.startswith('_'):
        __all__.append(name)
        obj = getattr(_pyhelayers_cppwrappers, name)
        if (isinstance(obj, typing.Callable) or inspect.isclass(obj)):
            if (obj.__module__ != 'pyhelayers'):
                obj.__module__ = 'pyhelayers'
