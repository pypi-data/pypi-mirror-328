...
def modify(instance_id, vcpus, gpu, num_gpus, disk_size_gb):
    ...
    if is_running:
        if any([vcpus, gpu, num_gpus]):
            raise click.ClickException("Can only modify disk size while instance is running. Stop the instance to modify other properties.")
        if not disk_size_gb:
    ...
    message = [
        ...
        f"[blue]- {vcpus} vCPUs" if vcpus else None,
        f"[blue]- {gpu.upper()} GPU" if gpu else None,
        f"[blue]- {disk_size_gb}GB disk" if disk_size_gb else None,
        "[yellow]This will take a few minutes to complete.[/yellow]",
    ]
    panel = Panel(
    ...
    if success:
        click.echo(click.style(
            f"Successfully queued instance modification! Track instance status with 'tnr status --wait'",
            fg="cyan",
        ))
    else:
