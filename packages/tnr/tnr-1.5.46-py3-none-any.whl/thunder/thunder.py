...
@click.argument("instance_id", required=True)
@click.argument("new_size_gb", required=True, type=int, metavar="SIZE_GB")
def resize(instance_id, new_size_gb):
    if new_size_gb > 1024:
        raise click.ClickException(
            f"❌ The requested size ({new_size_gb}GB) exceeds the 1TB limit."
        )
    with DelayedProgress(
        SpinnerColumn(spinner_name="dots", style="white"),
        TextColumn("[white]{task.description}"),
    ...
    if current_size >= new_size_gb:
        click.echo(click.style(
            f"❌ The current disk size ({current_size}GB) is already greater than or equal to the requested size ({new_size_gb}GB). No resize needed.",
            fg="yellow"
        ))
        ssh.close()
        return
    ...
    with DelayedProgress(
        ...
        success, error = utils.resize_instance(instance_id, new_size_gb, token)
        if success:
    ...
    if success:
        click.echo(
            click.style(
                f"Successfully resized the persistent disk for instance {instance_id} to {new_size_gb}GB.",
                fg="cyan",
            )
        )
    else:
            ...
            if instance.get("status") == "RUNNING" and instance.get("ip"):
                ...
                try:
                    ...
                    if error:
                        ...
                        try:
                            # Write config file using SSH command
                            remote_config_path = '/home/ubuntu/.thunder/config.json'
                            remote_token_path = '/home/ubuntu/.thunder/token'
                            config_json = json.dumps(config, indent=4)
                            config_cmd = f"""mkdir -p /home/ubuntu/.thunder && echo '{config_json}' > {remote_config_path}"""
                            _, stdout, stderr = ssh.exec_command(config_cmd)
                            if stdout.channel.recv_exit_status() != 0:
                                error = stderr.read().decode().strip()
                                raise Exception(f"Failed to write config file: {error}")

                            # Write token file using SSH command
                            token_cmd = f"""echo '{token}' > {remote_token_path}"""
                            _, stdout, stderr = ssh.exec_command(token_cmd)
                            if stdout.channel.recv_exit_status() != 0:
                                error = stderr.read().decode().strip()
                                raise Exception(f"Failed to write token file: {error}")

                            # Create symlinks (these were already using SSH)
...
def connect(tunnel, instance_id=None, mount=None, nomount=False, debug=False):
    ...
    try:
        ...
        # Check for active SSH sessions before updating binary
        ...
        timings['config_write'] = time.time() - config_write_start
