...
class TestFmt:
    ...
    def test_custom_locale(self, data_dir, messages) ->None:
        shutil.copy(data_dir / "foo-fr.po", messages / "foo-fr.po")
        hook = build_hook({"domain": "foo", "locale": "foo/locale"}, root=messages.parent)
        build_data = {"force_include": {}}
        with patch("hatch_msgfmt.plugin.make"):
            hook.initialize("standard", build_data)
            # noinspection PyUnresolvedReferences
            plugin.make.assert_called_with(
                str(messages / "foo-fr.po"),
                str(messages.parent / "foo" / "locale" / "fr" / "LC_MESSAGES" / "foo.mo"),
            )
