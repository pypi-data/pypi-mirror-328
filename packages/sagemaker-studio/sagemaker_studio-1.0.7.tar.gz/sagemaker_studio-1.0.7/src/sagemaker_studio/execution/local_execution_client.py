...
class LocalExecutionClient(ExecutionClient):
        ...
        try:
            ...
            job_scheduler_list_response: dict = LocalExecutionUtils.fetch_data(
                LocalExecutionUtils.get_url_with_query_params(
                    base_url=LocalExecutionUtils.LOCAL_JOBS_SCHEDULER_URL, params=query_params
                )
            )
    ...
    Args:
        ...
        compute (Optional[str]): Configuration for the compute environment (not used for local)
        ...
        termination_condition (Optional[Dict[str, Any]]): Condition for terminating the execution (not used for local)
    ...
    def start_execution(
        ...
        if client_token:
            parameters["client_token"] = client_token
