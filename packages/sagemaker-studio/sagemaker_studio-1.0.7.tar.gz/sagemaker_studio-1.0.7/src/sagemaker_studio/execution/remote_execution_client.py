...
class RemoteExecutionClient(ExecutionClient):
    ...
    Args:
        ...
        sort_order (Optional[str]): ASCENDING or DESCENDING order
        sort_by (Optional[str]): Field to sort the results by (e.g., 'start_time', 'status', 'end_time', 'name')
    ...
    Args:
        ...
        input_config (Optional[Dict[str, Any]]): Configuration for the input data for the execution
        ...
        client_token (Optional[str]): A unique token to ensure idempotency (not used for remote)
    ...
    def start_execution(
        ...
        if execution_type:
            parameters["execution_type"] = execution_type
        if input_config:
            parameters["input_config"] = input_config
        if compute:
            parameters["compute"] = compute
        if output_config:
            parameters["output_config"] = output_config
        if termination_condition:
            parameters["termination_condition"] = termination_condition
    ...
    def _start_execution_internal(self, request: StartExecutionRequest):
        ...
        if request.get("tags"):
            execution_tags.extend(
                {"Key": key, "Value": value} for key, value in request.tags.items()
            )
        ...
        if "ecr_uri" in image_details:
            ...
            if image_name != image_name_default:
                ...
                response = self.sagemaker_client.describe_image_version(
                    ImageName=image_name, Version=int(image_details.get("image_version"))
                )
        ...
        try:
            ...
            create_training_job_response = self.sagemaker_client.create_training_job(
                ...
                Tags=execution_tags,
                VpcConfig={"SecurityGroupIds": [self.security_group], "Subnets": self.subnets},
            )
