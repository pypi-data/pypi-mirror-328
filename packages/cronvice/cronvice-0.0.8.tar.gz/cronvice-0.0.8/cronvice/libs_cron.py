...
def fill_current_DTS():
    ...
    for job in mycron:
        tag2 = str(job).split(file_path)[-1].strip().split(" ")[1]
...
def start(tag, duotag): #REAL
    ...
    process.poll()
    config.INSIDE_PROC_OBJ[tag] = process
    with open("/tmp/cronvice.log", "a") as f:
        f.write(f"i... in START ... {tag} is launched at {config.INSIDE_LAST_RUN[tag]} ErrCode={process.poll()} \n")
