...
def crtable():
    ...
    for i in crolist:
        ...
        ###    f.write(f"i... in DF ... {name} check ... obj:{config.INSIDE_PROC_OBJ.keys()} run:{config.INSIDE_LAST_RUN.keys()} \n")

        # if name in config.INSIDE_PROC_OBJ.keys():
        #     with open("/tmp/cronvice.log", "a") as f:
    ...
    for i in scrlist:
        ...
        if len(item) < 2:
            with open("/tmp/cronvice.log", "a") as f:
                f.write(f"X... in DF ... screen ls error: {i} {scrlist}\n")
            return df # HARDER !!!!!
