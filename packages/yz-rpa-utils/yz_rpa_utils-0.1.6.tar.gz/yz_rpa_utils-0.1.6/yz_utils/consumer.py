...
class YuanmeiJob:
    ...
    def date_int_to_str(obj: dict):
        for key in ["startDate", "endDate", "createDate", "lastHeartbeatTime"]:
            if obj.get(key):
                obj[key] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(obj[key]))

    def sum_total_time(self):
        self.totalTime = (self.endDate - self.startDate) * 1000
...
class Consumer:
                ...
                if currentJob:
                    ...
                    try:
                        ...
                        for line in str(app_code).splitlines():
                            code_block += f"\t{line}\n"
                        code_block += "run_code()"
                        res_job = eval(code_block, local_vars, local_vars)
                        if not res_job:
                            raise Exception("任务执行完毕,无结果返回")
                        # 更新任务结果
                        self.currentJob.__dict__.update(res_job.__dict__)
                        self.end_job()
    ...
    def update_job(self):
        job_json = self.currentJob._to_json()
