...
from pydantic import AnyUrl, BaseModel, FilePath, validator, DirectoryPath, field_validator
...
class MLGameArgument(BaseModel):
    ...
    def group_ai(self)-> List[GroupAI]:
        ...
        elif self.ai_clients:
            for i,ai_client in enumerate(self.ai_clients,start=1):
                # group, ai_name, ai_path = entry.split(",")
                group_ai.append(GroupAI(group=GroupEnum.O, ai_name=AINameEnum(f"{i}P"), ai_path=Path(ai_client)))
        return group_ai
