...
class Network:
    def __init__(self, passport, base_url=None, time_out=None):
        self.crypto = Encryption()
        self.http = PoolManager()
        self.time_out = time_out or 8
        self.base_url = base_url or "http://iran.fruitcraft.ir"
    ...
    def sendRequest(self, path: str, input: dict = {"client": "iOS"}, method: str = "POST", max_attempts: int = 5):
        ...
        data = f"edata={self.crypto.encrypt(dumps(input))}" if method == "POST" else None
