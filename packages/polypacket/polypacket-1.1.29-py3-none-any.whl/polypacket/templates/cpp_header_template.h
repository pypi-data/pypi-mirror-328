...
typedef enum{
  ...
  ${proto.namespace.upper()+"_"+field.name.upper() + "_" + val.name.upper()},              /* ${val.desc} */
  % endfor
  ${proto.namespace.upper()+"_"+field.name.upper()}_MAX_LIMIT
} ${proto.namespace}_${field.name.lower()}_e;
%if proto.snippets:
//Switch Snippet
/*
...
typedef enum{
  ...
  ${proto.namespace.upper()+"_"+field.name.upper() + "_" + val.name.upper()} = ${ field.valsFormat % (1 << idx)},    /* ${val.desc} */
  % endfor
  ${proto.namespace.upper()+"_"+field.name.upper()}_MAX_LIMIT
} ${proto.namespace}_${field.name.lower()}_e;
...
class ${proto.camelNamespace()}Packet {
  public:
    ...
    /**
      ...
      */
    ${proto.camelNamespace()}Packet(poly_packet_desc_t* desc)
    {
    ...
    /**
      ...
      */
    ~${proto.camelNamespace()}Packet() {clean();}

    /**
      *@brief builds a new ${proto.camelNamespace()}Packet
      *@param desc ptr to packet descriptor to model packet from
      */
    ...
    /**
      ...
      */
    void copyFrom(${proto.camelNamespace()}Packet& src) {poly_packet_copy(&mPacket , &src.mPacket );}
...
typedef ${proto.camelNamespace()}Packet ${proto.camelNamespace()}Struct;
...
class ${proto.cppFileName} {
  public:
    ...
    /**
      ...
      *@param iface index of interface to send on
      */
    HandlerStatus_e send( int iface, ${proto.camelNamespace()}Packet& packet);
    ...
    /**
      ...
      *@param packet packet to be sent
      */
    HandlerStatus_e send(${proto.camelNamespace()}Packet& packet);
  ...
  private:
    ${proto.camelNamespace()}Packet mRequest;
    ${proto.camelNamespace()}Packet mResponse;
    ${proto.camelNamespace()}Packet mDespool;
    uint8_t mBuffer[512];
    uint8_t mBufferLen;
