...
poly_service_t ${proto.camelNamespace()}Packet::DESC_INDEX;
...
void ${proto.cppFileName}::buildDescriptors()
{
  ...
  /************************************************************************
                        Descriptor Index
  ************************************************************************/
  poly_service_init(&${proto.camelNamespace()}Packet::DESC_INDEX,${len(proto.packets) + len(proto.structs)}, 0);

  //Register standard packet descriptors with the service
% for packet in proto.packets:
%if packet.standard:
  poly_service_register_desc(&${proto.camelNamespace()}Packet::DESC_INDEX, ${packet.globalName});
%endif
% endfor
...
HandlerStatus_e ${proto.cppFileName}::dispatch(${proto.camelNamespace()}Packet& packet, ${proto.camelNamespace()}Packet& response)
{
  ...
  switch(packet.mPacket.mDesc->mTypeId)
  {
    case ${proto.namespace.upper()}_PACKET_PING_ID:
      response.build(${proto.namespace.upper()}_PACKET_ACK);
      ${proto.namespace}_status = PingHandler(packet, response);
      break;
    case ${proto.namespace.upper()}_PACKET_ACK_ID:
      ${proto.namespace}_status = AckHandler(packet);
      break;
% for packet in proto.packets:
% if not packet.standard:
    ...
    %if packet.hasResponse:
     ...
     ${proto.namespace}_status = ${packet.name}Handler(packet , response );
    %else:
      ${proto.namespace}_status = ${packet.name}Handler(packet);
    %endif
      break;
% endif
% endfor
  ...
  {
    ${proto.namespace}_status = defaultHandler(packet, response);
  }
...
void ${proto.cppFileName}::process()
{
  ...
  if(poly_service_try_parse(&mService, &mRequest.mPacket) == PACKET_VALID)
  {
    ...
    ${proto.namespace}_status = dispatch(mRequest,mResponse);
    ...
    if(( ${proto.namespace}_status == PACKET_HANDLED) && (mResponse.mPacket.mBuilt) && ((mRequest.mPacket.mHeader.mToken & POLY_ACK_FLAG)==0) )
    {
      //set response token with ack flag
			mResponse.mPacket.mHeader.mToken = mRequest.mPacket.mHeader.mToken | POLY_ACK_FLAG;

      ${proto.namespace}_status = poly_service_spool(&mService, mResponse.mPacket.mInterface, &mResponse.mPacket);
...
{
  ${proto.camelNamespace()}Packet packet;
  ${proto.camelNamespace()}Packet response;
  ...
  if(poly_service_parse_json(&mService, &packet.mPacket, req, len) == PACKET_VALID)
  {
    ...
    ${proto.namespace}_status = dispatch(packet,response);


    //If a response has been build and the ${proto.namespace}_status was not set to ignore, we send a response on the intrface it came from
    if(( ${proto.namespace}_status == PACKET_HANDLED) && (response.mPacket.mBuilt) )
    {
      //set response token with ack flag
...
HandlerStatus_e ${proto.cppFileName}::send(int iface, ${proto.camelNamespace()}Packet& packet)
{
  MRT_MUTEX_LOCK(mMutex);
  HandlerStatus_e ${proto.namespace}_status;

  ${proto.namespace}_status = poly_service_spool(&mService, iface, &packet.mPacket);
...
HandlerStatus_e ${proto.cppFileName}::send(${proto.camelNamespace()}Packet& packet)
{
