...
class Kiteworks_io():
   ...
   def download_files(self, 
                      ...
                      local_path: Path, 
                      file_extension: str,
                      remove_from_kiteworks: bool = False):
      try:
         returned_result = ""
            ...
            if returned_result == "":
               ...
               if stat.S_ISREG(self.sftp_client.stat(self.__remote_path).st_mode):
                  ...
                  if os.path.splitext(filename)[-1].lower() == self.__file_extension:
                     ...
                     returned_result = self.__get_latest_file(self.__remote_path, full_local_path, remove_from_kiteworks)

               # is it a directory?
               elif stat.S_ISDIR(self.sftp_client.stat(self.__remote_path).st_mode):
                  returned_result = self.__download_files_recursive(self.__remote_path, self.__local_path, "", remove_from_kiteworks)
      except Exception as err:
         ...
         if (self.sftp_client == None):
            ...
            if returned_result == "":
               ...
               # is it a directory?
                  ...
                  returned_result = self.__download_files_recursive(self.__remote_path, "", configurationfile_path, remove_from_kiteworks)
   ...
   def __get_latest_file(self, 
      ...
      try:
         ...
         if returned_result == "": # No file to be downloaded
            returned_result = self.__OUTCOME_SUCCESS
            return returned_result
         else: # the full path of the last file copied
            return returned_result
      ...
      try:
         ...
         for filename in self.sftp_client.listdir(remote_path):
               ...
               if os.path.splitext(filename)[-1].lower() == self.__file_extension:
                  ...
                  if configurationfile_path and not local_path:
                     ...
                     if local_path_temp and len(local_path_temp) > 0:
                        ...
                        returned_result = self.__get_latest_file(full_remote_path, full_local_path, remove_from_kiteworks)
                  elif local_path:
                     full_local_path = os.path.join(local_path, filename)
                     returned_result = self.__get_latest_file(full_remote_path, full_local_path, remove_from_kiteworks)
            ...
            # is it a directory?
               ...
               returned_result = self.__download_files_recursive(full_remote_path, os.path.join(local_path, filename), configurationfile_path)
      except Exception as err:
         returned_result = self.__ERROR_OCCURRED + str(err)
      finally:
         if returned_result == "": # No file to be downloaded
