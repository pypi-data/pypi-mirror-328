...
class MultiTableSchema(dict[str, TableSchema]):
    def __init__(self, data: Union[dict, "RDBMS_Schema"] | None, tables: list[str] | None = None):  # noqa: F821
        ...
        try:
            self.total_cols = sum([len(v['columns'].keys()) for k,v in data.items()])
        except KeyError:
            # Whenever the provided input is a dictionary missing the information regarding tables columns
            # This is a flow that is only used for tests
