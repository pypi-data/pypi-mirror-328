...
from webserver import config, env
...
async def _lifespan(app: FastAPI):
    ...
    from webserver.commons import db

    # startup
    env.load_modules(settings.MODULES)  # Load modules to allow orm metadada creation
...
if settings.STATIC_FOLDER and Path(settings.STATIC_FOLDER).exists():
    from starlette.staticfiles import StaticFiles
    app.mount("/static", StaticFiles(directory=f"{settings.RESOURCES_FOLDER}/{settings.STATIC_FOLDER}"))

if settings.TEMPLATES_FOLDER and Path(settings.TEMPLATES_FOLDER).exists():
    templates = Jinja2Templates(directory=f"{settings.RESOURCES_FOLDER}/{settings.TEMPLATES_FOLDER}")
