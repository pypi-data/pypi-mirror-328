...
class DatabaseMigrationExecutor:
    ...
    def _execute(self, file: Path) -> Migration | None:
        ...
        if not self._get_last_execution_of(file):
            try:
                ...
                migration = Migration(filename=file.name)
    ...
    def _get_last_execution_of(self, file: Path) -> Migration | None:
        return self.session.exec(select(Migration).where(Migration.filename == file.name)).first()
    ...
    def _migrate_json_data_from(self, file: Path) -> Migration | None:
        from webserver.commons import runtime

        root: dict = json.loads(file.read_text())
        for cls in root.keys():  # Reads all the classes
            for obj in root[cls]:  # Reads the list of class elements
