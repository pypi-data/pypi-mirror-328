...
def remote_run_prompt(file_path, config_path, exclude_file, dynamic_params, gpu):
    ...
    if app_module:
        for _, obj in inspect.getmembers(app_module, inspect.isclass):
            for _, func in inspect.getmembers(obj, inspect.isfunction):
                if (
                    (hasattr(func, "_is_loader") and func._is_loader)
                    or (hasattr(func, "_is_infer") and func._is_infer)
                ):
