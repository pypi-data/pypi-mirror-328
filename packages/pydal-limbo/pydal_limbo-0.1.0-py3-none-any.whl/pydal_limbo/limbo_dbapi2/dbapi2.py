...
import time
import warnings
from sqlite3 import (  # noqa: re-exports
    PARSE_DECLTYPES,
    register_adapter,
    register_converter,
)
...
class Connection:
    connection: limbo.Connection

    def __init__(self, database, **options):
        self.connection = limbo.connect(database)
    ...
    def commit(self):
        ...
        except limbo.NotSupportedError as e:
            warnings.warn(
                f"Transactions are not supported in this version",
    ...
    def create_function(self, fn_name, *a, **kw):
        warnings.warn(
            f"create_function is not supported! {fn_name}", category=RuntimeWarning
        )
