"""
Standard implementation of pnb.mcl.test.mcl model.
"""
...
def _private_init(cls_or_hint: type | str):

    def make_private(cls, hint):
        # TOFIX: in-/dedent
        doc = cls.__doc__
        if doc:
            doc += '\n\n'
            ...
            doc = ''
        cls.__doc__ = doc + '''*Private constructor.*'''
        if hint:
            cls.__doc__ += '\n\n' + hint

        init = vars(cls).get('__init__')

        # TOFIX: keep signature, doc, etc. -> use decorator lib
    ...
    if isinstance(cls_or_hint, type):
        return make_private(cls_or_hint, None)
    else:
        return functools.partial(make_private, hint=cls_or_hint)


@_private_init('''
    To obtain a Members object, get it from a Namespace object.
    ...
    .. code-block:: python
        ...
        spam = Package('spam')
        spam.packagedElements.add(Package('SubSpam'))
        for member in spam.members:
            print(member)
    
    - :code:`spam.packagedElements` is a :class:`MutableMembers` object that contains the
      :py:attr:`packagedElements <Package.packagedElements>` of :class:`Package` spam. We call its :meth:`add() <MutableMembers.add>` method to add
      another Package to the :code:`packagedElements`.
    
    
    # spam.members is a :ref:`Members` object that contains all members of Package
    # spam, including the packagedElements. Here, we iterate over spam.members
    # (cf. __iter__).
    # another Package to the packagedElements.
    
    ...
    print('names in package')
    for 
...
class Members(
    ...
    def __iter__(self) -> T.Iterator[MemberType]:
        """Iterate over the contained members. If not mentioned otherwise,
        iteration is in alphanumeric order w.r.t. the members' names."""
        return self._members_property._iter_(self._owner)

    def __len__(self) -> int:
        """Get the member of contained members."""
    ...
    def get(self, name, default=None):
        """Get the member with the given name, or default if there is no such
        member."""
        return self._members_property._get_(self._owner, name, default)

    @property
    def names(self) -> T.Iterator[str]:
        """Iterate over the names of the contained members. If not mentioned
        otherwise, iteration is in alphanumeric order."""
...
class MembersProperty(T.Generic[MemberType], ABC):
    ...
    def __init__(self,
        ...
        self._derived_properties_by_owner: dict[
        ...
        self._owner: type[Namespace] = None # type: ignore
    ...
    def __set_name__(self, owner, name: str):
        # Correct annotation of owner would be Type[Namespace]. However, this method is called
        ...
        self._owner = owner
        self._name = name
        self._info = f'{owner.__name__}.{name}'
    ...
    def _get_derived_properties(self, owner: type[Namespace]):
        derived_properties = self._derived_properties_by_owner.get(owner)
        if derived_properties is None:
            derived_properties = tuple(
                prop for prop in self._derived_properties_
                if issubclass(owner, prop.owner))
            self._derived_properties_by_owner[owner] = derived_properties
    ...
    def _iter_(self, owner: 'Namespace') -> T.Iterator[MemberType]:
        ...
        for name, value in members_dict.items():
            if name != _UNNAMED_MEMBERS:
                yield value
        yield from members_dict.get(_UNNAMED_MEMBERS, [])
        # return iter(self._get_members_dict_(owner, True).values())
...
class MutableMembersProperty(
    ...
    def _get_members_dict_(self, owner: Namespace, sorting_required: bool) -> dict[str, MemberType]:
        ...
        if sorting_required:
            ...
            if not is_sorted:
                ...
                unnamed_members = members_dict.pop(_UNNAMED_MEMBERS, [])
                items = sorted(members_dict.items())
                members_dict.clear()
                members_dict.update(items)
                if unnamed_members:
                    members_dict[_UNNAMED_MEMBERS] = unnamed_members
    ...
    def _add_(self, owner: Namespace, member: MemberType):
        ...
        if name:
            ...
            if name_is_registered:
                ...
                if member is own_member:
                    ...
                    raise TypeError(
                        f'{member!r} cannot be added to {self._get_members(owner)!r} because {owner!r} '
        ...
        if name:
            ...
            members_dict = self._get_members_dict_(owner, False)
            members_dict.setdefault(_UNNAMED_MEMBERS, []).append(member)
...
class DerivedMembersProperty(T.Generic[MemberType], MembersProperty[MemberType]):
    ...
    def _get_direct_base_properties(self, owner: type[Namespace]):
        direct_base_properties = self._direct_base_properties_by_owner.get(owner)
        if direct_base_properties is None:
            direct_base_properties = tuple(
                prop for prop in self._direct_base_properties
                if issubclass(owner, prop.owner))
            self._direct_base_properties_by_owner[owner] = direct_base_properties
        return direct_base_properties


class UnionMembersProperty(T.Generic[MemberType], DerivedMembersProperty[MemberType]):
    """A MembersProperty that is the union of several other _MembersProperties.
    """
...
class NamedElement(Element):
    ...
    @property
    def info(self):
        return f"{type(self).__name__} '{self.qualifiedName}'"
...
class Type(Namespace, PackageableElement):
    ...
    __slots__ = ['_superTypes', '_allSuperTypes']
    ownedAttributes = []
    
    name: string
    
    def __init__(self, name: str, superTypes: T.Iterable[Type], **kwargs):
        if name is None:
            raise TypeError(f'A {self.get_meta_class_name()} must have a name.')
...
class Model(Package):
    ...
    def __tattr__(self, name) -> NamedElement:
        # TODO: quick hack for package import
        try:
            attr = getattr(self.members, name)
            if isinstance(attr, Package):
                raise AttributeError()
            return attr
        except AttributeError:
            for pe in self.packagedElements:
                try:
...
class TypedElement(NamedElement):
    """
    merged with multiplicityelement
...
class DataProperty(Property):
    ...
    def __init__(self, name: str, type_: CDataType, lower: int=1, upper: T.Optional[int | str]='lower', isOrdered: bool=False, isUnique: bool=False, redefines: T.Iterable[ReferenceProperty]=()):
