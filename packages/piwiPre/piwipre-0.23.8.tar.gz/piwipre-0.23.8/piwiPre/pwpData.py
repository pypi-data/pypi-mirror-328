...
class PwpVideo(PwpData):
    ...
    def build_representative(source, destination, config: PwpConfig,
        ...
        # keep the aspect-ratio   width/height = logo.width / logo.height

        if jpg.image.width > jpg.image.height:
            width = min(int(jpg.image.width / 4), logo.width)
            height = int(width * logo.height / logo.width)
        else:
            height = min(int(jpg.image.height / 4), logo.height)
            width = int(height * logo.width / logo.height)

        if width != logo.width or height != logo.width:
            logo = ImageOps.contain(logo, (width, height))

        x = int(max((jpg.image.width - width) / 2, 0))
        y = int(max((jpg.image.height - height) / 3, 0))

        jpg.image.paste(logo, (x, y, x+logo.width, y+logo.height), mask=logo)
