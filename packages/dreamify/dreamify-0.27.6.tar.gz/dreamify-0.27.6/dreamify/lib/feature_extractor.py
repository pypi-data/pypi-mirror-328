...
class FeatureExtractor:
    def __init__(self, model_name, dream_style, layer_settings):
        ...
        if isinstance(self.layer_settings, list):
            ...
            outputs = [self.model.get_layer(name).output for name in self.layer_settings]
        else:
            # Otherwise, assume layer_settings is a dict and its keys are layer names.
            outputs = {
                layer.name: layer.output
                for layer in [self.model.get_layer(name) for name in self.layer_settings.keys()]
            }
        self.feature_extractor = keras.Model(
            inputs=self.model.inputs, outputs=outputs
        )
