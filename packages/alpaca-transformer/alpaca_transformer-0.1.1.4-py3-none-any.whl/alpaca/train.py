...
def train(epochs, transformer, loss_fn, train_dl, optimizer=torch.optim.Adam, lr=1e-4, device=None, validate_data=False, validation_dl=None):
    if not device:
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if validate_data:
            validate(transformer, validation_dl, loss_fn, device)
