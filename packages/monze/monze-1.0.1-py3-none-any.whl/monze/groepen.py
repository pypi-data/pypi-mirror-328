...
def groep_none():
	...
	mijngroepen = views_o.mijn_groepen()
	# list of group dicts
	allegroepen = ListDicts.sortlistofdicts(list(sysls_o.get_sysl('s_group').values()), 'ordering')
	for i in range(len(list(allegroepen))):
		if not allegroepen[i]['id'] in mijngroepen:
			allegroepen.pop(i)
