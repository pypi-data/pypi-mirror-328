...
def cli_run(cmd: Union[str, list], stdout: bool = False, label: str = None, silent: bool = False) -> CompletedProcess[str]:
    if label is None:
        label = " ".join(cmd) if isinstance(cmd, list) else cmd
    cli_print(f"Executing: \"{label}\"")
    result = run(cmd if isinstance(cmd, list) else cmd.split(), stdout=PIPE if stdout else DEVNULL, stderr=PIPE, universal_newlines=True)
