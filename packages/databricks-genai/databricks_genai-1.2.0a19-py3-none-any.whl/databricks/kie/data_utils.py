...
def create_training_data(
    ...
    if use_ai_query:
        if len(unlabeled_dataset_path) == 0:
            raise ValueError("unlabeled_dataset_path must be provided when use_ai_query is True")
        # Based on the UC table `unlabeled_table`, if the table contains more than MAX_DOCS_TO_TRAIN documents,
        # limit the number of documents to MAX_DOCS_TO_TRAIN.
        # Write the df to a new UC table `unlabeled_table_filtered` for ai_query. Eventually, this should happen
        # in the workflows' data processing step.
        filtered_unlabeled_table = unlabeled_dataset_path + "_filtered"

        # Read training documents into the table and save it out
        train_df = get_all_unlabeled(train_df.limit(MAX_DOCS_TO_TRAIN))
        # Overwrite schema, if the table already exists
        train_df.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(filtered_unlabeled_table)

        result_df = endpoint.generate_ai_query(
            data_path=filtered_unlabeled_table,
            system_prompt=prompt,
            response_format=response_format,
        )
        print(f"Finished generating {result_df.count()} responses from ai_query")
        # Write to train_jsonl_cache_path
        result_df.toPandas().to_json(train_jsonl_cache_path + ".jsonl",
          orient='records',
          lines=True)
