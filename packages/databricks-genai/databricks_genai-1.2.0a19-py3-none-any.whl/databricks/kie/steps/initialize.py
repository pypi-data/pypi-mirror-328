...
class TaskInitializer:
    ...
    def validate_labeled_data_with_response_format(self, labeled_split_df: DataFrame,
                                                   response_format: Type[BaseModel]) -> None:
        """Validate labeled data against response format."""

        def _validate_pandas_df(iterator, response_format_json: str):
            ...
            for pdf in iterator:
                ...
                for _, row in pdf.iterrows():
                    try:
                        response_format.model_validate(json.loads(row["expected_response"]))
