    ...
    **kwargs
    ...
    try:
        for data in batch:
            ...
            form_data.add_field('created_at', created_at)
        ...
        for field in form_data:
            logger.info(f"Form data field: {field}")
...
async def delete_item_func(
    ...
    **kwargs
    ...
    async with aiohttp.ClientSession() as session:
        async with rate_limiter:
            return await session.delete(url, headers=kwargs['headers'], params=params)
    ...
    async with aiohttp.ClientSession() as session:
        async with rate_limiter:
            ...
            if res.status != 401:
                result = await PollTaskStatusNamespace.poll_task_status_handler(
                    res,
                    status_url="http://192.168.1.27:5002/api/v1/tasks/status",
                    task_id_key='task_token',
                    **kwargs
                )
                return result
