...
@pytest.mark.skip("Test is flaky -- logs can take a while to propagate")
def test_fetch_service_logs(services, temp_service, instance_family):
    ...
    for _ in range(5):
        ...
        try:
            ...
            if logs is not None:  # get prefix of 100 characters of logs
                logs = logs[:100]
            print(f"Failed to get message of service, logs: {logs}")
            sleep(5)
