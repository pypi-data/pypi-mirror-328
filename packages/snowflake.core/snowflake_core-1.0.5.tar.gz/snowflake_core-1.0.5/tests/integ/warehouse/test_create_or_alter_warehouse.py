...
def test_create_or_alter_with_fetch(warehouses, cursor):
    ...
    try:
        ...
        # this can only be set for adapative warehouses
        warehouse_fetched.warehouse_credit_limit = None
        warehouse_fetched.target_statement_size = None
...
@pytest.mark.use_accountadmin
def test_alter_comment(warehouses, comment):
    new_w_name = random_string(3, "test_alter_comment_")
    new_w = Warehouse(name=new_w_name)
    w = warehouses.create(new_w)
    try:
        new_w.comment = comment
        w.create_or_alter(new_w)
