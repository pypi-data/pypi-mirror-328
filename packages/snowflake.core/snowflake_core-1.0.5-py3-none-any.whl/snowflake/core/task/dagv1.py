...
from snowflake.core.exceptions import APIError, InvalidOperationError, NotFoundError
...
class DAGTask:
    ...
    def add_predecessors(
        ...
        if isinstance(tasks, DAGTask):
            if tasks._is_finalizer:
                raise InvalidOperationError(f"Finalizer task {self.name} cannot be predecessor of any task")
...
class DAGOperation:
    ...
    @api_telemetry
    def drop(self, dag: Union[DAG, str]) -> None:
