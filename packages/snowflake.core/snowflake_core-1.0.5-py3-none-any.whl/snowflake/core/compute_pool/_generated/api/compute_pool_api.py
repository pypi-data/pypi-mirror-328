...
class ComputePoolApi(object):
    ...
    def create_or_alter_compute_pool(
        ...
        """
        kwargs['_return_http_data_only'] = True
        return self.create_or_alter_compute_pool_with_http_info(
            name, compute_pool, **kwargs)  # noqa: E501

    @validate_call
    def create_or_alter_compute_pool_with_http_info(self, name: Annotated[
        str,
        Field(strict=True,
              description="Identifier (i.e. name) for the resource.")],
                                                    compute_pool: ComputePool,
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise _APITypeError("Got an unexpected keyword argument '%s'"
                                    " to method create_or_alter_compute_pool" %
                                    _key)
            _params[_key] = _val
        del _params['kwargs']
