...
config = {
    ...
    'dep_command_enables': {
        'shell'            : True, # Enable DEPS to process shell@ commands to run in subprocess
        'work_dir_add_srcs': True, # Enable DEPS to process work_dir_add_srcs@ commands
        'peakrdl'          : True, # Enable DEPS to process peakrdl@ commands to run in subprocess
        'var_subst_args'   : True,
        'var_subst_os_env' : True,
        'tee'              : True, # built-in logging file name for 'shell' commands, otherwise uses default log filename.
    },
    'deps_yaml_supported': True,
    'deps_subprocess_shell': True, # allow DEPS.yml 'shell' and 'peakrdl' commands to run in subprocess shell=True.
...
class CommandDesign(Command):
    ...
    def run_dep_shell_commands(self):
        ...
        for iter,d in enumerate(self.dep_shell_commands):
            ...
            clist = util.ShellCommandList(d['exec_list'])
            # NOTE(drew): shell=True subprocess call, can disable with global config
    ...
    def run_commands_check_logs(self, commands, check_logs=True, log_filename=None,
        ...
        for obj in commands:
            ...
            if check_logs and log_fname:
                self.check_logs_for_errors(filename=log_fname, bad_strings=bad_strings, must_strings=must_strings)
...
class CommandSimVerilator(CommandSim, ToolVerilator):
    def __init__(self, config:dict):
        ...
        for x in ['%Error', '%Fatal']:
            self.args['log-bad-strings'].append(x)
    ...
    def compile(self):
        ...
        if self.args.get('lint-only', False):
            self.run_commands_check_logs(self.lint_only_command_lists)
        else:
            self.run_commands_check_logs(self.verilate_command_lists)
    ...
    def get_simulate_command_lists(self):
        ...
        # Note the | tee sim.log will ruin bash error codes, so although we scrape
        # sim.log for errors later, we also test ${PIPESTATUS} -eq 0 to percolate the
...
class CommandSimModelsimAse(CommandSim, ToolQuesta):
    def __init__(self, config:dict):
        ...
        self.args['log-must-strings'] = [' vsim ', 'Errors: 0']
...
class CommandSimIverilog(CommandSim, ToolIverilog):
    def __init__(self, config:dict):
        ...
        self.args['log-bad-strings'] = ['ERROR: ', 'Error:', 'error: ', 'FATAL: ', 'Fatal:', 'fatal: ', ': syntax error', 'I give up']
    ...
    def get_simulate_command_lists(self):
        ...
        return [ util.ShellCommandList(['./sim.exe'], tee_fpath='sim.log') ]
