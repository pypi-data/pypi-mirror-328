...
### TODO: itertools.compress when using the pixelbasedclusters variable.
...
class RAPIDGUI():
    ...
    ### TODO: Change variable names to be more standard ("train")
    def __init__(self):
    ...
    ### TODO: Make size dependent on max image size?
        ...
        if len(self.objecttrainlist[curimg]) > 0:
            ...
            if biaxial.colorbyindivclusters or biaxial.colorbycombclusters:
                if len(self.objecttrainlist[curimg]) > 1:
                    iteration = GUIUtils.ObjectTrainIteration(self.objecttrainlist[biaxial.imageindex])
                    iteration.exec()
                    if not iteration.OK:
                        return
        ...
        if tablefilters.reset:
            ...
            if self.mode == "Segmentation":
                ...
                for i in range(self.tablewidget.columnCount()):
                    ...
                    for j in range(self.numimgs):
                        self.lowerboundslist[startind + j] = copy.deepcopy(self.minvalssegment[self.index])
                        self.lowerboundslist[startind + j] = copy.deepcopy(self.maxvalssegment[self.index])
                imind = int(self.index // self.numimgs) * self.numimgs
                for i in range(self.numimgs):
                    self.combcellnums[imind + i] = copy.deepcopy(self.orders[imind + i])
        ...
        # If applying a new filter, add to the existing filters and update the table accordingly.
            ...
            # If filtering an object clustering table.
                ...
                if len(self.objectdatalist[self.index]) > len(currentdata) > 0:
                    ...
                    for index in self.objectclusternums:
                        currentclusters[self.greyobjects[trainnum] == index+1] = 1
            ...
            # If filtering a pixel clustering table.
                ...
                for i in range(currentdata.shape[0]):
                    if self.order[i] in self.currentlyselectedpixelclusters[trainnum] and self.order[i] not in self.pixelclusternums:
                        self.currentlyselectedpixelclusters[trainnum].remove(self.order[i])

    ### TODO: Test this for Windows.
    ### TODO: Looks slightly different from CLI than when loading pretrained model in GUI.
    def generate_RAPID_data(self, markerindices, markernames, outfolder, denoise, normalizeeach, normalizeall, normtype,
                            pca):
        """
    ...
    def load_environment(self):
        ...
        print(config.get("Variables", 'objectcolor'))
        self.objectcolor = [np.array(l) for l in ast.literal_eval(config.get("Variables", 'objectcolor'))]
        ...
        self.pixelcolor = [np.array(l) for l in ast.literal_eval(config.get("Variables", 'pixelcolor'))]
    ...
    def load_object_clusters(self):
        ...
        self.objectcolor.append(color[:-1,:])
    ...
    def manual_annotation(self):
        ...
        ### TODO: Doesn't work -- expected 12 got 16 -- didn't include morphological params originally?
        ...
        fulltabDF.insert(1, "Cluster", [str(labelnames[int(id) - 1]) for id in to_values])
    ...
    def merge_clusters(self, checked=[]):
        ...
        if self.mode == "Pixel":
            ...
            ind = [i for i,m in enumerate(self.pixelbasedclusters) if m][n]

            if not self.annotatedclusters[ind] == []:
                name = self.annotatedclusters[ind][checked[-1]]
                for cluster in checked:
                    self.annotatedclusters[ind].pop(cluster)
                self.annotatedclusters[ind].append(name)
        ...
        # If the uer is merging object-based clusters.
            ...
            ind = [i for i, m in enumerate(self.pixelbasedclusters) if not m][n]
            newclusterval = int(np.max(grey) + 1)

            if not self.annotatedclusters[ind] == []:
                newname = self.annotatedclusters[ind][checked[-1]]
                print(newname)
                for cluster in checked:
                    oldname = self.annotatedclusters[ind].pop(cluster)
                    print(self.annotatedclusters[ind])
                    self.tabdata[n]['Cluster'][self.tabdata[n]['Cluster'] == oldname] = newname
                    cluster += 1
                    grey[grey == cluster] = newclusterval
                    grey[grey > cluster] = grey[grey > cluster] - 1
                    newclusterval -= 1
                    ...
                    for i in reversed(range(len(self.viewer.layers))):
                        if self.viewer.layers[i].name == index:
                            self.viewer.layers.pop(i)
                            break
                self.annotatedclusters[ind].append(newname)
                self.tabdata[n].to_csv(os.path.join(self.objectclusterdirectories[n], "FullObject_segmentation_dataCluster.csv"))
                ...
                for cluster in checked:
                    ...
                    for i in reversed(range(len(self.viewer.layers))):
                        if self.viewer.layers[i].name == index:
                            ...
                            break
                clusterids += 1
                self.tabdata[n]['Cluster'] = [str(id) for id in clusterids]
                self.tabdata[n].to_csv(os.path.join(self.objectclusterdirectories[n], "FullObject_segmentation_dataCluster.csv"))
            ...
            self.update_table(newdisplaydata, self.lowerbounds, self.upperbounds, newclusterval, self.objectclusternums,
                              headernames=self.annotatedclusters[ind])
    ...
    def minimum_spanning_tree(self):
        ...
        # If multiple rounds of clustering have been performed, prompt the user to select which one to use.
            ...
            GUIUtils.log_actions(self.actionloggerpath, f"MST: {selectclusteringround.overallclusterindex}")
        ...
        arrays = np.array([cv.imread(fn) for fn in pathlist])
        self.set_invisible(self.viewer)
        self.viewer.add_image(arrays, name='MST (' + name + ")", blending="additive")
    ...
    def nearest_neighbours(self):
        ...
        clusteredsegresults = [self.objectimgnames[i] for i, l in enumerate(self.objecttrainlist) if len(l) > 0]
        if len(clusteredsegresults) > 1:
            segmentedimage = GUIUtils.SelectSegmentedImage(clusteredsegresults)
            segmentedimage.exec()
            if not segmentedimage.OK:
                return
            imgindex = self.objectimgnames.index(segmentedimage.image)
        elif len(clusteredsegresults) == 1:
            imgindex = self.objectimgnames.index(clusteredsegresults[0])
        else:
        ...
        if len(self.objecttrainlist[imgindex]) > 1:
            iteration = GUIUtils.ObjectTrainIteration(self.objecttrainlist[imgindex])
            iteration.exec()
            if not iteration.OK:
                return
            it = iteration.iteration
        else:
            it = self.objecttrainlist[imgindex][0]

        # Get all the cluster IDs in the selected clustered image, and prompt the user to define source and
        # target clusters.
        tabdata = self.tabdata[it]
        numtabs = 1
        if len(self.filenames) > 1:
            numtabs += len(self.filenames)
        clusteringround = it * numtabs + selectedimgindex
        data = self.objectdatalist[clusteringround]

        # Find current names of clusters.
        annotatedobjectclusters = [self.annotatedclusters[i] for i in range(len(self.annotatedclusters)) if not self.pixelbasedclusters[i]]
        if len(annotatedobjectclusters[it]) == 0:
            currentnames = [i + 1 for i in range(len(data)) if data[i, 0] != 0.0]
        else:
            currentnames = annotatedobjectclusters[it]
    ...
    def object_clustering(self):
        ...
        if not continuetraining:
            ...
            self.test_object(model, expressionavgs, args, [i + 1 for i in range(len(self.markers) + 4)], colored, grey,
                             "RAPID", imagenum, optimizer=optimizer, outputpath=dir, predict=True)
        else:
            if alg.alg == "Phenograph":
                model = 0
                self.test_object(model, expressionavgs, args, [i + 1 for i in range(len(self.markers) + 4)], colored,
                                 grey, "Phenograph", imagenum, outputpath=dir)
                pass
            elif alg.alg == "SciPy":
                model = 0
                self.test_object(model, expressionavgs, args, [i + 1 for i in range(len(self.markers) + 4)], colored,
                                 grey, setParams.scipyalgo, imagenum, outputpath=dir)
                pass
            else:
                hf = zarr.open(dir, 'a')
                ...
                self.test_object(model, expressionavgs, args, [i + 1 for i in range(len(self.markers) + 4)], colored,
                                 grey, "RAPID", imagenum, optimizer=optimizer, outputpath=dir)
        self.viewer.status = "RAPID clustering done."
    ...
    def open_images(self, segmentedimgpaths=[]):
        ...
        if not self.imagehasbeenloaded:
            ...
            print(type(self.viewer.layers[0].data))
            self.imagehasbeenloaded = True
    ...
    ### TODO: Color of training loaded model? Black cluster not the correct one anymore.
    ...
    def pixel_clustering(self):
            ...
            def finish_pixelgui() -> Image:
                ...
                if loadmodel.load:
                    ...
                    params = GUIUtils.RAPIDPixelParameters(len(trainmarkers.markernums), maximgshape,
                                                           israndompatches=False)
                    args.rfold = self.outputfolder
                    args.loadmodel = False
                params.exec()
                if not params.OK:
                    return
                args.normalize, args.normalizeall, args.normtype, args.pca = GUIUtils.pixel_normtype(params.normalize)

                # Normalize data for RAPID input.
                self.viewer.status = "Generating RAPID data..."
                self.generate_RAPID_data(trainmarkers.markernums, trainmarkers.pixeltrainmarkers, outfolder,
                                         params.denoise, args.normalize, args.normalizeall, args.normtype, args.pca)

                # Update parameters and save them to the output folder.
                logstr.append(f"Parameters ({params.nc}, {params.nit}, {params.bs}, {params.mse}, "
                              f"{params.RCN}, {params.lr}, {params.SCAN}, {params.normalize}, "
                              f"{params.denoise}), ")
        ...
        # If randomly generating patches.
            ...
            if loadmodel.load and predict:
                ...
                # Normalize data for RAPID input.
                self.viewer.status = "Generating RAPID data..."
                self.generate_RAPID_data(trainmarkers.markernums, trainmarkers.pixeltrainmarkers, outfolder, False,
                                         args.normalize, args.normalizeall, args.normtype, args.pca)
            ...
            # If training a model.
                ...
                # If training a new model.
                    ...
                    args.normalize,args.normalizeall,args.normtype,args.pca = GUIUtils.pixel_normtype(params.normalize)
                    logstr.append(f"Parameters ({params.nc}, {params.nit}, {params.bs}, {params.ps}, {params.nop}, "
                                  f"{params.mse}, {params.RC}, {params.RCN}, {params.lr}, {params.SCAN}, {params.normalize}, "
                                  f"{params.denoise})")
    ...
    def rename_clusters(self):
        ...
        # Find current names of clusters.
        currentnames = self.annotatedclusters[overallclusterindex]

        # If list is empty, find number of clusters and use those for the names.
        if len(currentnames) == 0:
            if ispixelcluster:
                numclusters = len(np.unique(self.greypixels[clusteringnum]))
            else:
                numclusters = len(np.unique(self.greyobjects[clusteringnum])) - 1
        ...
        self.annotatedclusters[overallclusterindex] = renameclusters.newclusternames

        if not ispixelcluster:
            self.tabdata[clusteringnum]['Cluster'] = [renameclusters.newclusternames[oldnames.index(name)] for name in self.tabdata[clusteringnum]['Cluster']]
            self.tabdata[clusteringnum].to_csv(os.path.join(self.objectclusterdirectories[clusteringnum], "FullObject_segmentation_dataCluster.csv"))
        ...
        if self.numimgs == 1:
            ...
            numtabs = self.numimgs + 1
        index = self.index // numtabs

        # If table is currently visible, update the names accordingly.
        if index == clusteringnum and ((self.mode == "Pixel" and ispixelcluster) or (self.mode == "Object" and not ispixelcluster)):
            self.verticalheaderlabels[3:] = [renameclusters.newclusternames[clusternum] for clusternum in self.objectclusternums]
            self.tablewidget.setVerticalHeaderLabels(np.asarray(self.verticalheaderlabels))
            vwidth = GUIUtils.font_width("Helvetica", 12, self.verticalheaderlabels)
    ...
    def save_environment(self):
        ...
        config.set("Variables", 'objectcolor', unicode([arr.tolist() for arr in self.objectcolor]))
        ...
        config.set("Variables", 'pixelcolor', unicode([arr.tolist() for arr in self.pixelcolor]))
    ...
    def segment(self):
        ...
        for i, path in enumerate(modelpaths):
            if not os.path.exists(path):
                gdown.download(fileurls[i], path)
        modelpath = modelpaths[segmentationmodel.modelindex]
        ...
        def finish_segmentgui() -> Image:
            ...
            if not self.addedtable:
                ...
                self.upperbounds = copy.deepcopy(self.maxvalssegment[0])
                self.update_table(self.Qtab[0][:, 1:], self.lowerbounds, self.upperbounds, len(self.Qtab[0]),
                                  self.Qtab[0][:, 0].astype(np.int).tolist())
                self.cellnums = copy.deepcopy(self.orders[0])
        ...
        if (self.segmentcount > 0 or self.pixeltraincount > 0) and not self.loadingenv:
            ...
            # If displaying object-based clustering results.
                ...
                if self.numimgs == 1:
                    ...
                    numtabs = self.numimgs + 1
                clusterindex = self.index // numtabs

                # Find names for the clusters for the round of clustering being displayed in the table.
                annotationindex = [j for j, n in enumerate(self.pixelbasedclusters) if not n][clusterindex]
            ...
            # If displaying pixel-based clustering results.
                ...
                # Find names for the clusters for the round of clustering being displayed in the table.
                annotationindex = [i for i, n in enumerate(self.pixelbasedclusters) if n][clusterindex]
    ...
    def spatial_analysis(self):
        ...
        if not spatialparams.OK:
            return
        pval, tab = KNN.random_kdtree_single(rclusters, spatialparams.npix, spatialparams.nsim, objectclusters=True)
        ...
        GUIUtils.log_actions(self.actionloggerpath, f"Spatial Analysis: Cluster ({overallclusterindex})")
    ...
    def subcluster(self):
        ...
        self.test_object_subcluster(model, currentimage, args, numcellsperimage, it, clusternum,
                                    imgindex * self.numimgs, startindex, trainmarkers.markernums, alg)
        self.viewer.status = "RAPID subclustering complete"

    def test_object(self, model, quantifiedvals, args, markerindices, createcolorimg, creategreyimg, alg, startind=0,
        ...
        for i in range(self.numimgs):
            complete_tab.append(self.Qtab[i + startind][:, qtabindices])
        complete_tab = np.vstack(complete_tab)
        complete_tab_DF = pd.DataFrame(complete_tab)
        ...
        fulltab.insert(1, "Cluster", [str(val) for val in to_values])
        ...
        generate_mst(distancematrix=DistMatrix, normalizeddf=my_data_scaled[my_data_scaled.columns], colors=color,
                     randomseed=0, clusterheatmap=True, outfolder=outputpath, displaymarkers=mergemarkerlist,
                     uniqueclusters=uniqueClusters, samplenames=list(np.unique(my_data['Sample'])), displaysingle=False)

    ### TODO: Update table like is done in test_object
    def test_object_subcluster(self, model, quantifiedvals, args, numcellsperimage, iteration, clusternum, startind,
                               objectclustersstartindex, markerindices, alg):
        """
        ...
        for i in range(len(numcellsperimage)):
            if numcellsperimage[i] != 0:
                images.append(i)
        data = np.zeros((len(images), len(np.unique(to_values)), len(self.markers) + 5))
        ...
        for i in range(numtabs):
            ...
            ''' Find weighted average data '''
            weighted_average = np.zeros((len(np.unique(to_values)), len(self.markers) + 5))
            for j in range(len(np.unique(to_values))):
                weighted_average[j, 0] = np.sum(np.array(c[:, j]))
                weighted_average[j, 1:] = np.average(data[:, j, 1:], axis=0, weights=c[:, j])
            newtable = np.zeros(
                (len(self.objectdatalist[it + i]) - 1 + len(np.unique(to_values)), len(self.markers) + 5))
        ...
        for i in range(self.numimgs):
            ...
            for j in range(numcellsperimage[i]):
                ...
                count += 1
            self.objectclusters[objectclustersstartindex + i] = updated_to_values
            ...
            #tabdata['Cluster'][counter:counter + len(list(updated_to_values))] = list(updated_to_values.astype(str))
        ...
        if len(self.objecttrainlist[setumapParams.imageindex]) > 0:
            ...
            if setumapParams.colorbyindivclusters or setumapParams.colorbycombclusters:
                if len(self.objecttrainlist[setumapParams.imageindex]) > 1:
                    iteration = GUIUtils.ObjectTrainIteration(self.objecttrainlist[setumapParams.imageindex])
                    iteration.exec()
                    if not iteration.OK:
                        return
    ...
    ### TODO: Disappears when filtering down to one.
