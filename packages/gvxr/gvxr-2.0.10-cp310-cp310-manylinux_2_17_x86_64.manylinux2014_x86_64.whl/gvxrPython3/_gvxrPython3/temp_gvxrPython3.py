...
def lineariseTest(aThreshold, aScalingFactor):
    ...
    \max(aThreshold, \hat{\theta}(i))) \end{eqnarray*}  
...
def computeRMSE(*args):
    ...
    \hat{\theta}(i,j,k)\right)^2}{I \times J \times K}\\ RMSE(\theta,
    \hat\theta) &=& \sqrt{MSE\left(\theta, \hat\theta\right)}
    \end{eqnarray*}  
...
def addControlPoint(aLabel, aPositionX, aPositionY, aPositionZ, aUnitOfLength, aDentityFunction, aParameterA, aParameterB=0.0):
    ...
    * `aDentityFunction`, `:` :  
        ...
        curve centered at the origin, actually a half Gaussian as $r \geq 0$.  
         "metaball", which uses the density function $\left\{ \begin{array}{ c
        l } a \left(1 - \frac{3 r^2}{b^2}\right) & \quad \textrm{if } 0 \leq r
        \leq b/3 \\ \frac{3 a}{2} \left(1 - \frac{r}{b}\right)^2 & \quad
        \textrm{if } b/3 \leq r \leq b \\ 0 & \quad \textrm{otherwise}
        \end{array} \right.$  $a$ is a scaling factor, $b$ is the maximum distance
        a control primitive contributes to the field.  
         "soft object", which uses the density function $\left\{ \begin{array}{
        c l } a \left(1 - \frac{4r^6}{9b^6} + \frac{17r^4}{9b^4} -
        \frac{22r^2}{9b^2}\right) & \quad \textrm{if } 0 \leq r \leq b \\ 0
        & \quad \textrm{otherwise} \end{array} \right.$  $a$ scales the
        function, each control primitive has no influence after a distance $b$. This
        function has a slight advantage over that for Meta balls as it only uses
        squares of the distance and so it isn't necessary to compute square roots.  
    * `aParameterA`, `:` :  
