...
##############################################                        
    ...
    def handle_request(self, method):
        ...
        if not 'X-Forwarded-Proto' in request_headers.keys(): request_headers['X-Forwarded-Proto'] = 'https' if CONFIG['secure'] else 'http'
        if not 'X-Forwarded-Host' in request_headers.keys(): request_headers['X-Forwarded-Host'] = request_headers.get('Host')
        if not 'X-Forwarded-For' in request_headers.keys(): request_headers['X-Forwarded-For'] = self.client_address[0]
        else : request_headers['X-Forwarded-For'] = request_headers['X-Forwarded-For'] + ', %s' %self.client_address[0]
