...
"""
#　update_info 20240920
1. 修改了数据库匹配中mz_error的计算方法 （freeSpec也要修改，数据库匹配方式也要修改）；
2. 修改了one_step_process_DDA的bug
#　update_info 20240920_1
3. 针对first, second, database matching 做了捕获异常操作。
# update_info 20240924
4. import traceback
5. fix num in DDA_to_DIA
6. delete t3 in some functions.
# update_info 20240929
7. 修改了了ChainedAssignmentError的问题
如df['index']改成df.loc[:,'index'] 几十处修改
20241011:
修改了DDA和DIA存储raw spec方式，全部使用字典存储。
20241027:
修复了formula_prediction的bug
20241104:
添加了fingerprint相关组件
20241105
one_step_process_swath 增加了sat_intensity 参数
修改了identify_isotopes,用字典存储了。
20241118
限制molmass的版本2021.6.18
20241128
修复了extract的bug
peak_checking_area的质量阈值要和alignment一致才行,对df1重新做了排序
isotope score小数进行了round
20241129：
修复了isotope_identify的bug
20241130
将DDA_df的iso_distribution转成str，避免报错
修改了DDA_df 的MS2_spec_DDA列名称和存储方式，改用字典存储
20241202
fingerprint_generation增加了blank_names参数
20241202
fingerprint_generation做了修改和升级，source_info传入字典，可以支持每个源有多个文件
20241209
(1)添加了suspect_screen,calculate_similarity_score
(2) 重新修改了formula_to_distribution 和isotope_score功能
20241214
(1) 解决one_step_process_DDA删除相同alignment的问题
20241220
（1） 更新了formula_to_distribution功能，增加了多种adduct的形式
(2) 增加了adducts_type_converter，可以将不规范的adduct type改成标准的
(3) 修改了isotope_score,增加了adducts_type_converter，可以做标准转换
20241223
(1) 修改了ms2_matching匹配中，isotope_score的参数。
20241224
（1） 屏蔽了警告
20241225
（1）修改了gen_DDA_df中的一些bug
20241230
（1）更新了suspect_screening，可以导出全面的信息
(2) 修复了当使用sat_intensity时引用的错误
20250101
（1）修改了PCA_analysis，增加了additional info

20250102
(1) 修复了get_chemical_name无法使用的问题

20250111
(1) 修改了split_n的范围，从1000改成1700

20250115
(1)更新了second_step的问题，有时候用户使用0.5min.mzml命名，使用‘.’分割的时候出问题

# 20250211
(1) 修复了DDA only时，最后两个数据点读取不到的问题

# 20250212
（1） 修复了所有intensity都是2的情况

# 20250217
(1) 修改了峰提取时zscore阈值，发现>1的情况下，有一些峰无法找到，因此修改为>0.
...
def suspect_screen(unique_cmps, database, mode, ms1_error=5, ms2_error=0.015, rt_error=0.1, iso_score=0.7,precursor_window = 2,
            ...
            if ms2_analysis is True:
                ...
                if any(item.lower() in basename_file.lower() for item in control_group):
                    ...
                    for i in tqdm(range(len(peak_all)), desc=f'{message}Assign DIA MS2 spectrum', leave=False,
                        ...
                        # append fragments
                        frag = str(list(df_DIA['mz'].values)[:max_frag_num])
...
def peak_finding(eic, threshold=10, width=1):
    ...
    peaks, pro_dict = scipy.signal.find_peaks(eic, width=1)
...
def peak_picking(df1, threshold=10, i_threshold=500, SN_threshold=3, peak_width=2, area_threshold=500,
    ...
    if len(data) == 0:
        ...
        if alignment:
            ...
            if len(data) != 0:
                ...
                peak_all = peak_all.sort_values(by='intensity', ascending=False)
...
def split_peak_picking(ms1, profile=True, split_n=20, threshold=10, i_threshold=500, SN_threshold=3, peak_width=1,
    ...
    for idx, scaled_pair in tqdm(enumerate(scaled_data), desc='Aligning all rt_mz pairs(split)', leave=False, colour='Green'):
        if idx in visited:
            continue
...
def identify_isotopes(cmp, iso_error=0.005,rt_error = 0.015):
    ...
    cmp.drop(columns=['Ciso', 'Cliso', 'Na adducts', 'NH4 adducts', 'Briso', 'K adducts'], errors='ignore',inplace=True)
...
def sort_columns_name(columns):
    ...
    locators = ['new_index','rt', 'mz', 'intensity', 'S/N', 'area', 'area_mean', 'area_std', 'mz_opt',
                'frag_DIA', 'frag_DDA', 'iso_distribution', 'resolution', 'isotope/adducts label','fold_change', 'p_values']
...
def gen_ref(files_excel, rt_error=0.1, mz_error=0.015):
    ...
    for idx, scaled_pair in tqdm(enumerate(scaled_data), desc='Aligning all rt_mz pairs(gen_df)', leave=False, colour='Green'):
        if idx in visited:
            continue
...
def fold_change_filter(path, control_group=['lab_blank'], filter_type=1):
    ...
    if filter_type == 1:
        ...
        for i in tqdm(range(len(area_files_sample)), desc='Fold change processing', leave=False, colour='Green'):
            ...
            alignment_path = [file for file in alignment if name in os.path.basename(file)][0]
        ...
        for i in tqdm(range(len(unique)), desc='Starting DIA ms2 matching:', leave=False):
            ...
            if len(match_result) == 0:  # 匹配失败
                ...
                for j in range(len(match_result)):
                    ...
                    try:
                        iso_score = isotope_score(iso_info, formula, mode=mode, adducts_type = adduct_type) # 这里会自动转换，所以不需要提前转换
                    except:
                        iso_score = 0
...
def gen_ref_precursor_frag(data, ms1_error=0.01, ms2_error=0.015):
    ...
    for idx, scaled_pair in tqdm(enumerate(scaled_data), desc='Aligning all rt_mz pairs(precursor_frag)', leave=False, colour='Green'):
        if idx in visited:
            continue
