...
class HttpClient:
    ...
    async def call_api(
        ...
        # Ensure that both json and form_data are not provided at the same time.
        ...
        payload_param = {}
        if json is not None:
            # For JSON, set the Content-Type explicitly.
            headers.setdefault("Content-Type", "application/json;charset=utf-8")
            payload_param["json"] = json
        elif form_data is not None:
            # For multipart data, let the client set the Content-Type (with the proper boundary).
            headers.pop("Content-Type", None)
            payload_param["data"] = form_data

        # Build request arguments. The payload is passed using either the 'json' or 'data' keyword.
