...
class TemporalMixin:
    ...
    def t_created_at(cls):
        def generate_created_at(context):
    ...
    def soft_delete(self, session: Session) -> None:
        """Soft delete the current record and create a explicit diff entry"""
        ...
        # Create an explicit diff entry for soft deletion
        diff_entry = DiffEntry(
            entity_type=self.__tablename__,
            t_id=self.t_id,
            from_version=self.t_version,
            to_version=self.t_version  # Same version, just marking deletion
        )
        session.add(diff_entry)

        # Create a special change record for soft deletion
        diff_change = DiffChange(
            ...
            new_value=None,
            value_type='dict'  # Serialize the entire state
        )
...
def track_processed_temporal_records(session: Session, flush_context, instances=None) -> None:
    """
    Track all temporal mixin objects that have been flushed.

    This method accumulates flushed items across multiple flush operations
    within the same session.
    """
    for obj in session.new | session.dirty | session.deleted:
        if isinstance(obj, TemporalMixin):
            # Ensure t_id is available before adding
            if hasattr(obj, 't_id') and obj.t_id is not None:
                _processed_temporal_records.add((type(obj), obj.t_id))
