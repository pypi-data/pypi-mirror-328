...
try:
    ...
    pymysql = mysqlclient = mysql.connector = None
except:
    try:
        import pymysql
        import pymysql as mysql_api
        from pymysql.cursors import SSCursor
        
        mysql.connector = mysqlclient = MySQLdb = None    
    except:
        try:
            ...
            pymysql = mysql.connector = MySQLdb = None
        except:
            try: #mysql.connector started to fail in Debian after security updates
                import mysql.connector
                import mysql.connector as mysql_api

                pymysql = mysqlclient = MySQLdb = None
            except:                
                pymysql = mysql.connector = mysqlclient = MySQLdb = mysql_api = None
...
class FriendlyDB(log.Logger):
    ...
    def __init__(
        ...
        # self.api=api
        if '@' in db_name:
            up, hd = db_name.split('@',1)
            if not user:
                user, passwd = up.split(':') if ':' in up else (up,'')
            if not host:
                host, db_name = hd.split('/') if '/' in hd else (
                    hd.split('.') if '.' in hd else ('localhost',hd))

        self.db_name = db_name
        self.host = host
        self.use_tuples = (
            use_tuples  # It will control if data is returned in tuples or lists
        )
        ...
        self.default_cursor = (
            default_cursor
            or (MySQLdb and MySQLdb.cursors.SSCursor)
            or None
    ...
    def setAutocommit(self, autocommit=None):
        try:
            ...
            self.error("Unable to set MySQL.connection.autocommit to %s" % autocommit)
            # raise Exception,e
    ...
    def renewMySQLconnection(self):
        try:
            ...
            if mysql_api == MySQLdb:
                self.db = mysql_api.connect(
                    db=self.db_name, host=self.host, user=self.user, passwd=self.passwd
                )
            ...
            self.error(
                "Unable to create a mysql_api connection to "
                '"%s"@%s.%s: %s' % (self.user, self.host, self.db_name, str(e))
            )
            self.error(traceback.format_exc())
            raise_(Exception, e)
            
    def getSSCursor(self):
        try:
            return mysql_api.cursors.SSCursor
        except:
            return None
        ...
        try:
            ...
            if (renew or klass) and self._cursor:
                if not self._recursion:
                    self._cursor.close()
                    del self._cursor
                elif self._recursion > 10:
                    print('Max Recursion Limit Reached! (10)')
                    return None
                
            if renew or klass or not self._cursor:
                #print('getCursor(%s, %s)' % (mysql_api, klass))
                if mysql_api == MySQLdb:
                    self._cursor = (
                        self.db.cursor(self.default_cursor)
                        if klass is None
                        else self.db.cursor(cursorclass=klass)
                    )
                else:
                    self._cursor = self.db.cursor(cursor=klass)
            ...
            self.renewMySQLconnection()
            self._recursion += 1
            if self._recursion < 10:
                return self.getCursor(renew=True, klass=klass)
            else:
                raise Exception('Max Recursion Limit Reached! (10)')
    ...
    def table2dicts(self, keys, table):
        ...
        for line in table:
            d = {}
            [
                d.__setitem__(keys[i], line[i])
                for i in range(min([len(keys), len(line)]))
            ]
            result.append(d)
        return result
    ...
    def Select(
        ...
        if type(clause) is list:
            ...
            for i in range(len(clause)):
                ...
                clause1 += (
                    "%s like '%s'" % (k, v) if type(v) is str else "%s=%s" % (k, str(v))
                )
                if (i + 1) < len(clause):
                    clause1 += " and "
            #' and '.join
    ...
    def getDatabases(self, load=False):
        """Gets the list of available databases."""
        q = self.Query("show databases", False)
        return [t[0] for t in q.fetchall()]
    ...
    def check(self, method=None, tables=None, verbose=False):
        ...
        if verbose:
            ...
            return not any(
                isinstance(v, (Exception, type(None)))
                for v in list(self.check_result.values())
            )
    ...
    def checkTable(self, table, partition=None):
        ...
        try:
            ...
            self.error('checkTable(%s,%s): %s' % (
                table, partition, traceback.format_exc()))
            return False
    ...
    def getTableLength(self, table=""):
        ...
        res = self.Query(
            ...
            "table_schema = '%s' and table_name like '%s';" % (self.db_name, table)
        )
        return 0 if not res else (int(res[0][1]) if len(res) == 1 else dict(res))

    def getTableSize(self, table="", index=True):
        """
        Returns size in bytes of table files (data and index)
        """
        table = table or "%"
        q = "select table_name,sum(data_length)"
        if index:
            q += "+sum(index_length)"
        res = self.Query(
            q + " from information_schema.partitions where "
            " table_schema = '%s' and table_name like '%s';" % (self.db_name, table)
        )
        return 0 if not res else (int(res[0][1]) if len(res) == 1 else dict(res))
    ...
    def getPartitionSize(self, table="", partition=""):
        ...
        q = (
            ...
            " and partition_name %s;" % (self.db_name, table, partition)
        )
        res = self.Query(q)
        try:
            return 0 if not res else (int(res[0][1]) if len(res) == 1 else dict(res))
        except Exception as e:
            self.error('getPartitionSize(%s) = %s: %s' % (
                q, res, traceback.format_exc()))
            raise e
    ...
    def getPartitionRows(self, table="", partition=""):
        ...
        try:
            ...
            self.error('getPartitionRows(%s) = %s: %s' % (
                q, res, traceback.format_exc()))
            raise e
    ...
    def getBigTables(self, ratio=0.1, dbsize=0):
        ...
        return dict((t, s) for t in list(sizes.items()) if s > ratio * dbsize)
