...
from .excepts import getLastException, exc2str, ExceptionContext, CatchedArgs
from .objects import *
...
from .log import Logger, printf, tracer, TRACE_LEVEL
...
class API_CantConnectToDevice(PyTango.DevFailed):
    pass
...
EVENT_TYPES = {
    "attr_conf": EventType.ATTR_CONF_EVENT,
...
try:
    EVENT_TYPE['quality'] = EventType.QUALITY_EVENT
    EVENT_TYPES['QUALITY_EVENT'] = EventType.QUALITY_EVENT
except:
    'Tango Quality Event not available'
    pass

try:
    EVENT_TYPE['alarm'] = EventType.ALARM_EVENT
    EVENT_TYPES['ALARM_EVENT'] = EventType.ALARM_EVENT
except:
    'Tango Alarm Event not available'
    pass
...
def event2str(event):
    """
    Parse Tango Event as received by push_event callback
    """
    name = event.attr_name
    ttype = event.event
    value = event.attr_value
    date = event.get_date().isoformat()
    if event.err:
      r = date, name, ttype, 'Error', event.errors[0].desc
    else:
      r = date, name, ttype, getattr(value,'value',None), getattr(value,'quality',None)
...
class EventListener(Logger, Object):  # Logger,
    ...
    def push_event(self, event):
        ...
        """
        return self.eventReceived(event.attr_name, event.event_, event.attr_value)
    ...
    def eventReceived(self, src, type_, value):
        ...
        delay = int(
            1e3 * (t0 - (ctime2time(value.time) if hasattr(value, "time") else t0))
        )

        error = isinstance(value, (PyTango.DevError, Exception)) or getattr(
            value, "err", False
        )  # getattr(value,'err',False))
        if error:
            l = self.info if str(src) not in self.error_sources else self.debug
            l(
                "eventReceived(%s,ERROR): +%2.1f ms (%2.1f delay)"
                % (src, 1e3 * inc, delay)
            )
            ...
            if self.error_hook is not None:
                try:
                    self.error_hook(src, type_, value)
                except:
                    self.warning(" %s:%s" % (src, traceback.format_exc()))
            ...
            if rvalue is not None:
                if isinstance(rvalue, PyTango.DevError):
                    ...
                    self.warning("%s restored: %s ..." % (str(src), str(rvalue)[:80]))
                    self.error_sources.remove(str(src))

                if self.value_hook is not None:
                    try:
                        self.value_hook(src, type_, rvalue)
                    except:
                        self.error(" %s:%s" % (src, traceback.format_exc()))

        if self.event_hook:
            try:
                self.event_hook(src, type_, value)
            except:
                self.error(" %s:%s" % (src, traceback.format_exc()))
    ...
    def process(self):
        ...
        while True:
            try:
                ...
                if not self.filtered or (e[0] not in [l[0] for l in self.lasts]):
...
class EventThread(Logger, ThreadedObject):
    ...
    def get_sources(self, source=""):
        return [s for s in self.sources if not source or clmatch(source, s.full_name)]
    ...
    def process(self):
        ...
        lg = (
            partial(tracer, obj=self)
            if not self.count % self.SHOW_ALIVE
            else self.debug
        )
        lg(
            "EventThread.process(filtered=%s,count=%d,period=%f)"
            % (filtered, self.count, self.get_period())
            + "<" * 16
        )
        ...
        for i in range(self.EVENT_POLLING_RATIO):
            ...
            if (False < self.filtered < True) and (
                (time.time() - t0) > (self.filtered * 1e-3 * self.latency)
                or self.queue.qsize() < self.EVENT_POLLING_RATIO
            ):
                filtered = True
        ...
        if pending:
            self.info("%d events processed, %d still in queue" % (len(queue), pending))
        ...
        for s in self.get_sources():
            ...
            if s.getMode():
                ...
                if t0 > (
                    s.last_read_time
                    + 1e-3 * (s.polling_period if polled else s.KeepAlive)
                ):

                    alive = check_device_cached(s.device)
                    if not alive and s.last_read_time < (t0 - s.KeepAlive):
                        self.debug(
                            "%s seems not alive, last read was at %s"
        ...
        for nxt, s in reversed(ksorted(pollings)):
            if t0 > nxt and (s.isPollingActive() or WAS_EMPTY):
                try:
                    ...
                    self.debug(
                        "Executing pollings (%d/%d/%d)"
                        % (polls, len(pollings), len(self.sources))
                    )
                    self.debug(
                        "process.poll(%s,%s,%s) after %s ms"
                        % (
                            s.full_name,
                            s.polling_period,
                            WAS_EMPTY,
                    ...
                    self.warning("%s is faulty!, polling will be reduced" % s.full_name)
                    s.polling_period = max((s.KeepAlive, s.polling_period or 0))
        ...
        asynch_hw = [
            s[1] for s in pollings if getattr(s, "pending_request", None) is not None
        ]
        ...
        if notupdated:
            self.info(
                "%d/%d sources not updated yet" % (len(notupdated), len(self.sources))
            )
            ...
            if hasattr(source, "fireEvent"):
                ...
                for l in listeners:
                    try:
                        ...
                        self.event.wait(old_div(self.MinWait, len(source.listeners)))
...
class EventSource(Logger, SingletonMap):
    ...
    All types are: 'CHANGE_EVENT,PERIODIC_EVENT,QUALITY_EVENT,ALARM_EVENT,
    ...
    DEFAULT_LOG = "WARNING" if TRACE_LEVEL < 4 else "DEBUG"
    DEFAULT_EVENTS = ["change"]
    VALUE_EVENTS = [k for k in
        ["periodic", "change", "archive", "quality", "user_event", "alarm"]
        if k in EVENT_TYPES]
    ...
    def __init__(self, name, keeptime=1000.0, fake=False, parent=None, **kw):
        """
        **kw used to keep compatibility with old api
        ...
        try:
            ...
            traceback.print_exc()
            raise Exception(
                "A valid device name is needed: %s" % ([name, self.simple_name, parent])
            )

        self.full_name = get_full_name("/".join((self.device, self.simple_name)))
        self.normal_name = "/".join(self.device.split("/")[-3:] + [self.simple_name])
        assert self.fake or self.proxy, "%s: A valid and existing device is needed" % (
            name
        )
        ...
        self.forced = kw.get(
            "enablePolling",
            kw.get(
                "enable_polling", kw.get("forcePolling", kw.get("force_polling", False))
            ),
        )
        self.polled = self.forced  # Forced is permanent, polled may change
        # current polling period in milliseconds
        self.polling_period = kw.get(
            "pollingPeriod",
            kw.get(
                "polling_period", 
                iif(isNumber, self.forced, falsepart=self.DefaultPolling,
                    forward=float)
            ),
        ) or self.DefaultPolling
    ...
    def setLogLevel(self, level):
        ...
        #self.get_thread().setLogLevel(level) #too much!
    ...
    def isUsingEvents(self):
        ...
        s = self.checkState("SUBSCRIBED")
        with ExceptionContext(self.warning,message=str(s)):
            return bool(s)

    def enablePolling(self, force=False):
        if force:
            self.activatePolling(force=force)

    def forcePolling(self, period=None):
        period = (
            period
            if isNumber(period)
            else float(self.getPollingPeriod() or self.DefaultPolling)
        )
        if period:
    ...
    def activatePolling(self, period=None, force=None):
        ...
        try:
            ...
            self.info("activatePolling(%s,%s)" % (self.forced, self.polling_period))

            self.resetStats()
            self.get_thread().register(self)
        except:
            self.warning("activatePolling() failed!: %s" % traceback.format_exc())
    ...
    def addListener(self, listener, use_events=True, use_polling=False):
        ...
        """
        if not isCallable(listener) and not any(
            hasattr(listener, c)
            for c in ("eventReceived", "event_received", "push_event")
        ):
        ...
        self.info(
            "addListener(%s,use_events=%s,polled=%s)"
        ...
        if use_events:
            ...
            self.subscribeEvents(
                types=use_events, asynchronous=self.checkState("UNSUBSCRIBED")
            )
        ...
        self.debug(
            "addListener(%s): %d listeners registered" % (listener, len(self.listeners))
        )
        return True
    ...
    def removeListener(self, listener, exclude="dummy"):
        ...
        """
        self.warning("removeListener(%s)" % str(listener))
        if listener == "*":
            self.warning("Removing all listeners")
            listener = [
                k for k in list(self.listeners.keys()) if not k().name == exclude
            ]
        ...
        if isSequence(listener):
            [self.removeListener(l) for l in listener]
            return
    ...
    def fireEvent(self, event_type, event_value, listeners=None):
        ...
        self.debug(
            "fireEvent(): %d/%d listeners" % (len(listeners), len(self.listeners))
        )
        for l in listeners:
            try:
                ...
                if (
                    evtype not in ("periodic", EventType.PERIODIC_EVENT)
                    and evtype not in evlist
                ):
                    self.debug(
                        "%s event discarded (not in %s - %s)"
        ...
        try:
            ...
            self.stats["latency"] = old_div(
                self.stats["acc_latency"], self.stats["fired"]
            )
        except:
            pass
    ...
    def subscribeEvents(self, types=None, asynchronous=True):
        try:
            ...
            if not isIterable(self.use_events):
                self.use_events = []
            self.use_events = sorted(
                set((self.use_events + types) or self.DEFAULT_EVENTS)
            )
    ...
    def checkEvents(self, tdiff=None, vdiff=None):
        ...
        if self.use_events:
            ...
            if tdiff and (
                self.checkState("SUBSCRIBING") or (vdiff and self.isUsingEvents())
            ):

                if self.isUsingEvents():
                    self.warning(
                        "Values differ and no event received "
                        "(check attr_conf)! (tdiff=%s,vdiff=%s)" % (tdiff, vdiff)
                    )

                elif delta > self.EVENT_TIMEOUT:
                    self.info(
                        "Event subscribing failed (tdiff=%s,vdiff=%s)"
        ...
        if self.listeners:
            ...
            check = check_device_cached(self.proxy) #.ping()
        ...
        try:
            ...
            for type_, ID in list(self.event_ids.items()):
                try:
                    ...
                    self.debug("Error unsubscribing {} events: {}".format(type_, e))
    ...
    def set_cache(self, value, t=None):
        ...
        self.attr_value = (
            value if hasattr(value, "value")
            else fakeAttributeValue(self.full_name, value, error=False)
        )
    ...
    def read(self, cache=None, asynch=None, _raise=True):
        ...
        if not cache or self.attr_value is None:
            ...
            self.read_hw(asynch=asynch, rethrow=_raise)

            if self.attr_value is not None:
                # if None, asynch has not been read yet
                # on failed Tango events, value is None
                # but on callbacks, it is replaced by exception value 
                # and then fired (err=True)
                if (getattr(self.attr_value,'err',False)
                    or getattr(self.attr_value,'error',False)):
                    self.warning('firing error event (%s)' % (self.full_name))

                self.fireEvent(EventType.PERIODIC_EVENT, self.attr_value)

        if _raise and isinstance(self.attr_value.value, Exception):
            #and getattr(self.attr_value, "error", False):
            raise self.attr_value.value
        else:
            return self.attr_value

    @CatchedArgs(throw=True)
    def read_hw(self, asynch=False, rethrow=True):
        self.debug("read_hw(asynch=%s,\n\tpending=%s)" % (asynch, self.pending_request))
        try:
            ...
            if asynch:
                ...
                ## Unnecessary string conversion triggers cpu peak!
                # self.debug(
                #     "read_hw(asynch=False):\n\tvalue:%s\n\tdata:%s"
                #     % (
            ...
            if 'AttrNotAllowed' in str(e):
                self.attr_value = fakeAttributeValue(self.full_name, value=None, error='AttrNotAllowed')
            else:
                # fakeAttributeValue initialized with full_name
                self.warning("EventSource.read(%s) failed!,"
                    " polling deactivated if %s:\n%s" % (
                    self.full_name, self.forced, traceback.format_exc()))
                if rethrow:
                    self.attr_value = fakeAttributeValue(self.full_name, value=e, error=e)
                else:
                    self.attr_value = fakeAttributeValue(self.full_name, value=None, error=str(e))

            if not check_device_cached(self.device) and self.polled and not self.forced:
                self.deactivatePolling()
        finally:
            self.event_lock.release()
    ...
    def poll(self):
        ...
        if tdiff < (old_div(1e-3 * self.polling_period, 2)):
            log = self.warning
        else:
            log = self.debug
        ...
        try:
            ...
            self.debug("poll(%s) exception: %s" % (self.full_name,
                                                   traceback.format_exc()))
    ...
    def push_event(self, event):
        ...
        try:
            ...
            if event.err:
                ...
                (
                    self.info
                    if (self.last_event_time < time.time() - 5 * self.KeepAlive)
                    else self.debug
                )(
                    "push_event(%s,err=%s,has_events=%s,polled=%s)"
                ...
                (self.debug if self.last_event.get(type_, None) else self.info)(
                    "push_event(%s,err=%s,has_events=%s)"
                    % (type_, event.err, self.isUsingEvents())
                )
                ...
                if (
                    str(event.event) == "periodic"
                    and (t0 - self.last_event_time) < 0.01
                ):
            ...
            if delay > 1e3:
                self.warning("push_event was %f seconds late" % delay)
            self.last_event[type_] = event
...
def get_test_objects(model="controls02:10000/test/sim/tonto/Pressure", period=1.0):
    """
...
class TAttr(EventStruct):
    ...
    def __init__(self, name, dev_name="", proxy=None, event_id=None):
        ...
        self.receivers = (
            []
        )  # This is the list of composers that must receive this event
        self.attr_value = None
...
class EventCallback(object):  # SHOULD BE RENAMED OR REMOVED
    """
    It provides persistent storage.
    lock.acquire and lock.release should be used to prevent threading problems!,
    Use of the lists inside push_event is safe
    ...
    def __init__(self):
        ...
        self.NotifdExceptions = [
            "OutOfSync",
            "EventTimeout",
            "NotificationServiceFailed",
        ]
    ...
    def push_event(self, event):
        ...
        try:
            ...
            if not event.err and event.attr_value is not None:
                ...
                #print(
                    #"in EventCallback.push_event(...): Received an Error Event!: ",
                    #event.errors,
                #)
                _EventsList[attr_name.lower()].set(event)
                ...
                reasons = [
                    getattr(
                        e,
                        "reason",
                        e.get("reason", str(e)) if hasattr(e, "get") else str(e),
                    )
                    for e in event.errors
                ]  # Needed to catch both PyTango3 and PyTango7 exceptions
                if any(n in r for n in self.NotifdExceptions for r in reasons):
                    #print(
                        #"callbacks=> DISCARDED EVENT FOR NOTIFD REASONS!!! {}({})".format(
                            #dev_name,
                            #reasons,
                        #)
                    #)
                    ...
                    if attr_name.lower().endswith("/state"):
                        _StatesList[
                            dev_name.lower()
                        ] = None  # An unreaded State cannot be UNKNOWN, it must be None to notify that an exception occurred!
                    _AttributesList[attr_name.lower()] = None
            # Launching Device.push_event()
            for rec in _EventsList[attr_name].receivers:
                if rec in list(_EventReceivers.keys()):
                    _EventReceivers[rec].push_event(event)
                elif hasattr(rec, "push_event"):
                    rec.push_event(_event)
                elif isinstance(rec, threading.Event):
                    rec.set()
                elif callable(rec):
                    rec()
                else:
                    raise Exception("UnknownEventReceiverType")
        except Exception as e:
            if self.last_exception < now()-60:
                print(
                    "exception in EventCallback.push_event(): ", e, ";", getLastException()
                )
...
def getStateFor(devname):
    ...
    GlobalCallback.lock.acquire()
    state = (
        _StatesList[devname.lower()]
        if devname.lower() in list(_StatesList.keys())
        else None
    )
    GlobalCallback.lock.release()
    return state
...
def setAttributeValue(attr_name, attr_value):
    #print("In callbacks.setAttributeValue({})".format(attr_name))
    GlobalCallback.lock.acquire()
...
def subscribeDeviceAttributes(self, dev_name, attrs):
    ...
    dev.ping()
    # Initializing lists
    if dev_name not in callbacks._StatesList:
        callbacks._StatesList[dev_name] = PyTango.DevState.UNKNOWN
    if dev_name not in callbacks._AttributesList:
        callbacks._AttributesList[dev_name] = None
    ...
    for att in attrs:
        ...
        if not dev.is_attribute_polled(att) and self.ForcePolling:
            ...
            period = dev.get_attribute_poll_period(att) or EventSource.DefaultPolling
        ...
        if not att_name in list(callbacks._EventsList.keys()):
            ...
            print(
                ...
                self.get_name(),
                "::AddDevice()",
                ": Listing Device/Attributes in _EventsList:",
            )
            for a, t in list(callbacks._EventsList.items()):
                print("\tAttribute: ", a, "\tDevice: ", t.dev_name, "\n")
        else:
            self.warning(
                "::AddDevice(%s): This attribute is already in the list, adding composer to receivers list."
                % att_name
            )
            ...
            if callbacks._EventsList[att_name].attr_value is not None:
                if att == "State":
                    callbacks._StatesList[dev_name] = _EventsList[
                        att_name
                    ].attr_value.value
                else:
                    callbacks._AttributesList[dev_name] = _EventsList[
                        att_name
                    ].attr_value.value
    return
