...
class Plotter:
    ...
    def plot(self, config, save_as=None, data=None, dpi=None):
        ...
        config['x_var'] = config['x_var'] if config['x_var'] in data.columns else None
        config['y_var'] = [y for y in config['y_var'] if y in data.columns]

        if sum([(config[v] not in data.columns) for v in ['hue', 'style', 'size'] if config[v] is not None]) or \
