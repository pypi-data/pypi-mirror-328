...
class Plotter:
    ...
    def plot(self, config, save_as=None, data=None, dpi=None):
        ...
        if sum([(config[v] not in data.columns) if v is not None else False for v in ['hue', 'style', 'size']] + \
               [config['x_var'] not in data.columns] + [y not in data.columns for y in config['y_var']]):
    ...
    def serial_plot(self, config, data=None,
        ...
        for index, row in config.iterrows():
            cfg = row.to_dict()  # Convert row to dictionary
            file_name = cfg.pop('savn', None)
            cfg.update(dict(theme=theme, palette=palette, aspect=aspect))
