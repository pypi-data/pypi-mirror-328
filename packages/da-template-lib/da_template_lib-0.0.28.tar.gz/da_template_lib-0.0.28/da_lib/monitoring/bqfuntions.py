...
class BigQueryTableUpdater:
    ...
    def __init__(self, project_id: str, sa_email=None, yaml_file=None):
        ...
        try:
            ...
            # Dicionário para armazenar os valores de qt_rows_ok para cada tracker_id e step
            self.step_values = {}
    ...
    def get_qt_rows_ok_difference(self, tracker_id, step_1, step_2, volumetria, nm_project):
        ...
        """
        # Se os valores de step para o tracker_id já existirem, vamos usá-los
        if tracker_id not in self.step_values:
            self.step_values[tracker_id] = {}

        # Se o valor de RAW já foi registrado, não precisamos consultar o BigQuery de novo
        if step_1 == "RAW" and step_1 not in self.step_values[tracker_id]:
            # Para o primeiro RAW, vamos simplesmente salvar o valor e não calcular a diferença
            self.step_values[tracker_id][step_1] = int(volumetria)
            logging.info(f"Valor de {step_1} registrado para o tracker_id {tracker_id} (sem cálculo de diferença).")
            return 0  # Não calcula a diferença na primeira vez

        # Caso contrário, fazemos a consulta ao BigQuery
        client = bigquery.Client(project=self.project_id)

        query = f"""
        SELECT nm_step, qt_rows_ok
        FROM `{self.config.project_id_bq}.dados_alternativos.tl_ingestions`
        WHERE tracker_id = @tracker_id AND nm_project = "{nm_project}" AND nm_step IN (@step_1, @step_2)
        """
        ...
        # Agora, calculamos a diferença para todos os casos em que o RAW já foi registrado
        if step_1 == "RAW":
            self.step_values[tracker_id][step_1] = step_1_qt_rows_ok  # Armazena o valor de RAW

        # Se ambos os valores estão registrados, calcula a diferença
        if step_1 in self.step_values[tracker_id] and step_2 in self.step_values[tracker_id]:
            step_1_qt_rows_ok = self.step_values[tracker_id][step_1]
            step_2_qt_rows_ok = self.step_values[tracker_id][step_2]

            difference = abs(step_1_qt_rows_ok - step_2_qt_rows_ok)
            logging.info(f"Diferença entre {step_1} e {step_2}: {difference}")
            return difference
        else:
            logging.warning(f"Um dos steps não foi registrado para o tracker_id {tracker_id}.")
            return 0
    ...
    #     if step_1 == "RAW":
    #         # Quando o step_1 é "RAW", considera a diferença como 0 (sem comparação anterior).
    #         logging.warning("Step 'RAW' não será considerado para cálculo de diferença (diferenca = 0).")
    #         difference = 0
    #     else:
    #         difference = abs(step_1_qt_rows_ok - step_2_qt_rows_ok)
