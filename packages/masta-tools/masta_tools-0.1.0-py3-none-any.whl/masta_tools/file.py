...
class File:
    ...
    @staticmethod
    def find_file(title:str='', message:str=None):
        msg = f"File Finder: {title}"
        msg += f"\nPlease hold ALT and press TAB until you find the window titled '{title}'"
        if message is None:
            msg += f"\n{message}"
        print(msg)
        root=tk.Tk()
        root.withdraw()
        filepath = askopenfilename(title=title)
        root.destroy()
        return filepath
