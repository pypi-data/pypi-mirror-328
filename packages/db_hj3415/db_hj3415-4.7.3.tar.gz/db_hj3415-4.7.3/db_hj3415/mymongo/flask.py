...
class Favorites(Flask):
    def __init__(self):
        ...
        self.collection = self._client['flask']['favorites']

    def save(self, data:Stock) -> bool:
        result = self.collection.insert_one(asdict(data)) # type: ignore
        return result.acknowledged

    def get(self, code: str) -> Optional[Stock]:
        data = self.collection.find_one({"code": code}, {"_id": 0})  # _id 제외
        if data:
    ...
    def get_all(self) -> List[Stock]:
        documents = list(self.collection.find({}, {"_id": 0}))  # 모든 문서 조회, _id 제외
