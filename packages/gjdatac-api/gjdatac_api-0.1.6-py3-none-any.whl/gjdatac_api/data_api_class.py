...
class GJDataC:
    ...
    def get_active_contract(self, code=None, start_date=datetime.date.today(),
                            end_date=datetime.date.today(), asset_type=None,
                            fields=None, source=None):
        """
        获取主力/次主力合约信息接口
        ...
        """ 校验参数 """
        self.get_active_contract_validate_params(code, start_date, end_date, asset_type)

        """ 获取、筛选数据 """
        data, db_session = self.get_active_contract_get_data(code, start_date, end_date, asset_type, source)
        db_session.close()

        """ 处理返回结果 """
        data = self.get_active_contract_process_result(data, fields)

        return data

    def get_active_contract_validate_params(self, code, start_date, end_date, asset_type):
        """
        校验参数
        Args:
        ...
        # 校验必填参数
        self.get_active_contract_validate_required_params(code, start_date, end_date, asset_type)
    ...
    def get_active_contract_validate_required_params(self, code, start_date, end_date, asset_type):
        """
        校验参数是否必填
        :param code: str or str list--必填，品种
        :param start_date: date--必填，默认当天
    ...
    def get_active_contract_get_data(self, code, start_date, end_date, asset_type, source):
        """
        获取数据
        :param code: str or str list--必填，品种
        :param start_date: str--必填，开始日期
        :param end_date: str--必填，结束日期
        :param asset_type: str--必填，资产类型
        ...
        # 根据 asset_type 筛选数据 (现无次主力合约信息，不清楚数据结构和字段信息，后续需要根据实际数据进行调整)
        # 根据表中的 rank 字段筛选主力/次主力合约，rank == 1是主力合约， 2 是次主力合约
        rank = 1 if asset_type == "active" else 2
        rank_data = date_data.where(f"rank = {rank}")

        # 根据 source 筛选数据，如果有，就筛选，反之直接返回 date_data
        if source:
            source_dict = {
                "1": "dc",
                "2": "mick",
            ...
            source_data = rank_data.where(f"source = '{source_dict[str(source)]}'")

            return source_data.toDF(), db_session
    ...
    def _get_basic_data_date_to_dolphin_time(self, date, asset_type):
        ...
        # 在此需求中，因为所需的数据源表中对于日期字段的定义不一样，所以需要根据不同情况进行处理
        if asset_type == "库存":
            if isinstance(date, str):
    ...
    def subscribe(self, subscribe_ip=None, subscribe_port=None, subscribe_user_id=None, subscribe_password=None,
        ...
        try:
            ...
            # 关闭连接和资源释放逻辑
            s.unsubscribe(subscribe_ip, subscribe_port, "QcMinKlineTable")
        except Exception as e:
            raise e

    """ get_history_bar 查询合约指定日期前的行情数据（2024-12-24提出） """

    def get_history_bar(self, datetime_str=None, instrument_id=None, bar_count=None,
        ...
        mk_instrument_id: 米筐合约代码， instrument_id: CTP合约代码
        Args:
            datetime_str: str, 查询的日期时间，格式为'YYYY-MM-DD HH:MM:SS'
    ...
    @staticmethod
    def _get_history_bar_parse_datetime(datetime_str):
        """
        解析日期时间字符串
        Args:
    ...
    def _get_history_bar_filter_data(self, table_data, instrument_id, datetime_str, bar_count):
        """
        筛选数据
        Args:
            table_data: 表数据
        ...
        if isinstance(instrument_id, str):
            query_data = (table_data.where(f"instrument_id='{instrument_id}'").where(f"trade_time <= {dolphin_date}")
                          .contextby("instrument_id").sort("trade_time", ascending=True).limit(-bar_count))
        else:
            query_data = (table_data.where(f"instrument_id in {instrument_id}").where(f"trade_time <= {dolphin_date}")
                          .contextby("instrument_id").sort("trade_time", ascending=True).limit(-bar_count))
    ...
    def get_history_bars(self, instrument_id, start_date, end_date, frequency, fields=None):
        """
        Args:
            instrument_id: str or List[str], 合约代码，可以是单个合约或合约列表
    ...
    def _get_history_bars_execute_query(self, frequency, instrument_id, start_date, end_date):
        ...
        # 筛选数据
        query_data = self._get_history_bars_filter_data(table_data, instrument_id, start_date, end_date)
