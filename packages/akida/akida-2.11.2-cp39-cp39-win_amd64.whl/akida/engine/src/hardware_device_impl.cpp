  ...
  if (effective_batch_size != pipeline_state_.max_size()) {
    ...
    if (version_.product_id == 0xA2) {
      ...
      const auto skipdma_store_tracks =
          current_program_info_.skipdma_store_track();
      const auto skipdma_load_tracks =
          current_program_info_.skipdma_load_track();
      for (uint32_t idx = 0; idx < skipdma_store_tracks.size(); idx++) {
        const auto& store_track = skipdma_store_tracks[idx];
        const auto& load_track = skipdma_load_tracks[idx];
        //------------- allocate skipdma desc buffer memory
        const uint8_t max_outbound = skipdma::program_store_channel_cont_size(
            dma_config_, store_track, &mem_mgr_, &external_mem_, driver_,
            is_pipeline, effective_batch_size);
        skipdma::program_load_channel_cont_size(dma_config_, load_track,
                                                &mem_mgr_, &external_mem_,
                                                driver_, max_outbound);
        ...
        skipdma::program_store_channel_desc_buff_addr(
            ...
            store_track, &external_mem_, driver_);
        skipdma::program_load_channel_desc_buff_addr(
            skipdma_mem_info.engine.descriptor_base_addr, dma_config_,
            load_track, &external_mem_, driver_);
        ...
        skipdma_mem_info.outputs_base_address =
            skipdma::program_store_channel_ob_buff_addr(
                max_outbound, dma_config_, store_track, &mem_mgr_,
                &external_mem_, driver_);
        //------------- enable PLD clear
        skipdma::store_channel_enable_pld_clr(
            dma_config_, store_track, &mem_mgr_, &external_mem_, driver_);
