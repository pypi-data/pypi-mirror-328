...
# rename to SolidModel?
    ...
    def cell_rotation(self, tag, state=None):
        pass
...
def _add_section_shape(section, sections, outlines):
    ...
    if "section" in section:
        ...
        if section["section"] not in outlines:
            outlines[int(section["name"])] = _add_section_shape(sections[section["section"]], sections, outlines)
        else:
            outlines[int(section["name"])] = outlines[int(section["section"])]
        ...
        try:
            ...
            outlines[section["name"]] = np.array([points[i] for edge in alpha_shape(points, 1.0) for i in edge])
...
def _get_frame_outlines(model):
    ...
    for elem in model["assembly"].values():
        if "sections" in elem:
            outlines[elem["name"]] = [
                SectionGeometry(shapes[int(i)]) for i in elem["sections"]
                if i in shapes and shapes[i] is not None
            ]
            continue
