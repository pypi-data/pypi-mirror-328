...
def skin_frames_(model, canvas, config=None):
    """
...
class Motion:
    ...
    def draw_sections(self,
                      state=None, rotation=None, position=None,
                      time=None):
        """
        ...
        if self._section_skins is None:
            self._section_skins = skin_frames(self.model, self.artist,
    ...
    def add_to(self, canvas):
        ...
        #    Create BufferViews and Accessors, then fix up each sampler's input/output
        #    to reference the newly created accessor indices.
        #    Do this "in bulk," iterating over new_animation.samplers
...
def create_animation(artist, states=None, skin_nodes=None):
    ...
    for time in states.times:
        animation.draw_sections(state=states[time]) #, skin_nodes)
        animation.advance()
