...
class DateTime(DateMixin, datetime):

    def __new__(cls, year: int, month: int = None, day: int = None, hour: int = 0, minute: int = 0, second: int = 0, microsecond: int = 0, tzinfo: ZoneInfo = None, fold: int = 0) -> Self:
        """
        ...
        Args:
            ...
            tzinfo (ZoneInfo, optional): The time zone information. Defaults to None.
        ...
        Criteria:
            - `year`, `month`, and `day` are required. Other parameters are optional.
            - If `tzinfo` is not provided, it defaults to 'UTC'.
            - The `fold` parameter should be 0 or 1.
        ...
        Example of exception message:
            'ValueError: Invalid input values for creating a DateTime object.'

        See Also:
            [Python Documentation](https://docs.python.org/3/library/datetime.html#datetime.datetime.fold)
        """
        # We check for tzinfo to enforce the UTC if empty
        if not tzinfo:
            tzinfo = ZoneInfo('UTC')
    ...
    def ensure(cls, value: 'DateTime') -> Self:
        ...
        Criteria:
            - The method accepts either a DateTime object, a datetime object or a DatetimeWithNanoseconds object.
            - It raises an error if the value cannot be converted to a DateTime object.
        ...
        """
        try:
            value = cls(value.year, value.month, value.day, value.hour, value.minute, value.second, value.microsecond, value.tzinfo)
        except AttributeError as error:
            raise ValueError(f"Invalid instantiation of class '{cls.__name__}' from '{value.__class__.__name__}'") from error
    ...
    def fromtimestamp(cls, timestamp: float, tz: ZoneInfo = None) -> Self:
        ...
        Criteria:
            - The method accepts a timestamp in float format.
            - If `tz` is not provided, the time zone is enforced to be UTC.
            - It raises an error if the timestamp is not a valid number.
    ...
    def adjust_time_zone(self, time_zone: str = date_settings.DEFAULT_TIME_ZONE) -> Self:
        ...
        Returns:
            Self: A new DateTime object with the replaced timezone information.

        Criteria:
            - The method accepts an optional `time_zone` parameter.
            - The default time zone is the value of `date_settings.DEFAULT_TIME_ZONE`.
            - It raises a ValueError if the provided `time_zone` is not a valid time zone string.
        ...
        Example of exception message:
            'ValueError: Invalid time zone string.'
        """
    ...
    def astimezone(self, tz: str = date_settings.DEFAULT_TIME_ZONE) -> Self:
        ...
        Returns:
            Self: A new DateTime object representing the adjusted date and time in the specified time zone.
    ...
    def now(cls, tzinfo: timezone = None) -> Self:
        ...
        Criteria:
            - The method accepts an optional `tzinfo` parameter.
            - If `tzinfo` is not provided, the time zone defaults to 'UTC'.
            - If `tzinfo` is provided as a string, it's used to create a `zoneinfo.ZoneInfo` object.
            - It raises a ValueError if the provided `tzinfo` is not a valid time zone.
        ...
        Example of exception message:
            'ValueError: Invalid time zone provided.'
        """
        if tzinfo is None:
            tzinfo = ZoneInfo('UTC')
    ...
    def fromisoformat(cls, date_string: str) -> Self:
        ...
        Criteria:
            - The method accepts an ISO formatted date or datetime string.
            - It raises a ValueError if the input date_string is not in a valid ISO format.
        ...
        Example of exception message:
            'ValueError: Invalid ISO formatted date or datetime string.'
    ...
    def force_time(self, force_time: str = date_settings.DEFAULT_FORCE_TIME) -> Self:
        ...
        Criteria:
            - The method accepts an optional `force_time` parameter.
            - It raises a ValueError if an invalid `force_time` is provided.
            - It returns a new DateTime object with the specified time.
    ...
    def strptime(cls, date_string: str, format: str = date_settings.DEFAULT_DATE_TIME_FORMAT) -> Self: # pylint: disable=redefined-builtin
        ...
        Criteria:
            - The method accepts an input `date_string` and an optional `format` format string.
            - It raises a ValueError if the input `date_string` does not match the provided `format` format.
            - It returns a DateTime object representing the parsed date and time.
    ...
    def strptime_or_null(cls, date_string: str, format: str = date_settings.DEFAULT_DATE_TIME_FORMAT) -> Union[Self, None]: # pylint: disable=redefined-builtin
        ...
        Criteria:
            - The method accepts an input `date_string` and an optional `format` format string.
            - It returns a DateTime object representing the parsed date and time, or None if parsing fails.
    ...
    def string_date_to_date_time(cls, date_string: str, force_time: str = date_settings.DEFAULT_FORCE_TIME) -> Self:
        ...
        Args:
            ...
            force_time (str, optional): A string indicating the desired time for the datetime object.
                                       Valid values are 'MIDDAY', 'NOW', 'FIRST_MINUTE', 'LAST_MINUTE',
                                       or a custom time in the format 'HH:MM:SS'. Defaults to 'MIDDAY'.

        Raises:
            ValueError: If an invalid `force_time` value is provided.

        Returns:
            DateTime: A DateTime object representing the combination of the input string date
                      and the specified or default time.

        Criteria:
            - The method accepts an input `date_string` and an optional `force_time` parameter.
            - It raises a ValueError if an invalid `force_time` value is provided.
        ...
        Example of exception message:
            'ValueError: Invalid force_time. Please choose one of the following: MIDDAY, NOW, FIRST_MINUTE, LAST_MINUTE, or a custom time in the format HH:MM:SS.'
        """
        if force_time == 'MIDDAY':
            force_time = '12:00:00'
        elif force_time == 'NOW':
    ...
    def strftime(self, format: str = date_settings.DEFAULT_DATE_TIME_FORMAT) -> str: # pylint: disable=redefined-builtin, useless-parent-delegation
        ...
        Args:
            format (str, optional): The desired date format. This should be a string representing the desired format using
                                 the appropriate format codes.
        ...
        Criteria:
            - The method accepts an optional `format` parameter for specifying the date format.
            - It raises a ValueError if the provided `format` format string is invalid.
            - It returns a string representing the DateTime object in the specified format.
    ...
    def strftime_or_null(cls, datetime_: Any, format: str = date_settings.DEFAULT_DATE_TIME_FORMAT) -> Union[str, None]: # pylint: disable=redefined-builtin
        ...
        Args:
            ...
            format (str, optional): The desired date format. This should be a string representing the desired format using
                                 the appropriate format codes.
                - Defaults to `date_settings.DEFAULT_DATE_TIME_FORMAT`.
        ...
        Criteria:
            - The method accepts an input `datetime_` and an optional `format` parameter.
            - It raises a ValueError if the provided `format` format string is invalid.
            - It returns a string representation of the input date in the specified format, or None if the input date is None.
