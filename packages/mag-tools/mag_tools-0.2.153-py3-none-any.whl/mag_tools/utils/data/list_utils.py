...
class ListUtils:
    ...
    def pick_block(lines: List[str], begin_keyword: str, end_keyword: str) -> List[str]:
        start_index = next((i for i, line in enumerate(lines) if begin_keyword in line.strip()), None)
        end_index = next((i for i, line in enumerate(lines) if end_keyword in line.strip() and i > start_index), None)
