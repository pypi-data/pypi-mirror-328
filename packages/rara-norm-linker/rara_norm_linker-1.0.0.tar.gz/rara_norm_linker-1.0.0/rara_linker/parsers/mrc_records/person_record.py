...
class PersonRecord(GeneralRecord):
    def __init__(self, record: str, add_variations: bool = False) -> NoReturn:
        ...
        self.__name_field_id = ["100"]
        self.__name_variations_field_id = ["400"]
        self.__source_field_id = ["670"]
        self.__description_field_id = ["680"]
        self.__default_year = None
    ...
    def name(self) -> str:
        if not self.__name:
            ...
            self.__name = self._clean_value(values[0]) if values else ""
        return self.__name
    ...
    def birth_year(self) -> int:
        if self.__birth_year == -1:
            try:
                ...
                pass
                #print(e, "life years", self.life_years)
    ...
    def name_variations(self) -> List[str]:
        if not self.__name_variations:
            values = self.get_values(
                self.__name_variations_field_id, 
                subfield_id="a"
            )
            self.__name_variations = [self._clean_value(value) for value in values]
