...
all its data is raw.
"""
from yta_core.classes.project_for_database import ProjectForDatabase
...
# TODO: I think we should rename this to ProjectRaw
class ProjectFromFile:
    """
    Class to represent a raw project that has
    been read from a .json file.
    """
    ...
    def for_database(
        ...
        ProjectForDatabase instance.
        """
        if not hasattr(self, '_for_database'):
            data = ProjectForDatabase(
            ...
            for segment in self.json['segments']:
                ...
                # Manually handle 'text_to_narrate'
    ...
    def __init__(
        ...
        self.json = ProjectFromFile.validate_project_json_from_file(json)
    ...
    def init_from_file(
        ...
        """
        Initializes a ProjectFromFile instance reading the
...
# create a ProjectFromFileValidator static class
# to wrap the methods
