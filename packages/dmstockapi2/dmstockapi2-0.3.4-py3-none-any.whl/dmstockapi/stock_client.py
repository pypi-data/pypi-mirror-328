...
from dmstockapi.constant import TradeSideEnum, SellStypeEnum, PositionOrderEnum, \
    CreditFlagEnum
from dmstockapi.model import *
...
class StockClient:
    ...
    def query_algo_summary(
        self, isdataframe=True, **kwargs
    ) -> List[OrderSummaryModel]:
        params = self._merge_two_dicts(
            {
                "isdataframe": isdataframe,
            },
    ...
    def query_algo_netstat(
        self, isdataframe=True, **kwargs
    ) -> List[OrderNetstatModel]:
        params = self._merge_two_dicts(
            {
                "isdataframe": isdataframe,
            },
    ...
    def query_plan_position(self, params: QueryPlanPosition, isdataframe=True, **kwargs):
        if params.planid == "":
            raise DMStockRequestException(f"Invalid Request Params: {params.__dict__}")
    ...
    def check_plan(self, account: str, portfolio: str, planid: str, sid: str):
        plans_resp = self.query_account_planning(QueryAccountPlans(account=account), isdataframe=False)
        if plans_resp["status"] == 200:
            for plan in plans_resp["data"]:
                if plan["portfolioid"] == portfolio and plan["planid"] == planid and plan["sym"] == sid:
                    return True
        return False

    def stock_buy(self, params: StockTradeBuyModel, isdataframe=False, **kwargs):
        if params.amount <= 0:
            raise DMStockRequestException(f"Invalid Request Params: (amount), {params.__dict__}")
        ...
        if not self.check_plan(params.account, params.portfolioid, params.planid, params.sym):
            raise DMStockRequestException(f"交易计划不存在或填写错误: {params.__dict__}")
    ...
    def stock_sell(self, params: StockTradeSellModel, isdataframe=False, **kwargs):
        ...
        position_resp = self.query_plan_position(QueryPlanPosition(planid=params.planid), isdataframe=False)
        ...
        if params.stype == SellStypeEnum.Positionid:
            if len(params.positionids) == 0:
                raise DMStockRequestException(f"Invalid Request Params: (positionids), {params.__dict__}")
            ...
            if len(old_positionids) != 0:
                raise DMStockRequestException(f"Invalid Request Params: (positionids), {old_positionids}")

        else:
            if params.position_order not in PositionOrderEnum:
                raise DMStockRequestException(f"Invalid Request Params: (position_order), {params.__dict__}")
        ...
        if availablevol < params.volume:
            raise DMStockRequestException(f"可卖量不足: {availablevol} < {params.volume}")
