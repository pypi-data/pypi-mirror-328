...
class WavList(object):
    ...
    def __init__(self, wav_folder: str | Path, classify_file:str = "classification.csv",
        ...
        for x in self.wavs:
            es = x.subject_code()
            if es in wav_grouped:
                wav_grouped[es].append(x)
            else:
                wav_grouped[es] = [x]
        self.wav_grouped = OrderedDict(sorted(wav_grouped.items()))
    ...
    def get_unclassified(self, subject_code: str) -> Iterable:
        return self.get_classify(subject_code=subject_code, cat=WavList.UNCAT)

    def get_classify(self, subject_code: str, cat: int) -> Iterable:
        return filter(lambda x: x.cat == cat, self.wav_grouped[subject_code])
    ...
    def all_stats(self) -> dict:
        ...
        for k, lst in self.wav_grouped.items():
            ...
            n.append(s[0])
            n_un.append(s[1])
        return {"subject_code": es, "n": n, "n_unclass": n_un}
    ...
    def overview(self, only_unclassified: bool = False) -> Tuple[str, List]:
        ...
        if not only_unclassified:
            for es, n, n_un in zip(s["subject_code"], s["n"], s["n_unclass"]):
                if n_un == 0:
    ...
    def subject_code_list(self, with_unclassified_cnt:bool=False) -> List[str]:
        s = self.all_stats()
        if with_unclassified_cnt:
            rtn = []
            for es, _, n_un in zip(s["subject_code"], s["n"], s["n_unclass"]):
                if n_un == 0:
                    rtn.append(es)
                else:
                    rtn.append(f"{es}           ({n_un})")
            return rtn
        else:
