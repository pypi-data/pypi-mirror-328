...
class Style:
    ...
    This class provides functionality to load, apply, and save styles 
    for matplotlib visualizations. It supports both built-in styles 
    ...
    _mandatory_params: Dict[str, Any] = {
        ...
        'figure.constrained_layout.h_pad':  0.02,   # Padding around axes objects. Float representing
        'figure.constrained_layout.w_pad':  0.02,   # inches. Default is 3/72 inches (3 points)
    ...
    @property
    def follow_palette_order(self) -> bool:
        """Indicates whether to follow the specified order of colors in 
        the palette (read-only).

        This property determines how colors are assigned to categories 
        in visualizations. If `True`, colors will be assigned in the
        order they appear in the defined palette. If `False`, colors 
        may be distributed differently across categories, allowing for 
        a more varied representation.

        The value of this property can change based on the currently 
        loaded matplotlib style. Specifically, styles such as 'daspi', 
        'daspi-dark', and 'ggplot2' do not follow the palette order, 
        ensuring consistency with their respective color configurations.
        """
    ...
    def use(self, name: str | Path) -> None:
        ...
        dsp.style.use('path/to/custom.mplstyle')    # Use custom style file
        ```
    ...
    def save(
        ...
        This function saves the current matplotlib rcParams into a .mplstyle 
        file in the specified folder. It also allows for the inclusion of 
        custom comment lines at the top of the file.

        Parameters
        ----------
        folder : Path or str
            The directory where the .mplstyle file will be saved.
        style_name : str
            The name of the style file to be saved. If it does not end with 
            '.mplstyle', this suffix will be added automatically.
        ...
        comment_lines = [
            ...
            "# Used rcParams of seaborn by running:",
            "# >>> import seaborn as sns",
            "# >>> sns.set_theme(context='paper')"
        ]
        dsp.style.save(Path('./styles'), 'seaborn', comment_lines)
        """
        folder = Path(folder)
        ...
        for key, value in rc_params.items():
            if key.startswith('_') or key in self._ignore_params:
                continue

            if 'Style.' in str(value):
                value = str(value).split('Style.')[1]
            elif isinstance(value, list):
                value = ', '.join(map(str, value))
            elif 'color' in key and str(value).startswith('#'):
                value = value[1:]
            elif key == 'savefig.bbox' and value is None:
                value='tight'
...
styles for plotting. This instance provides functionality to load, 
apply, and save styles for matplotlib visualizations. It supports both 
built-in styles from the matplotlib library and custom styles defined in 
.mplstyle files."""
...
def cmap_from_lut(
    ...
    if reverse:
        ...
        cmap = ListedColormap(lut, name)
    return cmap

_luts = {n.split('_')[1]: t for n, t in locals().items() if isinstance(t, list)}
