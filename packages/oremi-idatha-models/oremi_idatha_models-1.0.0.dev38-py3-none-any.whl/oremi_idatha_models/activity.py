...
class WriteTextAction(BaseModel):
  type: Literal['tts', 'print'] = 'tts'
  text: str
...
ActionsByType: dict[str, BaseModel] = {
  'tts': WriteTextAction,
  'media:play': PlayAction,
  'media:pause': PauseAction,
  'media:resume': ResumeAction,
  'media:stop': StopAction,
  'media:mute': MuteAction,
  'media:unmute': UnmuteAction,
  'media:set-volume': SetVolumeAction,
  'media:next': NextAction,
  'media:previous': PreviousAction,
  'system:notify': NotifyAction,
  'system:announcement': AnnounceAction,
  'system:request': RequestAdditionalInfoAction,
  'request:done': DoneAction,
  'request:cancel': CancelAction,
}
...
class Request(BaseModel):
  ...
  # @classmethod
  # def model_validate(cls, data_dict: dict):
  #   prediction_data = data_dict.get('prediction')
  #   prediction = ClassificationPrediction.model_validate(prediction_data) if prediction_data else None

  #   actions_data = data_dict.get('actions', [])
  #   actions = []

  #   for action_data in actions_data:
  #     action_type = action_data['type']
  #     action_cls = ActionsByType.get(action_type)
  ...
  #   return cls(
  #     type=data_dict['type'],
  #     processed_text=data_dict['processed_text'],
...
DetectionDocumentType = Document[Detection, None, UserMeta]
DetectionDocument = DetectionDocumentType.create_alias_from_user_meta(
  data_cls=Detection,
)


RequestDocumentType = Document[Request, None, UserMeta]
