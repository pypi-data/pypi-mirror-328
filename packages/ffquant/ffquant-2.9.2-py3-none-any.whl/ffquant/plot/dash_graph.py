...
def init_graph_callback(app, debug=False):
    ...
    def update_graph(n, data):
        ...
        for origin in origins + ["ALL"]:
            ...
            if origin == "ALL":
                window_size = 0
            win_rates = get_win_rates(origin=origin if origin != "ALL" else None, window_size=window_size, backtest_data=backtest_data)
            ...
            if origin == "ALL" or sum(1 if item["win_rate"] is not None else 0 for item in win_rates.data) > 0:
                figure.add_trace(
                    go.Scatter(
                        ...
                        mode='lines',
                        name=f"{origin}胜率" + (f"(窗口{window_size})" if window_size > 0 else "")
                    ),
...
def get_win_rates(origin=None, window_size=15, backtest_data=None):
    ...
    win_rate_line = Line(maxlen=len(klines[list(klines.keys())[0]]))
