...
from collectors.metrics import MetricCollector
...
class TrackExecution(BaseModel):
    ...
    def __call__(self, original_func=None):
        ...
        async def wrapper(*args, **kwargs):
            ...
            start = datetime.now()
            try:
                original_func_result = await original_func(*args, **kwargs)
                end, execution_time = _execution_time(start)
                await self.metrics_collector.send_metric(dict(
                ...
                error_msg = traceback.format_exception_only(type(e), e)[0].strip()
                end, execution_time = _execution_time(start)
                self.logger.error(f"Error Message: {error_msg}; Execution Time: {execution_time}")
