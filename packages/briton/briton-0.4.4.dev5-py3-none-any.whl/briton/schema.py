...
class ModelInput(BaseModel):
    ...
    def combined_messages(self) -> List[Dict[str, Any]]:
        ...
        for message in combined_messages:
            message["content"] = message["content"].encode("unicode_escape").decode("utf-8")
    ...
    def prompt(self, tokenizer: PreTrainedTokenizerFast) -> str:
        ...
        if tokenizer.chat_template is not None:
            logger.info("Applying chat template to prompt.")
            return cast(
                str,
                tokenizer.apply_chat_template(
                    conversation=[{"role": "user", "content": self.prompt_}],
                    tokenize=False,
                    add_generation_prompt=True,
                ),
            )
        return self.prompt_.encode("unicode_escape").decode("utf-8")
