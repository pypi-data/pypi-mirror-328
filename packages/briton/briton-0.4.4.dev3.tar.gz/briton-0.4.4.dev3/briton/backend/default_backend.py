...
class DefaultBackend(InferBackend):
    ...
    async def _generate(
        self, request: InferenceRequest, is_cancelled: Callable[[], Awaitable[bool]]
        ...
        if (
            ...
            return self._generate_chunked(request, is_cancelled)
        else:
    ...
    async def _generate_chunked(
        ...
        if request.output_schema_hash is not None:
            request.write_output_states = True
        ...
        while True:
            ...
            if await is_cancelled():
                logger.info("Request cancelled. Stopping request %d.", request.request_id)
                break
    ...
    async def infer(
        ...
        resp_iters = [await self._generate(briton_request, is_cancelled)]
        if model_input.n is not None and model_input.n > 1:
            for _ in range(1, model_input.n):
                ...
                resp_iters.append(await self._generate(new_request, is_cancelled))
