...
from colppy.models.empresas import EmpresasRequest, Empresa
from colppy.models.clientes import ClientesRequest, Cliente
from colppy.models.proveedores import ProveedoresRequest, Proveedor
...
class ColppyAPIClient:
    ...
    async def request_items_paginated(self, request, model):
        ret: list = []
        items = await self.request_items(request, model)
        while items:
            ret.extend(items)
            request.next_page()
            items = await self.request_items(request, model)

        logger.debug(f"REQUEST: Se consiguio una respuesta de {model} con {len(ret)} items")
        return ret

    async def request_items(self, request, model):
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                response = await client.post(self._base_url, json=request.to_dict())
                response.raise_for_status()

                response = Response(model, response.json())
    ...
    async def get_empresas(self, filters=None, start=0, limit=10) -> list[Empresa]:
        ...
        empresas = await self.request_items(empresas_request, Empresa)
    ...
    async def get_clientes_by_empresa(self, empresa: Empresa) -> list[Cliente]:
        ...
        return await self.request_items_paginated(clientes_request, Cliente)


    async def get_all_proveedores(self, start=0, limit=50) -> list[Proveedor]:
        ret: list[Proveedor] = []
        empresas = await self.get_empresas()
        for empresa in empresas:
    ...
    async def get_proveedores_by_empresa(self, empresa: Empresa) -> list[Proveedor]:
        ...
        return await self.request_items_paginated(proveedores_request, Proveedor)
    ...
    async def get_movimientos(self, start=0, limit=50, from_date="01/01/2013", to_date="01/01/2030") -> list[Movimiento]:
        ret: list[Movimiento] = []
        empresas = await self.get_empresas()
        for empresa in empresas:
            ret += await self.get_movimientos_by_empresa(empresa, start=start, limit=limit, from_date=from_date, to_date=to_date)
        return ret
