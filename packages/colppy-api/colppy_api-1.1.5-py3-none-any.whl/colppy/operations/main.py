...
class ColppyAPIClient:
    ...
    async def get_all_clientes(self, start=0, limit=50) -> list[Cliente]:
        ret: list[Cliente] = []
        empresas = await self.get_empresas()
        for empresa in empresas:
            ret += await self.get_clientes_by_empresa(empresa, start=start, limit=limit)
        return ret
    ...
    async def get_clientes_by_empresa(self, empresa: Empresa, start=0, limit=50) -> list[Cliente]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                logger.debug(f"Clientes de la empresa {empresa.nombre} (id: {empresa.id_empresa}): {len(clientes_response.get_clientes())}")
    ...
    async def get_all_proveedores(self, start=0, limit=50) -> list[Proveedor]:
        ...
        empresas = await self.get_empresas()
        for empresa in empresas:
            ret += await self.get_proveedores_by_empresa(empresa, start=start, limit=limit)
        return ret
    ...
    async def get_proveedores_by_empresa(self, empresa: Empresa, start=0, limit=100) -> list[Proveedor]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                logger.debug(f"Proveedores de la empresa {empresa.nombre} (id: {empresa.id_empresa}): {len(proveedores_response.get_proveedores())}")
    ...
    async def get_movimientos(self, start=0, limit=50) -> list[Movimiento]:
        ret: list[Movimiento] = []
        empresas = await self.get_empresas()
        for empresa in empresas:
            ret += await self.get_movimientos_by_empresa(empresa, start=start, limit=limit)
        return ret
