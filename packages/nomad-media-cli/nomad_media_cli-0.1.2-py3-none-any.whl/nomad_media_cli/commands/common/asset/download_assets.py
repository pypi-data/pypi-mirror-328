...
def download_assets(ctx, id, url, object_key, destination, threads, include_empty_folders, download_proxy, recursive):
    ...
    try:
        ...
        filtered_assets = []
        while True:
            assets = get_assets(nomad_sdk, id, offset)
        ...
        download_assets_exec(ctx, filtered_assets, destination, threads, download_proxy, recursive)
...
def download_assets_exec(ctx, assets, destination, threads, download_proxy, recursive):
    ...
    for batch_num in range(0, len(assets), 100):
        ...
        download_batch(ctx, batch, threads, download_proxy, is_folder_asset, asset_name, asset_date, 
                       display_path, destination, num_file_assets, batch_num, recursive)
...
def download_batch(ctx, batch, threads, download_proxy, is_folder_asset, asset_name, asset_date, 
                   display_path, destination, num_file_assets, batch_num, recursive):
    with ThreadPoolExecutor(max_workers=threads) as executor:
        for idx, asset in enumerate(batch):
            executor.submit(download_asset, ctx, asset, idx, batch_num, download_proxy, is_folder_asset, 
                            asset_name, asset_date, display_path, destination, num_file_assets, recursive)

def download_asset(ctx, asset, idx, batch_num, download_proxy, is_folder_asset, asset_name, asset_date, 
                   display_path, destination, num_file_assets, recursive):
...
def get_assets(nomad_sdk, id, offset):
    filter = [{
