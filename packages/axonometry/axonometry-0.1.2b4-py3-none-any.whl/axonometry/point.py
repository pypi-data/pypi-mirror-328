...
from .utils import pair_projections_points
...
class Point:
    ...
    def __init__(self, **kwargs: float) -> None:
        self.plane = None
        """The plane in which the point has been added."""
        self.matrix_applied = False
        ...
        self.data: CPoint | None = None  #: Axonometry plane case
    ...
    def reset_data(self) -> CPoint:
        ...
        help of the ```matrix_applied``` boolean flag attribute.
        """
        if self.key == "xy":
    ...
    def project(
        ...
        auxilaray_plane_key: str | None = None,
    ) -> "Point":
    ...
    def _project_on_axonometry_plane(
        ...
        if self.plane.key == "xy":
            ...
            if auxilaray_plane_key == "yz":
                auxilary_point = self.plane.axo[auxilaray_plane_key].add_point(
    ...
    def _project_on_reference_plane(self, ref_plane_key: str) -> "Point":
        if self == self.projections[ref_plane_key]:
            ...
            if ref_plane_key == "xy":
                ...
                new_point = self.plane[ref_plane_key].add_point(
...
def random_point(key: str = "xyz") -> Point:
    """Get a random point.

    To follow architecture standards, z is always equal or higher than 0.
    """
    if key == "xy":
        return Point(x=random.randint(-20, 50), y=random.randint(-20, 50))  # noqa: S311
    if key == "yz":
        return Point(y=random.randint(-20, 50), z=random.randint(0, 60))  # noqa: S311
    if key == "zx":
        return Point(x=random.randint(-20, 50), z=random.randint(0, 60))  # noqa: S311
    return Point(
        x=random.randint(-20, 50),  # noqa: S311
        y=random.randint(-20, 50),  # noqa: S311
        z=random.randint(0, 60),  # noqa: S311
    )
