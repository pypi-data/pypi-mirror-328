...
from axonometry import Axonometry, Line, Point, config_manager, random_line, random_point
...
class TestAxonometryLines(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.line_xyz_1 = self.axo.add_line(random_line())
        self.line_xyz_2 = self.axo.add_line(random_line())
        self.axo.add_line(Line(self.line_xyz_1.end, self.line_xyz_2.start))
        # ref plane line projection propagation
        self.p_xy_1 = self.axo["xy"].add_point(random_point("xy"))
        self.p_xy_2 = self.axo["xy"].add_point(random_point("xy"))
        self.p_xy_1.project(distance=random.randint(50,100))
        self.p_xy_2.project(distance=random.randint(50,100))
        self.axo["xy"].add_line(Line(self.p_xy_1, self.p_xy_2))
