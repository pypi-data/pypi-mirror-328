...
TdxMarketToExchange = {0: "SZ", 1: "SH", 3: "BJ", 4: "HK"}


class GetSecurityListV2(BaseParser):
    """fix bug"""
        ...
        for i in range(num):
            ...
            one = OrderedDict(
                [
                    (
                        "symbol",
                        f"{code}.{TdxMarketToExchange[self.market]}",
                    ),
                    ("volunit", volunit),
...
class GetSecurityQuotesCmdV2(BaseParser):
    """fix bug"""
        ...
        for _ in range(num_stock):
            ...
            one_stock = OrderedDict(
                [
                    (
                        "symbol",
                        f"{code.decode('utf-8','ignore')}.{TdxMarketToExchange[market]}",
                    ),
                    ("lasttrade", self._cal_price(price, 0)),
                    ("yclose", self._cal_price(price, last_close_diff)),
                    ...
                    ("s_vol", s_vol),
                    ("b_vol", b_vol),
                    ("bid1_p", self._cal_price(price, bid1)),
                    ("ask1_p", self._cal_price(price, ask1)),
                    ("bid1_v", bid_vol1),
                    ("ask1_v", ask_vol1),
                    ("bid2_p", self._cal_price(price, bid2)),
                    ("ask2_p", self._cal_price(price, ask2)),
                    ("bid2_v", bid_vol2),
                    ("ask2_v", ask_vol2),
                    ("bid3_p", self._cal_price(price, bid3)),
                    ("ask3_p", self._cal_price(price, ask3)),
                    ("bid3_v", bid_vol3),
                    ("ask3_v", ask_vol3),
                    ("bid4_p", self._cal_price(price, bid4)),
                    ("ask4_p", self._cal_price(price, ask4)),
                    ("bid4_v", bid_vol4),
                    ("ask4_v", ask_vol4),
                    ("bid5_p", self._cal_price(price, bid5)),
                    ("ask5_p", self._cal_price(price, ask5)),
                    ("bid5_v", bid_vol5),
                    ("ask5_v", ask_vol5),
    ...
    def _cal_price(self, base_p, diff):
        return float(base_p + diff)


class MockTdxHq_API(TdxHq_API):
    """修订版通达信行情接口"""

    @update_last_ack_time
    def get_security_list(self, market: int, start: int) -> List[OrderedDict[str, Any]]:
        cmd = GetSecurityListV2(self.client, lock=self.lock)
        cmd.setParams(market, start)
        return cmd.call_api()

    @update_last_ack_time
    def get_security_quotes(
        self, all_stock: List[Tuple[int, str]], code: Optional[str] = None
    ) -> List[OrderedDict[str, Any]]:
        cmd = GetSecurityQuotesCmdV2(self.client, lock=self.lock)
