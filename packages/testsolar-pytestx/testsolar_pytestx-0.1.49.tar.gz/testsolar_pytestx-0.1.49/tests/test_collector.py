...
class CollectorTest(unittest.TestCase):
    ...
    def test_collect_testcases_when_selector_is_valid(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            entry = EntryParam(
                ...
                FileReportPath=os.path.join(tmpdir, "load.json"),
            )

            collect_testcases(entry, None)

            re = read_file_load_result(file_report_path=Path(entry.FileReportPath))
    ...
    def test_collect_testcases_with_case_drive_separator(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            collect_testcases(entry, None, extra_load_function=loader_extend)
