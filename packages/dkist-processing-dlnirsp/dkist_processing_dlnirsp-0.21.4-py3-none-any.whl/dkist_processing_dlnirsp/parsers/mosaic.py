...
class locking_cached_property(cached_property):
    ...
    def __get__(self, instance, owner=None):
        ...
        if instance is None:
            raise ValueError(f"{self.__class__.__name__} does work outside a class instance")
...
class MosaicPiece(BaseModel):
    ...
    def round_float(cls, value: float) -> float:
        """Round CRPIX values so numeric jitter does not give the impression of more steps than actually exist."""
...
class MosaicPieceBase(Stem):
    ...
    def __init__(self, stem_name: str, crpix_correction_method: CRPIX_CORRECTION_TYPE_HINT):
...
class NumMosaicRepeatsBud(MosaicPieceBase):
    ...
    def __init__(self, crpix_correction_method: CRPIX_CORRECTION_TYPE_HINT):
        super().__init__(
...
class XYMosaicTilesBase(MosaicPieceBase):
    ...
    def __init__(
        ...
        super().__init__(stem_name=stem_name, crpix_correction_method=crpix_correction_method)
