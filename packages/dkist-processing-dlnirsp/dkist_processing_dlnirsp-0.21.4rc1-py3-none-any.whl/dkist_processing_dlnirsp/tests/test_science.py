                ...
                for Y_tile in range(1, num_Y_tiles + 1):
                    for stokes in task.constants.stokes_params:
                        ...
                        if not is_polarimetric and stokes in ["Q", "U", "V"]:
                            ...
                            # Still need to round by a small amount because some randomness is added to the CRPIX
                            # values in `ModulatedObserveHeaders`.
                            assert round(header["CRPIX1"], 1) == round(expected_CRPIX1, 1)
                            assert round(header["CRPIX2"], 1) == round(expected_CRPIX2, 1)
