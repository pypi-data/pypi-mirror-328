...
def plot_leaf(T, pos, u_node, numbers, ax, delta_txt):
    x_u,y_u= pos[u_node]
    txt= f'{get_txt(u_node, numbers, "S")}'
    #txt= f"{numbers[u_node]['S']} ({numbers[u_node]['Sr']})"
    species= T.nodes[u_node]['label']
    ax.text(x_u, y_u, txt, c='red')
    ax.text(x_u, y_u-delta_txt, species, c='k', rotation=-30, rotation_mode= 'anchor',)
    return None

def get_txt(node, numbers, symbol, prefix=''):
    txt= ''
    if numbers[node][f'{symbol}']:
        txt+= f"{numbers[node][f'{symbol}']} "
    if numbers[node][f'{symbol}r']:
        txt+= f"({numbers[node][f'{symbol}r']})"
    if len(txt)>0:
        txt= prefix+txt
    return txt
...
def plot_inner(T, pos, u_node, numbers, ax, delta_txt):
    ...
    for v_node in ch:
        ...
        # Numbers
        txt= f"{get_txt(v_node, numbers, 'D', '+ ')}"
        ax.text(x_v, y_mid, txt, c='blue')
        txt= f"{get_txt(v_node, numbers, 'X', '- ')}"
        ax.text(x_v, y_mid-delta_txt, txt, c='green')
    return None
...
def og_class_statistics(df, species_list): # previous name of this function: def clade_info()
    ...
    F= lambda row: list( row[[x>0 for x in row]].index )
    ...
    class_statistics= DataFrame({
        ...
        'Genes per OG' : gb_n_genes_per_orthogroup.mean(),
        'Genes per species' : av_genes_per_speciesper_orthogroup.groupby(species_list).mean(),
    })
