                ...
                if m.has_bit_granularity():
                    ...
                    if not isinstance(size, int):
                        ...
                        for s in free_symbols:
                            ss = str(s)
                            new_s = symbols(f'{state.parent}{name}.{ss}')
                            expr_ctx[ss] = new_s
...
"""
        ...
        if self.is_primitive() or self.is_array():
            offset_str = get_offset(self,state.parent)
            if self.has_bit_granularity():
                total_bit_size = self.bit_size()
                full_bytes, bits = divmod(total_bit_size, 8)
                out += f'<| {offset_str} <| {state.parent} <| {full_bytes}.{bits} <|'
            else:
                size = self.size()
                size_str = str(size)
                if not isinstance(size, int):
                    grandparent = '.'.join(state.parent.split('.')[0:-1])
                    if len(grandparent) > 0:
                        free_symbols = size.free_symbols
                        for s in free_symbols:
                            ss = str(s)
                            size_str = size_str.replace(ss, f'{grandparent}.{ss}')
                out += f'<| {offset_str} <| {state.parent} <| {size_str} <|'
            if values:
            ...
            for name in self._members:
                ...
                if deep:
                    ...
                    full_name = f'{state.parent}{name}'
                    offset_str = get_offset(m,full_name)
                    out += f'<| {offset_str} <| {full_name} <| {m.size()} <| '
                    if values:
                        if m.is_initialized():
