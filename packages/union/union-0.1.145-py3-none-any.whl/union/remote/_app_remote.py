...
class AppRemote:
    ...
    def deploy(self, app: App, project: Optional[str] = None, domain: Optional[str] = None):
        try:
            self.get(name=app.name, project=project, domain=domain)
        except grpc.RpcError as e:
            if e.code() == grpc.StatusCode.NOT_FOUND:
                self.create(app, project=project, domain=domain)
                return
            raise

        try:
            self.update(app, project=project, domain=domain)
        except grpc.RpcError as e:
    ...
    def stop(self, name: str, project: Optional[str] = None, domain: Optional[str] = None):
        app_idl = self.get(name, project=project, domain=domain)
    ...
    def upload_files(self, app: App) -> Optional[str]:
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            _, upload_native_url = self._union_remote_ref().upload_file(Path(tar_path))
    ...
    def materialize_values(self, app: App, project: Optional[str] = None, domain: Optional[str] = None) -> dict:
        ...
        for user_input in app.inputs:
            if isinstance(user_input.value, ArtifactQuery):
                ...
                query.domain = domain or self.default_project

                result = self._union_remote_ref().get_artifact(query=query.to_flyte_idl())
    ...
    def generate_console_url(self, app_idl: AppIDL) -> str:
        ...
        http_domain = self._union_remote_ref().generate_console_http_domain()
