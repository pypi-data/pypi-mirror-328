...
class HorizontalSlideBar(BasicSlider):
    def __init__(self, size, pos, screen, movable_width, drag_index=0, movable_color=((177, 177, 177), (127, 127, 127)), background_color=(255, 255, 255)):
        super().__init__(size, pos, screen)
        ...
        if self.in_area(mouse_pos) or self.sliding:
            if self._lock:
                ...
                if not self._pre_clicked and mouse_press[self._drag_index]:
                    ...
                    self.sliding = False
                    self._lock = False
                    if self.slide_pos - self.movable_width // 2 - self._size[1] // 2 <= mouse_pos[0] - self._pos[0] <= self.slide_pos + self.movable_width // 2 + self._size[1]:
