...
class HorizontalSlideBar(BasicSlider):
    def __init__(self, size, pos, screen, movable_width, drag_index=0, movable_color=(127, 127, 127), background_color=(255, 255, 255)):
        super().__init__(size, pos, screen)
    ...
    def operate(self, mouse_pos, mouse_press):
        ...
        self._pre_clicked = mouse_press[self._drag_index]
        self.surface.fill((0, 0, 0, 0))
        pygame.draw.rect(self.surface, self._background_color, (0, 0, *self._size), border_radius=self._size[1])
        if self.background is not None:
            self.surface.blit(self.background, (0, 0))
        self.percent = self.slide_pos - (self.slide_range[1] - self.slide_range[0])
        self._screen.blit(self.surface, self._pos)
        pygame.draw.rect(self._screen, self._movable_color,
                         (self.slide_pos + self._pos[0] + self._size[1] // 2 + 1 - self.movable_width // 2, self._pos[1] + 1, self.movable_width,
                          self._size[1] - 2))
...
if __name__ == '__main__':
    ...
    hsb = HorizontalSlideBar([200, 10], [100, 20], sc, 30)
    running = 1
