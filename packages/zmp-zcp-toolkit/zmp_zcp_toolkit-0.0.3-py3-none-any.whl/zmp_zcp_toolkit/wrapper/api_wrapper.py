...
class ZcpAnalysisAPIWrapper(BaseAPIWrapper):
    ...
    async def alert_action(self, query: AlertAction) -> dict:
        ...
        response = await self.async_client.patch(api_path, params=params)
        return response.json()
    ...
    def run(self, mode: str, query: str) -> str:
        if mode == "get_alerts":
            ...
            return json.dumps(result)
        elif mode == "alert_action":
            result = asyncio.run(self.alert_action(query))
            return json.dumps(result)
        else:
