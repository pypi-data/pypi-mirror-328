...
import traceback
from hvseispy.specsignal import window, taper, spectrum, konnoohmachi_smoothing_opt
from hvseispy.IO import read_cires, read_file, read_mseed, read_sac
from hvseispy.plot_tools import *
from hvseispy.misc import *
...
# Add FIR filtering function
# Add rotated H/V
# Horizontal particle motion
# Add support for GCF files


def hv_ratio(
        amp_north:np.ndarray,
        amp_vertical:np.ndarray,
        amp_east:np.ndarray,
        freq:np.ndarray,
        fmin:float=0.1,
        fmax:float=10.0
    ...
    Args
    ----
        amp_north (np.ndarray): North-component amplitude spectrum
        amp_vertical (np.ndarray): Vertical-component amplitude spectrum
        amp_east (np.ndarray): East-component amplitude spectrum
        freq (np.ndarray): Frequency array
        fmin (float): Minimum frequency for the H/V spectrum
        fmax (float): Maximum frequency for the H/V spectrum
...
def _process_hvsr(
    ...
    Returns:
    --------
        tuple: Tuple with shape (3,N) in the form (HV_mean, HV, freq)
    """    
    """
    Workflow (for development purposes):
        1. Read file (specify)
    ...
    data_windowed = window(data, win_len, fs, overlap=overlap)
    # print('Data windowed')
    ...
    # ------ #
    # TODO (fixed): This process is already performed in the window function
    # taper returns a 3-element tuple (N-tapered, V-tapered, E-tapered)
    data_tapered = taper(north, vertical, east, taper_win, amount=taper_amount)
...
def hvsr(
        ...
        win_len: float,
        file_type:str = None,
        taper:tuple|list = ('cosine',0.05),
        smooth_bandwidth:float = 40.0,
        overlap:float = 0,
        fftmin:float = 0.1,
        fftmax:float = 50.0,
        hvmin:float = 0.1,
    ...
    if file_type is not None:
        ...
        hvmean, hv, freq = _process_hvsr(data=data, dt=dt, win_len=win_len, taper_win=taper[0], taper_amount=taper[1], smooth_coeff=smooth_bandwidth, overlap=overlap, fftmin=fftmin, fftmax=fftmax, hvmin=hvmin, hvmax=hvmax)
    else:
        # if acc_data is a variable (override reading the file)
        hvmean, hv, freq = _process_hvsr(data=acc_data, dt=dt, win_len=win_len, taper_win=taper[0], taper_amount=taper[1], smooth_coeff=smooth_bandwidth, overlap=overlap, fftmin=fftmin, fftmax=fftmax, hvmin=hvmin, hvmax=hvmax)
