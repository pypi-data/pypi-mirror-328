...
# Initialize app
kitchen = KitchenAIApp(namespace="storage-example")
...
@kitchen.storage.handler("custom-storage")
async def handle_storage(data: StorageRequest) -> StorageResponse:
    """Example storage handler that implements basic file operations"""
    logger.info(f"Handling storage request: {data.action}")
    ...
    if data.action == "upload":
        ...
        # Return the list directly instead of wrapping in StorageResponse
        return files
