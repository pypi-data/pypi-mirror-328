...
def build_models(extra_args=None, species=None):
    ...
    for org_id in species:
        ...
        taxonomical_level_par = taxonomical_level[org_id]
        taxonomy_par = taxonomy[org_id]
        media = set()
        ...
        call(f'carve {fasta_file} -u {gram_status[org_id]} -o {model_file} {gapfill} --fbc2 {extra_args} -tl {taxonomical_level_par} -t {taxonomy_par}', shell=True)
...
def run_essentiality_benchmark(models, essential, non_essential, media_db, species=None):
    ...
    for org_id in species:
        ...
        # Calculate F1-Score
        f1_score = 2 * (precision * sensitivity) / (precision + sensitivity+1)
        #Calculate MCC score
        MCC = (TP * TN - FP * FN) / math.sqrt((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN)+1)
        print(f"precision: {precision:.2f} specificity: {specificity:.2f} sensitivity: {sensitivity:.2f} accuracy: {accuracy:.2f} f1_score: {f1_score:.2f} MCC: {MCC:.2f}")
