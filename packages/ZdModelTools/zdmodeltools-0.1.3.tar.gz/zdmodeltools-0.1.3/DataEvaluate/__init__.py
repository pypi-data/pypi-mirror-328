...
def save_eval_result_v1(output_path, df, label_cols, score_cols, month_col, projec_col):
    eval_v1 = evaluator(df, label_cols, score_cols, month_col, projec_col)
    eval_result_dict = get_all_eval_result_v1(eval_v1)
...
def get_all_eval_result_v1(evaluator):
    eval_result_dict = {}
    eval_result_names = [
    'auc_ks_byMonth',
    'binned_byMonth',
...
def get_one_eval_result(evaluator, name):
    if name == 'auc_ks_byMonth':
        ...
        evaluate_result = evaluator.get_psi(base_month_list= ['2024-10','2024-11','2024-09'])
