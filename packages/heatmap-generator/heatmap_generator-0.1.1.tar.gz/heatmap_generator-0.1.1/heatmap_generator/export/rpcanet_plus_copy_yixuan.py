...
__all__ = ['RPCANet0', 'RPCANet1', 'RPCANet2', 'RPCANet3', 'RPCANet4', 'RPCANet5',
           'RPCANet6','RPCANet7','RPCANet8',
           'RPCANet9', 'RPCANet10','RPCANet11', 'RPCANet_Experi', 'RPCANet_wo_Merge', 'RPCANet', 'RNet',
           'RPCANet12',
           'RPCANet13','RPCANet14','RPCANet_wo','RPCANetMA','TRPCANet','RPCANetW','RPCANetW1','RPCANetW2',
           'RPCANetW3', 'RPCANetW4', 'RPCANetW5', 'RPCANetMA1',
            'RPCANetMA2',
        'RPCANetMA3','RPCANetMA4','RPCANetMA5','RPCANetMA6','RPCANetMA7','RPCANetMA8','RPCANetMA9','RPCANetMA10',
...
# #====================================================================================================
class RPCANet9_CBAM(nn.Module):
    def __init__(self, stage_num=6, slayers=6, llayers=3, mlayers=3, channel=32, mode='train'):
        super(RPCANet9_CBAM, self).__init__()
        self.stage_num = stage_num
        self.decos = nn.ModuleList()
        self.mode = mode
        for _ in range(stage_num):
            self.decos.append(DecompositionModule9_CBAM(slayers=slayers, llayers=llayers,
                                                  mlayers=mlayers, channel=channel))
        # 迭代循环初始化参数
        for m in self.modules():
            # 也可以判断是否为conv2d，使用相应的初始化方式
...
class DecompositionModule9_CBAM(nn.Module):
    def __init__(self, slayers=6, llayers=3, mlayers=3, channel=32):
        super(DecompositionModule9_CBAM, self).__init__()
        self.lowrank = LowrankModule9_CBAM(channel=channel, layers=llayers)
        self.sparse = SparseModule9_CBAM(channel=channel, layers=slayers)
        self.merge = MergeModule9_CBAM(channel=channel, layers=mlayers)
...
class LowrankModule9_CBAM(nn.Module):
    def __init__(self, channel=32, layers=3):
        super(LowrankModule9_CBAM, self).__init__()
...
class SparseModule9_CBAM(nn.Module):
    def __init__(self, channel=32, layers=6) -> object:
        super(SparseModule9_CBAM, self).__init__()
        convs = [nn.Conv2d(1, channel, kernel_size=3, padding=1, stride=1),
                 nn.ReLU(True)]
        for i in range(layers):
            convs.append(nn.Conv2d(channel, channel, kernel_size=3, padding=1, stride=1))
...
class MergeModule9_CBAM(nn.Module):
    def __init__(self, channel=32, layers=3):
        super(MergeModule9_CBAM, self).__init__()
        convs = [nn.Conv2d(1, channel, kernel_size=3, padding=1, stride=1),
                 nn.BatchNorm2d(channel),
                 nn.ReLU(True)]
        for i in range(layers):
