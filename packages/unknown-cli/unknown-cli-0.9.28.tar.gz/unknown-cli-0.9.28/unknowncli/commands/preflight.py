...
def main(
    ...
    if get_opened_file_list(change_obj.change):
        ...
        if use_server_submit and commit_on_success:
            secho(f"Reverting files from CL {change_obj.change} to allow for submit on the server..")
            revert_file_in_cl("//...", change_obj.change)
    else:
    ...
    use_content_build = shelf_is_content_only(changelist)
