...
def resolve_rag_dir(rag_dir: str) -> str:
...
class AgentManager:
    ...
    @staticmethod
    def _get_memory_config(memory_types: list) -> dict:
        """Convert memory types list to memory configuration dictionary"""
        config = {
            'memory_types': memory_types,
            'short_term_limit': 10,
            'long_term_limit': 100,
            'semantic_backend': 'default'
        }

        # Adjust configuration based on memory types
        if 'Short-term' not in memory_types:
            config['short_term_limit'] = 0
        if 'Long-term' not in memory_types:
            config['long_term_limit'] = 0
        if 'Semantic' not in memory_types:
            config['semantic_backend'] = None
