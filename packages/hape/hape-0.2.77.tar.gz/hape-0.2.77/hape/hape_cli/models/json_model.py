...
class Json(FormatModel):
    ...
    def get(self):
        self.logger.debug(f"Getting JSON {{'self.model_schema_json': {self.model_schema_json}'}}")
        self.generate()
            
    def generate(self):
        self.logger.debug(f"Generating JSON {{'self.model_schema_json': {self.model_schema_json}'}}")
