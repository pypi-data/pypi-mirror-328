...
class Json(FormatModel):
    ...
    def get(self):
        self.logger.debug(f"Getting JSON {{'self.model_schema': {self.model_schema}'}}")
        self.generate()
            
    def generate(self):
        self.logger.debug(f"Generating JSON {{'self.model_schema': {self.model_schema}'}}")
        if self.model_schema:
            print(Crud._model_schema)
        else:
