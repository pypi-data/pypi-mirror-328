...
def _run(dict_result,
    ...
    label, mask_gradient, img_cellbound, dapi, rna_img =  (dict_result["mask_flow"], dict_result["mask_gradient"], dict_result["img_cellbound"],
    ...
    if path_save_model is not None and  save_ouptut:# (len(training_loader) % max(i_train, 1) <= print_loss_every):
        ...
        for index in range(imgs.shape[0]):
            ...
            tifffile.imwrite(path_save_plot_epoch / f"mask_gradient{patch_index[index]}.tif", mask_gradient[index].numpy())
            tifffile.imwrite(path_save_plot_epoch / f"img_cellbounds{patch_index[index]}.tif", img_cellbound[index].numpy())
