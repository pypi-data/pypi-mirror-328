...
def get_mean_density_per_polygon(gdf_polygon : gpd.GeoDataFrame,
    ...
    mask_instance = rasterize(((polygons[i], i+1) for i in range(len(polygons))), out_shape=shape, transform=transform, fill=0, all_touched=True, dtype=np.uint16)
    ...
    #print(f"Time to compute the density per instance: {time()-t:.2f}")
    return list_density
...
if __name__ == '__main__':
    ...
    ### test different background mask ####
