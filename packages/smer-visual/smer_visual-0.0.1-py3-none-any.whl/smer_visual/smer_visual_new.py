...
class ImageClassifier:
    ...
    def predict_probabilities_from_text(self, texts: List[str]) -> np.ndarray:
        """
    ...
    def process_instance_lime(self, i, text, label):
        explainer = LimeTextExplainer(class_names=[], random_state=42)
    ...
    def process_instance_smer(self, text, label):
        """
        Calculate word importance by removing each word individually (SMER) and measuring
        the change in predicted probability.

        Parameters:
        - i (int): Index of the instance.
        - text (str): The input text.
        - true_label (int/str): The true label of the instance.

        Returns:
        - pd.DataFrame: DataFrame with words, their calculated importance, and other metrics.
        """
        # Original prediction for the full text
        original_pred = self.predict_probabilities_from_text([text])[0]
        original_class = np.argmax(original_pred)  # Determine predicted class

        # List to store each word's importance row
        importance_rows = []
    ...
    @staticmethod
    def _remove_words(text, words_to_remove):
        return ' '.join([word for word in text.split() if word not in words_to_remove])

    def compute_AOPC_custom(self, df, top_words, max_K=6):
        """
        Compute the AOPC by removing up to K top globally important words from each text.

        Parameters:
        - df (pd.DataFrame): DataFrame containing texts.
        - top_words (list): List of top globally important words.
        - max_K (int): Maximum number of words to remove.

        Returns:
        - list: AOPC values for each value of K.
        """
        avg_drops = []
        ...
        for K in range(0, max_K + 1):
            ...
            for idx, row in df.iterrows():
                ...
                # Determine the number of words to remove
                ...
                altered_text = self._remove_words(text, words_to_remove)
    ...
    def plot_aopc(self):
        ...
        The calculations for AOPC align with a custom method using globally important words.
        """
        max_K = 6  # Maximum number of top words to remove
        avg_drops_SMER = []
        avg_drops_LIME = []

        # Define top globally important words for both SMER and LIME
        top_words_SMER = self.get_top_words(method="SMER")
        top_words_LIME = self.get_top_words(method="LIME")
        ...
        for K in range(max_K + 1):
            ...
            for idx, row in self.df_AOPC.iterrows():
                ...
                processed_text = row["Description_processed"]

                # Original prediction
                original_pred = self.predict_probabilities([processed_text])[0]
                original_class = np.argmax(original_pred)
                original_prob = original_pred[original_class]

                # SMER calculation
                top_words_in_text_SMER = [word for word in top_words_SMER if word in processed_text.split()]
                words_to_remove_SMER = top_words_in_text_SMER[:min(K, len(top_words_in_text_SMER))]
                altered_text_SMER = self.remove_words(processed_text, words_to_remove_SMER)

                if processed_text == altered_text_SMER:
                    drop_SMER = 0
                else:
                    altered_pred_SMER = self.predict_probabilities([altered_text_SMER])[0]
                    altered_prob_SMER = altered_pred_SMER[original_class]
                    drop_SMER = original_prob - altered_prob_SMER

                drops_SMER.append(drop_SMER)

                # LIME calculation
                top_words_in_text_LIME = [word for word in top_words_LIME if word in processed_text.split()]
                words_to_remove_LIME = top_words_in_text_LIME[:min(K, len(top_words_in_text_LIME))]
                altered_text_LIME = self.remove_words(processed_text, words_to_remove_LIME)

                if processed_text == altered_text_LIME:
                    drop_LIME = 0
                else:
                    altered_pred_LIME = self.predict_probabilities([altered_text_LIME])[0]
                    altered_prob_LIME = altered_pred_LIME[original_class]
                    drop_LIME = original_prob - altered_prob_LIME

                drops_LIME.append(drop_LIME)

            # Average drops for this K
            avg_drops_SMER.append(np.mean(drops_SMER))
            avg_drops_LIME.append(np.mean(drops_LIME))

        # Plot the results (optional, if needed)
        plt.plot(range(max_K + 1), avg_drops_SMER, label="SMER", marker="o")
        plt.plot(range(max_K + 1), avg_drops_LIME, label="LIME", marker="x")
        plt.xlabel("Number of Words Removed (K)")
        plt.ylabel("Average Output Probability Change (AOPC)")
        plt.legend()
        plt.title("AOPC as a Function of Top Word Removal")
        plt.show()
