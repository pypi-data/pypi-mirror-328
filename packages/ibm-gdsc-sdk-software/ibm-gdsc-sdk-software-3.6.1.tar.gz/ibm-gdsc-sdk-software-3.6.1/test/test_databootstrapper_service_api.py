# coding: utf-8

"""
    IBM Guardium Data Security Center

    Use Guardium Data Security Center SDK (application programming interfaces) to interact with other applications, such as Guardium Data Protection, universal connectors, and Guardium Data Security Center reports.

    The version of the OpenAPI document: 3
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from ibm_gdsc_sdk_software.api.databootstrapper_service_api import DatabootstrapperServiceApi


class TestDatabootstrapperServiceApi(unittest.TestCase):
    """DatabootstrapperServiceApi unit test stubs"""

    def setUp(self) -> None:
        self.api = DatabootstrapperServiceApi()

    def tearDown(self) -> None:
        pass

    def test_databootstrapper_service_load_data(self) -> None:
        """Test case for databootstrapper_service_load_data

        Summary: Load data Description: Load data for a tenant.
        """
        pass


if __name__ == '__main__':
    unittest.main()
