...
from ..models import Auth, DetailInfo, ExtendField, Platform
...
class UserService:
    ...
    def user_add(params):
        ...
        try:
            ...
            base_info = {
                'username': account if account_type == 'username' else '',
                'fullname': fullname,
                "nickname": nickname,
    ...
    def user_basic_message(user_id=None, nickname=None, need_detail_info=True):
        ...
        if detail_info:
            ...
            [detail_alias_info] = filter_result_field(
                [detail_info],
                filter_filed_list=[i.name for i in DetailInfo._meta.fields if not i.name[0:6] == "field_"] + list(
                    extend_field_map.keys()),
    ...
    def user_edit(params):
        ...
        try:
            ...
            if account_type == 'phone':
                ...
                user_list = BaseInfo.objects.filter(Q(username=account), ~Q(id=user_id))
            if user_list.count() and account_type == 'phone':
                return None, "手机已被注册: " + account
            elif user_list.count() and account_type == 'email':
                return None, "邮箱已被注册: " + account
    ...
    @staticmethod
    def user_edit_v2(params=None, user_id=None):
        """
        用户修改服务。
        :param params: 修改参数集合
        :param user_id: 用户ID
