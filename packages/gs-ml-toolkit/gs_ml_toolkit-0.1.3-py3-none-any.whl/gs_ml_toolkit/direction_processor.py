...
class DirectionProcessor:
    ...
    @staticmethod
    def clean_stop_code(stop_id: str) -> str:
        numbers = re.sub(r'[^0-9]', '', str(stop_id))
        return str(int(numbers)) if numbers else ''
    ...
    def create_direction_sequence(
        ...
        df[stop_col_id] = df[stop_col_id].apply(self.clean_stop_code)
