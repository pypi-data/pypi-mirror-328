...
These should be imported from the top-level `NValidator` module.

Methods decorated with
`pre_load <NValidator.decorators.pre_load>`, `post_load <NValidator.decorators.post_load>`,
`pre_dump <NValidator.decorators.pre_dump>`, `post_dump <NValidator.decorators.post_dump>`,
and `validates_schema <NValidator.decorators.validates_schema>` receive
``many`` as a keyword argument. In addition, `pre_load <NValidator.decorators.pre_load>`,
`post_load <NValidator.decorators.post_load>`,
and `validates_schema <NValidator.decorators.validates_schema>` receive
...
class NValidatorHook:
    __nvalidator_hook__: dict[str, list[tuple[bool, Any]]] | None = None
...
def validates_schema(
    ...
    argument passed to the `Schema`'s :func:`~NValidator.Schema.validate` call.
...
def pre_dump(
    ...
    By default it receives a single object at a time, transparently handling the ``many``
    argument passed to the `Schema`'s :func:`~NValidator.Schema.dump` call.
    If ``pass_many=True``, the raw data (which may be a collection) is passed.
...
def pre_load(
    ...
    By default it receives a single object at a time, transparently handling the ``many``
    argument passed to the `Schema`'s :func:`~NValidator.Schema.load` call.
    If ``pass_many=True``, the raw data (which may be a collection) is passed.
...
def set_hook(
    ...
    # Set a __nvalidator_hook__ attribute instead of wrapping in some class,
    # because I still want this to end up as a normal (unbound) method.
    function = cast(NValidatorHook, fn)
    try:
        hook_config = function.__nvalidator_hook__
    except AttributeError:
        function.__nvalidator_hook__ = hook_config = defaultdict(list)
    # Also save the kwargs for the tagged function on
    # __nvalidator_hook__, keyed by <tag>
