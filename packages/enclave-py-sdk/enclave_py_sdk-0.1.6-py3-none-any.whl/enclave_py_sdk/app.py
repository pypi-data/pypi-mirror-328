import requests
import json
from .interfaces import TransactionDetails, OrderData, UserOperationStruct, CreateAccountResponse, GasResponse, ComputeQuoteResponse, GetSmartBalanceResponse, GetBalanceResponse,SubmitTransactionResponse, BuildUserOpResponse
...
class Enclave:
    ...
    def search_token(self, query: str) -> BuildUserOpResponse:
        try:
            response = requests.get(
                f"{BASE_URL}/token/search?{query}",
                headers={
                    'Authorization': self.API_KEY,
                    'Content-Type': 'application/json'
                }
