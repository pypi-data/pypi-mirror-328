...
from titanq._model.constraints import Constraints
from titanq._model.manifest import Manifest
from titanq._model.objective import Objective
...
class SwInfinityQModel(Model):
    def __init__(self):
        """This is just wrapping the titanq.Model class.
        So that it will skip the auth checks.
        This way users should be able to create their own Model
        Without first passing in an auth token.
        We will then pass in the auth token in the service side.
        """
...
class StrangeworksInfinityQModelModel(StrangeworksModel):
    def __init__(self, model: SwInfinityQModel):
        self.model = model
        self.model_type = StrangeworksModelType.InfinityQModel

    def to_str(self) -> str:
        return base64.b64encode(dill.dumps(self.model)).decode()

    @staticmethod
    def from_str(model_str: str) -> Any:
        """
        Returns an SwInfinityQModel model parsed from the slug for the Strangeworks
        workspace file.

        Args:
            model_str (str): the file string.

        Returns:
            SwInfinityQModel.
        """
        return dill.loads(base64.b64decode(model_str.encode()))
...
class StrangeworksModelFactory:
    ...
    def from_model(model: Any) -> StrangeworksModel:
        ...
        elif isinstance(model, SwInfinityQModel):
            return StrangeworksInfinityQModelModel(model=model)
        else:
    ...
    def from_model_str(
        ...
        elif strangeworks_model_type == StrangeworksModelType.InfinityQModel:
            m = StrangeworksInfinityQModelModel.from_str(model_str)
        else:
            raise ValueError("Unsupported model type")
