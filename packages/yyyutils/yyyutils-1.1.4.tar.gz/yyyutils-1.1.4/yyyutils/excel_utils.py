...
import psutil
import pandas as pd
from mytools.yyyutils.os_utils import OSUtils
...
class ExcelUtils:
    ...
    def two_tables_is_same(excel1_path, excel2_path, table1_name='Sheet1', table2_name='Sheet1') -> bool:
        # 打开两个excel文件
        excel1 = openpyxl.load_workbook(excel1_path)
        excel2 = openpyxl.load_workbook(excel2_path)
        # 获取两个工作表
        table1 = excel1[table1_name]
        table2 = excel2[table2_name]
        # 使用pandas对两个工作表进行对比
        df1 = pd.DataFrame(table1.values)
        df2 = pd.DataFrame(table2.values)
        print(df1)
        print(df2)
