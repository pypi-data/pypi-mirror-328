...
class QteaTensor(_AbstractQteaBaseTensor):
    ...
    **Details**
    ...
    @classmethod
    def mpi_bcast(cls, tensor, comm, tn_mpi_types, tensor_backend, root=0):
        """
        ...
        comm.Bcast([elem, tn_mpi_types[np.dtype(dtype).str]], root=root)
    ...
    def mpi_recv(cls, from_, comm, tn_mpi_types, tensor_backend):
        ...
        **Arguments**
        ...
        obj.convert(dtype=dtype, device=tensor_backend.device)
    ...
    def get_attr(self, *args):
        ...
        for elem in args:
            if elem in ["linalg.eigh", "eigh"]:
                attributes.append(np.linalg.eigh)
                continue
