...
class XBase(iGeo, Particle):
    ...
    async def _wip_build_walk(self, data) -> List:
        ...
        geo_key_values, _geo_hash, geo_json = await self._get_geolocation(data)
    ...
    def _get_wip_completed_edge(self) -> EDGE:
        ...
        if self.delivery in ("sync", "asap") and all(self._wip_cursor):
            ...
            if self._milk and self.delivery in ("sync",):
                ...
                log.debug(
                    "Policy: [%s] needed buffers filled for [%s], start computing",
                    self.delivery,
                    min_requiered_key,
                )
                foo = 1
...
class XParticle(XBase):
    ...
    def floatwhen_cloner(self, wdata: Walk, lambda_name):
        new_value = pickle_cloner(wdata)
        for key, value in new_value.ordered:
            if isinstance(
                value,
                (
