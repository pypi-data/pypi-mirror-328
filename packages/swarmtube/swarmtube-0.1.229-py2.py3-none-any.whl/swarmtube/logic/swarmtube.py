...
class XBase(iGeo, Particle):
    ...
    async def _wip_build_walk(self, data) -> List:
        ...
        geo_key_values, _geo_hash, geo_json = await self._get_geolocation(
            data
        )
...
#                         return key0, key1
    ...
    async def _get_geolocation(self, data):
        ...
        return geo_key_values, _geo_hash, geo_json
...
class XParticle(XBase):
    ...
    def _lambda_peak(self, current, value, key):
        ...
        if not isinstance(value, FloatWhen):
            value = FloatWhen(value, self.context["datetime"])
    ...
    def _pre_peak(self, old_value, new_value, lambda_name):
        return self._pre_lambda(old_value, new_value, self._lambda_peak, lambda_name)
    ...
    def _post_peak(self, wdata: Walk):
        """
        create historogram and get the middle point of the max frequency
        """
        assert isinstance(wdata, Walk)
        ...
        for key, value in wdata.ordered:
            if key:
                skey = str(key[-1])
                if isinstance(value, (int, float)) and not (
                    re.match(self.lambda_exclude, skey)
                ):
        ...
        for col_name, data in samples.items():
            ...
            if (last_value := existing.get(col_name)) is not None:
                ...
                col = data
#
#             data = np.array([1.6, 0.4, 2.0, 1.5, 3.0, 2.5])
...
#             decrease_points = np.where((diff[:-1] > 0) & (diff[1:] < 0))[0] + 1
#
#             # Extract the values at these points
#             decrease_values = df['value'].iloc[decrease_points]
#
#             print("Points where the series starts to decrease:", decrease_points)
#             print("Values at these points:", decrease_values.tolist())



            # peaks, properties = find_peaks(col)
            peaks = list(find_peaks(col))
            if peaks:
                wdata[col_name] = peaks

            if col:
                self._tmp_value.setdefault("peak", {})[col_name] = col[-1:][0]
        ...
        for stage in ["post", "final"]:
            ...
            for keys, results in self._last_output.items():
                for lambda_name in set(results).intersection(lambdas):
                    ...
                    if (_value := func(wdata)) is None:
                        results.pop(lambda_name)
                    else:
        ...
        if lq := self._live_queries.get(uri):
            ...
            if not lq.callbacks:
                ...
                if result.status in ('OK', ):
                    ...
                    log.error("Error Deactivating subscription for [%s] -> [%s]", uri, result)
