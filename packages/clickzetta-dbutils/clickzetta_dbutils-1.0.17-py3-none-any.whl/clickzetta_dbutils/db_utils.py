...
class DatabaseConnectionManager:
    ...
    def load_connection_configs(cls) -> Dict[str, ConnectionConfig]:
        ...
        if not hasattr(DatabaseConnectionManager, '_connection_cache'):
            ...
            conn_info_str = None
            if log_id := os.environ.get('EXECUTE_LOG_ID'):
                pipe_path = f"/tmp/{log_id}"
                _log.info(f"Loading EXECUTE_LOG_ID is : {pipe_path}")
                try:
                    with open(pipe_path, 'r') as pipe:
                        conn_info_str = pipe.read()
                except FileNotFoundError:
                    raise DatabaseConnectionError(f"Path not found: {pipe_path}")
