...
from crow_client.utils.auth import get_google_credentials, get_password_credentials
...
class RestClient:
    ...
    def _run_auth(self) -> str:
        ...
        try:
            with self.get_client(with_auth=False) as client:
    ...
    def _fetch_my_orgs(self) -> list[str]:
        with self.get_client() as client:
            response = client.get(f"/v0.1/organizations?filter={True}")
            response.raise_for_status()
            orgs = response.json()
    ...
    def get_client(
        ...
        """Get HTTP client to the base service uri. This will include an auth header to any requests outside of /login.

        Args:
            content_type: Optional content type header. Set to None for multipart uploads.
            with_auth: Whether to include the Authorization header. False only works on the init call to /login.
        """
        headers = {}
        if with_auth:
    ...
    def create_job(self, data: dict):
        ...
        try:
            with self.get_client() as client:
                response = client.post("/v0.1/crows", json=data)
                response.raise_for_status()
    ...
    def get_build_status(self, build_id: UUID | None = None) -> dict[str, Any]:
        ...
        with self.get_client() as client:
            ...
            response = client.get(f"/v0.1/builds/{build_id}")
            response.raise_for_status()
            return response.json()
