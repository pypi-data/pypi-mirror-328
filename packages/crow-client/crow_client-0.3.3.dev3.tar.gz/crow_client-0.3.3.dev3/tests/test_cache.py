...
class TestRestClient:
    ...
    def test_client_cleanup(self, rest_client):
        ...
        if rest_client._client:
            rest_client._client.close = MagicMock()
        if rest_client._auth_client:
            rest_client._auth_client.close = MagicMock()

        rest_client.__del__()  # noqa: PLC2801

        if rest_client._client:
            rest_client._client.close.assert_called_once()
        if rest_client._auth_client:
            rest_client._auth_client.close.assert_called_once()
