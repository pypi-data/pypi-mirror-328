...
from crow_client.utils.auth import (
    AuthTokenCache,
...
def get_cached_token(service_uri: str, auth_type: AuthType) -> str | None:
    return _auth_cache.get_token(service_uri, auth_type)


def store_cached_token(
    service_uri: str, auth_type: AuthType, token: str, expiry: datetime
):
    _auth_cache.store_token(service_uri, auth_type, token, expiry)
...
class RestClient:
    ...
    def auth_client(self) -> Client:
        """Lazily initialized and cached HTTP client without authentication for auth endpoints."""
    ...
    def get_client(
        ...
        """Get HTTP client with specified content type.

        Args:
            content_type: Optional content type header. Set to None for multipart uploads.
            with_auth: Whether to include the Authorization header.
    ...
    def __del__(self):
        """Ensure clients are properly closed when the instance is destroyed."""
    ...
    def _run_auth(self) -> str:
        cached_token = get_cached_token(self.base_url, self.auth_type)
        if cached_token:
            return cached_token

        auth_payload: FirebaseCreds | ProviderResponse
        if self.auth_type == AuthType.PASSWORD:
            auth_payload = get_password_credentials()
        elif self.auth_type == AuthType.GOOGLE:
            auth_payload = get_google_credentials()
        else:
    ...
    def create_job(self, data: dict):
        ...
        try:
            response = self.client.post("/v0.1/crows", json=data)
            response.raise_for_status()
    ...
    def create_crow(self, config: CrowDeploymentConfig) -> dict[str, Any]:  # noqa: PLR0915
        ...
        try:
            ...
            response = self.get_client(content_type=None).post(
                "/v0.1/builds",
                data=data,
                files=[*files, markdown_template_file]
