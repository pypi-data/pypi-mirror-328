...
def get_year_and_month(invoice_date):
    try:
        ...
        return date_obj.year, date_obj.strftime("%B").upper()  # Month in uppercase
    except ValueError:
        return None, None

def sanitize_filename(filename):
    # Check if the filename already has spaces
    if ' ' in filename:
        return filename.strip()
    
    # For concatenated names like "MANSHAPURNACHAIN", insert spaces before uppercase letters
...
def process_main_folder(main_folder):
    ...
    print("The Processing of all the Subfolders adn files are complete")    
