...
# Initialize the AWS Bedrock client (ensure the region is correctly specified)
...
def extract_text_from_image(image_path):
    ...
    # Build a detailed, efficient, and strict prompt for AWS Bedrock
    prompt = (
        ...
        "3. 'Supplier Name' - the supplier's name, which must never be 'Safari Exim House Pvt Ltd'. Don't mix address with name\n\n"
    ...
    try:
        ...
        # Directly parse the JSON from the "content" field
...
def extract_invoice_details(extracted_text):
    """
    Attempt to parse the extracted text as JSON. If parsing fails,
...
def get_year_and_month(invoice_date):
    try:
        # Remove extra quotes or commas if present
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=os.getcwd(),
        help="Folder containing PDFs (default: current working directory)"
    )
