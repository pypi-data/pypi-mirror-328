...
def test_pcmd_accounts(pcmd, snapshot):
    account_name = f"test-pcmd-account.{random_id()}"
    new_account = pcmd(f"accounts new {account_name}", masked=False)
    account_id = safe_load(new_account)["id"]
    assert mask(new_account) == snapshot

    accounts = safe_load(pcmd("accounts list"))["accounts"]
    assert safe_load(mask(new_account)) in accounts
...
def test_pcmd_admin_config(pcmd, tmpdir, capsys):
    ...
    pcmd(
        ...
        + f' \'{{"enabled": false, "path": "{tmpdir}", "type": "local"}}\''
    )
...
def test_pcmd_collection(pcmd, scratch_collection):
    rv = pcmd("collection list", masked=False)
    assert scratch_collection[0] in rv
    assert scratch_collection[1] in rv
