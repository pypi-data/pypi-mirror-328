...
ENVIRONMENTS: Dict[str, str] = {
    "production": "https://brainbase-monorepo-api.onrender.com",
    "development": "https://brainbase-monorepo.onrender.com",
}
...
class BrainbaseLabs(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        This automatically infers the `api_key` argument from the `BRAINS_BASE_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("BRAINS_BASE_API_KEY")
        if api_key is None:
            raise BrainbaseLabsError(
                "The api_key client option must be set either by passing api_key to the client or by setting the BRAINS_BASE_API_KEY environment variable"
            )
        self.api_key = api_key

        self._environment = environment

        base_url_env = os.environ.get("BRAINBASE_LABS_BASE_URL")
        if is_given(base_url) and base_url is not None:
            ...
            if base_url_env and base_url is not None:
                raise ValueError(
                    "Ambiguous URL; The `BRAINBASE_LABS_BASE_URL` env var and the `environment` argument are given. If you want to use the environment, you must pass base_url=None",
                )
