...
async def main() -> None:
    voice_deployments = await client.workers.deployments.voice.list(
        "REPLACE_ME",
    )
...
try:
    client.workers.deployments.voice.list(
        "REPLACE_ME",
    )
...
client.with_options(max_retries=5).workers.deployments.voice.list(
    "REPLACE_ME",
)
```
...
client.with_options(timeout=5.0).workers.deployments.voice.list(
    "REPLACE_ME",
)
```
...
voice = response.parse()  # get the object that `workers.deployments.voice.list()` would have returned
print(voice)
```
