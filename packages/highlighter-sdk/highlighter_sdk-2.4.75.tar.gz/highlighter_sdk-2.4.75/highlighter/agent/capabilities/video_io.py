...
class ImageGather(Capability):
    ...
    def process_frame(self, stream, data_files) -> Tuple[aiko.StreamEvent, dict]:
    ...
    def _start_upload(self, stream, image_gather_list):
        ...
        path = template.format(stream.frame_id)
    ...
    def _assessment_finalise(self, cache):
        assessments.finalise(HLClient.get_client(), cache["assessment"])
