...
class DataFrameDiff:
            ...
            if col_name in left_df.columns and col_name in right_df.columns:
                select_exprs.append(
                    F.when(
                        ...
                        F.array(F.col(f"left_values.{col_name}"), F.col(f"right_values.{col_name}"))
                    ).alias(col_name)
                )
    ...
    def create_diff(cls, expected_df, generated_df, key_columns, diff_key):
        ...
        clean_joined_df = cls.clean_joined_df(joined_df, key_columns, value_columns, left_df=generated_df, right_df=expected_df)
