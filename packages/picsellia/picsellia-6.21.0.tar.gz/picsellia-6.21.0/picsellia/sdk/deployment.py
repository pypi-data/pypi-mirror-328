...
class Deployment(Dao, Taggable):
    ...
    def predict_bytes(
        ...
        Returns:
            A (dict) with information of the prediction
        """
        payload = self._prepare_payload_for_prediction(tags, source, monitor)
    ...
    def predict_shadow(
        ...
        if resp.status_code != 200:  # pragma: no cover
            raise PredictionError(f"Could not predict shadow because {resp.text}")
    ...
    def _fetch_target_datalake_connector(
        ...
        r = self.connexion.get(f"/sdk/deployment/{self.id}/datalake/connector").json()
        self._target_datalake_connector = TargetDatalakeConnectorSchema(**r)
        return self._target_datalake_connector

    @staticmethod
    def _prepare_payload_for_prediction(
        tags: Union[str, Tag, list[Union[Tag, str]], None],
        ...
        payload = {"tags": sent_tags, "monitor": monitor}
        if source:
    ...
    def _prepare_payload_monitor(
        ...
        shadow_prediction: Optional[PredictionFormat],
    ...
    def _request_monitor(self, payload: dict) -> dict:
        ...
        if resp.status_code != 201:  # pragma: no cover
            raise MonitorError(
                f"Our monitoring service could not handle your prediction: {resp.status_code}.\n Error : {resp.text}"
            )
    ...
    def monitor_shadow_from_oracle_prediction_id(
        self,
        oracle_prediction_id: Union[str, UUID],
        shadow_latency: float,
        shadow_prediction: PredictionFormat,
    ) -> None:
        ...
        if resp.status_code != 201:  # pragma: no cover
            raise MonitorError(
                f"Our monitoring service could not handle your shadow prediction: {resp.status_code}. Check {resp.text}"
            )
    ...
    def find_predicted_asset(
        ...
        oracle_prediction_id: Union[str, UUID, None] = None,
    ...
    def _do_list_predicted_assets(
        ...
        return (
            [PredictedAsset(self.connexion, self.id, item) for item in r["items"]],
            r["count"],
        )
    ...
    def list_workers(self) -> list[Worker]:
        ...
        """
        r = self.connexion.get(
            f"/api/deployment/{self.id}/deploymentpermissionners"
        ).json()
        return [
            Worker(self.connexion, item, WorkerType.DEPLOYMENT) for item in r["items"]
        ]
