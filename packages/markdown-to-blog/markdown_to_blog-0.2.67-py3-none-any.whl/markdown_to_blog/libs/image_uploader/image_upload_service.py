...
class DefaultImageUploader(ImageUploadService):
    ...
    def _update_progress(self, image_path: str, status: str, service: str = "", 
                        attempt: int = 0, progress: int = 0, url: str = "", 
                        error: str = ""):
        """진행 상황 업데이트"""
        if self.progress_viewer:
            self.progress_viewer.update(image_path, status, service, attempt, 
                                      progress, url, error)

    @log_operation
    def upload_markdown_images(self, markdown_file: str, image_files: List[str]) -> List[str]:
        """마크다운 파일의 이미지들을 업로드
        ...
        try:
            ...
            for attempt in range(self.max_retries):
                ...
                try:
                    ...
                    try:
                        result = self._handle_upload_process(process, service, attempt + 1, file_path)
                        if not self.use_tui:
                            logger.info(format_log_message(
                                service=service,
                                status="SUCCESS",
    ...
    def _handle_upload_process(self, process: subprocess.Popen[str], service: str, 
                             attempt: int, file_path: str) -> str:
        stdout_lines = []
        stderr_lines = []
