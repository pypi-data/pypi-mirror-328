...
class ProgressViewer:
    """진행 상황 뷰어 인터페이스"""
    def __init__(self, markdown_file: str, image_files: List[str], use_tui: bool = True):
    ...
    def update(self, image_path: str, status: str, service: str = "", 
               attempt: int = 0, progress: int = 0, url: str = "", 
               error: str = "") -> None:
        """진행 상황 업데이트"""
        ...
        # 완료된 파일 수 업데이트
        self.completed_files = sum(1 for s in self.image_statuses.values() 
                                 if s.status in ["완료", "실패"])
    ...
    def stop(self):
        """진행 상황 표시 종료"""
        print("\n업로드 완료!")
        print(f"성공: {sum(1 for s in self.image_statuses.values() if s.status == '완료')}개")
        print(f"실패: {sum(1 for s in self.image_statuses.values() if s.status == '실패')}개\n")
    ...
    def _print_progress_bar(self):
        ...
        print(f"\r진행률: [{bar}] {progress:.1f}% ({self.completed_files}/{self.total_files})", 
              end='', flush=True)
