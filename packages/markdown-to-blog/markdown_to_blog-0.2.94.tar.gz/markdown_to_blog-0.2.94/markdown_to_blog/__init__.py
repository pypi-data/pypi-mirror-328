...
def run_upload_images(input_, service: str = None, tui: bool = False):
    try:
        upload_markdown_images(input_, service=service, use_tui=tui)
...
@click.option("--description", "-d", help="검색 엔진용 설명 (메타 설명)")
@click.argument("filename")
def run_publish(
...
def run_publish_folder(blogid, interval, service, tui, folder_path):
    if blogid is None:
        blog_id = get_blogid()
    else:
        blog_id = blogid
    ...
    for file in file_list:
        ...
        try:
            ...
            upload_markdown_images(file_path, service=service, use_tui=tui)
