...
def _bq_timeline(parameters, qname, dbtoplist = None):
    ...
    if is_toplist:
        ...
        subquery = psycopg2.sql.SQL('toplist_with_rest AS (SELECT set::text, sum FROM toplist UNION (SELECT {} as set, total - SUM(sum)::bigint as sum FROM {}, toplist GROUP BY total HAVING total - SUM(sum)::bigint > 0) ORDER BY sum DESC)').format(
    ...
    subquery = psycopg2.sql.SQL('raw AS (SELECT GREATEST(%s, %s + %s * (width_bucket({}, (SELECT array_agg(bucket) FROM generate_series(%s, %s - INTERVAL \'1 microsecond\', %s) AS bucket)) - 1)) AS bucket{}, COUNT(*) AS count FROM {}').format(
        time_column,
        ...
        psycopg2.sql.Identifier('events') if not is_toplist else psycopg2.sql.SQL('(SELECT {}, {} AS set FROM events').format(
            time_column,
            _bq_get_unnested_set(parameters)
        )
    )
    ...
    if is_toplist:
        subquery = psycopg2.sql.SQL('raw_with_rest AS (SELECT bucket, set::text, count FROM raw UNION ALL SELECT bucket, {} AS set, raw_totals.count - raw_sums.count AS count FROM (SELECT GREATEST(%s, %s + %s * (width_bucket({}, (SELECT array_agg(bucket) FROM generate_series(%s, %s - INTERVAL \'1 microsecond\', %s) AS bucket)) - 1)) AS bucket, {} AS count FROM events').format(
            psycopg2.sql.Literal(ST_SKEY_REST),
            time_column,
...
def _bq_where(parameters, query, params, end_inclusive=True):
    ...
    if parameters:
        ...
        if parameters.get('description', None):
            chunks.append(psycopg2.sql.SQL("{} ILIKE '%%' || %s || '%%' ESCAPE '&'").format(psycopg2.sql.Identifier('description')))
...
class EventStorageCursor:
    ...
    def fetch_thresholds(self):
        ...
        self.cursor.execute(
            "SELECT id, report_label FROM thresholds"
        )
        return self.cursor.fetchall()
