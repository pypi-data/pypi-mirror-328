...
class PDNSRService:
    ...
    def get_url_lookup_hostname(self, hostname):
        ...
        """
        return "{}?query={}&type=domain&since=&until=".format(
            self.base_url,
            hostname
        )
    ...
    def lookup_ip_or_hostname(self, search, sortby = None, limit = None):
        ...
        """
        for (url, lookup_type) in [(self.get_api_url_lookup_ip(search), "ip"),
            ...
            try:
                ...
                raise PDNSRRuntimeException(
                    "Can't get data from PassiveDNS service for search: '{}'.".format(search)
                ) from exc
