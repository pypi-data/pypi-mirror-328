...
def t_network_record(val, source = None):
    ...
    if 'abuse_group' in val:
        ...
        emails = []
        for severity in 'emails_fallback', 'emails_info', 'emails_low', 'emails_medium', 'emails_high', 'emails_critical':
...
def t_detector_record(val, source):
    ...
    try:
        ...
        if 'credibility' in val:
            record['credibility'] = val['credibility']
        else:
            record['credibility'] = 1.0
        if 'registered' in val:
            record['registered'] = val['registered']
    except Exception as exc:
        raise ValueError('Unknown detector record {}'.format(pprint.pformat(val))) from exc
