        ...
        if hawat.const.FORM_ACTION_SUBMIT in flask.request.args:
            if form.validate():
                ...
                try:
                    ...
                    url = pdnsr_service.get_url_lookup_ip if lookup_type == "ip" else pdnsr_service.get_url_lookup_hostname
                    self.response_context.update(
                    ...
                    self.flash(
                        markupsafe.Markup(lazy_gettext("<b>This search was not successful.</b> PassiveDNS returned error: ") + str(exc)),
