...
class ListView(HTMLMixin, SQLAlchemyMixin, ItemListView):
    ...
    def build_query(query, model, form_args):
        ...
        # Adjust query based on user membership amd management selection.
        # This is done together, so we do not need more than 1 join with GroupModel.
        membership_present = 'membership' in form_args and form_args['membership']
        management_present = 'management' in form_args and form_args['management']
        if membership_present or management_present:
            query = query.join(model.memberships)
...
class ShowView(HTMLMixin, SQLAlchemyMixin, ItemShowView):
    ...
    def authorize_item_action(cls, **kwargs):
        ...
        return (hawat.acl.PERMISSION_POWER.can() or permission_me.can()
                or (permission_mngr.can() and len(needs) > 0))
...
class AddMembershipView(HTMLMixin, SQLAlchemyMixin,
...
class RejectMembershipView(HTMLMixin, SQLAlchemyMixin,
                           ItemObjectRelationView):  # pylint: disable=locally-disabled,too-many-ancestors
    """
...
class RemoveMembershipView(HTMLMixin, SQLAlchemyMixin,
                           ItemObjectRelationView):  # pylint: disable=locally-disabled,too-many-ancestors
    """
...
class AddManagementView(HTMLMixin, SQLAlchemyMixin,
                        ItemObjectRelationView):  # pylint: disable=locally-disabled,too-many-ancestors
    """
...
class RemoveManagementView(HTMLMixin, SQLAlchemyMixin,
                           ItemObjectRelationView):  # pylint: disable=locally-disabled,too-many-ancestors
    """
