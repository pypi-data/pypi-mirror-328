...
import mentat.const
from mentat.datatype.sqldb import FilterModel, GroupModel, SettingsReportingModel, UserModel
...
CHART_SECTIONS = [
    ...
    charts.ChartSection(
        ...
        charts.DataComplexity.SINGLE,
        lazy_gettext('Table name'),
        value_formats=charts.ValueFormats(
...
class ViewView(HTMLMixin, PsycopgMixin, SimpleView):
    ...
    def do_before_response(self, **kwargs):
        self.response_context.update(
            ...
            chart_sections=CHART_SECTIONS.copy()
        )
...
class DashboardView(HTMLMixin, SQLAlchemyMixin, SimpleView):  # pylint: disable=locally-disabled,abstract-method
    ...
    def do_before_response(self, **kwargs):
        ...
        self.response_context['settingsrep_emailsnotset'] = self.dbquery(SettingsReportingModel). \
            filter(SettingsReportingModel.emails_info == [] and SettingsReportingModel.emails_low == []
                   and SettingsReportingModel.emails_medium == [] and SettingsReportingModel.emails_high == []
                   and SettingsReportingModel.emails_critical == []
                   ). \
            order_by(SettingsReportingModel.createtime.desc()). \
            all()
