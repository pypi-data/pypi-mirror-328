...
class SimpleEventSearchForm(hawat.forms.BaseSearchForm):
    ...
    def is_multivalue(field_name):
        ...
        """
        if field_name in (
            'source_addrs', 'target_addrs', 'host_addrs', 'source_ports', 'target_ports', 'host_ports', 'source_types',
            'target_types', 'host_types', 'detectors', 'detector_types', 'categories', 'severities', 'classess',
            'protocols', 'groups', 'inspection_errs'
        ):
            return True
        return False
    ...
    def is_csag_context_insignificant(cls, field_name):
        return field_name in ('dt_from', 'dt_to', 'st_from', 'st_to') or super().is_csag_context_insignificant(field_name)
...
class EventDashboardForm(hawat.forms.HawatBaseForm):
    ...
    @classmethod
    def is_csag_context_insignificant(cls, field_name):
        return field_name in ('dt_from', 'dt_to') or super().is_csag_context_insignificant(field_name)
