...
from hawat.view import BaseSearchView, FileIdView, ItemDeleteView, ItemShowView, RenderableView
...
DASHBOARD_CHART_SECTIONS = [
    ...
    charts.ChartSection(
        ...
        lazy_gettext(
            'This view shows total numbers of report e-mails generated by the Mentat system per '
            'day.'
        ),
        charts.DataComplexity.SINGLE,
        lazy_gettext('E-mail')
...
def add_search_events_entry(action_menu):
    ...
    def get_legend_text(report):
        ...
        """
        legend_text = lazy_gettext('Search for all events related to report "%(label)s".',
...
class SearchView(HTMLMixin, SQLAlchemyMixin, BaseSearchView):  # pylint: disable=locally-disabled,too-many-ancestors
    ...
    def get_search_form(request_args):
        return EventReportSearchForm(request_args,
                                     meta={'csrf': False},
    ...
    def build_query(query, model, form_args):
        ...
        if 'categories' in form_args and form_args['categories']:
            query = query.filter(or_(*[model.statistics['categories'].has_key(category) for category in form_args['categories']]))
        # Adjust query based on event classes selection.
        if 'classes' in form_args and form_args['classes']:
        ...
        if 'detectors' in form_args and form_args['detectors']:
            query = query.filter(or_(*[model.statistics['detectors'].has_key(detector) for detector in form_args['detectors']]))
        # Adjust query based on source ip addresses selection.
        if 'source_ips' in form_args and form_args['source_ips']:
        ...
        if 'target_ips' in form_args and form_args['target_ips']:
...
class ReportsBlueprint(HawatBlueprint):
    ...
    def register_app(self, app):
        ...
        app.set_csag(
            ...
            SearchView,
            URLParamsBuilder({'submit': tr_('Search')}).add_rule('source_ips', True)
                                                       .add_kwrules_from_form(EventReportSearchForm),
        ...
        app.set_csag(
            ...
            SearchView,
            URLParamsBuilder({'submit': tr_('Search')}).add_rule('target_ips', True)
                                                       .add_kwrules_from_form(EventReportSearchForm),
