    ...
    def build_query(query, model, form_args):
        ...
        if 'search' in form_args and form_args['search']:
            ...
            try:
                ...
                query = query.filter(text("network >>= '{}-{}'".format(net.single(net.low()), net.single(net.high()))))
            except ValueError:
...
class APINetworksView(AJAXMixin, SQLAlchemyMixin, SimpleView):  # pylint: disable=locally-disabled,too-many-ancestors
    ...
    def _get_resolved_abuses(self, network):
        ...
        if network.is_base:
            return {"fallback": next((email for email in [settings.emails_info, settings.emails_low, settings.emails_medium, settings.emails_high, settings.emails_critical] if email), [])}
        out = {}
...
def get_blueprint():
    ...
    hbp.register_view_class(NetworkResolverSnippetView, '/snippet/{}/resolver'.format(BLUEPRINT_NAME))
