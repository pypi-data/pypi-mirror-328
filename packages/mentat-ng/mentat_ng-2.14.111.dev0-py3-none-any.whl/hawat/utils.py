...
class URLParamsBuilder:
    ...
    def _add_rule(self, rule: UrlParamRule) -> None:
        if rule.key in self._used_keys:
            raise ValueError(f"Key '{rule.key}' is already used. If using in combination with the `add_kwrule_from_form` method, use it last.")
    ...
    def add_kwrule(self, key: str, as_list: bool = False, optional: bool = False, insignificant: bool = False) -> Self:
        """
