...
class MentatNetmngrScript(mentat.script.fetcher.FetcherScript):
    ...
    def _process_groups_and_networks(self, status_only):
        ...
        abuse_groups = self.sqlservice.session.query(GroupModel).filter(GroupModel.enabled).order_by(GroupModel.name).all()
        ...
        for abg in abuse_groups:
            ...
            emails = list(set((abg.settings_rep.emails_info or []) + (abg.settings_rep.emails_low or []) + (abg.settings_rep.emails_medium or []) + (abg.settings_rep.emails_high or []) + (abg.settings_rep.emails_critical or [])))
            name = '_'.join(sorted(map(lambda e: e.lower().replace('+', '_'), emails)))
    ...
    def _load_whois_file(self, whois_file):
        ...
        try:
            ...
            raise pyzenkit.zenscript.ZenScriptException("Invalid whois file '{}', expected JSON formatted file"
        ...
        for net in networks:
            ...
            # Set local id if no local id is set.
                ...
                if status_only:
                    ...
                    continue
                self.logger.warning("'%s' Updating existing network '%s', setting new local id.", gkey, nkey)
