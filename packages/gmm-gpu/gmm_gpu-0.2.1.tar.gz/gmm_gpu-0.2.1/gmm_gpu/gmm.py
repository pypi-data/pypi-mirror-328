...
class GMM:
    ...
    def fit(self, X):
        ...
        single_component = (N_actual >= N_actual_total.unsqueeze(1) - 1).any(1)
    ...
    def _init_clusters(self, X):
        ...
        retries = 0
        while retries < 3:
            seed = self._rand_seed + retries if self._rand_seed else None
            _, assignment = self._kmeans(X,
                                         self._n_components,
                                         random_seed=seed)
    ...
    def _kmeans(self, X, n_clusters=2, max_iter=10, tol=0.001, random_seed=None):
        """
        ...
        centers = self._kmeans_pp(X, C, valid_points, random_seed=random_seed)
        ...
        while i < max_iter and diff > tol:
            ...
            cluster_counts = torch.zeros((B, C, 1), dtype=torch.float32, device=X.device)
            cluster_sums.scatter_add_(1, assignment.unsqueeze(-1).expand(-1, -1, D), X)
            cluster_counts.scatter_add_(1, assignment.unsqueeze(-1), torch.ones_like(X[:, :, :1]))
            new_centers = cluster_sums / cluster_counts.clamp_min(1e-8)
    ...
    def _select_random_valid_points(self, X, valid_mask):
        ...
        rand_offsets = torch.randint(0, counts.max(), (B,), device=X.device) % counts
    ...
    def _kmeans_pp(self, X, C, valid_points, random_seed=None):
        B, N, D = X.shape
        device = X.device
        std = self._nanstd(X)
