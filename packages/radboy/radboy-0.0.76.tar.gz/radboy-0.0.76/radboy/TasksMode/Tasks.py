        ...
        if (fieldname in self.valid_fields) or (load == True and fieldname == 'ListQty'):
            with Session(self.engine) as session:
                ...
                for num,result in enumerate(results):
                    print(f"{numColor}{num}{Style.reset} -> {color1}{result.Name}{Style.reset}|{color2}{result.rebar()}|{result.ALT_Barcode}{Style.reset}|{color3}{result.Code}{Style.reset}|{color4}{getattr(result,fieldname)}{Style.reset}|{eidColor}{getattr(result,'EntryId')}{Style.reset}")
        print(f"{m}\n{hr}")
                            ...
                            if text.startswith("-") or text.startswith("+"):
                                ...
                                if result:
                                    ...
                                    if callable(repack_exec):
                                        repack_exec(result)
                                    print(f"{Fore.light_red}0{Style.reset} -> {color1}{result.Name}{Style.reset}|{color2}{result.rebar()}|{result.ALT_Barcode}{Style.reset}|{color3}{result.Code}{Style.reset}|{color4}{getattr(result,fieldname)}{Style.reset}|{color4}{getattr(result,'EntryId')}{Style.reset}")
                                    print(f"{m}\n{hr}")
...
""")
    ...
    def __init__(self,engine,parent,init_only=False,root_modes={}):
        ...
        self.options["setListQty True"]={
                    'cmds':["#"+str(count),f"setListQty","setListQty True","set ListQty True","slq","slq True"],
        ...
        self.options["setListQty False"]={
                    'cmds':["#"+str(count),f"setListQty","set ListQty False","slq","slq False"],
        ...
        location_fields["set ListQty True"]=self.options["setListQty False"]['cmds']
        count+=1
    ...
    def fastShelf(self,count_field="ShelfCount",set_field="Shelf",mode=""):
        ...
        with Session(ENGINE) as session:
            while True:
                ...
                result=selected
                msg=f"{Fore.light_red}0{Style.reset} -> {color1}{result.Name}{Style.reset}|{color2}{result.rebar()}|{result.ALT_Barcode}{Style.reset}|{color3}{result.Code}{Style.reset}|{color4}{getattr(result,fieldname)}{Style.reset}|{color4}{getattr(result,'EntryId')}{Style.reset}"
                print(msg)
