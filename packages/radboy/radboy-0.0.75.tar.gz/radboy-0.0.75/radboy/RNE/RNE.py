	...
	def getUpcomingExpiry(self,oldest=False):
		...
		with Session(ENGINE) as session:
			...
			for num,i in enumerate(results):
				...
				print(f"{Fore.light_green}{num}/{Fore.light_yellow}{num+1} of {Fore.light_red}{ct}{Fore.dark_goldenrod} -> {Fore.light_magenta}{i.Name}|{Fore.plum_2}{i.Barcode}|{Fore.cyan}{bb}|{Fore.medium_violet_red}{doe}{Fore.orange_red_1}|eid={i.eid}{Style.reset}")
		...
		with Session(ENGINE) as session:
			while True:
				...
				if ct == 0:
					...
					for num,entry in enumerate(results):
						msg=f'''{Fore.light_yellow}{num}/{Fore.dark_goldenrod}{num+1}/{Fore.light_red}{ct}{Fore.light_magenta} -> [BARCD]={Fore.light_green}{entry.Barcode}|[NM]={Fore.green_yellow}{entry.Name}|{entry.Note}|[BB_EXP]={Fore.orange_red_1}{Style.bold}{entry.BB_Expiry}|[DTOE]={Fore.medium_violet_red}{Style.bold}{entry.DTOE}|{Fore.magenta}[eid]={entry.eid}{Style.reset}'''
						print(msg)
