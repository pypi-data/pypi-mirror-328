...
{Fore.light_green}DOE:{Fore.light_magenta}{log.DayLogDate}{Style.reset}'''
	...
	def listDateRange(self):
		...
		occurances=Prompt.__init2__(None,func=FormBuilderMkText,ptext="All Fields Qty Above or Equal To How Many?",helpText="above or == To this number",data="integer")
		if occurances in [None,]:
			return
		elif occurances == 'd':
			occurances=2

		occurances_max=Prompt.__init2__(None,func=FormBuilderMkText,ptext="All Fields Qty Below or Equal To How Many?",helpText="Below or == To this number",data="integer")
...
"PalletCount",]
		...
		with Session(ENGINE) as session:
			...
			if code in ['d',]:
				...
				query=session.query(DayLog).filter(and_(DayLog.DayLogDate.between(date_from,date_to),
					or_(
