...
{Fore.light_green}DOE:{Fore.light_magenta}{log.DayLogDate}{Style.reset}'''
	...
	def listDateRange(self):
		...
		occurances=Prompt.__init2__(None,func=FormBuilderMkText,ptext="Above or Equal To How Many Times Marked?",helpText="above or == To this number",data="integer")
		if occurances in [None,]:
			return
		elif occurances == 'd':
			occurances=2

		occurances_max=Prompt.__init2__(None,func=FormBuilderMkText,ptext="Below or Equal To How Many Times Marked?",helpText="Below or == To this number",data="integer")
		...
		with Session(ENGINE) as session:
			query=session.query(DayLog).filter(date_from>DayLog.DayLogDate,DayLog.DayLogDate<date_to).all()
			totals={}
			totals_dl={}
			...
			for num,i in enumerate(totals):
				msg=f"{Fore.light_cyan}{num}/{num+1} of {ct} - {totals_dl[i].seeShort()} {Fore.light_magenta}Total{Fore.green_yellow}={Fore.light_sea_green}{totals[i]}{Style.reset}"
