...
from stratio.cli.config import check_for_updates, copy_default_config_if_missing, load_config, logger
from stratio.cli.utils import parse_filter_option
from stratio.config import Config

app = typer.Typer(help="Stratio CLI Tool: Manage customers, clusters, and bootstrap instances.")
...
def start_cluster(
    ...
    if not yes and not typer.confirm(
        f"Are you sure you want to start cluster '{cluster_id}' in region '{INFRASTRUCTURE_REGION}'?"
    ):
...
def stop_cluster(
    ...
    """
    if not yes and not typer.confirm(
        f"Are you sure you want to stop cluster '{cluster_id}' in region '{INFRASTRUCTURE_REGION}'?"
    ):
        console.print("Operation cancelled.", style="red")
        raise typer.Exit()
    client = get_cluster_client()
...
def force_uninstall_cluster(
    ...
    """
    if not yes and not typer.confirm(
        f"Are you sure you want to force uninstall cluster '{cluster_id}' in region '{INFRASTRUCTURE_REGION}'?"
    ):
        console.print("Operation cancelled.", style="red")
        raise typer.Exit()
...
def stop_bootstrap(
    ...
    """
    if not yes and not typer.confirm(
        f"Are you sure you want to stop bootstrap instance '{bootstrap_id}' in region '{INFRASTRUCTURE_REGION}'?"
    ):
        console.print("Operation cancelled.", style="red")
        raise typer.Exit()
    client = get_operations_client()
...
def exec_keos(
    ...
    try:
        client.exec_keos(cluster_identifier, "/tmp")
    except Exception as e:
...
def download_keos(
    ...
    client = get_keos_client()
    try:
        client.download_keos_workspace(cluster_identifier, "/tmp")
    except Exception as e:
