...
from otter.config.models import Settings
from otter.errors import DownloadError, NotFoundError, PISCriticalError, PreconditionFailedError
from otter.manifest import Manifest
from otter.manifest.models import Result, RootManifest, StepManifest

s = Settings(step='step1', release_uri='gs://test')
steps = ['step1', 'step2']
content_dict = {
    'result': 'failed',
...
def mocked_settings():
    with patch('pis.manifest.manifest.settings') as mock_settings:
...
@pytest.fixture(autouse=True)
def mocked_steps():
    with patch('pis.manifest.manifest.steps') as mock_steps:
...
def mocked_absolute_path():
    with patch('pis.manifest.manifest.absolute_path') as mock_absolute_path:
...
@patch('pis.manifest.manifest.get_remote_storage')
...
@patch('pis.manifest.manifest.Manifest._load_remote')
...
@patch('pis.manifest.manifest.Manifest._load_remote')
@patch('pis.manifest.manifest.Manifest._load_local')
...
@patch('time.sleep')
@patch('pis.manifest.manifest.Manifest._refresh_from_remote')
@patch('pis.manifest.manifest.get_remote_storage')
...
def test_save_remote_ko(mock_remote_storage, mock_refresh_from_remote):
    mock_remote_storage.return_value.download_to_string.return_value = (content_json, 81235723895)
    mock_remote_storage.return_value.upload.side_effect = DownloadError('test')
    m = Manifest()
    m._revision = 1
...
@patch('pis.manifest.manifest.Manifest._load_remote')
@patch('pis.manifest.manifest.FileLock')
@patch('pis.manifest.manifest.absolute_path')
...
@patch('pis.manifest.manifest.Manifest._load_remote')
@patch('pis.manifest.manifest.Manifest._serialize')
@patch('pis.manifest.manifest.FileLock')
@patch('pis.manifest.manifest.absolute_path')
