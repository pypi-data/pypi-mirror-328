...
class AssistantModel:
    ...
    def chat(
        ...
        if stream:
            return self._chat_streaming(messages=messages, model=model, filter=filter, include_highlights=include_highlights)
        else:
            return self._chat_single(messages=messages, model=model, filter=filter, json_response=json_response, include_highlights=include_highlights)
    ...
    def _chat_single(
        ...
        chat_request = (
            ChatRequest(messages=messages, filter=filter,
                        model=model, json_response=json_response, include_highlights=include_highlights)
            if filter
            else ChatRequest(messages=messages, model=model, json_response=json_response, include_highlights=include_highlights)
        )
    ...
    def context(
        ...
        :param top_k: Optional integer to specify the number of context snippets to return.
