...
from deepquant.quest.const import EXIT_CODE
import  deepquant.quest.mod.mod_sys_progress.progress_client as ProcessClient
...
class ProgressMod(AbstractMod):
    ...
    def start_up(self, env, mod_config):
        ...
        self._backtestid = env.config.extra.backtest_id
        if self._show:
            env.event_bus.add_listener(EVENT.POST_SYSTEM_INIT, self._init)
            env.event_bus.add_listener(EVENT.AFTER_TRADING, self.after_trading)
    ...
    def after_trading(self, __):
        self._cur_traded_cnt = self._cur_traded_cnt + 1
        self._progress_bar.update(self._cur_traded_cnt/self._trading_length*100)
        self.update_progress(self._cur_traded_cnt/self._trading_length*100)
        print()

    def update_progress(self, progress):
        ProcessClient.update_progress(progress, self._backtestid)
    
    def update_status(self, status):
        ProcessClient.update_status(status, self._backtestid)
