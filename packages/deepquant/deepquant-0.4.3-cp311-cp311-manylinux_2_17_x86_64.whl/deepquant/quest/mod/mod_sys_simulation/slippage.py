...
from deepquant.quest.const import POSITION_EFFECT, ORDER_TYPE
    ...
    def __init__(self, module_name, rate):
        try:
            if "." not in module_name:
                module = importlib.import_module("deepquant.quest.mod.mod_sys_simulation.slippage")
...
class PriceRatioSlippage(BaseSlippage):
    ...
    def get_trade_price(self, order, price):
        ...
        high_limited = env.price_board.get_high_limited(order.market_code)
        low_limited = env.price_board.get_low_limited(order.market_code)
        if is_valid_price(high_limited):
            temp_price = min(temp_price, high_limited)
        if is_valid_price(low_limited):
            temp_price = max(temp_price, low_limited)
        return temp_price
...
class TickSizeSlippage(BaseSlippage):
    ...
    def get_trade_price(self, order, price):
        ...
        tick_size = Environment.get_instance().data_proxy.instrument(order.market_code).tick_size()
