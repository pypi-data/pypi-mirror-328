...
from deepquant.quest.interface import AbstractBroker, Persistable
from deepquant.quest.utils.i18n import gettext as _
from deepquant.quest.core.events import EVENT, Event
from deepquant.quest.const import MATCHING_TYPE, ORDER_STATUS, POSITION_EFFECT, EXECUTION_PHASE, INSTRUMENT_TYPE
...
class SimulationBroker(AbstractBroker, Persistable):
    ...
    @lru_cache(1024)
    def _get_matcher(self, market_code):
        # type: (str) -> AbstractMatcher
        instrument_type = self._env.data_proxy.instrument(market_code).type
        try:
    ...
    def set_state(self, state):
        def _account_order_from_state(order_state):
            ...
            account = self._env.get_account(o.market_code)
            return account, o
    ...
    def after_trading(self, __):
        for account, order in self._open_orders:
            order.mark_rejected(_(u"Order Rejected: {market_code} can not match. Market close.").format(
    ...
    def pre_settlement(self, __):
        for account, order in self._open_exercise_orders:
            self._get_matcher(order.market_code).match(account, order, False)
    ...
    def on_tick(self, event):
        tick = event.tick
        self._get_matcher(tick.market_code).update(event)
        self._match(tick.market_code)

    def _match(self, market_code=None):
        # 撮合未完成的订单，若指定标的时只撮合指定的标的的订单
        order_filter = lambda a_and_o: not (a_and_o[1].is_final() or (market_code and a_and_o[1].market_code != market_code))
        for account, order in filter(order_filter, self._open_orders):
            self._get_matcher(order.market_code).match(account, order, open_auction=False)
        for account, order in filter(order_filter, self._open_auction_orders):
            self._get_matcher(order.market_code).match(account, order, open_auction=True)
    ...
    def _check_subscribe(self, order):
        if self._env.config.base.frequency == "tick" and order.market_code not in self._env.get_universe():
            raise RuntimeError(_("{market_code} should be subscribed when frequency is tick.").format(
                market_code=order.market_code))
