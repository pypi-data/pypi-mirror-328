...
from deepquant.quest.const import POSITION_DIRECTION, SIDE, POSITION_EFFECT, DEFAULT_ACCOUNT_TYPE, INSTRUMENT_TYPE
from deepquant.quest.environment import Environment
from deepquant.quest.portfolio.position import Position, PositionProxy
...
from deepquant.quest.utils.class_helper import deprecated_property, cached_property
...
class StockPosition(Position):
    ...
    def __init__(self, market_code, direction, init_quantity=0, init_price=None):
        super(StockPosition, self).__init__(market_code, direction, init_quantity, init_price)
    ...
    def before_trading(self, trading_date):
        ...
        if self.direction != POSITION_DIRECTION.LONG:
            raise RuntimeError("direction of stock position {} is not supposed to be short".format(self._market_code))
    ...
    def settlement(self, trading_date):
        ...
        instrument = self._env.data_proxy.instrument(self._market_code)
        delta_cash = 0
        if instrument.de_listed_at(next_date):
            try:
                transform_data = self._env.data_proxy.get_share_transformation(self._market_code)
...
class FuturePosition(Position):
    ...
    def prev_close(self):
        if not is_valid_price(self._prev_close):
            if self._env.config.mod.sys_accounts.futures_settlement_price_type == "settlement":
                self._prev_close = self._env.data_proxy.get_prev_settlement(self._market_code, self._env.trading_dt)
            else:
    ...
    def settlement(self, trading_date):
        ...
        if self._quantity == 0:
            return delta_cash
        data_proxy = self._env.data_proxy
        instrument = data_proxy.instrument(self._market_code)
        next_date = data_proxy.get_next_trading_date(trading_date)
        if self._env.config.mod.sys_accounts.futures_settlement_price_type == "settlement":
            # 逐日盯市按照结算价结算
            self._last_price = self._env.data_proxy.get_settle_price(self._market_code, self._env.trading_dt)
        delta_cash += self.equity
        self._avg_price = self.last_price
        if instrument.de_listed_at(next_date):
            user_system_log.warn(_(u"{market_code} is expired, close all positions by system").format(
                market_code=self._market_code
            ))
            account = self._env.get_account(self._market_code)
            side = SIDE.SELL if self.direction == POSITION_DIRECTION.LONG else SIDE.BUY
            trade = Trade.__from_create__(
                None, self.last_price, self._quantity, side, POSITION_EFFECT.CLOSE, self._market_code
            )
            self._env.event_bus.publish_event(Event(EVENT.TRADE, account=account, trade=trade, order=None))
            self._quantity = self._old_quantity = 0
        return delta_cash
...
class StockPositionProxy(PositionProxy):
    __repr_properties__ = (
        "market_code", "quantity", "avg_price", "market_value"
    )
    __instrument_types__ = INST_TYPE_IN_STOCK_ACCOUNT
...
class FuturePositionProxy(PositionProxy):
    __repr_properties__ = (
        "market_code", "buy_quantity", "sell_quantity", "buy_market_value", "sell_market_value",
