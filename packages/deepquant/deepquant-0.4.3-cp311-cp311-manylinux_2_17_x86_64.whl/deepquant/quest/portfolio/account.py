...
import six
from deepquant.quest.const import POSITION_DIRECTION, POSITION_EFFECT, DEFAULT_ACCOUNT_TYPE, DAYS_CNT
from deepquant.quest.environment import Environment
from deepquant.quest.core.events import EVENT
from deepquant.quest.model.order import Order, OrderStyle
from deepquant.quest.model.trade import Trade
from deepquant.quest.utils.class_helper import deprecated_property
...
from deepquant.quest.portfolio.position import Position, PositionProxyDict
from deepquant.quest.mod.mod_sys_accounts.position_model import FuturePosition
...
class Account(metaclass=AccountMeta):
    ...
    def __init__(
        ...
        for market_code, (init_quantity, init_price) in init_positions.items():
            ...
            self._get_or_create_pos(market_code, position_direction, init_quantity, init_price)

    def __repr__(self):
        positions_repr = {}
        for market_code, positions in self._positions.items():
            for direction, position in positions.items():
                if position.quantity != 0:
                    positions_repr.setdefault(market_code, {})[direction.value] = position.quantity
    ...
    def set_state(self, state):
        ...
        for market_code, positions_state in state['positions'].items():
            for direction in POSITION_DIRECTION:
                position = self._get_or_create_pos(market_code, direction)
    ...
    def get_position(self, market_code: str, direction: POSITION_DIRECTION = POSITION_DIRECTION.LONG) -> Position:
        """
        获取某个标的的持仓对象
        ...
        """
        try:
            return self._positions[market_code][direction]
        except KeyError:
            return Position(market_code, direction)

    def calc_close_today_amount(self, market_code, trade_amount, position_direction, position_effect):
        return self._get_or_create_pos(market_code, position_direction).calc_close_today_amount(trade_amount, position_effect)
    ...
    def _on_before_trading(self, _):
        for market_code, positions in list(self._positions.items()):
            if all(p.quantity == 0 and p.equity == 0 for p in six.itervalues(positions)):
                del self._positions[market_code]
    ...
    def apply_trade(self, trade, order=None):
        ...
        if trade.position_effect == POSITION_EFFECT.MATCH:
            delta_cash = self._get_or_create_pos(
                market_code, POSITION_DIRECTION.LONG
            ).apply_trade(trade) + self._get_or_create_pos(
                market_code, POSITION_DIRECTION.SHORT
            ).apply_trade(trade)
            self._total_cash += delta_cash
        else:
            delta_cash = self._get_or_create_pos(market_code, trade.position_direction).apply_trade(trade)
            self._total_cash += delta_cash
    ...
    def _get_or_create_pos(
        ...
        if market_code not in self._positions:
            ...
            positions = self._positions.setdefault(market_code, {
                POSITION_DIRECTION.LONG: Position(market_code, POSITION_DIRECTION.LONG, long_quantity, init_price),
                POSITION_DIRECTION.SHORT: Position(market_code, POSITION_DIRECTION.SHORT, short_quantity, init_price)
            })
            if not init_price:
                last_price = self._env.get_last_price(market_code)
            ...
            positions = self._positions[market_code]
        return positions[direction]

    def _on_tick(self, event):
        tick = event.tick
        try:
            positions = self._positions[tick.market_code]
        except KeyError:
            return
        for position in positions.values():
            position.update_last_price(tick.last_price)

    def _on_bar(self, _):
        for market_code, positions in self._positions.items():
            price = self._env.get_last_price(market_code)
            if price == price:
                for position in six.itervalues(positions):
    ...
    def _frozen_cash_of_order(self, order):
        if order.position_effect == POSITION_EFFECT.OPEN:
            instrument = self._env.data_proxy.instrument(order.market_code)
