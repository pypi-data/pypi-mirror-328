...
from deepquant.quest.apis import names
...
from deepquant.quest.utils.logger import user_log as logger, user_system_log, user_print
from deepquant.quest.model.instrument import Instrument
from deepquant.quest.model.tick import TickObject
from deepquant.quest.const import (
    EXECUTION_PHASE, ORDER_STATUS, SIDE, POSITION_EFFECT, ORDER_TYPE, MATCHING_TYPE, RUN_TYPE, POSITION_DIRECTION,
    DEFAULT_ACCOUNT_TYPE
)
from deepquant.quest.model.order import Order, MarketOrder, LimitOrder, OrderStyle, VWAPOrder, TWAPOrder
from deepquant.quest.core.events import EVENT, Event
from deepquant.quest.core.strategy_context import StrategyContext
...
def assure_instrument(id_or_ins):
    if isinstance(id_or_ins, Instrument):
        ...
        raise RQInvalidArgument(_(u"unsupported market_code type"))


def assure_market_code(id_or_ins):
    return assure_instrument(id_or_ins).market_code
...
def submit_order(id_or_ins, amount, side, price=None, position_effect=None):
    ...
    if (
            ...
            and env.get_instrument(market_code).type == "Future"
    ):
    ...
    market_price = env.get_last_price(market_code)
    if not is_valid_price(market_price):
        ...
        env.order_creation_failed(market_code, reason)
        return
...
def update_universe(id_or_symbols):
    ...
    market_code = set(
        assure_market_code(market_code) for market_code in id_or_symbols
    )
    if market_code != Environment.get_instance().get_universe():
        Environment.get_instance().update_universe(market_code)
...
def subscribe(id_or_symbols):
    ...
    if isinstance(id_or_symbols, six.string_types):
        market_code = instruments(id_or_symbols).market_code
        current_universe.add(market_code)
    elif isinstance(id_or_symbols, Instrument):
        current_universe.add(id_or_symbols.market_code)
    elif isinstance(id_or_symbols, Iterable):
        for item in id_or_symbols:
            current_universe.add(assure_market_code(item))
    else:
        raise RQInvalidArgument(_(u"unsupported market_code type"))
...
def unsubscribe(id_or_symbols):
    ...
    if isinstance(id_or_symbols, six.string_types):
        market_code = instruments(id_or_symbols).market_code
        current_universe.discard(market_code)
    elif isinstance(id_or_symbols, Instrument):
        current_universe.discard(id_or_symbols.market_code)
    elif isinstance(id_or_symbols, Iterable):
        for item in id_or_symbols:
            i = assure_market_code(item)
...
@apply_rules(
    verify_that("market_code", pre_check=True).is_listed_instrument(),
...
def history_bars(
    ...
    if fields is None:
        fields = ["orig_time", "open_price", "high_price", "low_price", "close_price", "volume"]
...
def history_ticks(market_code, count):
    ...
    return env.data_proxy.history_ticks(market_code, count, dt)
...
def all_instruments(type=None, date=None):
    ...
    :param type: 需要查询合约类型，例如：type='stock'代表股票。默认是所有类型
    :param date: 查询时间点
    ...
    return pd.DataFrame(
        [
            [i.market_code, i.security_name, i.type, i.listed_date, i.de_listed_date]
            for i in result
        ],
        columns=["market_code", "symbol", "type", "listed_date", "de_listed_date"],
    )
...
def instruments(id_or_symbols):
    ...
    *   获取单一股票合约的详细信息:
        ...
        ..  code-block:: python3
            ...
            [Out]
            Instrument(market_code=000001.XSHE, symbol=平安银行, abbrev_symbol=PAYH, listed_date=19910403, de_listed_date=null, board_type=MainBoard, sector_code_name=金融, sector_code=Financials, round_lot=100, exchange=XSHE, special_type=Normal, status=Active)
    ...
    *   获取多个股票合约的详细信息:
        ...
        ..  code-block:: python3
            ...
            [Out]
            [Instrument(market_code=000001.XSHE, symbol=平安银行, abbrev_symbol=PAYH, listed_date=19910403, de_listed_date=null, board_type=MainBoard, sector_code_name=金融, sector_code=Financials, round_lot=100, exchange=XSHE, special_type=Normal, status=Active), Instrument(market_code=000024.XSHE, symbol=招商地产, abbrev_symbol=ZSDC, listed_date=19930607, de_listed_date=null, board_type=MainBoard, sector_code_name=金融, sector_code=Financials, round_lot=100, exchange=XSHE, special_type=Normal, status=Active)]
...
def current_snapshot(id_or_symbol):
    ...
    ..  code-block:: python3
        ...
        Snapshot(market_code: '000001.XSHE', datetime: datetime.datetime(2016, 1, 4, 9, 33), open: 10.0, high: 10.025, low: 9.9667, last: 9.9917, volume: 2050320, total_turnover: 20485195, prev_close: 9.99)
    """
    env = Environment.get_instance()
    frequency = env.config.base.frequency
    market_code = assure_market_code(id_or_symbol)
    ...
    if env.config.base.run_type == RUN_TYPE.BACKTEST:
        if ExecutionContext.phase() == EXECUTION_PHASE.BEFORE_TRADING:
            dt = env.data_proxy.get_previous_trading_date(env.trading_dt.date())
            return env.data_proxy.current_snapshot(market_code, "1d", dt)
        elif ExecutionContext.phase() == EXECUTION_PHASE.AFTER_TRADING:
            return env.data_proxy.current_snapshot(market_code, "1d", dt)

    # PT、实盘直接取最新快照，忽略 frequency, dt 参数
    return env.data_proxy.current_snapshot(market_code, frequency, dt)
...
def get_positions():
    ...
    ..  code-block:: python3
        ...
        [In] get_positions()
        [Out]
        [StockPosition(market_code=000001.XSHE, direction=LONG, quantity=1000, market_value=19520.0, trading_pnl=0.0, position_pnl=0),
        StockPosition(market_code=RB2112, direction=SHORT, quantity=2, market_value=-111580.0, trading_pnl=0.0, position_pnl=0)]
...
def get_position(market_code, direction=POSITION_DIRECTION.LONG):
    # type: (str, Optional[POSITION_DIRECTION]) -> Position
    """
    获取某个标的的持仓对象。

    :param market_code: 标的编号
    ...
    ..  code-block:: python3
        ...
        [Out]
        StockPosition(market_code=000001.XSHE, direction=LONG, quantity=268600, market_value=4995960.0, trading_pnl=0.0, position_pnl=0)

    """
    portfolio = Environment.get_instance().portfolio
    return portfolio.get_position(market_code, direction)
...
@export_as_api
def symbol(market_code, sep=", "):
    if isinstance(market_code, six.string_types):
        return "{}[{}]".format(market_code, Environment.get_instance().get_instrument(market_code).security_name)
    else:
        s = sep.join(symbol(item) for item in market_code)
        return s
