...
from deepquant.quest.const import POSITION_DIRECTION, TRADING_CALENDAR_TYPE, INSTRUMENT_TYPE, SIDE
...
class AbstractEventSource(with_metaclass(abc.ABCMeta)):
    """
    事件源接口。yhalpha 从此对象中获取事件，驱动整个事件循环。
...
class AbstractPriceBoard(with_metaclass(abc.ABCMeta)):
    """
    yhalpha多个地方需要使用最新「行情」，不同的数据源其最新价格获取的方式不尽相同
...
class AbstractDataSource(object):
    """
    数据源接口。yhalpha 中通过 :class:`DataProxy` 进一步进行了封装，向上层提供更易用的接口。
    ...
    def get_instruments(self, id_or_syms=None, types=None):
        ...
        """
        获取 instrument，
        可指定 market_code 或 symbol 或 instrument type，id_or_syms 优先级高于 types，
    ...
    def get_merge_ticks(self, market_code_list, trading_date, last_dt=None):
        ...
        :param list market_code_list: 合约名列表
...
class AbstractBroker(with_metaclass(abc.ABCMeta)):
    ...
    yhalpha 将产生的订单提交给此对象，此对象负责对订单进行撮合（不论是自行撮合还是委托给外部的真实交易所），
    并通过 ``EVENT.ORDER_*`` 及 ``EVENT.TRADE`` 事件将撮合结果反馈进入 yhalpha。
    ...
    def submit_order(self, order):
        # type: (Order) -> None
        # 提交订单。yhalpha 会生成 :class:`~Order` 对象，再通过此接口提交到 Broker。
        raise NotImplementedError
...
class AbstractMod(with_metaclass(abc.ABCMeta)):
    ...
    def start_up(self, env, mod_config):
        """
        yhalpha 在系统启动时会调用此接口；在此接口中，可以通过调用 ``env`` 的相应方法来覆盖系统默认组件。
    ...
    def tear_down(self, code, exception=None):
        """
        yhalpha 在系统退出前会调用此接口。

        :param code: 退出代码
        :type code: deepquant.quest.const.EXIT_CODE
