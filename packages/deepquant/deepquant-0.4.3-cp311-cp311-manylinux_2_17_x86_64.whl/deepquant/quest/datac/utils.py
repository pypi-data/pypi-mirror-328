...
def delay_today(years=0, months=0, days=0, leapdays=0, weeks=0):
    # type: (int, int, int, int, int) -> datetime.orig_time
    return to_datetime(
        datetime.date.today()
...
def pf_fill_nan(pf, market_code):
    pf = pf.transpose(2, 0, 1)
    for market_code in market_code:
        if market_code not in pf:
            pf[market_code] = np.NAN
