...
from .utils import listify, to_date, is_panel_removed
...
def _to_date_int(date):
    # type: (datetime.orig_time or datetime.date) -> int
    return date.year * 10000 + date.month * 100 + date.day
...
def ensure_dates_base_on_listed_date(instrument, start_date, end_date, market):
    from .services.calendar import get_previous_trading_date, get_latest_trading_date
    if to_date(instrument.listed_date) > datetime.date.today():
        raise ValueError("instrument {} is not listed yet".format(instrument.market_code))
    ...
    if start_date is None:
        ...
        warnings.warn("{} has been delisted on {}".format(instrument.market_code, instrument.de_listed_date))
        return None, None
...
def ensure_instruments(market_code, type=None, market="cn"):
    market_code = ensure_list_of_string(market_code)
    from .services.basic import _all_obid_to_type, _get_instrument

    obid_to_type = _all_obid_to_type(market)
    result = []
    obid_set = set()
    for ob in market_code:
        if ob not in obid_to_type:
            warnings.warn("invalid market_code: {}".format(ob), stacklevel=0)
            continue
    ...
    if not result:
        raise ValueError("market_code: at least one valid instrument expected, got none")
    return result


def ensure_market_codes(market_code, type=None, market="cn"):
    market_code = ensure_list_of_string(market_code)
    from .services.basic import _all_obid_to_type
    ...
    for ob in market_code:
        if ob not in obid_to_type:
            warnings.warn("invalid market_code: {}".format(ob))
            continue
        ob_type, ob = obid_to_type[ob]
        if ob in obid_set:
            continue
...
def ensure_market_code(ob, type=None, market="cn"):
    ob = ensure_string(ob, "market_code")
    from .services.basic import _all_obid_to_type

    obid_to_type = _all_obid_to_type(market)
    if ob not in obid_to_type:
        raise ValueError("invalid market_code: {}".format(ob))
    ob_type, ob = obid_to_type[ob]
    if type is not None and ob_type != type:
        raise ValueError(
...
def ensure_trading_date(date):
    from .services.calendar import get_trading_dates, get_previous_trading_date, get_next_trading_date
