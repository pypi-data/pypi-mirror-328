...
from deepquant.quest.const import DAYS_CNT, DEFAULT_ACCOUNT_TYPE, POSITION_DIRECTION, RUN_TYPE
from deepquant.quest.environment import Environment
from deepquant.quest.core.events import EVENT, EventBus
...
from deepquant.quest.data import DataProxy
...
class Portfolio(object, metaclass=PropertyReprMeta):
    ...
    def __init__(
        ...
        for market_code, quantity in init_positions:
            account_type = self.get_account_type(market_code)
            if account_type in account_args:
                price = data_proxy.get_bar(market_code, last_trading_date).close_price
                if not is_valid_price(price):
                    raise ValueError(_("invalid init position {market_code}: no valid price at {date}").format(
                        market_code=market_code, date=last_trading_date
                    ))
                account_args[account_type]["init_positions"][market_code] = quantity, price
    ...
    def get_position(self, market_code, direction):
        # type: (str, POSITION_DIRECTION) -> AbstractPosition
        account = self._accounts[self.get_account_type(market_code)]
        return account.get_position(market_code, direction)

    @classmethod
    def get_account_type(cls, market_code):
        instrument = Environment.get_instance().data_proxy.instrument(market_code)
        return instrument.account_type

    def get_account(self, market_code):
        return self._accounts[self.get_account_type(market_code)]
...
class MixedPositions(Mapping):
    ...
    def __repr__(self):
        ...
        for account in six.itervalues(self._accounts):
            keys += [
                market_code for market_code, position in account.positions.items()
