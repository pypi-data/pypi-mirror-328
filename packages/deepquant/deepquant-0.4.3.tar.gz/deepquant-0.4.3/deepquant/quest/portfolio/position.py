...
from deepquant.quest.const import POSITION_DIRECTION, POSITION_EFFECT
...
from deepquant.quest.utils.repr import PropertyReprMeta, property_repr
...
class Position(AbstractPosition, metaclass=PositionMeta):
    __repr_properties__ = (
        "market_code", "direction", "quantity", "market_value", "trading_pnl", "position_pnl", "last_price"
    )

    # 用于注册该 Position 类型适用的 instrument_type
    __instrument_types__ = []

    def __new__(cls, market_code, direction, init_quantity=0, init_price=None):
        if cls == Position:
            ins_type = Environment.get_instance().data_proxy.instrument(market_code).type
            try:
                position_cls = POSITION_TYPE_MAP[ins_type]
            except KeyError:
                raise NotImplementedError("")
            return position_cls.__new__(position_cls, market_code, direction, init_quantity, init_price)
        else:
    ...
    def __init__(self, market_code, direction, init_quantity=0, init_price=None):
        ...
        self._instrument = self._env.data_proxy.instrument(market_code)  # type: Instrument
    ...
    def daily_pnl(self):
        ...
        返回该持仓的浮动盈亏
        """
        return self.position_pnl + self.trading_pnl - self.transaction_cost
    ...
    def prev_close(self):
        ...
        if not is_valid_price(self._prev_close):
            self._prev_close = self._env.data_proxy.get_prev_close(self._market_code, self._env.trading_dt)
            if not is_valid_price(self._prev_close):
                user_system_log.warn("invalid prev close of {}: {}".format(self._market_code, self._prev_close))
    ...
    def last_price(self):
        ...
        if not is_valid_price(self._last_price):
            self._last_price = self._env.data_proxy.get_last_price(self._market_code)
            if not is_valid_price(self._last_price):
                user_system_log.warn("invalid last price of {}: {}".format(self._market_code, self._last_price))
    ...
    def _open_orders(self):
        # type: () -> Iterable[Order]
        for order in self._env.broker.get_open_orders(self.market_code):
...
class PositionProxy(metaclass=PositionProxyMeta):
    ...
    def open_orders(self):
        return Environment.get_instance().broker.get_open_orders(self.market_code)
...
class PositionProxyDict(UserDict):
    ...
    def __getitem__(self, market_code):
        position_type, position_proxy_type = self._get_position_types(market_code)
        if market_code not in self._positions:
            long = position_type(market_code, POSITION_DIRECTION.LONG)
            short = position_type(market_code, POSITION_DIRECTION.SHORT)
        else:
    ...
    def _get_position_types(self, market_code):
        # type: (str) -> Tuple[type, type]
        instrument_type = Environment.get_instance().data_proxy.instrument(market_code).type
