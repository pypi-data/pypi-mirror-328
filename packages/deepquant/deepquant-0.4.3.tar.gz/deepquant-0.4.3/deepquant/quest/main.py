...
import logbook
import six
from deepquant.quest import const
from deepquant.quest.core.executor import Executor
from deepquant.quest.core.strategy import Strategy
from deepquant.quest.core.strategy_context import StrategyContext
from deepquant.quest.core.strategy_loader import FileStrategyLoader, SourceCodeStrategyLoader, UserFuncStrategyLoader
from deepquant.quest.data.base_data_source import BaseDataSource
from deepquant.quest.data.data_proxy import DataProxy
...
from deepquant.quest.interface import Persistable
from deepquant.quest.mod import ModHandler
from deepquant.quest.model.bar import BarMap
from deepquant.quest.utils import YhAttrDict, create_custom_exception, init_yhdatac_env
from deepquant.quest.utils.exception import CustomException, is_user_exc, patch_user_exc
from deepquant.quest.utils.i18n import gettext as _
from deepquant.quest.utils.log_capture import LogCapture
from deepquant.quest.utils.logger import release_print, system_log, user_log, user_system_log
from deepquant.quest.utils.persisit_helper import PersistHelper
...
def get_strategy_apis():
    from deepquant.quest import api
    return {n: getattr(api, n) for n in api.__all__}


def init_yhdatac(yhdatac_uri):
    if yhdatac_uri in ["disabled", "DISABLED"]:
        return
    ...
    if deepquant.quest.datac.initialized():
        return True
    else:
        ...
        try:
            ...
            system_log.warn(_('yhdatac init failed, some apis will not function properly: {}').format(str(e)))
            return


def run(config, source_code=None, user_funcs=None):
    env = Environment(config, init_yhdatac(getattr(config.base, 'yhdatac_uri', None)))
    persist_helper = None
    init_succeed = False
    ...
    try:
        ...
        env.set_data_proxy(DataProxy(env.data_source, env.price_board))#设置实时行情，基础行情，历史行情

        _adjust_start_date(env.config, env.data_proxy)#根据数据时间更新策略开始结束时间
        ...
        if env.portfolio is None:
            from deepquant.quest.portfolio import Portfolio
...
def enable_profiler(env, scope):
    ...
    for name in scope:
        obj = scope[name]
        # 针对 run_func
        func_cond = "__globals__" in dir(obj) and obj.__globals__.get("__name__", None) == "deepquant.quest.user_module"
        # 针对 run_code 和 run_file
        file_or_code_cond = "__module__" in dir(obj) and obj.__module__ == "deepquant.quest.user_module"
...
def set_loggers(config):
    from deepquant.quest.utils.logger import user_log, user_system_log, system_log
    from deepquant.quest.utils.logger import init_logger
    from deepquant.quest.utils import logger
