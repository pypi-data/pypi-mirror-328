...
from deepquant.quest.const import MATCHING_TYPE, ORDER_TYPE, POSITION_EFFECT, SIDE
from deepquant.quest.environment import Environment
from deepquant.quest.core.events import EVENT, Event
from deepquant.quest.model.order import Order, ALGO_ORDER_STYLES
...
class DefaultBarMatcher(AbstractMatcher):
    ...
    def _current_bar_close_decider(self, market_code, _):
        try:
            return self._env.get_bar(market_code).close_price
        except (KeyError, TypeError):
            return 0

    def _next_bar_open_decider(self, market_code, _):
        try:
            return self._env.get_bar(market_code).open_price
        except (KeyError, TypeError):
            return 0

    def _vwap_decider(self, market_code, _):
        try:
            contract_multiplier = self._env.get_instrument(market_code).contract_multiplier
            bar = self._env.get_bar(market_code)
            return bar.total_turnover / bar.volume / contract_multiplier
        except (KeyError, TypeError, ZeroDivisionError):
            return 0

    def _open_auction_deal_price_decider(self, market_code, _):
        return self._env.data_proxy.get_open_auction_bar(market_code, self._env.trading_dt).open_price
    ...
    def _get_bar_volume(self, order, open_auction=False):
        if open_auction:
            volume = self._env.data_proxy.get_open_auction_volume(order.market_code, self._env.trading_dt)
        else:
            if isinstance(order.style, ALGO_ORDER_STYLES):
                _, volume = self._env.data_proxy.get_algo_bar(order.market_code, order.style, self._env.calendar_dt)
            else:
                volume = self._env.get_bar(order.market_code).volume
        return volume

    def _get_deal_price(self, order, open_auction=False):
        if open_auction:
            deal_price = self._open_auction_deal_price_decider(order.market_code, order.side)
        else:
            if isinstance(order.style, ALGO_ORDER_STYLES):
                deal_price, v = self._env.data_proxy.get_algo_bar(order.market_code, order.style, self._env.calendar_dt)
            else:
                deal_price = self._deal_price_decider(order.market_code, order.side)
        return deal_price
    ...
    def match(self, account, order, open_auction):
        ...
        instrument = self._env.get_instrument(market_code)
        ...
        if not is_valid_price(deal_price):
            ...
            if listed_date == self._env.trading_dt.date():
                reason = _(
                    u"Order Cancelled: current security [{market_code}] can not be traded"
                    u" in listed date [{listed_date}]").format(
                    market_code=order.market_code,
                    listed_date=listed_date,
                )
            elif isinstance(order.style, ALGO_ORDER_STYLES):
                reason = _(u"Order Cancelled: {market_code} bar no volume").format(market_code=order.market_code)
            else:
                # 撮合的时候无行情数据也不需要撤单，等到有行情再撮合
                reason = None
                # reason = _(u"Order Cancelled: current bar [{market_code}] miss market data.").format(
                #     market_code=order.market_code)
            if reason:
        ...
        if order.type == ORDER_TYPE.LIMIT:
            ...
            if self._price_limit:
                if order.side == SIDE.BUY and deal_price >= price_board.get_high_limited(market_code):
                    return
                if order.side == SIDE.SELL and deal_price <= price_board.get_low_limited(market_code):
                    return
        else:
            if self._price_limit:
                if order.side == SIDE.BUY and deal_price >= price_board.get_high_limited(market_code):
                    reason = _(
                        "Order Cancelled: current bar [{market_code}] reach the high_limited price."
                    ).format(market_code=order.market_code)
                    order.mark_rejected(reason)
                    return
                if order.side == SIDE.SELL and deal_price <= price_board.get_low_limited(market_code):
                    reason = _(
                        "Order Cancelled: current bar [{market_code}] reach the low_limited price."
                    ).format(market_code=order.market_code)
                    order.mark_rejected(reason)
                    return
        ...
        if self._volume_limit:
            ...
            if volume == volume:
                volume_limit = round(volume * self._volume_percent) - self._turnover[order.market_code]

                round_lot = instrument.round_lot
                volume_limit = (volume_limit // round_lot) * round_lot
                if volume_limit <= 0:
                    if order.type == ORDER_TYPE.MARKET:
                        reason = _(u"Order Cancelled: market order {market_code} volume {order_volume}"
        ...
        ct_amount = account.calc_close_today_amount(market_code, fill, order.position_direction, order.position_effect)
        ...
        if order.position_effect == POSITION_EFFECT.OPEN and self._slippage_decider.decider.rate != 0:
            ...
            if cost_money > account.cash + order.init_frozen_cash:
                reason = _(u"Order Cancelled: not enough money to buy {market_code}, needs {cost_money:.2f}, cash {cash:.2f}").format(
                        market_code=market_code, cost_money=cost_money, cash = account.cash + order.init_frozen_cash
                        )
                order.mark_rejected(reason)
                return

        order.fill(trade)
        self._turnover[order.market_code] += fill
        ...
        if order.type == ORDER_TYPE.MARKET and order.unfilled_quantity != 0:
            reason = _(
                u"Order Cancelled: market order {market_code} volume {order_volume} is"
...
class DefaultTickMatcher(AbstractMatcher):
    ...
    def _create_deal_price_decider(self, matching_type):
        decider_dict = {
            MATCHING_TYPE.NEXT_TICK_LAST: lambda market_code, side: self._env.price_board.get_last_price(
                market_code),
            MATCHING_TYPE.NEXT_TICK_BEST_OWN: lambda market_code, side: self._best_own_price_decider(market_code,
                                                                                                       side),
            MATCHING_TYPE.COUNTERPARTY_OFFER: None,
            MATCHING_TYPE.NEXT_TICK_BEST_COUNTERPARTY: lambda market_code, side: (
                self._env.price_board.get_a1(market_code) if side == SIDE.BUY else self._env.price_board.get_b1(
                    market_code))
        }
        return decider_dict[matching_type]

    def _best_own_price_decider(self, market_code, side):
        """ 己方最优价 """
        price = self._env.price_board.get_b1(market_code) if side == SIDE.BUY else self._env.price_board.get_a1(
            market_code)
        if price == 0:
            price = self._env.price_board.get_last_price(market_code)
        return price
    ...
    def _get_today_history_ticks(self, market_code, count):
        """ 获取当前交易日的历史tick数据 """
        cal_dt = self._env.calendar_dt
        tick_list = self._env.data_proxy.history_ticks(market_code, count, cal_dt)
        ...
        ticks = [tick for tick in tick_list if start <= tick.orig_time <= cal_dt]
        return ticks

    def _get_last_tick(self, market_code):
        """ 获取上一个tick """
        _last_tick = self._last_tick.get(market_code)
        trading_dt = self._env.trading_dt
        # 上一根tick缺失
        if not _last_tick:
            tick_list = self._get_today_history_ticks(market_code, 2)
            _last_tick = tick_list[0] if len(tick_list) == 2 else None
        else:
            # 两个tick之间的时间差(秒)
            diff_time = trading_dt.timestamp() - _last_tick.orig_time.timestamp()
    ...
    def match(self, account, order, open_auction):  # type: (Account, Order, bool) -> None
        ...
        if not is_valid_price(deal_price):
            ...
            if listed_date == self._env.trading_dt.date():
                ...
                reason = _(u"Order Cancelled: current tick [{market_code}] miss market data.").format(
                    market_code=order.market_code)
            order.mark_rejected(reason)
            return
        ...
        if order.type == ORDER_TYPE.LIMIT:
            ...
            if self._liquidity_limit:
                if order.side == SIDE.BUY and price_board.get_a1(market_code) == 0:
                    return
                if order.side == SIDE.SELL and price_board.get_b1(market_code) == 0:
                    return
        else:
            if self._price_limit:
                if order.side == SIDE.BUY and deal_price >= price_board.get_high_limited(market_code):
                    reason = _(
                        "Order Cancelled: current tick [{market_code}] reach the high_limited price."
                    ).format(market_code=order.market_code)
                    order.mark_rejected(reason)
                    return
                if order.side == SIDE.SELL and deal_price <= price_board.get_low_limited(market_code):
                    reason = _(
                        "Order Cancelled: current tick [{market_code}] reach the low_limited price."
                    ).format(market_code=order.market_code)
                    order.mark_rejected(reason)
                    return
            if self._liquidity_limit:
                if order.side == SIDE.BUY and price_board.get_a1(market_code) == 0:
                    reason = _(
                        "Order Cancelled: [{market_code}] has no liquidity."
                    ).format(market_code=order.market_code)
                    order.mark_rejected(reason)
                    return
                if order.side == SIDE.SELL and price_board.get_b1(market_code) == 0:
        ...
        if _volume_limit_flag:
            # 获取上一个tick
            _last_tick = self._get_last_tick(market_code)
    ...
    def update(self, event):
        self._last_tick[event.tick.market_code] = self._cur_tick.get(event.tick.market_code)
        self._cur_tick[event.tick.market_code] = event.tick
        self._turnover.clear()
...
class CounterPartyOfferMatcher(DefaultTickMatcher):
    ...
    def match(self, account, order, open_auction):
        ...
        if order.side == SIDE.BUY:
            if len(self._a_volume[market_code]) == 0:
                return
            volume_limit = self._a_volume[market_code][0]
            matching_price = self._a_price[market_code][0]
        else:
            if len(self._b_volume[market_code]) == 0:
                return
            volume_limit = self._b_volume[market_code][0]
            matching_price = self._b_price[market_code][0]
        ...
        if instrument.during_call_auction(self._env.trading_dt):
            # 集合竞价期间一律使用last
            matching_price = self._cur_tick[market_code].last_price
            _volume_limit_flag = True
        else:
            _volume_limit_flag = self._volume_limit
        ...
        if _volume_limit_flag:
            # 获取tick
            _last_tick = self._get_last_tick(market_code)
            _cur_tick = self._cur_tick[market_code]
        ...
        if not instrument.during_call_auction(self._env.trading_dt):
            # 常规交易时间逐档
            if order.side == SIDE.BUY:
                self._a_volume[order.market_code][0] -= fill
            else:
                self._b_volume[order.market_code][0] -= fill

        if order.type == ORDER_TYPE.MARKET and order.unfilled_quantity != 0:
            reason = _("Order Cancelled: market order {market_code} fill {filled_volume} actually").format(
                market_code=order.market_code,
                filled_volume=order.filled_quantity,
            )
            order.mark_cancelled(reason)
    ...
    def _pop_volume_and_price(self, order):
        try:
            if order.side == SIDE.BUY:
                if self._a_volume[order.market_code][0] == 0:
                    self._a_volume[order.market_code].pop(0)
                    self._a_price[order.market_code].pop(0)
            else:
                if self._b_volume[order.market_code][0] == 0:
                    self._b_volume[order.market_code].pop(0)
                    self._b_price[order.market_code].pop(0)
        except IndexError:
            return

    def _pre_tick(self, event):
        market_code = event.tick.market_code
        self._a_volume[market_code] = event.tick.ask_vols
        self._b_volume[market_code] = event.tick.bid_vols

        self._a_price[market_code] = event.tick.asks
        self._b_price[market_code] = event.tick.bids
