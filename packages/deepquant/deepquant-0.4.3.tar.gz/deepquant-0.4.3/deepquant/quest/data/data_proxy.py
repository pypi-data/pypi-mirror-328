...
from deepquant.quest.const import INSTRUMENT_TYPE, TRADING_CALENDAR_TYPE, EXECUTION_PHASE
from deepquant.quest.utils import risk_free_helper, TimeRange, merge_trading_period
from deepquant.quest.data.trading_dates_mixin import TradingDatesMixin
from deepquant.quest.model.bar import BarObject, NANDict, PartialBarObject
from deepquant.quest.model.tick import TickObject
from deepquant.quest.model.instrument import Instrument
from deepquant.quest.model.order import ALGO_ORDER_STYLES
from deepquant.quest.utils.functools import lru_cache
from deepquant.quest.utils.datetime_func import convert_int_to_datetime, convert_date_to_int
from deepquant.quest.utils.typing import DateLike, StrOrIter
from deepquant.quest.interface import AbstractDataSource, AbstractPriceBoard
...
class DataProxy(TradingDatesMixin):
    ...
    def get_trading_minutes_for(self, market_code, dt):
        instrument = self.instruments(market_code)
    ...
    def get_dividend_by_book_date(self, market_code, date):
        table = self._data_source.get_dividend(self.instruments(market_code))
    ...
    def get_split_by_ex_date(self, market_code, date):
        df = self.get_split(market_code)
    ...
    @lru_cache(10240)
    def _get_prev_close(self, market_code, dt):
        instrument = self.instruments(market_code)
        prev_trading_date = self.get_previous_trading_date(dt)
        bar = self._data_source.history_bars(instrument, 1, '1d', 'close_price', prev_trading_date,
                                             skip_suspended=False, include_now=False, adjust_orig=dt)
        if bar is None or len(bar) < 1:
            return np.nan
        return bar[0]

    def get_prev_close(self, market_code, dt):
        return self._get_prev_close(market_code, dt.replace(hour=0, minute=0, second=0))
    ...
    def _get_settlement(self, instrument, dt):
        ...
        if bar is None or len(bar) == 0:
            raise LookupError("'{}', dt={}".format(instrument.market_code, dt))
        return bar[0]

    def get_prev_settlement(self, market_code, dt):
        instrument = self.instruments(market_code)
    ...
    def get_settlement(self, instrument, dt):
        ...
        if instrument.type != INSTRUMENT_TYPE.FUTURE:
            raise LookupError("'{}', instrument_type={}".format(instrument.market_code, instrument.type))
        return self._get_settlement(instrument, dt)

    def get_settle_price(self, market_code, date):
        instrument = self.instruments(market_code)
    ...
    @lru_cache(512)
    def get_bar(self, market_code: str, dt: date, frequency: str = '1d') -> BarObject:
        instrument = self.instruments(market_code)
        if dt is None:
    ...
    def get_open_auction_bar(self, market_code, dt):
        instrument = self.instruments(market_code)
        try:
            bar = self._data_source.get_open_auction_bar(instrument, dt)
        except NotImplementedError:
            # forward compatible
            tick = self.current_snapshot(market_code, "1d", dt)
            bar = {k: getattr(tick, k) for k in [
                "orig_time", "open_price", "high_limited", "low_limited", "volume", "total_turnover"
            ]}
        return PartialBarObject(instrument, bar)
    
    def get_open_auction_volume(self, market_code, dt):
        instrument = self.instruments(market_code)
        volume = self._data_source.get_open_auction_volume(instrument, dt)
        return volume

    def history(self, market_code, bar_count, frequency, field, dt):
        data = self.history_bars(market_code, bar_count, frequency,
    ...
    def fast_history(self, market_code, bar_count, frequency, field, dt):
        return self.history_bars(market_code, bar_count, frequency, field, dt, skip_suspended=False,
                                 adjust_type='pre', adjust_orig=dt)

    def history_bars(self, market_code, bar_count, frequency, field, dt,
    ...
    def history_ticks(self, market_code, count, dt):
        instrument = self.instruments(market_code)
        return self._data_source.history_ticks(instrument, count, dt)

    def current_snapshot(self, market_code, frequency, dt):
        ...
        if frequency == '1d':
            ...
            d['prev_close'] = self._get_prev_close(market_code, dt)
    ...
    def get_futures_trading_parameters(self, market_code, dt):
    ...
    def get_merge_ticks(self, market_code_list, trading_date, last_dt=None):
        return self._data_source.get_merge_ticks(market_code_list, trading_date, last_dt)

    def is_suspended(self, market_code, dt, count=1):
        # type: (str, DateLike, int) -> Union[Sequence[bool], bool]
        if count == 1:
            return self._data_source.is_suspended(market_code, [dt])[0]

        trading_dates = self.get_n_trading_dates_until(dt, count)
        return self._data_source.is_suspended(market_code, trading_dates)

    def is_st_stock(self, market_code, dt, count=1):
        if count == 1:
            return self._data_source.is_st_stock(market_code, [dt])[0]

        trading_dates = self.get_n_trading_dates_until(dt, count)
        return self._data_source.is_st_stock(market_code, trading_dates)

    def get_tick_size(self, market_code):
        return self.instruments(market_code).tick_size()

    def get_last_price(self, market_code):
        # type: (str) -> float
        return float(self._price_board.get_last_price(market_code))
    ...
    def get_future_contracts(self, underlying, date):
        ...
        return sorted(i.market_code for i in self.all_instruments(
            [INSTRUMENT_TYPE.FUTURE], date
        ) if i.underlying_symbol == underlying and not Instrument.is_future_continuous_contract(i.market_code))
    ...
    def get_algo_bar(self, id_or_ins, order_style, dt):
        ...
        day_bar = self.get_bar(market_code=id_or_ins.market_code, dt=dt, frequency="1d")
        if day_bar.volume == 0:
            return np.nan, 0
