...
"""
yhalpha - a Algorithm Trading System
"""
from typing import Optional

from deepquant.quest.cmds import cli, inject_run_param
from deepquant.quest.api import export_as_api
from deepquant.quest.apis import subscribe_event
from . import data
...
def load_ipython_extension(ipython):
    ...
    ipython.register_magic_function(run_ipython_cell, 'line_cell', 'yhalpha')


def run(config, source_code=None):
    # [Deprecated]
    from deepquant.quest.utils.config import parse_config
    from deepquant.quest import main
...
def run_ipython_cell(line, cell=None):
    ...
    from deepquant.quest.utils.functools import clear_all_cached_functions
...
def run_func(**kwargs):
    ...
    :Keyword Arguments:
        * **config** (dict) -- 策略配置字典
        * **init** (Callable[[:class:`~deepquant.quest.core.strategy_context.StrategyContext`], Any]) -- 策略初始化函数，会在策略开始运行时被调用，仅会执行一次
        * **before_trading** (Callable[[:class:`~deepquant.quest.core.strategy_context.StrategyContext`], Any]) -- 盘前函数，会在每日盘前被调用一次
        * **open_auction** (Callable[[:class:`~deepquant.quest.core.strategy_context.StrategyContext`, dict[str, :class:`~deepquant.quest.model.bar.BarObject`]], Any]) -- 集合竞价函数，会在每日盘前集合竞价阶段被调用一次
        * **handle_bar** (Callable[[:class:`~deepquant.quest.core.strategy_context.StrategyContext`, dict[str, :class:`~deepquant.quest.model.bar.BarObject`]], Any]) -- k 线处理函数，会在盘中 k 线发生更新时被调用，适用于日/分钟级别回测
        * **handle_tick** (Callable[[:class:`~deepquant.quest.core.strategy_context.StrategyContext`, :class:`~deepquant.quest.model.tick.TickObject`], Any]) -- 快照数据处理函数，会在每个 tick 到达时被调用，适用于 tick 回测
        * **after_trading** (Callable[[:class:`~deepquant.quest.core.strategy_context.StrategyContext`], Any]) -- 盘后函数，会在每日交易结束后被调用一次
