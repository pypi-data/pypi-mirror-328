...
from deepquant.quest.cmds.entry import cli
from deepquant.quest.utils import init_yhdatac_env


@cli.command(help=_("create bundle using YHDatac"))
@click.option('-d', '--data-bundle-path', default=os.path.expanduser('~/.yhalpha'), type=click.Path(file_okay=False))
@click.option("yhdatac_uri", '--yhdatac', '--yhdatac-uri', default=None,
...
def create_bundle(data_bundle_path, yhdatac_uri, compression, concurrency):
    try:
        import deepquant.quest.datac
    except ImportError:
        click.echo(_(
            'yhdatac is required to create bundle. '
            'you can visit https://www.ricequant.com/welcome/rqdata to get yhdatac, '
            'or use "yhalpha download-bundle" to download monthly updated bundle.'
        ))
        return 1
    ...
    try:
        ...
        click.echo(_('yhdatac init failed with error: {}').format(e))
        return 1

    os.makedirs(os.path.join(data_bundle_path, 'bundle'), exist_ok=True)
    from deepquant.quest.data.bundle import update_bundle as update_bundle_
    update_bundle_(os.path.join(data_bundle_path, 'bundle'), True, compression, concurrency)


@cli.command(help=_("Update bundle using YHDatac"))
@click.option('-d', '--data-bundle-path', default=os.path.expanduser('~/.yhalpha'), type=click.Path(file_okay=False))
@click.option("yhdatac_uri", '--yhdatac', '--yhdatac-uri', default=None,
              help='yhdatac uri, eg user:password or tcp://user:password@ip:port')
@click.option('--compression', default=False, type=click.BOOL, help='enable compression to reduce file size')
@click.option('-c', '--concurrency', type=click.INT, default=1)
def update_bundle(data_bundle_path, yhdatac_uri, compression, concurrency):
    try:
        import deepquant.quest.datac
    except ImportError:
        click.echo(_(
            'yhdatac is required to update bundle. '
            'you can visit https://www.ricequant.com/welcome/rqdata to get yhdatac, '
            'or use "yhalpha download-bundle" to download monthly updated bundle.'
        ))
        return 1
    ...
    if not os.path.exists(os.path.join(data_bundle_path, 'bundle')):
        click.echo(_('bundle not exist, use "yhalpha create-bundle" command instead'))
        return 1
...
def download_bundle(data_bundle_path, confirm):
    default_bundle_path = os.path.abspath(os.path.expanduser('~/.yhalpha/bundle'))
