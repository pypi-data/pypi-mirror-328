...
def to_date(date):
    # type: (Union[str, datetime.date, datetime.orig_time]) -> datetime.date
...
def convert_dt_to_date_str(date):
    return date.strftime("%Y-%m-%d")

def convert_int_to_date_str(dt_int):
    date_obj = datetime.datetime.strptime(str(dt_int), "%Y%m%d")
    return date_obj.strftime("%Y-%m-%d")

def convert_data_str_to_int(data_str):
    date_obj = datetime.datetime.strptime(data_str,"%Y-%m-%d %H:%M:%S.%f")
    return convert_date_to_int(date_obj)