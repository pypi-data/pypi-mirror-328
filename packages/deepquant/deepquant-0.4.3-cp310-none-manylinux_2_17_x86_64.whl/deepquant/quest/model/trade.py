...
from deepquant.quest.utils import id_gen, get_position_direction
...
class Trade(object):
    ...
    @classmethod
    def __from_create__(
            cls, order_id, price, amount, side, position_effect, market_code, commission=0., tax=0.,
        ...
        trade._market_code = market_code
        trade._frozen_price = frozen_price
        trade._kwargs = kwargs
        return trade

    market_code = property(lambda self: self._market_code)
    trading_datetime = property(lambda self: self._trading_dt)
    orig_time = property(lambda self: self._calendar_dt)
