...
from deepquant.quest.const import COMMISSION_TYPE, INSTRUMENT_TYPE
...
class FutureInfoStore(object):
    ...
    def __init__(self, f, custom_future_info):
        with open(f, "r") as json_file:
            self._default_data = {
                item.get("market_code") or item.get("underlying_symbol"): self._process_future_info_item(
                    item
    ...
    @lru_cache(1024)
    def get_future_info(self, market_code, underlying_symbol):
        # type: (str, str) -> FuturesTradingParameters
        custom_info = self._custom_data.get(market_code) or self._custom_data.get(underlying_symbol)
        info = self._default_data.get(market_code) or self._default_data.get(underlying_symbol)
        if custom_info:
            info = copy(info) or {}
            info.update(custom_info)
        elif not info:
            raise NotImplementedError(_("unsupported future instrument {}").format(market_code))
    ...
    def __init__(self, instruments, instrument_type):
        ...
        for ins in instruments:
            if ins.type != instrument_type:
                continue
            self._instruments[ins.market_code] = ins
            self._sym_id_map[ins.security_name] = ins.market_code
    ...
    def get_instruments(self, id_or_syms):
        ...
        for i in id_or_syms:
            if i in self._instruments:
                ...
                market_code.add(self._sym_id_map[i])
        return (self._instruments[i] for i in market_code)
...
class ShareTransformationStore(object):
    ...
    def get_share_transformation(self, market_code):
        try:
            transformation_data = self._share_transformation[market_code]
        except KeyError:
            return
...
class DayBarStore(AbstractDayBarStore):
    ...
    def get_date_range(self, market_code):
        with h5_file(self._path) as h5:
            try:
                data = h5[market_code]
                return data[0]['orig_time'], data[-1]['orig_time']
            except KeyError:
...
class DateSet(AbstractDateSet):
    ...
    def contains(self, market_code, dates):
        date_set = self.get_days(market_code)
        if not date_set:
            return None
