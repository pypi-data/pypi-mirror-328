...
from deepquant.quest.utils.datetime_func import convert_date_to_date_int, convert_date_to_int, convert_dt_to_date_str, convert_int_to_date_str, convert_data_str_to_int
...
class DayBarTask(ProgressedTask):
    def __init__(self, market_code, variety='stock'):
        ...
        try:
            ...
            with h5:
                ...
                while True:
                    ...
                    df = yhdatac.get_price(market_code, convert_int_to_date_str(START_DATE),
                                           convert_dt_to_date_str(datetime.date.today()), '1d',
                                            adjust_type='none', fields=fields, expect_df=True, variety=self._variety)
                    if not (df is None or df.empty):
                        df.reset_index(inplace=True)
                        df['orig_time'] = [convert_data_str_to_int(d) for d in df['orig_time']]
                        df['volume'] = [float(d) for d in df['volume']]
                        #del df['orig_time']
...
class UpdateDayBarTask(DayBarTask):
    ...
    def __call__(self, path, fields, **kwargs):
        ...
        if need_recreate_h5:
            yield from GenerateDayBarTask(self._market_code, self._variety)(path, fields, **kwargs)
        else:
            h5 = None
            try:
                h5 = h5py.File(path, 'a')
                ...
                for market_code in self._market_code:
                    ...
                    if market_code in h5 and not is_pre:
                        try:
                            last_date = int(h5[market_code]['orig_time'][-1] // 1000000)
                        except OSError:
                            system_log.error("File {} update failed, if it is using, please update later, "
                                            "or you can delete then update again".format(path))
                            sval.value = False
                        ...
                        start_date = START_DATE
                    df = yhdatac.get_price(market_code, convert_int_to_date_str(start_date), convert_int_to_date_str(END_DATE), '1d',
                    ...
                    if not (df is None or df.empty):
                        ...
                        df['volume'] = [float(d) for d in df['volume']]
                        #del df['date']
                        df.set_index('orig_time', inplace=True)
...
def update_bundle(path, create, enable_compression=False, concurrency=1):
    ...
    day_bar_args = (
        ("stocks.h5", yhdatac.all_instruments('stock').market_code.tolist(), STOCK_FIELDS, 'stock'),
        ("indexes.h5", yhdatac.all_instruments('index').market_code.tolist(), INDEX_FIELDS, 'index'),
        ("futures.h5", yhdatac.all_instruments('Future').market_code.tolist(), FUTURES_FIELDS, 'Future'),
        ("funds.h5", yhdatac.all_instruments('FUND').market_code.tolist(), FUND_FIELDS, 'FUND'),
    )
    ...
    import pandas as pd
    print(os.getcwd())
    df = pd.read_csv("D:\\Workspace\\python_space\\onequantsdk_pkg\\deepquant\\quest\\data\\historysymbollist.csv")
    day_bar_args = (("indexes_yh2.h5", df['market_code'].tolist(), INDEX_FIELDS, 'index'),)
    ...
    with ProgressedProcessPoolExecutor(
        ...
        for file, market_code, field, variety in day_bar_args:
            executor.submit(_DayBarTask(market_code, variety), os.path.join(path, file), field, **kwargs)
