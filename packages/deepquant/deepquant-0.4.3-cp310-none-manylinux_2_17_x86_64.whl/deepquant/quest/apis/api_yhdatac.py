...
from deepquant.quest.apis.names import (
...
from deepquant.quest.utils.logger import user_log
from deepquant.quest.utils import check_items_in_container


try:
    import deepquant.quest.datac as yhdatac
    from deepquant.quest.datac.services.future import _get_future_factors_df
    ...
    class DummyYHDatac:
        ...
        def __call__(self, *args, **kwargs):
            raise RuntimeError(_('yhdatac is not available, extension apis will not function properly'))


    yhdatac = DummyYHDatac()
...
def index_components(market_code, date=None):
    ...
    market_code = assure_market_code(market_code)
    return yhdatac.index_components(market_code, date=date)


@export_as_api
@apply_rules(verify_that('date').is_valid_date(ignore_none=True))
def index_weights(market_code, date=None):
    # type: (str, Optional[Union[str, datetime.date]]) -> pd.Series
    """
    获取T-1日的指数权重

    :param market_code: 指数
    ...
    market_code = assure_market_code(market_code)
    return yhdatac.index_weights(market_code, date)
...
def concept(*concept_names):
    ...
    :param concept_names: 概念名称。可以从概念列表中选择一个或多个概念填写, 可以通过 deepquant.quest.datac.concept_list() 获取概念列表
    :return: 属于该概念的股票 market_code
    ...
    return yhdatac.concept(*concept_names, date=dt)
...
def get_margin_stocks(exchange=None, margin_type="all"):
    ...
    if margin_type == "all":
        symbols.extend(yhdatac.get_margin_stocks(trade_dt, exchange, margin_type="stock", market="cn"))
        symbols.extend(yhdatac.get_margin_stocks(trade_dt, exchange, margin_type="cash", market="cn"))
    elif margin_type in ["cash", "stock"]:
        symbols.extend(yhdatac.get_margin_stocks(trade_dt, exchange, margin_type=margin_type, market="cn"))
    else:
...
@apply_rules(verify_that('market_code').are_valid_instruments(),
...
def get_price(
        market_code,  # type: Union[str, Iterable[str]]
        start_date,  # type: Union[datetime.date, str]
        end_date=None,  # type: Optional[Union[datetime.date, datetime.orig_time, str]]
    ...
    :param market_code: 合约代码，合约代码，可传入market_code, market_code list, symbol, symbol list
    ...
    :param skip_suspended: 是否跳过停牌数据。默认为False，不跳过，用停牌前数据进行补齐。True则为跳过停牌期。注意，当设置为True时，函数market_code只支持单个合约传入
    :param expect_df: 是否期望始终返回 DataFrame。pandas 0.25.0 以上该参数应设为 True，以避免因试图构建 Panel 产生异常

    当 expect_df 为 False 时，返回值的类型如下

        *   传入一个market_code，多个fields，函数会返回一个pandas DataFrame
        *   传入一个market_code，一个field，函数会返回pandas Series
        *   传入多个market_code，一个field，函数会返回一个pandas DataFrame
        *   传入多个market_code，函数会返回一个pandas Panel
    ...
    ..  code-block:: python3
        ...
        #[Out]
        #open    close    high    low    total_turnover    volume    high_limited    low_limited
    ...
    return yhdatac.get_price(market_code, start_date, end_date,
                             frequency=frequency, fields=fields, adjust_type=adjust_type,
                             skip_suspended=skip_suspended, expect_df=expect_df, variety=variety)
...
def get_securities_margin(
    ...
    :param market_code: 可输入market_code, market_code list, symbol, symbol list。另外，输入'XSHG'或'sh'代表整个上证整体情况；'XSHE'或'sz'代表深证整体情况
    ...
    当 expect_df 为 False 时，返回值的类型如下：
        ...
        *   多个market_code，单个field的时候返回DataFrame，index为date，column为market_code
        *   单个market_code，多个fields的时候返回DataFrame，index为date，column为fields
        *   单个market_code，单个field返回Series
        *   多个market_code，多个fields的时候返回DataPanel Items axis为fields Major_axis axis为时间戳 Minor_axis axis为market_code
    ...
    *   获取50ETF融资偿还额情况
    ...
    return yhdatac.get_securities_margin(market_code, start_dt, dt, fields=fields, expect_df=expect_df)
...
def get_shares(
        market_code,  # type: Union[str, List[str]]
        count=1,  # type: Optional[int]
        fields=None,  # type: Optional[str]
        expect_df=False  # type: Optional[bool]
):  # type: (...) -> Union[pd.DataFrame, pd.Series]
    """
    :param market_code: 可输入 market_code, market_code list, symbol, symbol list
    :param count: 回溯获取的数据个数。默认为当前能够获取到的最近的数据
    :param fields: 期望返回的字段，默认为所有字段。见下方列表
    :param expect_df: 是否期望始终返回 DataFrame。pandas 0.25.0 以上该参数应设为 True，以避免因试图构建 Panel 产生异常
    ...
    return yhdatac.get_shares(market_code, start_dt, dt, fields=fields, expect_df=expect_df)
...
def get_turnover_rate(
    ...
    当 expect_df 为 False 时，返回值的类型如下：
        ...
        *   如果只传入一个market_code，多个fields，返回 `pandas.DataFrame`
        *   如果传入market_code list，并指定单个field，函数会返回一个 `pandas.DataFrame`
        *   如果传入market_code list，并指定多个fields，函数会返回一个 `pandas.Panel`
   ...
   ..  code-block:: python3
    ...
    return yhdatac.get_turnover_rate(market_code, start_dt, dt, fields=fields, expect_df=expect_df)
...
def get_price_change_rate(
    ...
    当 expect_df 为 False 时，返回值的类型如下：
        ...
        *  传入多个market_code，函数会返回pandas DataFrame
        *  传入一个market_code，函数会返回pandas Series
    ...
    ..  code-block:: python3
        ...
        # [Out]
        # 2016-06-01 15:30:00.00  INFO   market_code  000001.XSHE  510050.XSHG
        #                                date
        #                                2016-05-31        0.026265     0.033964
        # 2016-06-02 15:30:00.00  INFO   market_code  000001.XSHE  510050.XSHG
        #                                date
    ...
    return yhdatac.get_price_change_rate(market_code, start_date, end_date, expect_df=expect_df)
...
def get_factor(
    ...
    :param market_code:  合约代码，可传入market_code, market_code list
    :param factors: 因子名称，可查询 deepquant.quest.datac.get_all_factor_names() 得到所有有效因子字段
    ...
    return yhdatac.get_factor(market_code, factors,
...
def calc_factor(
    ...
    :param market_code:  合约代码，可传入market_code, market_code list
    :param source_code: 计算函数的代码 inspect.getsource(xxx)
    :param factor_name: 自定义因子名称，
    :param count: 获取多少个交易日的数据
    :return: DataFrame
    """
    env = Environment.get_instance()
    ...
    return yhdatac.calc_factor(market_code, source_code,factor_name,
                              start_date=start_date, end_date=end_date,frequency=frequency)

@export_as_api
def get_industry(industry, source=12):
    # type: (str, Optional[str]) -> List[str]
    """
    通过传入行业名称、行业指数代码或者行业代号，拿到 T 日指定行业的股票列表

    :param industry: 行业名字
    :param source: 默认为中证监会(12)，可选万得到(gildata)，中信
    :return: list of market_code

    """
    env = Environment.get_instance()
    return yhdatac.get_industry(industry, source, env.calendar_dt)


@export_as_api
def get_instrument_industry(market_code, source='14', level=1):
    ...
    if isinstance(market_code, six.string_types):
        ...
        market_code = [assure_market_code(i) for i in market_code]
    env = Environment.get_instance()
    return yhdatac.get_instrument_industry(market_code, source, level, env.calendar_dt)


@export_as_api
@apply_rules(verify_that('count').is_instance_of(int).is_greater_than(0),
             verify_that('fields').are_valid_fields(VALID_STOCK_CONNECT_FIELDS, ignore_none=True))
def get_stock_connect(market_code, count=1, fields=None, expect_df=False):
    # type: (Union[str, List[str]], Optional[int], Optional[str], Optional[bool]) -> pd.DataFrame
    """
    获取截止T-1日A股股票在香港上市交易的持股情况

    :param market_code: 合约代码，可传入market_code, market_code list，这里输入的是A股编码
    :param count: 向前获取几个交易日
    :param fields: 持股量（shares_holding），持股比例（holding_ratio），默认为所有字段
    :param expect_df: 默认为False。当设置为True时，总是返回 multi-index DataFrame。pandas 0.25.0 以上该参数应设为 True，以避免因试图构建 Panel 产生异常

    当 expect_df 为 False 时，返回值的类型如下：
        *  多个market_code，多个fields的时候返回pandas Panel
        *  单个market_code，多个fields的时候返回pandas DataFrame
        *  单个market_code，单个field返回pandas Series
    ...
    return yhdatac.get_stock_connect(market_code, start_date, end_date,
                                     fields=fields, expect_df=expect_df)


@export_as_api
@apply_rules(verify_that('market_code').is_valid_instrument(),
...
def current_performance(market_code, info_date=None, quarter=None, interval='1q', fields=None):
    # type: (str, Optional[str], Optional[str], Optional[str], Optional[str, List[str]]) -> pd.DataFrame
    """
    默认返回给定的 market_code 当前最近一期的快报数据

    :param market_code: 合约代码
    ...
    if info_date is None and quarter is None:
        info_date = dt
    return yhdatac.current_performance(market_code, info_date, quarter, interval, fields)
...
def get_dominant_future(underlying_symbol, rule=0):
    ...
    ret = yhdatac.get_dominant_future(underlying_symbol, dt, dt, rule)
...
def _econ_get_reserve_ratio(reserve_type='all', n=1):
    ...
    """
    df = yhdatac.econ.get_reserve_ratio(reserve_type)
...
def _econ_get_money_supply(n=1):
    ...
    df = yhdatac.econ.get_money_supply(start_date, end_date)
...
def _futures_get_dominant(underlying_symbol, rule=0):
    ...
    """
    dt = Environment.get_instance().trading_dt.date()
    ret = yhdatac.futures.get_dominant(underlying_symbol, dt, dt, rule)
...
def _futures_get_member_rank(which, count=1, rank_by='short'):
    ...
    return yhdatac.futures.get_member_rank(which, start_date=start_date, end_date=end_date, rank_by=rank_by)
...
def _futures_get_warehouse_stocks(underlying_symbols, count=1):
    ...
    return yhdatac.futures.get_warehouse_stocks(underlying_symbols, start_date=start_date, end_date=end_date)
...
DOMINANT_PRICE_ADJUST_FIELDS = [
    'open_price', 'high_price', 'low_price', 'close_price', 'last_price', 'high_limited', 'low_limited', 'settlement', 'prev_settlement', 'prev_close',
...
def _futures_get_dominant_price(
    ...
    df = yhdatac.get_price(
        market_code=obs, start_date=start_date, end_date=end_date, frequency=frequency, adjust_type='none',
    ...
    df['underlying_symbol'] = df['market_code'].str[:-2]
    ...
    if frequency != '1d':
        df = df.reset_index().set_index(['underlying_symbol', 'orig_time'])
...
def get_fundamentals(query, entry_date=None, interval='1d', report_quarter=False, expect_df=False, **kwargs):
    ...
    result = yhdatac.get_fundamentals(query, query_date, interval, report_quarter=report_quarter, expect_df=expect_df)
    if result is None:
        return pd.DataFrame()
...
def get_financials(query, quarter=None, interval='4q', expect_df=False):
    ...
    for d in query.column_descriptions:
        if d['name'] == 'announce_date':
            include_date = True
    if not include_date:
        query = query.add_column(yhdatac.fundamentals.announce_date)

    result = yhdatac.get_financials(query, quarter, interval, expect_df=expect_df)
    if result is None:
        return pd.DataFrame()
    if isinstance(result, pd.Series):
        ...
        d = dict()
        for market_code in result.minor_axis:
            df = result.minor_xs(market_code)
...
def get_pit_financials(fields, quarter=None, interval=None, market_code=None, if_adjusted='all'):
    if quarter is None:
        valid = True
    else:
        valid = isinstance(quarter, six.string_types) and quarter[-2] == 'q'
    ...
    if quarter is None or quarter > default_quarter:
        quarter = default_quarter
    result = yhdatac.get_pit_financials(fields, quarter, interval, market_code, if_adjusted,
    ...
    if if_adjusted == 'ignore':
        ...
        result = result.groupby(['market_code', 'end_date'], as_index=False).fillna(method='ffill')
        result = result.drop(['info_date', 'if_adjusted'], axis=1)
        result = result.drop_duplicates(['market_code', 'end_date'], keep='last')
        result = result.set_index(['market_code', 'end_date']).sort_index()
    return result


@export_as_api
@apply_rules(verify_that('statements').is_in(['all', 'latest'], ignore_none=True))
def get_pit_financials_ex(market_code, fields, count, statements='latest'):
    ...
    for market_code in market_code:
        ...
        if env.calendar_dt > instrument.de_listed_date:
            ...
            active_list.append(market_code)
    ...
    def _get_data(symbol_list, start_dt):
        ...
        result = yhdatac.get_pit_financials_ex(
            fields=fields, start_quarter=start_quarter, end_quarter=end_quarter, market_code=symbol_list,
    ...
    if isinstance(result, pd.DataFrame):
        result_list = [group_df.iloc[-count:] for _, group_df in result.groupby("market_code")]

    # 获取退市的数据
    for instrument in de_listed_list:
        result = _get_data([instrument.market_code], instrument.de_listed_date)
        if isinstance(result, pd.DataFrame):
