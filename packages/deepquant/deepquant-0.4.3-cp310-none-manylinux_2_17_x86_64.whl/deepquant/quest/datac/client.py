...
URI_PATTERN = r"^(?P<schema>tcp|deepquant.quest.datac?)://(?P<username>\S+?):(?P<password>\S+)@(?P<hostname>\S+):(?P<port>\d+$)"
...
class DummyClient:
    ...
    def execute(self, *args, **kwargs):
        raise RuntimeError("deepquant.quest.datac is not initialized")

    def reset(self):
        pass

    def info(self):
        print('deepquant.quest.datac is not initialized')
...
def reset(flush_cache=False, enable_bjse=None):
    """
    reset all connections. this function is helpful when you fork from an connected client.

    :param flush_cache: boolean, flush all ttl cache, default False
    :param enable_bjse: bool, 默认不设置
    """

    _CLIENT.reset()
    _CLIENT.PID = os.getpid()
    if flush_cache:
        from deepquant.quest.datac.decorators import ttl_cache_clear
        ttl_cache_clear()
    if enable_bjse is not None:
        global _ENABLE_BJSE
        _ENABLE_BJSE = enable_bjse


def initialized():
    """
    is deepquant.quest.datac already initialized ?
...
def _set_sock_factory(proxy_info):
    from deepquant.quest.datac.connection import Connection
    if proxy_info is None:
        ...
        try:
            ...
            raise RuntimeError("PySocks is required when use proxy. You can install it using `pip install PySocks` or "
                               "`pip install deepquant.quest.datac[proxy]`")
...
def init(username=None, password=None, addr=("rqdatad-pro.ricequant.com", 16011), *_, **kwargs):
    """initialize deepquant.quest.datac.

    deepquant.quest.datac connection is thread safe but not fork safe. Every thread have their own connection by
    default. you can set param 'use_pool' to True to use a connection pool instead.

    NOTE: if you are using deepquant.quest.datac with python < 3.7 in a multi-process program, remember to call
    reset in child process.

    Environment Variable:
    deepquant.quest.datac_CONF / deepquant.quest.datac2_CONF: When init called with no argument, the value in deepquant.quest.datac2_CONF is used as uri, then deepquant.quest.datac_CONF.
    deepquant.quest.datac_PROXY: proxy info, e.g. http://username:password@host:port
    ...
    :kwyword enable_bjse: enable bjse data default False
    """
    extra_args = {k: kwargs.pop(k) for k in ('timeout', 'connect_timeout') if k in kwargs}
    proxy_info = kwargs.pop('proxy_info', None) or _parse_proxy_url(os.environ.get('deepquant.quest.datac_PROXY'))
    _set_sock_factory(proxy_info)

    logging.getLogger("deepquant.quest.datac").disabled = not kwargs.pop('debug', False)
    ...
    if kwargs.pop("use_zstd", False):
        ...
        from deepquant.quest.datac import connection
        from deepquant.quest.datac.share.protocol import COMPRESSION_METHOD
        connection.DEFAULT_COMPRESSION_METHOD = COMPRESSION_METHOD.ZSTD
    uri = kwargs.pop("uri", None)
    if not (username or password or uri):
        uri = os.environ.get("deepquant.quest.datac2_CONF") or os.environ.get("deepquant.quest.datac_CONF")
    ...
    if _CLIENT is not _DUMMY:
        warnings.warn("deepquant.quest.datac is already inited. Settings will be changed.", stacklevel=0)
        reset()
    ...
    auth_info = {'username': username, 'password': password, 'ver': deepquant.quest.datac.__version__}
    ...
    def show_info():
        print('deepquant.quest.datac version:', deepquant.quest.datac.__version__)
    ...
    global _ENABLE_BJSE
    _ENABLE_BJSE = kwargs.pop('enable_bjse', False)
