...
class AnalyserMod(AbstractMod):
        ...
        for market_code, weight in self._benchmark:
            ...
            if len(bars) == len(trading_dates) + 1:
                ...
                if len(bars) == 0:
                    ...
                    (available_s, available_e) = (convert_int_to_date(bars[0]['orig_time']).date(), convert_int_to_date(bars[-1]['close_price']).date())
                raise RuntimeError(
    ...
    def _to_position_record(self, date, market_code, long, short):
        ...
        data = {
            'market_code': market_code,
            'symbol': self._symbol(market_code),
            'date': date,
        }
    ...
    def _to_trade_record(self, trade):
        return {
            ...
            'symbol': self._symbol(trade.market_code),
