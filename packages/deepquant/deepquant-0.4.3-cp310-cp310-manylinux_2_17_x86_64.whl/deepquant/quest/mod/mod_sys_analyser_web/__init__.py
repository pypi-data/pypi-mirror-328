...
from deepquant.quest import cli, inject_run_param

__config__ = {
    # 策略基准，该基准将用于风险指标计算和收益曲线图绘制
    #   若基准为单指数/股票，此处直接设置 market_code，如："000300.SH"
    #   若基准为复合指数，则需传入 market_code 和权重构成的字典，如：{"000300.SH": 0.2. "000905.SH": 0.8}
    "benchmark": None,
...
def load_mod():
    from .mod import AnalyserWebMod
    return AnalyserWebMod()
...
"""
cli_prefix = "mod__sys_analyser_web__"

inject_run_param(click.Option(
    ('--report', 'mod__sys_analyser_web__report_save_path'),
    type=click.Path(writable=True),
    help=_("[sys_analyser_web] save report")
))
inject_run_param(click.Option(
    ('-o', '--output-file', 'mod__sys_analyser_web__output_file'),
    type=click.Path(writable=True),
    help=_("[sys_analyser_web] output result pickle file")
))
inject_run_param(click.Option(
    ('-p', '--plot', 'mod__sys_analyser_web__plot'),
    default=None, is_flag=False, flag_value="default",
    help=_("[sys_analyser_web] plot result")
))
inject_run_param(click.Option(
    ('--plot-save', 'mod__sys_analyser_web__plot_save_file'),
    default=None,
    help=_("[sys_analyser_web] save plot to file")
))
inject_run_param(click.Option(
    ("-bm", "--benchmark", cli_prefix + "benchmark"),
    type=click.STRING,
    help=_("[sys_analyser_web] market_code of benchmark")
))
inject_run_param(click.Option(
    ('--plot-open-close-points', cli_prefix + "plot_config__open_close_points"),
    is_flag=True, default=None,
    help=_("[sys_analyser_web] show open close points on plot"),
))
inject_run_param(click.Option(
    ("--plot-weekly-indicators", cli_prefix + "plot_config__weekly_indicators"),
    is_flag=True, default=None,
    help=_("[sys_analyser_web] show weekly indicators and return curve on plot")
))
...
@cli.command(help=_("[sys_analyser_web] Generate report from strategy output file"))
@click.argument('result_pickle_file_path', type=click.Path(exists=True), required=True)
