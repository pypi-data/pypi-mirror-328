...
class SimulationEventSource(AbstractEventSource):
    ...
    def _get_future_trading_minutes(self, trading_date):
        ...
        for market_code in universe:
            if self._env.get_account_type(market_code) == DEFAULT_ACCOUNT_TYPE.STOCK:
                continue
            trading_minutes.update(self._env.data_proxy.get_trading_minutes_for(market_code, trading_date))
                ...
                while True:
                    for tick in data_proxy.get_merge_ticks(self._get_universe(), date, last_dt):
                        ...
                        if last_tick is None:
                            ...
                            """
                            if self._env.get_instrument(tick.market_code).type == INSTRUMENT_TYPE.FUTURE:
                                yield Event(
                                    EVENT.BEFORE_TRADING,
                                    calendar_dt=calendar_dt - timedelta(minutes=30),
                                    trading_dt=trading_dt - timedelta(minutes=30),
