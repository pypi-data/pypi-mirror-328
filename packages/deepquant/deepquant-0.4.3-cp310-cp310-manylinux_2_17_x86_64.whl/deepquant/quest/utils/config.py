...
from deepquant.quest.const import RUN_TYPE, PERSIST_MODE, COMMISSION_TYPE
from deepquant.quest.utils import YhAttrDict, logger
from deepquant.quest.utils.i18n import gettext as _, set_locale
from deepquant.quest.utils.dict_func import deep_update
from deepquant.quest.utils.logger import system_log
from deepquant.quest.mod.utils import mod_config_value_parse
...
def user_mod_conf_path():
    return os.path.join(os.path.expanduser(yhalpha_path), 'mod_config.yml')


def get_mod_conf():
    base = load_yaml(default_mod_config_path)
    user_mod_conf = os.path.join(os.path.expanduser(yhalpha_path), 'mod_config.yml')
...
def user_config():
    return load_config_from_folder(yhalpha_path)
...
def parse_config(config_args, config_path=None, click_type=False, source_code=None, user_funcs=None):
    ...
    if config.base.data_bundle_path is None:
        config.base.data_bundle_path = os.path.join(os.path.expanduser(yhalpha_path), "bundle")
...
def parse_init_positions(positions):
    ...
    for s in positions.split(','):
        try:
            market_code, quantity = s.split(':')
        except ValueError:
            raise RuntimeError(_(u"invalid init position {}, should be in format 'market_code:quantity'").format(s))

        try:
            result.append((market_code, float(quantity)))
        except ValueError:
            raise RuntimeError(_(u"invalid quantity for instrument {market_code}: {quantity}").format(
                market_code=market_code, quantity=quantity))
    return result
