...
from deepquant.quest.utils.exception import CustomError, CustomException
from deepquant.quest.const import EXC_TYPE, INSTRUMENT_TYPE, DEFAULT_ACCOUNT_TYPE, SIDE, POSITION_EFFECT, POSITION_DIRECTION
from deepquant.quest.utils.datetime_func import TimeRange
from deepquant.quest.utils.i18n import gettext as _
from deepquant.quest.utils.functools import lru_cache


class YhAttrDict(object):
    ...
    def update(self, other):
        YhAttrDict._update_dict_recursive(self, other)
    ...
    def _update_dict_recursive(target, other):
        if isinstance(other, YhAttrDict):
            other = other.__dict__
        target_dict = target.__dict__ if isinstance(target, YhAttrDict) else target

        for k, v in other.items():
            if isinstance(v, YhAttrDict):
                v = v.__dict__
            if isinstance(v, Mapping):
                r = YhAttrDict._update_dict_recursive(target_dict[k] if target_dict.get(k) else {}, v)
                target_dict[k] = r
            else:
...
YHDATAC_DEFAULT_ADDRESS = "rqdatad-pro.ricequant.com:16011"


def init_yhdatac_env(uri):
    if uri is None:
        return

    if '@' not in uri:
        uri = "tcp://{}@{}".format(uri, YHDATAC_DEFAULT_ADDRESS)

    if not re.match(r"\w*://.+:.+@.+:\d+", uri):
        raise ValueError('invalid yhdatac uri. use user:password or tcp://user:password@ip:port')

    os.environ['YHDATAC2_CONF'] = uri
