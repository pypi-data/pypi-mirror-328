...
from deepquant.quest.utils.functools import instype_singledispatch
...
@instype_singledispatch
def order(market_code, quantity, price_or_style=None, price=None, style=None):
    ...
    """
      ...
      如果 market_code 是股票，等同于调用 order_shares

      如果 market_code 是期货，则进行智能下单:

          *   quantity 表示调仓量
          *   如果 quantity 为正数，则先平 Sell 方向仓位，再开 Buy 方向仓位
          *   如果 quantity 为负数，则先平 Buy 反向仓位，再开 Sell 方向仓位

      :param market_code: 下单标的物
      :param quantity: 调仓量
      :param price_or_style: 默认为None，表示市价单，可设置价格，表示限价单，也可以直接设置订单类型，有如下选项：MarketOrder、LimitOrder、
                            TWAPOrder、VWAPOrder
...
@apply_rules(verify_that("quantity").is_number(), *common_rules)
@instype_singledispatch
def order_to(market_code, quantity, price_or_style=None, price=None, style=None):
    # type: (Union[str, Instrument], int, PRICE_OR_STYLE_TYPE, Optional[float], Optional[OrderStyle]) -> List[Order]
    """
    全品种通用智能调仓函数
    ...
    如果 market_code 是股票，则表示仓位调整到多少股

    如果 market_code 是期货，则进行智能调仓:
...
def exercise(id_or_ins, amount, convert=False):
    ...
    """
        ...
        :param id_or_ins: 行权合约，market_code 或 Instrument 对象
