...
from deepquant.quest.const import INSTRUMENT_TYPE, TRADING_CALENDAR_TYPE
from deepquant.quest.interface import AbstractDataSource
from deepquant.quest.model.instrument import Instrument
from deepquant.quest.utils.datetime_func import (convert_date_to_int, convert_int_to_date, convert_int_to_datetime)
...
from deepquant.quest.utils.converttools import convert_exchange_to_wind
from deepquant.quest.environment import Environment
from deepquant.quest.data.base_data_source.adjust import FIELDS_REQUIRE_ADJUSTMENT, adjust_bars
from deepquant.quest.data.base_data_source.storage_interface import (AbstractCalendarStore, AbstractDateSet,
                                                                     AbstractDayBarStore, AbstractDividendStore,
                                                                     AbstractInstrumentStore)
from deepquant.quest.data.base_data_source.storages import (DateSet, DayBarStore, DividendStore,
...
class BaseDataSource(AbstractDataSource):
    ...
    def __init__(self, path: str, custom_future_info: dict, *args, **kwargs) -> None:
        ...
        with open(_p('instruments.pk'), 'rb') as f:
            for i in pickle.load(f):
                if i["type"] == "Future" and Instrument.is_future_continuous_contract(i["market_code"]):
    ...
    def get_dividend(self, instrument):
        ...
        return dividend_store.get_dividend(instrument.market_code)

    def get_trading_minutes_for(self, market_code, trading_dt):
    ...
    def get_share_transformation(self, market_code):
        return self._share_transformation.get_share_transformation(market_code)

    def is_suspended(self, market_code, dates):
        # type: (str, Sequence[DateLike]) -> List[bool]
        for date_set in self._suspend_days:
            result = date_set.contains(market_code, dates)
    ...
    def is_st_stock(self, market_code, dates):
        result = self._st_stock_days.contains(market_code, dates)
        return result if result is not None else [False] * len(dates)

    @lru_cache(None)
    def _all_day_bars_of(self, instrument):
        return self._day_bars[instrument.type].get_bars(convert_exchange_to_wind(instrument.market_code))
    ...
    def get_bar(self, instrument, dt, frequency):
        ...
        #dt = dt + 93000  ###just for test
        flag = ""
        if 'datetime' in bars.dtype.names:
            pos = bars['datetime'].searchsorted(dt)
            flag = 'datetime'
        else:
            pos = bars['orig_time'].searchsorted(dt)
            flag = 'orig_time'

        if pos >= len(bars) or bars[flag][pos] != dt:
            return None

        return bars[pos]

    OPEN_AUCTION_BAR_FIELDS = ["orig_time", "open_price", "high_limited", "low_limited", "volume", "total_turnover"]
    ...
    def get_ex_cum_factor(self, market_code):
        return self._ex_cum_factor.get_factors(market_code)
    ...
    def resample_week_bars(self, bars, bar_count, fields):
        ...
        df_bars['orig_time'] = df_bars.apply(lambda x: convert_int_to_datetime(x['orig_time']), axis=1)
        df_bars = df_bars.set_index('orig_time')
        nead_fields = fields
        ...
        df_bars['orig_time'] = df_bars.apply(lambda x: np.uint64(convert_date_to_int(x['orig_time'].date())), axis=1)
        df_bars = df_bars.set_index('orig_time')
    ...
    def history_bars(self, instrument, bar_count, frequency, fields, dt,
        ...
        if frequency == '1w':
            if include_now:
                dt = np.uint64(convert_date_to_int(dt))
                i = bars['orig_time'].searchsorted(dt, side='right')
            else:
                monday = dt - timedelta(days=dt.weekday())
                monday = np.uint64(convert_date_to_int(monday))
                i = bars['orig_time'].searchsorted(monday, side='left')

            left = i - bar_count * 5 if i >= bar_count * 5 else 0
            bars = bars[left:i]

            if adjust_type == 'none' or instrument.type in {'Future', 'index'}:
                # 期货及指数无需复权
            ...
            adjust_bars_date = adjust_bars(bars, self.get_ex_cum_factor(instrument.market_code),
        ...
        bars = adjust_bars(bars, self.get_ex_cum_factor(instrument.market_code),
                           fields, adjust_type, adjust_orig)
    ...
    def get_split(self, instrument):
        ...
        return splilt_store.get_factors(instrument.market_code)
    ...
    def available_data_range(self, frequency):
        ...
        if frequency in ['tick', '1d']:
            s, e = self._day_bars[INSTRUMENT_TYPE.INDX].get_date_range('000001.SH') # just for test
    ...
    def get_futures_trading_parameters(self, instrument: Instrument, dt: datetime.date) -> FuturesTradingParameters:
        return self._future_info_store.get_future_info(instrument.market_code, instrument.underlying_symbol)
    ...
    def get_algo_bar(self, id_or_ins, start_min, end_min, dt):
        raise NotImplementedError("open source yhalpha not support algo order")

    def get_open_auction_volume(self, instrument, dt):
        # type: (Instrument, datetime.orig_time) -> float
