...
from deepquant.quest.const import ORDER_STATUS, ORDER_TYPE, SIDE, POSITION_EFFECT, POSITION_DIRECTION, ALGO
from deepquant.quest.utils import id_gen, decimal_rounding_floor, get_position_direction
from deepquant.quest.utils.repr import property_repr, properties
...
class Order(object):
    ...
    def set_state(self, d):
        ...
        self._market_code = d['market_code']
    ...
    @classmethod
    def __from_create__(cls, market_code, quantity, side, style, position_effect, **kwargs):
        ...
        order._market_code = market_code
        order._side = side
        ...
        if isinstance(style, LimitOrder):
            if env.config.base.round_price:
                tick_size = env.data_proxy.get_tick_size(market_code)
            ...
            algo_price, _ = env.data_proxy.get_algo_bar(market_code, style, env.calendar_dt)
            order._frozen_price = env.get_last_price(market_code) if np.isnan(algo_price) else algo_price
            order._type = ORDER_TYPE.ALGO
        else:
            order._frozen_price = env.get_last_price(market_code)
