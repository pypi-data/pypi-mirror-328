...
from deepquant.quest.const import ORDER_TYPE, POSITION_EFFECT
...
class PriceValidator(AbstractFrontendValidator):
    ...
    def validate_submission(self, order: Order, account: Optional[Account] = None) -> Optional[str]:
        ...
        high_limited = round(self._env.price_board.get_high_limited(order.market_code), 4)
        if order.price > high_limited:
            reason = _(
                "Order Creation Failed: limit order price {limit_price} is higher "
                "than limit up {high_limited}, market_code={market_code}"
            ).format(
        ...
        low_limited = round(self._env.price_board.get_low_limited(order.market_code), 4)
        if order.price < low_limited:
            reason = _(
                "Order Creation Failed: limit order price {limit_price} is lower "
                "than limit down {low_limited}, market_code={market_code}"
            ).format(
                market_code=order.market_code,
                limit_price=order.price,
