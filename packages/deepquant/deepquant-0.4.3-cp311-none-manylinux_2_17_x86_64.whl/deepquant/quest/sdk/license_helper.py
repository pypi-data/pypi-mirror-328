...
from deepquant.quest.datac.errors import AuthenticationFailed
from deepquant.quest.sdk.const import PERMISSIONS_INFO_URL, RQDATAC_DEFAULT_ADDRESS
from deepquant.quest.sdk.script_update import update_bash_file
from tabulate import tabulate


def format_rqdatac_uri(uri):
    """
    整理deepquant.quest.datac_uri 并返回
    uri可能有以下三种方式
    * 纯license
    * 手机号:密码
    * 完整的deepquant.quest.datac_uri
    """
    if uri is None:
    ...
    if '@' not in uri:
        ...
        uri = "tcp://{}@{}".format(uri, deepquant.quest.datac_DEFAULT_ADDRESS)

    if not re.match(r"\w*://.+:[^\n\r/]+@.+:\d+", uri):
        raise ValueError('无效的 yhdatac_uri ,格式应为 user:password 或者 tcp://user:password@ip:port')
    return uri
...
def get_permissions_info(rqdtac_uri):
    ...
    if not proxy_uri:
        resp = requests.get(PERMISSIONS_INFO_URL, params={"yhdatac_uri": rqdtac_uri})
    else:
        resp = requests.get(PERMISSIONS_INFO_URL, params={"yhdatac_uri": rqdtac_uri}, proxies={
            'http': proxy_uri,
            'https': proxy_uri
        })
    res = resp.json()
...
def get_yhdata_info(uri=None, license_name='RQDATA LICENSE'):
    """从rqdata获取账户的相关信息"""
    try:
        deepquant.quest.datac.init(uri=uri)
        _info = deepquant.quest.datac.user.get_quota()
    except(AuthenticationFailed, ValueError) as err:
        click.echo("{}：{}".format(license_name, uri))
        click.echo("{} 不可用，原因：【{}】；请使用【yhsdk config --yhdatac】 命令更新".format(license_name, err.args[0]))
    else:
        return _info


def print_license_info(uri_license=os.environ.get('RQSDK_LICENSE') or os.environ.get('YGDATAC2_CONF'),
                       uri_rqdata=os.environ.get('YGDATAC2_CONF')):
    """展示 rqdata_uri 账户的相关信息"""
    license_info = get_license_info(uri_license)
    rqdata_info = get_yhdata_info(uri_rqdata)
...
def set_to_environ(variables, value):
    ...
    :param value: string
    """
    if set(variables) == {"YGDATAC_CONF", "RQSDK_LICENSE"}:
        var_name = "license"
    elif variables == ["YGDATAC2_CONF"]:
        var_name = "YGDATAC license"
    else:
        var_name = "代理"
...
def license_console():
    """license 交互环境"""
    # print
    uri_license = os.environ.get('YHSDK_LICENSE') or os.environ.get('YGDATAC2_CONF')
    if uri_license:
    ...
    while True:
        ...
        rqdata_license = os.environ.get('YGDATAC2_CONF') or rqsdk_license
        if print_license_info(rqsdk_license, rqdata_license):
            set_to_environ(['RQSDK_LICENSE', 'YGDATAC_CONF'], rqsdk_license)
            return
