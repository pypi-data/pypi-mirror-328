...
from deepquant.quest.apis.api_abstract import (order, order_percent, order_shares,
                                               order_target_percent,
                                               order_target_value, order_to,
                                               order_value,
                                               common_rules, TUPLE_PRICE_OR_STYLE_TYPE, PRICE_OR_STYLE_TYPE)
from deepquant.quest.apis.api_base import (assure_instrument, assure_market_code,
                                           cal_style, calc_open_close_style)
from deepquant.quest.const import (DEFAULT_ACCOUNT_TYPE, EXECUTION_PHASE,
...
from deepquant.quest.mod.mod_sys_risk.validators.cash_validator import validate_cash
from deepquant.quest.model.instrument import IndustryCode as industry_code
from deepquant.quest.model.instrument import IndustryCodeItem, Instrument
from deepquant.quest.model.instrument import SectorCode as sector_code
from deepquant.quest.model.instrument import SectorCodeItem
from deepquant.quest.model.order import LimitOrder, MarketOrder, Order, OrderStyle, ALGO_ORDER_STYLES
from deepquant.quest.utils import INST_TYPE_IN_STOCK_ACCOUNT, is_valid_price
from deepquant.quest.utils.arg_checker import apply_rules, verify_that
from deepquant.quest.utils.datetime_func import to_date
...
def _get_account_position_ins(id_or_ins):
    ...
    try:
        ...
        raise KeyError(_(
                u"market_code: {market_code} needs stock account, please set and try again!"
            ).format(market_code=ins.market_code))
...
def _get_order_style_price(market_code, style):
    if isinstance(style, LimitOrder):
        return style.get_limit_price()
    env = Environment.get_instance()
    if isinstance(style, MarketOrder):
        return env.data_proxy.get_last_price(market_code)
    if isinstance(style, ALGO_ORDER_STYLES):
        price, _ = env.data_proxy.get_algo_bar(market_code, style, env.calendar_dt)
        return price
...
def _submit_order(ins, amount, side, position_effect, style, current_quantity, auto_switch_order_value, zero_amount_as_exception=True):
    ...
    if not is_valid_price(price):
        ...
        env.order_creation_failed(market_code=ins.market_code, reason=reason)
        return
    ...
    if amount == 0:
        if zero_amount_as_exception:
            reason = _(u"Order Creation Failed: 0 order quantity, market_code={market_code}").format(market_code=ins.market_code)
            env.order_creation_failed(market_code=ins.market_code, reason=reason)
        return
    order = Order.__from_create__(ins.market_code, abs(amount), side, style, position_effect)
    ...
    if cash_amount > 0:
        ...
        while amount > 0:
            expected_transaction_cost = env.get_order_transaction_cost(Order.__from_create__(
                ins.market_code, amount, SIDE.BUY, LimitOrder(price), POSITION_EFFECT.OPEN
            ))
...
def stock_order(market_code, quantity, price_or_style=None, price=None, style=None):
    result_order = stock_order_shares(market_code, quantity, price, style, price_or_style)
    if result_order:
        return [result_order]
    return []
...
def stock_order_to(market_code, quantity, price_or_style=None, price=None, style=None):
    position = Environment.get_instance().portfolio.get_position(market_code, POSITION_DIRECTION.LONG)
    open_style, close_style = calc_open_close_style(price, style, price_or_style)
    quantity = quantity - position.quantity
    _style = open_style if quantity > 0 else close_style
    result_order = stock_order_shares(market_code, quantity, price, _style, price_or_style)
    if result_order:
        return [result_order]
    return []
...
def order_target_portfolio(
    ...
    该 API 的参数 target_portfolio 为字典，key 为 market_code 或 instrument，value 为权重。
    此时将根据参数 price_or_styles 中设置的价格来计算目标持仓数量并调仓。

    :param target_portfolio: 目标权重字典，key 为 market_code，value 为权重。
    :param price_or_styles: 目标下单价格字典，key 为 market_code, value 为价格或订单类型或订单类型和价格组成的 tuple
    ...
    for id_or_ins, percent in target_portfolio.items():
        ...
        if not ins:
            raise RQInvalidArgument(_(
                ...
                "expected market_code or Instrument objects, got {} (type: {})"
        ...
        if ins.type not in ORDER_TARGET_PORTFOLIO_SUPPORTED_INS_TYPES:
            ...
            ).format(ins.market_code))
        market_code = ins.market_code
        last_price = env.data_proxy.get_last_price(market_code)
        ...
        price_or_style = price_or_styles.get(ins.market_code)
        ...
        target[market_code] = percent, open_style, close_style, last_price
    ...
    # 先把不在目标权重中的仓位平掉
    current_quantities = {
        p.market_code: p.quantity for p in account.get_positions() if p.direction == POSITION_DIRECTION.LONG
    }
    for market_code, quantity in current_quantities.items():
        if market_code not in target:
            env.submit_order(Order.__from_create__(
                market_code, quantity, SIDE.SELL, MarketOrder(), POSITION_EFFECT.CLOSE
            ))
    ...
    if total_percent == 1:
        ...
        for market_code, (target_percent, open_style, close_style, last_price) in target.items():
            current_value = current_quantities.get(market_code, 0) * last_price
    ...
    for market_code, (target_percent, open_style, close_style, last_price) in target.items():
        open_price = _get_order_style_price(market_code, open_style)
        close_price = _get_order_style_price(market_code, close_style)
        if not (is_valid_price(close_price) and is_valid_price(open_price)):
            reason = _("Adjust position of {id_or_ins} Failed: Invalid close/open price {close_price}/{open_price}").format(
                id_or_ins=market_code, close_price=close_price, open_price=open_price
            )
            env.order_creation_failed(market_code=market_code, reason=reason)
            continue
        delta_quantity = (account_value * target_percent / close_price) - current_quantities.get(market_code, 0)
        delta_quantity = _round_order_quantity(env.data_proxy.instrument(market_code), delta_quantity, method=round)
        
        # 优先生成卖单，以便计算出剩余现金，进行买单数量的计算
        if delta_quantity == 0:
            continue
        elif delta_quantity > 0:
            waiting_to_buy[market_code] = (delta_quantity, POSITION_EFFECT.OPEN, open_style, last_price)
            continue
        else:
            quantity, side, position_effect = abs(delta_quantity), SIDE.SELL, POSITION_EFFECT.CLOSE
        order = Order.__from_create__(market_code, quantity, side, close_style, position_effect)
    ...
    for market_code, (delta_quantity, position_effect, open_style, last_price) in waiting_to_buy.items():
        ...
        if cost > estimate_cash:
            ...
            delta_quantity = _round_order_quantity(env.data_proxy.instrument(market_code), delta_quantity)
            if delta_quantity == 0:
                continue
            cost = delta_quantity * last_price + env.get_transaction_cost_with_value(delta_quantity * last_price)
        order = Order.__from_create__(market_code, delta_quantity, SIDE.BUY, open_style, position_effect)
...
def is_suspended(market_code, count=1):
    # type: (str, Optional[int]) -> Union[bool, pd.DataFrame]
    """
    判断某只股票是否全天停牌。

    :param market_code: 某只股票的代码或股票代码，可传入单只股票的market_code, symbol
    ...
    return Environment.get_instance().data_proxy.is_suspended(market_code, dt, count)
...
@ExecutionContext.enforce_phase(EXECUTION_PHASE.ON_INIT,
                                ...
                                EXECUTION_PHASE.SCHEDULED)
@apply_rules(verify_that('market_code').is_valid_instrument())
def is_st_stock(market_code, count=1):
    # type: (str, Optional[int]) -> Union[bool, pd.DataFrame]
    """
    ...
    :param market_code: 某只股票的代码，可传入单只股票的market_code, symbol
    :param count: 回溯获取的数据个数。默认为当前能够获取到的最近的数据
    """
    dt = Environment.get_instance().calendar_dt.date()
    market_code = assure_market_code(market_code)
    return Environment.get_instance().data_proxy.is_st_stock(market_code, dt, count)
...
def industry(code):
    ...
    return [i.market_code for i in cs_instruments if i.industry_code == code]
...
def sector(code):
    ...
    cs_instruments = Environment.get_instance().data_proxy.all_instruments((INSTRUMENT_TYPE.CS,))
    return [i.market_code for i in cs_instruments if i.sector_code == code]


@export_as_api
@apply_rules(
    verify_that("market_code").is_valid_instrument(),
    verify_that("start_date").is_valid_date(ignore_none=False),
)
def get_dividend(market_code, start_date):
    # type: (str, Union[str, datetime.date, datetime.orig_time, pd.Timestamp]) -> Optional[np.ndarray]
    """
    ...
    market_code = assure_market_code(market_code)
    array = env.data_proxy.get_dividend(market_code)
    if array is None:
        return None
