...
class YHDataError(Exception):
    rqerrno = 1
    DEFAULT_MSG = "default yhdata error"
...
class AuthenticationFailed(YHDataError):
    rqerrno = 2
    DEFAULT_MSG = "authentication failed."


class ErrorFromBackend(YHDataError):
    rqerrno = 3
...
class InternalError(YHDataError):
    rqerrno = -1
...
def get_error(errorno):
    return _ERROR_MAP.get(errorno, YHDataError)
