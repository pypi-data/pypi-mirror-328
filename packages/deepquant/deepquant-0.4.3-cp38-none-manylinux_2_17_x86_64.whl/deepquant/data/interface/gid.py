...
def get_kline_combine(klines,
    ...
    if adj==None:
        return klines;
            ...
            klines.drop(['ex_date','ex_factor','orig_time_key','new_key','market_code','security_code_y'], axis=1, inplace=True)
...
def dayprices(
    ...
    return to_query_by_batch('dayprices', fields=fields, symbols=market_code, start_time=start_time,
                    end_time=end_time, limit=limit, df=df)
...
def allFactorsList(
    ...
    data, code, msg = APIManager().api.get_factor(api_name='queryList', fields=fields, factorType=factorType,userName=userName,
                                             limit=limit,
                                             df=df)
    return data, code, msg

def get_turnover_rate(market_code, start_date=None, end_date=None, fields=None, df=True):
     return to_query_by_batch('get_turnover_rate', fields=fields, symbols=market_code,
                              start_date=start_date, end_date=end_date,df=df)
...
def get_share_daily(
   ...
   :return:
   '''
    return to_query_by_batch('queryShareDaily', symbols=market_code, start_time=start_time, end_time=end_time, fields=fields,
                    limit=limit, df=df)
...
def get_reserve_ratio(
    ...
    data, code, msg = APIManager().api.query(api_name='reserveRatio', fields=fields, reserveType=reserve, start_time=start_time, end_time=end_time,
                                                  limit=limit,df=df)
    return data, code, msg
...
def get_margin_stocks(
    ...
    data, code, msg = APIManager().api.query(api_name='marginStocks', fields=fields, exchange=exchange, margine_type=margine_type,date=date,
                                                  limit=limit,df=df)
    return data, code, msg