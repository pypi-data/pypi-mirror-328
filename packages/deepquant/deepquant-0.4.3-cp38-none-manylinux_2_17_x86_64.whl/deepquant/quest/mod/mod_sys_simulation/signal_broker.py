...
from deepquant.quest.interface import AbstractBroker
...
from deepquant.quest.const import SIDE, ORDER_TYPE, POSITION_EFFECT
...
class SignalBroker(AbstractBroker):
    ...
    def submit_order(self, order):
        ...
        account = self._env.get_account(order.market_code)
    ...
    def _match(self, account, order):
        ...
        last_price = price_board.get_last_price(market_code)
        ...
        if not is_valid_price(last_price):
            ...
            if listed_date == self._env.trading_dt.date():
                reason = _(
                    "Order Cancelled: current security [{market_code}] can not be traded in listed date [{listed_date}]").format(
        ...
        ct_amount = account.calc_close_today_amount(market_code, order.quantity, order.position_direction, order.position_effect)
