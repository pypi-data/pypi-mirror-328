...
from deepquant.quest.const import SIDE, HEDGE_TYPE, COMMISSION_TYPE, POSITION_EFFECT
...
class StockTransactionCostDecider(AbstractTransactionCostDecider):
    ...
    def _get_order_commission(self, market_code, side, price, quantity):
        commission = price * quantity * self.commission_rate * self.commission_multiplier
        return max(commission, self.min_commission)

    def _get_tax(self, market_code, side, cost_money):
    ...
    def get_trade_tax(self, trade):
        return self._get_tax(trade.market_code, trade.side, trade.last_price * trade.last_quantity)

    def get_order_transaction_cost(self, order):
        commission = self._get_order_commission(order.market_code, order.side, order.frozen_price, order.quantity)
        tax = self._get_tax(order.market_code, order.side, order.frozen_price * order.quantity)
...
class CNStockTransactionCostDecider(StockTransactionCostDecider):
    ...
    def _get_tax(self, market_code, side, cost_money):
        instrument = Environment.get_instance().get_instrument(market_code)
...
class CNFutureTransactionCostDecider(AbstractTransactionCostDecider):
    ...
    def _get_commission(self, market_code, position_effect, price, quantity, close_today_quantity, dt):
        # type: (str, POSITION_EFFECT, float, int, int, datetime.date) -> float
        info = self.env.data_proxy.get_futures_trading_parameters(market_code, dt)
        commission = 0
        if info.commission_type == COMMISSION_TYPE.BY_MONEY:
            contract_multiplier = self.env.get_instrument(market_code).contract_multiplier
    ...
    def get_trade_commission(self, trade):
        return self._get_commission(
            trade.market_code, trade.position_effect, trade.last_price, trade.last_quantity, trade.close_today_amount, trade.trading_datetime.date()
        )
    ...
    def get_order_transaction_cost(self, order):
        ...
        return self._get_commission(
            order.market_code, order.position_effect, order.frozen_price, order.quantity, close_today_quantity, order.trading_datetime.date()
        )
