...
from .errors import OverwriteWarning
...
def export_as_api(f=None, name=None, namespace=None, priority=0):
    ...
    if namespace:
        if hasattr(yhdatac, namespace):
            namespace = getattr(yhdatac, namespace)
        else:
            namespace_name = namespace
            namespace = types.ModuleType(namespace_name)
            namespace.__file__ = 'yhdatac plugin'
            namespace.__module__ = "yhdatac"
            setattr(yhdatac, namespace_name, namespace)
            yhdatac.__all__.append(namespace_name)
    else:
        namespace = yhdatac
        yhdatac.__all__.append(name)
...
def may_trim_bjse(func):
    @wraps(func)
    def wrap(*args, **kwargs):
        from deepquant.quest.datac import client
        result = func(*args, **kwargs)
        if not client.bjse_enabled():
            # 根据各种可能的数据类型进行处理.
            if isinstance(result, pd.DataFrame):
                if "market_code" in result.columns:
                    result = result[~result["market_code"].str.endswith("BJSE")]
                elif result.index.name == "market_code":
                    result = result[~result.index.str.endswith("BJSE")]
                elif result.index.names[0] == "market_code":
            ...
            elif isinstance(result, pd.Series) and result.index.name == "market_code":
                result = result[~result.index.str.endswith("BJSE")]
