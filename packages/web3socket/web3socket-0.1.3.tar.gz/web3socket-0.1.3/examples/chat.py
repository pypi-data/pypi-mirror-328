...
class Application(object):
    ...
    def __call__(self, environ, start_response):
        socket = environ.get('web3socket')
        if socket is not None:
            self.handle_web3socket(socket)
        else:
    ...
    def handle_web3socket(self, socket):
...
if __name__ == '__main__':
    from web3socket.server import Web3socketServer
    PORT = 8000
    Web3socketServer(('', PORT), Application()).serve_forever()
