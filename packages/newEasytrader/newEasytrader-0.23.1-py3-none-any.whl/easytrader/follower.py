...
class BaseFollower(metaclass=abc.ABCMeta):
    ...
    WEB_REFERER = ""
    WEB_ORIGIN = ""
    PRICE_DECIMAL_PLACES = 3  # 添加价格小数位数的配置
    ...
    def track_strategy_worker(self, strategy, name, interval=10, **kwargs):
        ...
        while True:
            try:
                # 打印时间
                # logger.info("%s query_strategy_transaction time: %s",
                #       name, datetime.datetime.now())
                transactions = self.query_strategy_transaction(
                    strategy, **kwargs
                )
                # logger.info("策略 %s 调仓信息: %s", name, transactions)
