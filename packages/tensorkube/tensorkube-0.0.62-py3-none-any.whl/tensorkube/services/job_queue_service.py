        ...
        if enable_efs:
            ...
            if gpus > 0:
                ...
                if 'nodeSelector' in scaled_job_json['spec']['jobTargetRef']['template']['spec']:
                    scaled_job_json['spec']['jobTargetRef']['template']['spec']['nodeSelector']['nodepool'] = get_gpu_nodepool()
                else:
                    scaled_job_json['spec']['jobTargetRef']['template']['spec']['nodeSelector'] = {'nodepool': get_gpu_nodepool()}
...
def create_sa_role_rb_for_job_sidecar(namespace: Optional[str] = None):
    ...
    attach_role_policy(account_no=get_aws_account_id(), policy_name=get_s3_access_policy_name(), role_name=role_name)
    attach_role_policy(account_no=get_aws_account_id(), policy_name=get_dynamodb_access_policy_name(), role_name=role_name)
    attach_role_policy(account_no=get_aws_account_id(), policy_name=get_sqs_access_policy_name(), role_name=role_name)
    patch_service_account(role_arn, get_job_sidecar_service_account_name(), "keda")
