...
def create_or_update_iam_role_with_service_account_cluster_access(account_no: str, oidc_issuer_url: str, role_name: str,
    ...
    try:
        ...
        current_sub_values = current_trust_policy['Statement'][0]['Condition']['StringEquals'].get("{}:sub".format(oidc_issuer), None)
...
def delete_policy(account_no, policy_name, region=REGION):
    ...
    policy = client.get_policy(PolicyArn='arn:aws:iam::{}:policy/{}'.format(account_no, policy_name))
    default_version = policy['Policy']['DefaultVersionId']
    for version in policy['Policy']['PolicyVersionList']:
        if version['VersionId'] != default_version:
            client.delete_policy_version(PolicyArn='arn:aws:iam::{}:policy/{}'.format(account_no, policy_name), VersionId=version['VersionId'])
