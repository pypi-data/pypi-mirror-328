...
class APIClient:
    ...
    def __init__(self, url: str, api_key: str):
        ...
        self.light_search_endpoint = urljoin(str(self.url), "search/light-query")
        ...
        self.add_entity_endpoint = urljoin(str(self.url), "alie/entity")
        self.predict_entities_endpoint = urljoin(str(self.url), "alie/entity/predict")
        self.delete_entity_endpoint = urljoin(str(self.url), "alie/entity")
    ...
    def delete_intent(
        ...
        """Delete the specified intent from the database. Use "*" to delete all intents from the database (clear the DB).

        Args:
            intent_name: The name of the intent to delete. Use "*" to delete all intents from the database (clear the DB).
    ...
    def add_entity(
        ...
        """Add entity data to the entity prediction engine.

        Args:
            entity_data: List of annotated string containing entity data like : `["hello I live in [France](country) and I want to oder a [airpods](product)"]`
        ...
        Usage:
        ```python
        entity_data = ["hello I live in [France](country) and I want to oder a [airpods](product)"]
        add_entity_response = client.add_entity(entity_data)
        add_entity_response.model_dump()
        ```
        """
        with requests.Session() as session:
            headers = {
            ...
            with session.post(
                    self.add_entity_endpoint, headers=headers, data=json.dumps(entity_data)
            ) as response:
                if response.status_code == 200:
                    response = AddEntityResponse(**response.json())
                else:
                    raise Exception(
                        f"Pythia API Error {response.status_code}: {self._process_error_response(response)}"
                    )
    ...
    def predict_entities(
        ...
        """Query the API to predict the entities in a user message.
        ...
        Usage:
        ```python
        entities = client.predict_entities("Hello I want to modify the delivery date of order FR73681920")
        entities.model_dump()
        ```
        """
        with requests.Session() as session:
            ...
            with session.get(
                    self.predict_entities_endpoint,
                    headers=headers,
                    params={"user_message": user_msg},
            ) as response:
                if response.status_code == 200:
                    response = PredictEntitiesResponse(**response.json())
                else:
                    raise Exception(
                        f"Pythia API Error {response.status_code}: {self._process_error_response(response)}"
                    )
    ...
    def delete_entity(
        ...
        """Delete all entities data the database.
        Returns:
            The response from the API as a Pydantic model. You can use response.model_dump() to get a Dict. Raise an exception with API status code and error message if the status code is not 200.

        Usage:
        ```python
        delete_entity_response = client.delete_entity()
        delete_entity_response.model_dump()
        ```
        """
        with requests.Session() as session:
            headers = {
            ...
            with session.delete(
                    self.delete_entity_endpoint,
                    headers=headers,
            ) as response:
                if response.status_code == 200:
                    response = DeleteEntityResponse(**response.json())
                else:
                    raise Exception(
                        f"Pythia API Error {response.status_code}: {self._process_error_response(response)}"
                    )
    ...
    def extract_structured_data(
        ...
        if prepared_file != [] and string_content:
            raise ValueError("You can't provide both a file and string content to extract the data from.")
