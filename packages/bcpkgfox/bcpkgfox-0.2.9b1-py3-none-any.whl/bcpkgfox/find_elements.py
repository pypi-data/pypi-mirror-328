...
class Elements:
    ...
    def wait_for_appear(
        ...
        attempt = 0
        timeout = float(inf) if timeout == 0 else None
        ...
        if type == "window":
            ...
            object = [object] if isinstance(object, str) == True else None
            while attempt < timeout:
                all_windows = gw.getWindowsWithTitle(title)
                for title_ in object: window_to_search = find_window_by_title(title_)
                if len(window_to_search) >= 1: return window_to_search
                else: attempt += 1

        raise ValueError(f" {RD}>>> {object.title} não encontrado {RESET}")
