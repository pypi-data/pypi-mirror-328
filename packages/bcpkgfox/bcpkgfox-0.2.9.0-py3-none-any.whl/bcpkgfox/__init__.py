...
dirs_sub = {
    ...
    "dir_GCPJ": "C:\\TMPIMGGCPJ\\LAST_IMG",
    "dir_GCPJ": "C:\\TMPIMGGCPJ\\FTP"
    }
...
def create_dirs(specifics_dirs: Optional[list] = None, disable_print_response: bool = False) -> str:
    ...
    if disable_print_response == False:
        if dirs_created:
            print(f" {DK_ORANGE}>{RESET} {len(dirs_created)} diretórios criados:") if len(dirs_created) > 1 else print(f" {DK_ORANGE}>{RESET} {len(dirs_created)} diretório criado:")
