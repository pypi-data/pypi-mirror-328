...
def create_dirs(specifics_dirs: Optional[list] = None, disable_print_response: bool = False) -> str:
    ...
    if disable_print_response == False:
        if dirs_created:
            print(f" {DK_ORANGE}>{RESET} {len(dirs_created)} pastas criadas:") if len(dirs_created) > 1 else print(f" {DK_ORANGE}>{RESET} {len(dirs_created)} pasta criada:")
            for pasta in dirs_created:
                print(f"  {ORANGE}>{RESET} {pasta}")

        else:
            print(f" {DK_ORANGE}>{RESET} Todas as pastas padrões já estão criadas")
...
print(f"\n\n{ORANGE}Biblioteca BCFOX importada - {re.sub(r"[^0-9.]", "", version_line)}{RESET}")
create_dirs()