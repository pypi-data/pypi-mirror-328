    ...
    def from_block(cls, block_lines: list[str]):
        ...
        if len(block_lines) >= 4:
            ...
            if 'COMPOSITION' in init_line:
                init_model.comp_model = InitCompModel.from_block(block_lines)
            elif 'OIL-WATER' in init_line:
                init_model.ow_model = InitOwModel.from_block(block_lines)
...
'''
    ...
    ----------------------------------------------------------------------
    '''
    init_model = InitModel.from_block(ow_str.split('\n'))
    block_ = init_model.to_block()
    print('\n'.join(block_))

    init_model = InitModel.from_block(comp_str.split('\n'))
    block_ = init_model.to_block()
    print('\n'.join(block_))
