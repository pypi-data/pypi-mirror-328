...
class SimulateLog:
    ...
    def set_id(self, uuid: str):
        self.simulate_id = uuid
        if self.model_params is None:
            self.model_params.simulate_id = uuid
        if self.primary_params is None:
            self.primary_params.simulate_id = uuid
        if self.init_ow_model is None:
            self.init_ow_model.simulate_id = uuid
        if self.performance_statistics is None:
            self.performance_statistics.simulate_id = uuid
        for process_stage in self.process_stages:
            process_stage.simulate_id = uuid
    ...
    def from_block(cls, block_lines):
        ...
        head_block = ListUtils.pick_block(block_lines, '--', 'Simulation starts at:')
