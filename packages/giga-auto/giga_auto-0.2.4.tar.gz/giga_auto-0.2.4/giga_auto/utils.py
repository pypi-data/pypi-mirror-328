...
def parse_case_data(case_file: str, has_sql=False):
    case_data = YamlUtils(case_file).read()
    cases = defaultdict(list)
    sql = defaultdict(dict)
    for key, case_config in case_data.items():
        for case in case_config.get('cases', []):
            case_items = [value for _, value in case.items()]
            cases[key].append(tuple(case_items))

        sql[key].update(case_config.get('sql', {}))
    return cases if not has_sql else (cases, sql)
