...
from gai.rag.client.dtos.encode_dto import EncodeResponse, EncodeRequest
from gai.rag.client.dtos.scrapers_dtos import SearchResponse
...
#Chunks-------------------------------------------------------------------------------------------------------------------------------------------
    ...
    async def web_search_async(self,search_query) -> SearchResponse:
        url = os.path.join(self.url,f"websearch/{search_query}")
        response = await http_get_async(url)
        result = json.loads(response.text)
