...
# NVCF-3001 - Enable NVCF debuggability commands when backend production ready
INSTANCE_FLAG = ENABLE_TYPE if (get_environ_tag() <= STAGING_ENV) else DISABLE_TYPE
...
class InstanceCommand(FunctionCommand):  # noqa: D101
    ...
    @CLICommand.arguments(
        "--container-name",
        metavar="<container-name>",
        help="Target container to query logs",
        type=str,
        default=None,
    )
    @CLICommand.arguments(
    ...
    @CLICommand.arguments("target", metavar=FUNCTION_METAVAR, help=TARGET_HELP, type=str, default=None)
    @CLICommand.command(
        help="Read the target pod container logs help",
        description="Read the target pod container logs",
        aliases=[],
        feature_tag=INSTANCE_FLAG,
    )
