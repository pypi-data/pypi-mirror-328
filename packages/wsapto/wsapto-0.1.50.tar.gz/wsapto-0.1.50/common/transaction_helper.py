...
def validate_method(
    ...
    if require_api_key:
        ...
        if abs(int(time.time()) - request_time) > API_KEY_EXPIRATION:
            raise ValueError("Unauthorized: Expired or invalid timestamp")

        expected_api_key = hmac.new(SECRET_KEY.encode(), 
                                    datetime.utcfromtimestamp(request_time).strftime("%Y-%m").encode(), 
