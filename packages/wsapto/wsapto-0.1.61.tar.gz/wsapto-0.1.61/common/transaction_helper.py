...
# Precompile regex untuk deteksi pola berbahaya
SUSPICIOUS_PATTERNS = [
    re.compile(r"<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>", re.IGNORECASE),
    re.compile(r"on\w+\s*=", re.IGNORECASE),  # Deteksi inline event handlers seperti onclick, onload, dll.
    re.compile(r"javascript:", re.IGNORECASE),  # Deteksi penggunaan pseudo-protokol javascript:
    re.compile(r"\b(union\s+select|drop\s+table|insert\s+into|delete\s+from|update\s+.*set|--|#|/\*|\*/)\b", re.IGNORECASE),
    re.compile(r"\b(alert|confirm|prompt|document\.cookie|eval\s*\()", re.IGNORECASE),
    # Pola tambahan untuk deteksi tag berbahaya lain
...
def iterative_unescape(value, iterations=3):
    """
    Lakukan unescape HTML secara iteratif untuk menangani double encoding.
    """
    previous = value
    for _ in range(iterations):
        unescaped = html.unescape(previous)
        if unescaped == previous:
            break
        previous = unescaped
    return previous
...
def contains_malicious_input(data):
    """
    Memeriksa payload input untuk karakter atau pola berbahaya (XSS, SQL Injection, JS Injection)
    dengan pendekatan yang lebih komprehensif.
    """
    for key, value in data.items():
        if isinstance(value, str):
            # Iteratif unescape untuk menangani encoding berlapis
            normalized_value = iterative_unescape(value)
            # Normalisasi whitespace untuk mengurangi teknik obfuscation
            normalized_value = re.sub(r'\s+', ' ', normalized_value).strip()

            # Sanitasi dengan bleach untuk menghapus tag HTML dan atribut yang tidak diizinkan
            # Di sini, kita mengizinkan tidak ada tag, sehingga jika ada yang terhapus,
            # berarti ada konten yang berpotensi berbahaya.
            sanitized_value = bleach.clean(normalized_value, tags=[], attributes={}, strip=True)

            # Jika hasil sanitasi berbeda, berarti ada konten yang tidak diizinkan
            if sanitized_value != normalized_value:
                return True

            # Periksa menggunakan pola-pola regex yang sudah disusun
