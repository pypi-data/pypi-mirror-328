...
from proxy_middleman.custom_exceptions import NoProxiesAvailable #, ProxyRetriesExceeded, CantGetProxiesFromAPI
from proxy_middleman.notification import SlackNotifier
...
class ProxyMiddleman:
    ...
    async def _get_available_proxies(self, connector_id=None):
        ...
        for conn_id, proxies in connectors.items():
            print(f"len(proxies)={len(proxies)}")
            filtered_proxies = [
            ...
            print(f"len(filtered_proxies)={len(filtered_proxies)}")
    ...
    async def _request(self, url, method="GET", **kwargs):
        ...
        for _ in range(self.number_of_retries):
            ...
            try:
                ...
                break
            except (TimeoutErrorCffi, ConnectionErrorCffi, ConnectTimeoutCffi):
