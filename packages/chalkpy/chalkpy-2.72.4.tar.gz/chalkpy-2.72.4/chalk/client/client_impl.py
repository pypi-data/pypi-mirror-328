...
"""
    ...
    def load_features(
        ...
        result = grpc_client._get_python_codegen()  # type: ignore
        ...
        for k, v in feature_sets.items():
            ...
            for f in v.__chalk_features_raw__:
                table.add_row(f.name, escape(str(f.typ).strip("'")))
        ...
        if branch_name in available_branches:
            ...
            try:
                ...
                client_grpc = ChalkGRPCClient(
                    client_id=self._client_id, client_secret=self._client_secret, environment=self._primary_environment
                )
