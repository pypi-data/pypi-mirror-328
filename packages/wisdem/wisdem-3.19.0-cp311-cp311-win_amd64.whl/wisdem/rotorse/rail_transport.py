...
class RailTransport(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        EIxy = inputs["EIxy"]
        lateral_clearance = 0.5 * float(inputs["lateral_clearance"][0])
        vertical_clearance = float(inputs["vertical_clearance"][0])
        max_strains = float(inputs["max_strains"][0])
        max_rot = float(inputs["max_root_rot_deg"][0])
        max_LV = float(inputs["max_LV"][0])
        if _8axle:
            mass_car = float(inputs["max_flatcar_weight_8axle"][0])
        else:
            mass_car = float(inputs["max_flatcar_weight_4axle"][0])
        flatcar_tc_length = float(inputs["flatcar_tc_length"][0])
