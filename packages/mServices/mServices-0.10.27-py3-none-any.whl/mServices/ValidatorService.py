...
class ValidatorService:
            ...
            for r in rules_list:
                if r == 'required':
                    ...
                    try:
                        ...
                        if len(parts) != 2:
                            ValidatorService._add_error(errors, field, "exists", {'_attribute': field})
                        
                        table_name, column = parts

                        if not ValidatorService.table_exists(table_name):
                            ValidatorService._add_error(errors, field, "exists", {'_attribute': "Table Not Found"})
                            continue
                        ...
                        ValidatorService._add_error(errors, field, "invalid_rule", message=str(e))
    ...
    def record_exists(table_name, column, value):
        """Check if a record exists in the database table."""
        with connection.cursor() as cursor:
