...
import nexon
from nexon.application_command import (
    BaseApplicationCommand,
    CallbackWrapper,
    SlashApplicationSubcommand,
)
from nexon.interactions import Interaction
...
def has_role(item: Union[int, str]) -> AC:
    ...
    def predicate(interaction: Interaction) -> bool:
        ...
        if isinstance(item, int):
            role = nexon.utils.get(interaction.user.roles, id=item)  # type: ignore
        else:
            role = nexon.utils.get(interaction.user.roles, name=item)  # type: ignore
        if role is None:
...
def has_any_role(*items: Union[int, str]) -> AC:
    ...
    def predicate(interaction: Interaction) -> bool:
        ...
        # interaction.guild is None doesn't narrow interaction.user to Member
        getter = functools.partial(nexon.utils.get, interaction.user.roles)  # type: ignore
        if any(
...
def bot_has_role(item: Union[int, str]) -> AC:
    ...
    def predicate(interaction: Interaction) -> bool:
        ...
        if isinstance(item, int):
            role = nexon.utils.get(me.roles, id=item)
        else:
            role = nexon.utils.get(me.roles, name=item)
        if role is None:
...
def bot_has_any_role(*items: Union[str, int]) -> AC:
    ...
    *items: Union[:class:`str`, :class:`int`]
    ...
    def predicate(interaction: Interaction) -> bool:
        ...
        getter = functools.partial(nexon.utils.get, interaction.guild.me.roles)
        if any(
            getter(id=item) is not None if isinstance(item, int) else getter(name=item) is not None
            for item in items
        ):
...
def has_permissions(**perms: bool) -> AC:
    ...
    invalid = set(perms) - set(nexon.Permissions.VALID_FLAGS)
    if invalid:
...
def is_nsfw() -> AC:
    ...
    def pred(interaction: Interaction) -> bool:
        ...
        if interaction.guild is None or (
            isinstance(ch, (nexon.TextChannel, nexon.Thread)) and ch.is_nsfw()
        ):
            return True
