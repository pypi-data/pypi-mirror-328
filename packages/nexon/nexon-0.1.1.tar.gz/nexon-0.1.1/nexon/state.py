...
class ConnectionState:
                ...
                for guild_id in app_cmd.guild_ids_to_rollout if use_rollout else app_cmd.guild_ids:
                    if guild_id not in data:
                        try:
                            ...
                            if ignore_forbidden:
                                _log.warning(
                                    "nexon.Client: Forbidden error for %s, is the applications.commands "
        ...
        for raw_response in data:
            ...
            if app_cmd:
                ...
                if app_cmd.is_payload_valid(raw_response, guild_id):
                    if associate_known:
                        _log.debug(
                            "nexon.ConnectionState: Command with signature %s associated with added command.",
                            response_signature,
                        )
                    ...
                    _log.debug(
                        "nexon.ConnectionState: Command with signature %s found but failed deep check, updating.",
                        response_signature,
                    )
                    await self.register_application_command(app_cmd, guild_id)
                elif delete_unknown:
                    _log.debug(
                        "nexon.ConnectionState: Command with signature %s found but failed deep check, removing.",
                        response_signature,
                    )
    ...
    async def register_application_command(
        ...
        _log.info(
            "nexon.ConnectionState: Registering command with signature %s",
