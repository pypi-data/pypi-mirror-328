...
def wf2matrix(vals):
    try:
        idx,val,fwfid,fptime,wf_dir=vals
...
def pred_model(model,in_mat,batch_size=None,itr_pred=0):
    ...
    if itr_pred==0:
        ...
        r_df = pd.DataFrame({'predict': i_mat, 'prob': v_mat})
        ...
        r_df = pd.DataFrame({'predict': i_mat, 'prob': v_mat,'std':s_mat})
        r_df['predict'] = r_df['predict'].replace({0: 'U', 1: 'D', 2: 'K'})
    return r_df
...
def pred_rpnet(model,in_mat,metadata,batch_size=2**13,iteration=100,gpu_num=-1,time_shift=0.5,mid_point=250):
    ...
    # mid point of matrix (if you want shift matrix, change it)
    if not time_shift == 0:
