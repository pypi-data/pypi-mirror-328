...
from lark import Tree, Token, Discard
...
class SimpleTransformer:
    ...
    def _transform_children(C, children):
        for A in children:
            if isinstance(A, Tree):
                B = C._transform_tree(A)
    ...
    def _transform_tree(A, tree): B = list(A._transform_children(
        tree.children)); return A._call_userfunc(tree, B)

    def transform(B, tree):
        A = list(B._transform_children([tree]))
        if not A:
            return
        return A[0]
...
class PreTransformer(SimpleTransformer):
    ...
    def with_paren_or_brace(A, t): return A.with_paren(t)
    ...
    def frac(D, t):
        ...
        return Tree('__formatted__', [A, Token('SLASH', '/'), B])

    def vector_frac(A, t): return A.vector_div(A.frac(t).children)
...
def _normalize_brackets(item):
    B = item
    if isinstance(B, Token) and B.value in PAREN_MAP:
        A = PAREN_MAP[B.value]
        return {'(': Token('LPAR', A), ')': Token('RPAR', A), '[': Token('LSQB', A), ']': Token('RSQB', A)}.get(A)
...
class Extractor:
            ...
            if is_variable_type(c):
                for E in B.inferred:
                    if get_print_value(c) == E[0]:
                        A = []
                        for C in E[1]:
                            D[C] = snake_to_camel(C)
                            A.append(Token(VarName.ANY_VAR, snake_to_camel(C)))
    ...
    def substitute(A, tree):
        ...
        for C in L:
            E = A.collect_variables_include(C)
            F.update(E.fn_names)
            D.update(E.vars)
        ...
        for C in find_from(B.children[2], 'func_simple_vars'):
            ...
            replace_from(C, _remove_paren)
        B.children.pop(4)
        A.inferred = infer_variable_grouping(vars, D)
        A.format_vars(B)
...
@cache
def load_parser():
    with files('algeb_parser').joinpath('parser').open('rb') as f:
        load, value = dill.load(f)
        return dill.loads(load(value))
...
def format_topic(topic, variable_hints=[]):
    ...
    try:
        ...
        H = load_parser()
        A = H(D, B._name)
        A = PreTransformer().transform(A)
        I = Extractor()
        I.lazy_visit_topdown(A)
        return B.from_string(print_tree(A))
    except:
        return


def infer_variable_grouping(ambiguous_vars, revolve_vars): return VariableGrouper(
    ambiguous_vars, revolve_vars).group_variables()
...
class VariableGrouper:
    def __init__(A, ambiguous_vars, revolve_vars):
        C = revolve_vars
        B = ambiguous_vars
        ...
        for D in B:
            if len(D) == 1 or '_' in D:
                E.append(D)
        A.ambiguous_vars = set(B)
        A.revolve_vars = set(C)
        A.revolve_vars.update(E)
        A.revolve_vars = sorted(list(A.revolve_vars), key=len, reverse=True)

    def _find_intervals(E, text):
        C = []
        for B in E.revolve_vars:
            D = 0
            while True:
                A = text.find(B, D)
                if A == -1:
