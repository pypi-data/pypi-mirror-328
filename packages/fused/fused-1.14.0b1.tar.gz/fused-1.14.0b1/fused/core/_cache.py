...
def _serialize_code(code: types.CodeType) -> bytes:
    logger.debug(f"code.co_code: {code.co_code}")
    func_code_bytes = base64.b64encode(code.co_code)
    logger.debug(f"b64 of code: {func_code_bytes}")
    ...
    logger.debug(f"func_const_values: {func_const_values}")
    ...
    logger.debug(f"func_name_values: {func_name_values}")
...
def _parse_max_age(max_age: str | int) -> timedelta:
    if isinstance(max_age, int):
        max_age = f"{max_age}s"

    quantifier = re.match(r"(\d+)([smhd])$", max_age.strip().lower())
    if not quantifier:
        raise ValueError(
            "Invalid cache max age format: Use a number followed by one of 's' (seconds), 'm' (minutes), 'h' (hours), or 'd' (days)."
        )
    value, unit = int(quantifier.group(1)), quantifier.group(2)
    if value < 0:
        raise ValueError("Cache max age cannot be less than zero")
...
def _set_max_age(max_age: timedelta) -> datetime:
    now = datetime.now(timezone.utc)
    return now + max_age


def _write_cache_status(cache_path: Path, uuid: str, status: str):
    status_file = cache_path / f"{uuid}.status"
    with open(status_file, "w") as f:
        f.write(status)
    logger.debug(f"Cache status {status} written to {status_file}")
...
def _write_cache_file(
    ...
    in_prog_expires = (datetime.now(timezone.utc) + timeout_delta).isoformat()
    ...
    **kwargs,
    ...
    Args:
        ...
        *args: Positional arguments to the decorated function.
        ...
        concurrent_lock_timeout: Max amount of time in seconds for concurrent calls to wait for the decorated function
    ...
    """
    if concurrent_lock_timeout < 0:
        raise ValueError("Timeout cannot be less than 0")

    # Calculate expires_on
    max_age_delta = _parse_max_age(cache_max_age)

    if max_age_delta.total_seconds() == 0:
        logger.debug(
            f"Cache max age for {func.__qualname__} set to zero, bypassing cache"
        )
    ...
    try:
        ...
        # 1. Hashify function
        id = _hashify(func)
        logger.debug(f"ID after Hashify function for {func.__qualname__}: {id}")
        # 2. Hashify args
        for v in args:
            id += "_" + _hashify(v)
        logger.debug(f"ID after Hashify args for {func.__qualname__}: {id}")
        # 3. Hashify kwargs
        for k in kwargs:
            id += k + _hashify(kwargs[k])
        logger.debug(f"ID after Hashify kwargs for {func.__qualname__}: {id}")
        # 4. Hashify cache_max_age
        id += _hashify(max_age_delta)
        logger.debug(f"ID after Hashify cache_max_age for {func.__qualname__}: {id}")
        # 5. Hashify composite id
        id = _hashify(id)
        logger.debug(f"ID after Hashify composite id for {func.__qualname__}: {id}")
        ...
        expires_on = _set_max_age(max_age_delta).isoformat()
        timeout_delta = timedelta(seconds=concurrent_lock_timeout)
        if cache_entry:
            now = datetime.now(timezone.utc)
            wait_time_seconds = 1
            max_retries = concurrent_lock_timeout // wait_time_seconds
            retries = 1
            while (
                concurrent_lock_timeout
                and cache_entry.status == "p"
...
# TODO: Replace synchronous calls inside with async
